...
**Metrics组件**(`PerformanceMonitorService`):
```typescript
...
| :--- | :--- | :--- | :--- |
| **`metrics/services/performance-monitor.service.ts`** | `getPerformanceSummary` (封装调用)<br>`calculateHealthScore` (封装调用) | `record*` 系列数据采集方法<br>`getEndpointMetrics`<br>`getDatabaseMetrics`<br>`getSystemMetrics`<br>原始数据获取方法<br>定时数据采集任务<br>事件发射逻辑 | `PerformanceAnalyticsService`<br>`HealthAnalyticsService` |
...
#### **阶段2: 迁移计算逻辑（2-3天）**
1. 从Metrics和Monitoring组件迁移方法到Analytics
2. 更新测试用例，确保功能不变

#### **阶段3: 重构依赖关系（1-2天）**
1. 更新Monitoring组件依赖Analytics
2. 更新其他组件的依赖注入
3. 运行集成测试
