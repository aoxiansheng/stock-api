...
async function main() {
  ...
  // 获取命令行参数，如果没有指定则默认分析 src 目录
  const targetDirectory = process.argv[2] || './src';
  const outputPath = process.argv[3] || './docs/duplicate-analysis-report.md';
  
  // 分析指定目录
  console.log(`🚀 开始分析目录: ${targetDirectory}`);
  console.log(`📁 报告将保存到: ${outputPath}`);
  
  const report = await analyzer.analyzeDirectory(targetDirectory);
  ...
  // 输出摘要
  console.log('\n📊 分析完成！');
  console.log(`📂 分析目录: ${targetDirectory}`);
  ...
  console.log(`\n📋 详细报告请查看: ${outputPath}`);
  console.log('\n💡 使用方法:');
  console.log('  bun run tools/duplicate-analyzer.util.ts [目录路径] [输出文件路径]');
  console.log('  例如: bun run tools/duplicate-analyzer.util.ts ./src/common/constants ./docs/constants-analysis.md');
}
