...
- 旧架构（应用内统计、存储驱动）
  - 核心：`MetricsPerformanceService`（应用内缓冲 + 仓储记录 + 全局拦截器/装饰器）
  - 关键入口：
```1:160:src/main.ts
...
```
  - 装饰器通过 `global["MetricsPerformanceService"]` 间接调用：
```1:101:src/metrics/decorators/database-performance.decorator.ts
const performanceMonitor =
  this.performanceMonitor || (global["MetricsPerformanceService"] as MetricsPerformanceService);
```
  - 服务实现（节选）：
```1:120:src/metrics/services/metrics-performance.service.ts
@Injectable()
...
- 新架构（Prometheus 注册中心统一指标）
  - 核心：`MonitoringRegistryService` + 轻量 `Metrics` 工具
...
```1:130:src/monitoring/helper/metrics-helper.ts
export class Metrics {
  static inc(registry: MonitoringRegistryService, name: string, labels?: object, value = 1, legacyCallback?: () => void)
```
    - Prometheus 导出端点（已存在）：
...
- 严重-Critical｜重复实例化 `MonitoringRegistryService`
  - 证据1：全局共享模块已提供注册中心（Global）
```1:65:src/core/shared/module/shared-services.module.ts
@Global()
@Module({
  providers: [FeatureFlags, MonitoringRegistryService, ...],
  exports:   [FeatureFlags, MonitoringRegistryService, ...],
})
```
...
- 高-High｜监控控制器混用两套体系
  - 证据：同时注入 `MetricsPerformanceService` 和 `MonitoringRegistryService`，并暴露两类接口
...
- 中-Medium｜`AnalyticsModule`（旧）仍被 `AppModule` 与 `MonitoringModule` 引入
```24:116:src/app.module.ts
imports: [ ..., MonitoringModule, ..., AnalyticsModule, ... ]
```
```1:20:src/monitoring/module/monitoring.module.ts
imports: [ ..., AnalyticsModule, SharedServicesModule ]
```
...
- Phase 0（立即，1 天）
  - 目标：消除“重复实例化”，对外行为不变
  - 动作：
    - 统一 `MonitoringRegistryService` 的提供者来源，仅保留 `SharedServicesModule`（Global）提供；`MonitoringModule` 不再单独提供/导出该服务，改为依赖全局导出。
    - 验证所有注入点均取到同一实例（在一处输出 `registry.metrics()` 长度，对比控制器导出的指标数一致）。
  - 风险控制：纯 DI 层调整，无业务逻辑变更。若必须临时保留 `MonitoringModule` 提供者，则改为 `useExisting: MonitoringRegistryService` 指向全局实例，避免双实例。

- Phase 1（短期，2-3 天）
  - 目标：停止新增旧体系依赖、收束入口
  - 动作：
    - 在 `MetricsPerformanceService` 中引入“适配层”，将核心埋点同时转发到 `MonitoringRegistryService`（通过 `Metrics` 工具），保持原仓储写入不变。对外 DTO/接口保持不变。
    - 在 `PerformanceInterceptor` 和装饰器中增加对 `Metrics` 的调用，但保留原逻辑，确保两边数据一致期。
    - 将 `MonitoringInitializerService` 的 `legacyMode` 固定为 false（现已如此），并标注弃用旧内存统计路径。
  - 风险控制：双写期间若 Prometheus 指标遗漏，可回溯旧仓储数据。

- Phase 2（中期，1-2 周）
  - 目标：替换调用方到新注册中心，淡化旧服务
  - 动作：
    - `MonitoringController` 的“性能/数据库/Redis/系统”查询接口维持 DTO 不变，但内部计算逐步改用 Prometheus 指标（或从 `MonitoringRegistryService.getMetricsSummary()` 衍生），以减少对旧仓储的强依赖。
    - 将 `main.ts` 的全局变量访问替换为标准 DI，用于装饰器/拦截器获取 `MetricsPerformanceService` 或直接接受 `MonitoringRegistryService` 的轻量封装。
...
- Phase 3（收尾，1 周）
  ...
  - 动作：
    - 停用全局变量 `global["MetricsPerformanceService"]`；将 `DatabasePerformance`/`CachePerformance`/`AuthPerformance` 装饰器改为直接使用 `Metrics` 写入注册中心。
    - 评估 `AnalyticsModule`（旧）是否仍需保留；若不再需要，进行模块下线（先从 `MonitoringModule` 移除，再从 `AppModule` 移除）。
    - 去除旧仓储路径的写入，保留只读迁移观测一段时间后完全删除。
  - 风险控制：以开关或环境变量控制“旧仓储写入是否开启”，确保灰度下线。

## 4. 详细问题项与修复步骤

### 4.1 问题 A（Critical）：MonitoringRegistryService 多实例冲突
...
@Global()
@Module({
  providers: [FeatureFlags, MonitoringRegistryService, CommonCacheService],
  exports: [FeatureFlags, MonitoringRegistryService, CommonCacheService],
})

// src/monitoring/module/monitoring.module.ts (本地重复提供)
@Module({
  providers: [MonitoringInitializerService, MonitoringRegistryService], // 🚨 重复实例化
  exports: [MonitoringInitializerService, MonitoringRegistryService],
})
```
...
@Module({
  ...
  providers: [
    ...
    // 移除 MonitoringRegistryService 或使用 useExisting
    {
      provide: MonitoringRegistryService,
      useExisting: MonitoringRegistryService, // 指向全局实例
    }
  ],
  exports: [MonitoringInitializerService], // 不再导出 MonitoringRegistryService
...
**问题识别与技术债务**：
- **全局污染**：`global["MetricsPerformanceService"]` 破坏模块边界
- **测试困难**：全局状态导致测试隔离问题
- **并发风险**：多线程环境下的竞态条件隐患
...
const performanceMonitor = app.get(MetricsPerformanceService);
app.useGlobalInterceptors(new PerformanceInterceptor(performanceMonitor, reflector));
global["MetricsPerformanceService"] = performanceMonitor; // 🚨 全局状态
...
1. **Phase 1 - 双写适配层**：
```typescript
// 增强 src/metrics/services/metrics-performance.service.ts
@Injectable()
export class MetricsPerformanceService implements OnModuleInit {
  ...
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      try {
        ...
        // 直接使用注入的 MonitoringRegistryService
...
describe('MonitoringRegistryService 单例验证', () => {
  let app: TestingModule;
  let metricsRegistry1: MonitoringRegistryService;
  let metricsRegistry2: MonitoringRegistryService;
  ...
  beforeAll(async () => {
    ...
    metricsRegistry1 = app.get(MonitoringRegistryService);
    metricsRegistry2 = app.get(MonitoringRegistryService, { strict: false });
  });
...
describe('指标迁移流程集成测试', () => {
  ...
  beforeAll(async () => {
    ...
    metricsRegistry = app.get(MonitoringRegistryService);
...
describe('监控系统容错性测试', () => {
  ...
  describe('部分服务故障', () => {
    it('装饰器异常不应影响业务逻辑', async () => {
      class TestService {
        constructor(public metricsRegistry: MonitoringRegistryService) {}
...
export class ProductionReadinessService {
  ...
  private async checkArchitectureReadiness(): Promise<ReadinessCategory> {
    const checks = [
      { name: '单例MonitoringRegistryService', check: () => this.validateSingletonRegistry() },
...
**Sprint 1 (第1周) - 基础设施修复**：
- ✅ 任务 A1：修复 `MonitoringRegistryService` 双实例问题
- ✅ 任务 A2：实施实例一致性验证
- ✅ 任务 A3：`MetricsPerformanceService` 双写适配层
- 🎯 **里程碑**：基础架构稳定，双写机制就绪
...
建议**立即开始任务A1**（MonitoringRegistryService单例修复），这是解决"写A读B"问题的关键第一步。 