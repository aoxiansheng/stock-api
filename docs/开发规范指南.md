...
{
  // 统一接口层 (直接使用能力命名)
  dataType: "get-stock-quote",       // 用户请求参数，直接对应能力名称
...
- **API接口层** (`dataType`, `capability`): 使用 `kebab-case` (连字符分隔)
- **存储内部层** (`dataClassification`, `dataRuleListType`): 使用 `snake_case` (下划线分隔)

#### 2.4.3 命名规范对照表

| 组件 | API接口层 | 系统内部层 | 文件名 | 存储分类 |
|------|-----------|-----------|--------|---------|
| 股票报价 | `get-stock-quote` | `get-stock-quote` | `get-stock-quote.ts` | `stock_quote` |
| 指数报价 | `get-index-quote` | `get-index-quote` | `get-index-quote.ts` | `index_quote` |
| 股票信息 | `get-stock-basic-info` | `get-stock-basic-info` | `get-stock-basic-info.ts` | `stock_basic_info` |
| 市场状态 | `get-market-status` | `get-market-status` | `get-market-status.ts` | `market_status` |
...
// ✅ 简化后的直接能力使用（无需映射）
// 用户直接使用能力名称，系统内部保持一致
private processCapability(dataType: string): string {
  // dataType 现在直接对应能力名称
  // 如: "get-stock-quote", "get-stock-basic-info", "get-index-quote"
  return dataType; // 无需映射转换
}
...
1. **使用旧的映射思维**
   ```typescript
   // 错误：仍然尝试映射数据类型
   const capability = mapDataType("stock-quote"); // 旧方式，已废弃
   ```

2. **文件名不一致**
   ```typescript
   // 错误：文件名缺少get-前缀
   // 文件：stock-quote.ts，应该是：get-stock-quote.ts
   ```

3. **API文档不统一**
   ```typescript
   // 错误：文档中仍使用旧的数据类型格式
   dataType: "stock-quote" // 应该是 "get-stock-quote"
   ```
...
2. **保持一致性**
   ```typescript
   // 文件名、能力名称、API字段保持完全一致
   // 文件：get-stock-quote.ts
   // 能力：name: "get-stock-quote" 
   // API：dataType: "get-stock-quote"
   ```
...
```typescript
// 📂 位置：@common/pagination/dto/paginated-data
...
const mockData = {
  market: 'HK',
  dataType: 'get-stock-quote', // 统一使用get-前缀格式
  symbols: ['700.HK', '0005.HK']
};
```
