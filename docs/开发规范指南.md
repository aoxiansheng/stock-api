...
{
  // Receiver层 - 能力路由
  capabilityType: "get-stock-quote",     // 用于提供商能力路由和匹配

  // Data Mapper/Transformer层 - 映射规则类型
  dataRuleListType: "quote_fields",     // 用于字段映射规则分类和查找

  // Storage层 - 数据分类
  dataClassification: "stock_quote",    // 用于数据存储分类和检索

  // Query层 - 数据过滤
  dataTypeFilter: "get-stock-quote",    // 用于查询时的数据类型过滤
...
- **能力路由层** (`capabilityType`): 使用 `kebab-case` (连字符分隔) - "get-stock-quote"
- **映射规则层** (`dataRuleListType`): 使用 `snake_case` (下划线分隔) - "quote_fields"  
- **存储分类层** (`dataClassification`): 使用 `snake_case` (下划线分隔) - "stock_quote"
- **查询过滤层** (`dataTypeFilter`): 灵活格式，通常沿用能力名称格式

#### 2.4.3 命名规范对照表

| 数据类型 | Receiver层<br>`capabilityType` | Data Mapper层<br>`dataRuleListType` | Storage层<br>`dataClassification` | Query层<br>`dataTypeFilter` | 文件名 |
|---------|--------------------------------|------------------------------------|------------------------------------|------------------------------|---------|
...
// Receiver层处理能力路由
private processCapability(capabilityType: string): string {
  // capabilityType 用于能力路由和匹配
  // 如: "get-stock-quote", "get-stock-basic-info", "get-index-quote"
  return capabilityType;
}

// Data Mapper/Transformer层处理映射规则查找
private findMappingRule(provider: string, dataRuleListType: string) {
  // dataRuleListType 用于查找字段映射规则
  // 如: "quote_fields", "basic_info_fields", "index_fields"
  return this.dataMapperService.findBestMatchingRule(provider, dataRuleListType);
}

// Storage层处理数据分类
private storeData(data: any, dataClassification: string) {
  // dataClassification 用于数据存储分类
  // 如: "stock_quote", "company_profile", "index_quote"
  return this.storageService.store(data, { classification: dataClassification });
}

// Query层处理数据过滤
private queryData(symbols: string[], dataTypeFilter?: string) {
  // dataTypeFilter 用于查询时的数据类型过滤（Query层语义）
  // 可以使用任意上述类型的字符串值
  return this.queryService.execute({ symbols, dataTypeFilter });
}
...
{
  provider: "preset",
  dataRuleListType: "quote_fields",             // 字段映射规则类型
  // 其他示例：
  // dataRuleListType: "basic_info_fields",     // 基本信息字段
  // dataRuleListType: "index_fields",          // 指数字段
  // dataRuleListType: "market_status_fields",  // 市场状态字段
}
```
...
1. **定义Receiver层能力类型**
   ...
   export const getCryptoQuote: ICapability = {
     name: "get-crypto-quote",  // capabilityType值
     // ...
   };
   ```
...
2. **定义Data Mapper层映射规则类型**
   ...
   export const SUPPORTED_DATA_RULE_LIST_TYPES = [
     // 现有类型...
     "crypto_quote_fields",  // dataRuleListType值 (snake_case)
   ] as const;
   ```
...
3. **定义Storage层数据分类**
   ...
   export enum DataClassification {
     // 现有分类...
     CRYPTO_QUOTE = "crypto_quote",  // dataClassification值 (snake_case)
   }
   ```
...
4. **更新字段映射配置**
   ...
   export const FIELD_MAPPING_CONFIG = {
     ...
     RULE_TYPE_TO_CLASSIFICATION: {
       // 现有映射...
       "crypto_quote_fields": DataClassification.CRYPTO_QUOTE,
     }
   } as const;
   ```
...
- [ ] **DTO字段**：`capabilityType` 使用 `get-{data-type}` 格式

**Data Mapper/Transformer层 (映射规则)**：
- [ ] **映射规则类型**：`dataRuleListType` 使用 `{type}_fields` 格式 (snake_case)
- [ ] **常量定义**：在 `SUPPORTED_DATA_RULE_LIST_TYPES` 中添加
- [ ] **Schema字段**：数据库中使用相同的 `dataRuleListType` 值

**Storage层 (数据分类)**：
- [ ] **分类枚举**：在 `DataClassification` 中使用 `{TYPE}` 格式 (UPPER_SNAKE_CASE)
- [ ] **分类值**：枚举值使用 `{type}` 格式 (snake_case)
- [ ] **DTO字段**：`dataClassification` 使用分类枚举值

**Query层 (数据过滤)**：
- [ ] **过滤字段**：`dataTypeFilter` 可灵活使用上述任意格式
...
1. **混淆不同组件的字段语义**
   ...
   dataClassification: "get-stock-quote" // 应该是 "stock_quote"
   
   // 错误：在Data Mapper中使用能力名称
   dataRuleListType: "get-stock-quote" // 应该是 "quote_fields"
   ```
...
3. **命名格式不一致**
   ...
   dataRuleListType: "quote-fields" // 应该是 "quote_fields"
   
   // 错误：在kebab-case场景使用snake_case  
   capabilityType: "get_stock_quote" // 应该是 "get-stock-quote"
   ```
...
1. **明确字段语义**
   ...
   interface DataRequestDto {
     capabilityType: string; // "get-stock-quote"
   }
   
   // Data Mapper层 - 映射规则
   interface DataMappingRule {
     dataRuleListType: string; // "quote_fields"  
   }
   
   // Storage层 - 数据分类
   interface StorageDocument {
     dataClassification: string; // "stock_quote"
   }
   
   // Query层 - 数据过滤
   interface QueryRequestDto {
     dataTypeFilter?: string; // 可灵活使用任意格式
   }
   ```
...
2. **使用统一的映射配置**
   ...
   const storageClassification = FIELD_MAPPING_CONFIG.CAPABILITY_TO_CLASSIFICATION[capabilityType];
   ```
...
2. **保持四层架构一致性**
   ...
   // Receiver API：capabilityType: "get-stock-quote"
   // Data Mapper：dataRuleListType: "quote_fields"
   // Storage：dataClassification: "stock_quote"
   // Query：dataTypeFilter: "get-stock-quote" (通常沿用能力名称)
   ```
...
import { DataClassification } from '@common/types/field-naming.types';
import { MARKETS } from '@common/constants/markets.constants';
import { PROVIDERS } from '@common/constants/providers.constants';
...
export class CreateExampleDto {
  ...
  // 🎯 根据组件语义使用相应的字段
  // 示例：如果这是Storage层的DTO，使用dataClassification
  @ApiPropertyOptional({
  ...
  @IsOptional()
  @IsEnum(DataClassification, { message: '不支持的数据分类' })
  dataClassification?: DataClassification;
  
  // 示例：如果这是Receiver层的DTO，使用capabilityType
  // @ApiPropertyOptional({
...
export class ExampleResponseDto {
  ...
  static fromEntity(entity: Example): ExampleResponseDto {
    ...
    dto.dataClassification = entity.dataClassification as DataClassification;
  ...
  static fromDocument(doc: any): ExampleResponseDto {
    ...
    dto.dataClassification = doc.dataClassification as DataClassification;
...
ExampleSchema.index({ market: 1, dataClassification: 1 }); // 业务查询优化
...
export class ExampleRepository {
  ...
  async findByMarketAndClassification(
    ...
    this.logger.debug(`按市场和数据分类查询: ${market}, ${dataClassification}`);
...
async findByBusinessCriteria(
  ...
  return this.exampleModel
    .find({ market, dataClassification, provider, isActive: true })
    .lean()
    .exec();
}
...
if (dto.dataClassification && !Object.values(DataClassification).includes(dto.dataClassification)) {
  throw new BadRequestException(`不支持的数据分类: ${dto.dataClassification}`);
}
```
...
describe('ExampleService', () => {
  ...
  describe('create', () => {
    it('应该成功创建实体', async () => {
      ...
      const createDto = {
        ...
        dataClassification: DataType.STOCK_QUOTE
      };
      const mockEntity = {
        id: '1',
        name: 'test',
  ...
  describe('findByMarketAndType', () => {
    it('应该按市场和数据类型查询', async () => {
      ...
      const dataClassification = DataType.STOCK_QUOTE;
      const mockResults = [
        { id: '1', market, dataClassification, name: 'test1' },
        { id: '2', market, dataClassification, name: 'test2' }
      ];

      jest.spyOn(repository, 'findByMarketAndClassification').mockResolvedValue(mockResults);

      // When
      const result = await service.findByMarketAndClassification(market, dataClassification);

      // Then
      expect(result).toHaveLength(2);
      expect(repository.findByMarketAndClassification).toHaveBeenCalledWith(market, dataClassification);
    });
  });
});
```
    ...
    it('应该返回标准分页响应格式', async () => {
      const response = await request(app.getHttpServer())
        ...
        .query({
          ...
          dataTypeFilter: DataType.STOCK_QUOTE  // 🎯 使用 common 枚举
        })
        .expect(200);
      ...
      if (response.body.data.items.length > 0) {
        ...
        expect(firstItem).toMatchObject({
          ...
          dataClassification: expect.stringMatching(new RegExp(Object.values(DataType).join('|')))
        });
      }
    });
  ...
  describe('POST /examples', () => {
    it('应该创建新实体并返回标准响应格式', async () => {
      const createDto = {
        ...
        dataClassification: DataType.STOCK_BASIC_INFO  // 🎯 使用 common 枚举
      };
      ...
      expect(response.body).toMatchObject({
        ...
        data: {
          ...
          dataClassification: createDto.dataClassification,
...
const mockData = {
  market: 'HK',
  capabilityType: 'get-stock-quote', // 统一使用get-前缀格式
  symbols: ['700.HK', '0005.HK']
};
```
