...
{
  // 用户接口层 (简洁、用户友好)
  dataType: "stock-quote",           // 用户请求参数

  // 系统内部层 (语义明确)
  capability: "get-stock-quote",     // 内部能力标识

  // 存储层 (结构化)
  storageKey: "get-stock-quote:longport:AAPL:US",  // 存储键
  dataClassification: "stock_quote"                 // 数据分类 (snake_case)
}
```
...
- **用户接口层** (`dataType`, `capability`): 使用 `kebab-case` (连字符分隔)
- **存储内部层** (`dataClassification`, `ruleListType`): 使用 `snake_case` (下划线分隔)

#### 2.4.3 命名规范对照表

| 组件 | 用户接口层 | 系统内部层 | 文件名 | 存储分类 |
|------|-----------|-----------|--------|---------|
| 股票报价 | `stock-quote` | `get-stock-quote` | `get-stock-quote.ts` | `stock_quote` |
| 指数报价 | `index-quote` | `get-index-quote` | `get-index-quote.ts` | `index_quote` |
| 股票信息 | `stock-basic-info` | `get-stock-basic-info` | `get-stock-basic-info.ts` | `stock_basic_info` |
| 市场状态 | `market-status` | `get-market-status` | `get-market-status.ts` | `market_status` |
...
private mapDataTypeToCapability(dataType: string): string {
  ...
  return dataTypeToCapabilityMap[dataType] || dataType;
}
```

#### 2.4.7 存储分类规范

**DataClassification 枚举定义**：
...
{
  provider: "preset",
  ruleListType: "quote_fields",             // 字段映射规则类型
  // 其他示例：
  // ruleListType: "basic_info_fields",     // 基本信息字段
  // ruleListType: "index_fields",          // 指数字段
  // ruleListType: "market_status_fields",  // 市场状态字段
}
```

#### 2.4.9 添加新数据类型的步骤

当需要添加新的数据类型时，请按以下步骤操作：

1. **定义用户接口名称**（简洁）
   ```typescript
   // 例如：crypto-quote（加密货币报价）
   ```
...
3. **更新数据类型枚举**
   ```typescript
   // src/common/enums/data-type.enum.ts
   export enum DataType {
     // 现有类型...
     CRYPTO_QUOTE = "crypto-quote",  // 用户接口名称
   }
   ```

4. **更新存储分类**
   ```typescript
   // src/core/storage/dto/storage-request.dto.ts
   export enum DataClassification {
     // 现有分类...
     CRYPTO_QUOTE = "crypto_quote",  // 存储分类名称 (snake_case)
   }
   ```

5. **更新映射关系**
   ```typescript
   // src/core/receiver/receiver.service.ts
   private mapDataTypeToCapability(dataType: string): string {
     const dataTypeToCapabilityMap: Record<string, string> = {
       // 现有映射...
       "crypto-quote": "get-crypto-quote",  // 新增映射
     };
     return dataTypeToCapabilityMap[dataType] || dataType;
   }
   ```
...
6. **创建预设字段配置**（如需要）
   ```typescript
   // 预设字段规则类型：get-crypto-quote-fields
   // 数据源映射规则类型：get-crypto-quote
   ```

#### 2.4.10 命名规范验证清单

在添加新能力时，请使用以下清单验证命名规范：

- [ ] **文件名**：使用 `get-{data-type}.ts` 格式
- [ ] **能力名称**：`name` 字段使用 `get-{data-type}` 格式
- [ ] **导出变量**：使用 `camelCase`，如 `getCryptoQuote`
- [ ] **用户接口**：`dataType` 使用简洁的 `{data-type}` 格式
- [ ] **存储分类**：使用内部能力名称 `get-{data-type}`
- [ ] **映射关系**：在 `mapDataTypeToCapability` 中添加映射
- [ ] **枚举更新**：更新 `DataType` 和 `DataClassification` 枚举
- [ ] **文档更新**：更新相关API文档和开发指南
...
1. **混用命名规范**
   ```typescript
   // 错误：在内部使用用户接口名称
   capability: "stock-quote"  // 应该是 "get-stock-quote"
   ```

2. **文件名不一致**
   ```typescript
   // 错误：文件名与能力名称不匹配
   // 文件：stock-quote.ts，能力名称：get-stock-quote
   ```
...
2. **保持一致性**
   ...
   // 映射："stock-quote" → "get-stock-quote"
   ```
...
export class ExampleResponseDto {
  ...
  static fromEntity(entity: Example): ExampleResponseDto {
    ...
    dto.dataType = entity.dataType as DataType;
  ...
  static fromDocument(doc: any): ExampleResponseDto {
    ...
    dto.dataType = doc.dataType as DataType;
...
##### 3. PaginatedResponseDto<T> - 分页响应格式
```typescript
...
async getUsers(@Query() pagination: PaginationDto): Promise<PaginatedResponseDto<UserDto>> {
  const result = await this.userService.findWithPagination(pagination);
...
ExampleSchema.index({ market: 1, dataType: 1 }); // 业务查询优化
...
export class ExampleRepository {
  ...
  async findByMarketAndType(
    ...
    this.logger.debug(`按市场和类型查询: ${market}, ${dataType}`);
  ...
  async findAll(query: ExampleQueryDto): Promise<{
    items: ExampleDocument[];
    total: number;
    page: number;
    limit: number;
  }> {
    const { page = 1, limit = 10, search, type, market, dataType } = query;
    ...
    // 🎯 使用 common 模块的类型进行筛选
    if (market && Object.values(MARKETS).includes(market)) {
...
async findByBusinessCriteria(
  ...
  return this.exampleModel
    .find({ market, dataType, provider, isActive: true })
    .lean()
    .exec();
}
...
if (dto.dataType && !Object.values(DataType).includes(dto.dataType)) {
  throw new BadRequestException(`不支持的数据类型: ${dto.dataType}`);
}
```
...
系统采用统一的权限验证架构，支持JWT用户和API Key两种认证方式：
...
// 角色 + 权限限制
@Auth([UserRole.DEVELOPER], [Permission.DATA_READ, Permission.QUERY_EXECUTE])
@Post('query/execute')
@JwtAuthResponses()
@ApiStandardResponses()
executeQuery() {
...
| 装饰器 | 状态码 | 用途 | 返回类型 |
|--------|--------|------|----------|
| `@ApiSuccessResponse()` | 200 | 通用成功操作 | `ApiResponseDto` |
| `@ApiCreatedResponse()` | 201 | 资源创建成功 | `ApiResponseDto` |
| `@ApiPaginatedResponse()` | 200 | 分页数据获取 | `PaginatedResponseDto` |

##### 错误响应装饰器
| 装饰器 | 覆盖状态码 | 用途 | 适用场景 |
|--------|------------|------|----------|
| `@ApiStandardResponses()` | 400,401,403,404,409,429,500 | 标准错误响应 | API Key认证、公开端点 |
| `@JwtAuthResponses()` | 400,401,403,404,500 | JWT认证错误响应 | JWT认证端点 |
| `@PermissionResponses()` | 403 | 权限验证错误 | 需要特殊权限说明的端点 |

##### 专用响应装饰器
| 装饰器 | 状态码 | 用途 | 适用场景 |
|--------|--------|------|----------|
| `@ApiHealthResponse()` | 200,503 | 健康检查响应 | 健康检查端点 |

#### 认证装饰器与响应装饰器匹配规则

| 认证装饰器 | 响应装饰器 | 使用场景 | 示例 |
|-----------|-----------|---------|------|
| `@Auth()` | `@JwtAuthResponses()` | 用户个人账户管理 | 获取用户信息、创建API Key |
| `@ApiKeyAuth()` | `@ApiStandardResponses()` | 第三方应用和系统管理 | 数据查询、系统监控 |
| `@Public()` | `@ApiStandardResponses()` | 公开访问 | 注册、登录、健康检查 |
...
@ApiKeyAuth([Permission.DATA_READ])
@Get('data-mappings')
@ApiOperation({ summary: '分页获取数据映射' })
@ApiPaginatedResponse(DataMappingDto)
@ApiStandardResponses()
async getDataMappings(@Query() query: PaginationDto) {}
...
##### 1. 装饰器组合原则
- **JWT认证端点**: 只使用 `@JwtAuthResponses()`（已包含完整错误响应）
- **API Key认证端点**: 使用 `@ApiStandardResponses()`
- **公开访问端点**: 使用 `@ApiStandardResponses()`
- **健康检查端点**: 使用 `@ApiHealthResponse()`

##### 2. 成功响应选择
- **通用操作**: 使用 `@ApiSuccessResponse()`
- **创建资源**: 使用 `@ApiCreatedResponse()`
- **分页查询**: 使用 `@ApiPaginatedResponse()`

##### 3. 权限设计原则
- **职责分离**: JWT用于用户管理，API Key用于第三方访问和系统管理
- **权限粒度**: 优先使用具体权限而非角色，提供更细粒度的访问控制
- **最小权限**: 每个端点只要求完成其功能所需的最小权限

##### 4. 常见错误避免
- ❌ 不要在JWT认证端点额外添加 `@ApiStandardResponses()`
- ❌ 不要混用不同的错误响应装饰器
- ❌ 不要忘记为成功响应指定具体的DTO类型
- ✅ 确保认证装饰器与响应装饰器匹配
- ✅ 为每个端点添加清晰的 `@ApiOperation()` 描述
...
describe('ExampleService', () => {
  ...
  describe('findByMarketAndType', () => {
    it('应该按市场和数据类型查询', async () => {
      ...
      const dataType = DataType.STOCK_QUOTE;
      const mockResults = [
        { id: '1', market, dataType, name: 'test1' },
        { id: '2', market, dataType, name: 'test2' }
      ];

      jest.spyOn(repository, 'findByMarketAndType').mockResolvedValue(mockResults);

      // When
      const result = await service.findByMarketAndType(market, dataType);

      // Then
      expect(result).toHaveLength(2);
      expect(repository.findByMarketAndType).toHaveBeenCalledWith(market, dataType);
    });
  });
});
```
    ...
    it('应该返回标准分页响应格式', async () => {
      const response = await request(app.getHttpServer())
        ...
        .query({
          ...
          dataType: DataType.STOCK_QUOTE  // 🎯 使用 common 枚举
        })
        .expect(200);

      // 🎯 验证响应符合 common 模块的标准格式
      ...
      if (response.body.data.length > 0) {
        ...
        expect(firstItem).toMatchObject({
          ...
          dataType: expect.stringMatching(new RegExp(Object.values(DataType).join('|')))
        });
      }
    });
  ...
  describe('POST /examples', () => {
    it('应该创建新实体并返回标准响应格式', async () => {
      const createDto = {
        ...
        dataType: DataType.STOCK_BASIC_INFO  // 🎯 使用 common 枚举
      };
