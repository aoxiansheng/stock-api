...
@Module({
  imports: [LongportModule, LongportSgModule, FooModule],
...
async onModuleInit(): Promise<void> {
  ...
  if (this.longportSgProvider) this.registerProvider(this.longportSgProvider, 2);
  if (this.fooProvider) this.registerProvider(this.fooProvider, 3);
}
```
...
> 
> 提示：当前标准名称为 `longport-sg`；注册表保留了 `longportsg` 的兼容别名。
