...
```
┌─────────────────────────────────────────────────┐
│  Layer 1: Smart Cache + Basic Cache (Redis)    │
│  存储：完整的API响应数据                         │
...
说明：以下为示例范围，实际 TTL 由 BasicCacheService.calculateOptimalTTL 动态计算，并受缓存策略影响（如 STRONG_TIMELINESS、MARKET_AWARE）；Receiver 默认使用 STRONG_TIMELINESS 策略。
...
| **Basic Cache** | 通用缓存服务 | Smart Cache的存储层 | Redis | 动态计算 |
