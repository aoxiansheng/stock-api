...
git add -A && git commit -m "常量重复去重优化结束" && git push
...
分析未使用的字段和属性
bun run tools/property-usage-analyzer-fast.ts 99999
...
# 1. 运行分析工具生成报告
  bun run tools/property-usage-analyzer-nested.ts > constants-usage-analysis.json

  # 2. 干运行查看删除计划
  bun run tools/property-cleanup-tool.ts property-usage-analysis-nested.json --dry-run
