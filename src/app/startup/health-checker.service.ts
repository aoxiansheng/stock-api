...
} from "../config/validation/config-validator.service";
...
export class StartupHealthCheckerService {
  ...
  async performStartupCheck(
    ...
    for (const phase of allPhases) {
      ...
      try {
        ...
        this.logger.debug(`Startup phase completed: ${phase.name}`, {
          duration,
        });
  ...
  private async checkExternalServices(): Promise<void> {
    ...
    if (!longportConfigured) {
      this.logger.warn(
        "LongPort API not configured - provider will be disabled",
      );
    } else {
