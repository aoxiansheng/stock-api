...
export class GracefulShutdownService implements OnApplicationShutdown {
  ...
  private async executeShutdownHooks(): Promise<ShutdownResult> {
    ...
    this.logger.debug(`Executing ${this.shutdownHooks.length} shutdown hooks...`);
    ...
    for (const hook of this.shutdownHooks) {
      ...
      try {
        ...
        this.logger.debug(`Shutdown hook completed: ${hook.name}`, { duration });
  ...
  private registerDefaultShutdownHooks(): void {
    ...
    const needsRegistration = defaultHooksNames.filter(name => !existingNames.includes(name));
      ...
      this.registerShutdownHook({
        ...
        execute: async () => {
          ...
          try {
            ...
            this.logger.warn('Error closing database connections', { error: error.message });
          }
        }
      });
    }
      ...
      this.registerShutdownHook({
        ...
        execute: async () => {
          ...
          try {
            ...
            this.logger.warn('Error closing cache connections', { error: error.message });
          }
        }
      });
    }
  ...
  private async executeHookWithTimeout(hook: ShutdownHook): Promise<void> {
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => {
        reject(new Error(`Shutdown hook '${hook.name}' timed out after ${hook.timeout}ms`));
      }, hook.timeout);
    });
  ...
  private setupSignalHandlers(): void {
    ...
    signals.forEach(signal => {
      process.on(signal, async (receivedSignal) => {
        this.logger.log(`Received ${receivedSignal} signal, initiating graceful shutdown...`);
    ...
    process.on('unhandledRejection', (reason) => {
      this.logger.error('Unhandled Rejection, initiating emergency shutdown...', {
   ...
   * 获取注册的关闭钩子信息
   */
  getShutdownHooks(): Array<{ name: string; priority: number; timeout: number }> {
