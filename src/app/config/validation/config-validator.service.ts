...
export class ConfigValidatorService {
  private readonly logger = createLogger(ConfigValidatorService.name);

  constructor(
    private readonly environmentValidator: EnvironmentValidatorService,
    private readonly dependenciesValidator: DependenciesValidatorService,
  ) {}

  /**
   * 执行完整的配置验证
   */
  async validateAll(options: ValidationOptions = {}): Promise<FullValidationResult> {
    ...
    try {
      ...
      const [environmentResult, dependenciesResult] = await Promise.all([
        ...
        this.dependenciesValidator.validateDependencies(options)
      ]);
    ...
    try {
      ...
      criticalEnvVars.forEach(envVar => {
        if (!process.env[envVar]) {
          startupErrors.push(`Critical environment variable ${envVar} is not set`);
        }
      });
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        if (!process.env.LONGPORT_APP_KEY || !process.env.LONGPORT_APP_SECRET) {
          startupWarnings.push('LongPort credentials not configured for production');
        }
      }
  ...
  private generateRecommendedActions(
    ...
    // 基于警告生成建议
    if (environmentResult.warnings.length > 0 || dependenciesResult.warnings.length > 0) {
  ...
  private logValidationSummary(result: FullValidationResult): void {
    ...
    this.logger.debug('Validation details', {
      ...
      dependencies: {
        ...
        errors: result.dependencies.errors.length,
        warnings: result.dependencies.warnings.length
      }
    });
  }
}