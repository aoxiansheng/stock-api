...
export const CACHE_QUALITY_STANDARDS = Object.freeze({
  ...
  HIT_RATE: {
    ...
    GOOD: 0.85,                           // 良好: 85%+
    ACCEPTABLE: 0.70,                     // 可接受: 70%+
    POOR: 0.50,                           // 较差: 50%+
    CRITICAL: 0.30,                       // 严重: 30%以下
  },
  ...
  ERROR_RATE: {
    ...
    POOR: 0.10,                           // 较差: 10%以下
    CRITICAL: 0.20,                       // 严重: 20%以上
  },
  
  // 内存使用率标准
  MEMORY_USAGE: {
    EXCELLENT: 0.60,                      // 优秀: 60%以下
    GOOD: 0.75,                           // 良好: 75%以下
...
export type CacheSystemType = typeof CACHE_TYPE_SEMANTICS.SYSTEM[keyof typeof CACHE_TYPE_SEMANTICS.SYSTEM];
export type CacheFunctionalType = typeof CACHE_TYPE_SEMANTICS.FUNCTIONAL[keyof typeof CACHE_TYPE_SEMANTICS.FUNCTIONAL];
export type CacheOperationalType = typeof CACHE_TYPE_SEMANTICS.OPERATIONAL[keyof typeof CACHE_TYPE_SEMANTICS.OPERATIONAL];

/**
 * 缓存键前缀类型定义
 */
export type CacheKeyPrefix = typeof CACHE_KEY_PREFIX_SEMANTICS[keyof typeof CACHE_KEY_PREFIX_SEMANTICS];

/**
 * 缓存操作类型定义
 */
export type CacheOperationBasic = typeof CACHE_OPERATION_SEMANTICS.BASIC[keyof typeof CACHE_OPERATION_SEMANTICS.BASIC];
export type CacheOperationBatch = typeof CACHE_OPERATION_SEMANTICS.BATCH[keyof typeof CACHE_OPERATION_SEMANTICS.BATCH];
export type CacheOperationAdvanced = typeof CACHE_OPERATION_SEMANTICS.ADVANCED[keyof typeof CACHE_OPERATION_SEMANTICS.ADVANCED];
export type CacheOperationMonitoring = typeof CACHE_OPERATION_SEMANTICS.MONITORING[keyof typeof CACHE_OPERATION_SEMANTICS.MONITORING];

/**
 * 缓存状态类型定义
 */
export type CacheStatusBasic = typeof CACHE_STATUS_SEMANTICS.BASIC[keyof typeof CACHE_STATUS_SEMANTICS.BASIC];
export type CacheStatusHealth = typeof CACHE_STATUS_SEMANTICS.HEALTH[keyof typeof CACHE_STATUS_SEMANTICS.HEALTH];
export type CacheStatusPerformance = typeof CACHE_STATUS_SEMANTICS.PERFORMANCE[keyof typeof CACHE_STATUS_SEMANTICS.PERFORMANCE];
export type CacheStatusOperation = typeof CACHE_STATUS_SEMANTICS.OPERATION[keyof typeof CACHE_STATUS_SEMANTICS.OPERATION];
...
export function isStrongConsistencyType(cacheType: string): boolean {
  return CACHE_BUSINESS_RULES.CONSISTENCY.STRONG_CONSISTENCY_TYPES.includes(cacheType as any);
}
 ...
 * 生成标准缓存键
 */
export function generateCacheKey(prefix: CacheKeyPrefix, ...parts: string[]): string {
 ...
 * 从质量标准获取等级
 */
export function getQualityLevel(value: number, standards: Record<string, number>): string {
