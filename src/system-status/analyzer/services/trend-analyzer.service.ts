...
export class TrendAnalyzerService {
  ...
  async calculatePerformanceTrends(
    ...
    try {
      ...
      // 缓存结果
      const ttl = this.cacheService.getTTL('TRENDS');
      ...
      this.eventBus.emit(SYSTEM_STATUS_EVENTS.ANALYSIS_ERROR, {
        timestamp: new Date(),
        source: 'trend-analyzer',
        metadata: { error: error.message, operation: 'calculatePerformanceTrends', period }
      });
  ...
  async getHistoricalTrends(
    ...
    try {
      this.logger.debug(`分析历史趋势: ${metricsHistory.length} 个数据点, 周期 ${period}`);
      ...
      for (let i = 1; i < metricsHistory.length; i++) {
        ...
        const trend = this.metricsCalculator.calculateTrends(currentMetrics, previousMetrics);
        trends.push(trend);
      }
  ...
  async detectAnomalies(
    ...
    try {
      ...
      // 计算当前指标
      const currentResponseTime = this.metricsCalculator.calculateAverageResponseTime(currentMetrics);
      const currentErrorRate = this.metricsCalculator.calculateErrorRate(currentMetrics);
      const currentThroughput = this.metricsCalculator.calculateThroughput(currentMetrics);
      ...
      // 响应时间异常检测
      const responseTimeDeviation = Math.abs(currentResponseTime - baseline.averageResponseTime) / baseline.averageResponseTime;
      if (responseTimeDeviation > defaultThresholds.responseTimeThreshold) {
        anomalies.push({
          ...
          expectedRange: {
            min: baseline.averageResponseTime * (1 - defaultThresholds.responseTimeThreshold),
            max: baseline.averageResponseTime * (1 + defaultThresholds.responseTimeThreshold)
          },
      ...
      // 错误率异常检测
      const errorRateDeviation = Math.abs(currentErrorRate - baseline.averageErrorRate) / (baseline.averageErrorRate || 0.01);
      if (errorRateDeviation > defaultThresholds.errorRateThreshold) {
        anomalies.push({
          ...
          expectedRange: {
            min: Math.max(0, baseline.averageErrorRate * (1 - defaultThresholds.errorRateThreshold)),
            max: baseline.averageErrorRate * (1 + defaultThresholds.errorRateThreshold)
          },
      ...
      // 吞吐量异常检测
      const throughputDeviation = Math.abs(currentThroughput - baseline.averageThroughput) / baseline.averageThroughput;
      if (throughputDeviation > defaultThresholds.throughputThreshold) {
        anomalies.push({
          ...
          expectedRange: {
            min: baseline.averageThroughput * (1 - defaultThresholds.throughputThreshold),
            max: baseline.averageThroughput * (1 + defaultThresholds.throughputThreshold)
          },
  ...
  async predictTrends(
    ...
    try {
      this.logger.debug(`开始趋势预测: ${predictionHours}小时, 基于${historicalMetrics.length}个历史数据点`);
      ...
      // 简化的线性趋势预测
      const predictions = this.calculateLinearTrendPrediction(historicalMetrics, predictionHours);
  ...
  private calculateBaseline(historicalMetrics: RawMetricsDto[]): {
    ...
    const responseTimes = historicalMetrics.map(m => this.metricsCalculator.calculateAverageResponseTime(m));
    const errorRates = historicalMetrics.map(m => this.metricsCalculator.calculateErrorRate(m));
    const throughputs = historicalMetrics.map(m => this.metricsCalculator.calculateThroughput(m));
  ...
  private calculateLinearTrendPrediction(
    ...
    const firstResponseTime = this.metricsCalculator.calculateAverageResponseTime(firstMetrics);
    const lastResponseTime = this.metricsCalculator.calculateAverageResponseTime(lastMetrics);
    const responseTimeTrend = (lastResponseTime - firstResponseTime) / recentMetrics.length;
    
    const firstErrorRate = this.metricsCalculator.calculateErrorRate(firstMetrics);
    const lastErrorRate = this.metricsCalculator.calculateErrorRate(lastMetrics);
    const errorRateTrend = (lastErrorRate - firstErrorRate) / recentMetrics.length;
    
    const firstThroughput = this.metricsCalculator.calculateThroughput(firstMetrics);
    const lastThroughput = this.metricsCalculator.calculateThroughput(lastMetrics);
    const throughputTrend = (lastThroughput - firstThroughput) / recentMetrics.length;

    return {
      responseTime: { 
        value: Math.max(0, lastResponseTime + responseTimeTrend * predictionHours), 
        confidence: 0.7 
      },
      errorRate: { 
        value: Math.max(0, Math.min(1, lastErrorRate + errorRateTrend * predictionHours)), 
        confidence: 0.6 
      },
      throughput: { 
   ...
   * 评估预测置信度
   */
  private assessPredictionConfidence(historicalMetrics: RawMetricsDto[]): 'high' | 'medium' | 'low' {
  ...
  private identifyTrendFactors(historicalMetrics: RawMetricsDto[]): string[] {
    ...
    const hasHighLoad = historicalMetrics.some(m => 
      this.metricsCalculator.calculateThroughput(m) > 100
    );
    if (hasHighLoad) {
      factors.push('高负载影响');
    }
    
    const hasErrors = historicalMetrics.some(m => 
      this.metricsCalculator.calculateErrorRate(m) > 0.05
    );
    if (hasErrors) {
      factors.push('错误率波动');
    }
   ...
   * 构建趋势缓存键
   */
  private buildTrendsCacheKey(type: string, period: string, metrics: RawMetricsDto): string {
