...
export class AnalyzerHealthScoreCalculator {
  private readonly logger = new Logger(AnalyzerHealthScoreCalculator.name);
  ...
  calculateOverallHealthScore(rawMetrics: RawMetricsDto): number {
    try {
      ...
      this.logger.debug('健康分计算完成', {
        overall: finalScore,
        components: { api: apiScore, database: databaseScore, cache: cacheScore, system: systemScore }
      });
  ...
  calculateApiHealthScore(rawMetrics: RawMetricsDto): number {
    ...
    // 错误率评分 (0-40分)
    let errorScore = 40;
    if (errorRate > 0.1) errorScore = 0;           // 错误率>10%
    else if (errorRate > 0.05) errorScore = 20;    // 错误率>5%
    else if (errorRate > 0.01) errorScore = 35;    // 错误率>1%

    // 响应时间评分 (0-60分)
    let responseScore = 60;
    if (avgResponseTime > 2000) responseScore = 0;        // >2秒
    else if (avgResponseTime > 1000) responseScore = 20;  // >1秒
  ...
  calculateDatabaseHealthScore(rawMetrics: RawMetricsDto): number {
    ...
    // 计算平均查询时间
    const avgQueryTime = dbOps.reduce((sum, op) => sum + op.duration, 0) / totalOps;

    // 失败率评分 (0-50分)
    let failureScore = 50;
    if (failureRate > 0.1) failureScore = 0;       // 失败率>10%
    else if (failureRate > 0.05) failureScore = 25; // 失败率>5%

    // 查询时间评分 (0-50分)
    let queryScore = 50;
    if (avgQueryTime > 1000) queryScore = 0;        // >1秒
  ...
  calculateCacheHealthScore(rawMetrics: RawMetricsDto): number {
    ...
    // 计算平均缓存响应时间
    const avgCacheTime = cacheOps.reduce((sum, op) => sum + op.duration, 0) / totalOps;

    // 命中率评分 (0-70分)
    let hitScore = 0;
    if (hitRate >= 0.9) hitScore = 70;        // 命中率>=90%
    else if (hitRate >= 0.7) hitScore = 50;   // 命中率>=70%
    else if (hitRate >= 0.5) hitScore = 30;   // 命中率>=50%

    // 响应时间评分 (0-30分)
    let responseScore = 30;
    if (avgCacheTime > 100) responseScore = 0;        // >100ms
   ...
   * 获取健康分等级
   */
  getHealthGrade(score: number): 'excellent' | 'good' | 'fair' | 'poor' | 'critical' {
  ...
  generateHealthRecommendations(rawMetrics: RawMetricsDto): string[] {
    ...
    try {
      ...
      const dbOps = rawMetrics.database || [];
      if (dbOps.length > 0) {
        const avgQueryTime = dbOps.reduce((sum, op) => sum + op.duration, 0) / dbOps.length;
        const failureRate = dbOps.filter(op => !op.success).length / dbOps.length;

        if (avgQueryTime > 500) {
          recommendations.push('数据库查询时间过长，建议优化索引或查询语句');
        }
        if (failureRate > 0.05) {
          recommendations.push('数据库操作失败率过高，建议检查连接池配置和网络状况');
        }
      }
      ...
      const cacheOps = rawMetrics.cache || [];
      if (cacheOps.length > 0) {
        const hitRate = cacheOps.filter(op => op.hit).length / cacheOps.length;
        
        if (hitRate < 0.7) {
          recommendations.push('缓存命中率偏低，建议优化缓存策略或增加缓存时间');
        }
      }
      ...
      if (system) {
        if (system.cpu.usage > 0.8) {
          recommendations.push('CPU使用率过高，建议优化计算密集型操作或增加服务器资源');
        }
        if (system.memory.percentage > 0.8) {
          recommendations.push('内存使用率过高，建议检查内存泄漏或增加内存容量');
        }
      }
