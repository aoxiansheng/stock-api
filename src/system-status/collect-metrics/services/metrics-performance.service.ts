...
import {
  ...
  COLLECT_METRIC_UNITS,
  COLLECT_METRIC_HEALTH_SCORE_CONFIG,
  COLLECT_METRIC_PERFORMANCE_DEFAULTS,
  COLLECT_METRIC_PERFORMANCE_EVENTS,
  API_KEY_CONSTANTS,
  REDIS_INFO,
...
export class MetricsPerformanceService {
  private readonly logger = createLogger(MetricsPerformanceService.name);
  ...
  async recordRequest(
    ...
    this.addMetric(
      COLLECT_METRIC_NAMES.API_REQUEST_DURATION,
      responseTime,
      COLLECT_METRIC_UNITS.MILLISECONDS,
      {
        endpoint,
        method,
  ...
  async recordDatabaseQuery(
    ...
    this.addMetric(
      COLLECT_METRIC_NAMES.DB_QUERY_DURATION,
      duration,
      COLLECT_METRIC_UNITS.MILLISECONDS,
      {
  ...
  recordCacheOperation(
    ...
    this.addMetric(COLLECT_METRIC_NAMES.CACHE_OPERATION_TOTAL, 1, COLLECT_METRIC_UNITS.COUNT, {
      operation,
    ...
    if (duration !== undefined) {
      this.addMetric(
        COLLECT_METRIC_NAMES.CACHE_OPERATION_DURATION,
        duration,
        COLLECT_METRIC_UNITS.MILLISECONDS,
        {
          operation,
  ...
  recordAuthentication(
    ...
    this.addMetric(
      COLLECT_METRIC_NAMES.AUTH_DURATION,
      duration,
      COLLECT_METRIC_UNITS.MILLISECONDS,
      {
        auth_type: type,
    ...
    this.addMetric(COLLECT_METRIC_NAMES.AUTH_TOTAL, 1, COLLECT_METRIC_UNITS.COUNT, {
      auth_type: type,
      status: success ? AuthStatus.SUCCESS : AuthStatus.FAILURE,
    });
  }
  ...
  recordRateLimit(apiKey: string, allowed: boolean, remaining: number): void {
    ...
    this.addMetric(COLLECT_METRIC_NAMES.RATE_LIMIT_CHECK, 1, COLLECT_METRIC_UNITS.COUNT, {
      api_key: apiKeyPrefix,
      result: allowed ? OperationStatus.ALLOWED : OperationStatus.BLOCKED,
    });

    this.addMetric(
      COLLECT_METRIC_NAMES.RATE_LIMIT_REMAINING,
      remaining,
      COLLECT_METRIC_UNITS.COUNT,
      {
        api_key: apiKeyPrefix,
      },
    );
  ...
  private checkSlowOperation(duration: number) {
    if (duration > PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.SLOW_REQUEST_MS) {
      this.logger.warn(
        "慢操作检测",
        sanitizeLogData({ duration, unit: COLLECT_METRIC_UNITS.MILLISECONDS }),
      );
    }
  }
  ...
  async getDatabaseMetrics(
    ...
    const metrics: DatabaseMetricsDto = {
      connectionPoolSize: this.configService.get<number>(
        "DB_POOL_SIZE",
        COLLECT_METRIC_PERFORMANCE_DEFAULTS.DB_POOL_SIZE,
      ),
      activeConnections: 0,
  ...
  async getPerformanceSummary(
    ...
    try {
      ...
      // ✅ 健康评分计算已移至Analytics层，Metrics层只负责数据聚合
      const summary = {
        timestamp: new Date().toISOString(),
        healthScore: 100, // 默认值，实际计算由Analytics层负责
        processingTime: 0,
        summary: {
  ...
  private getDefaultDatabaseMetrics(): DatabaseMetricsDto {
    return {
      connectionPoolSize: COLLECT_METRIC_PERFORMANCE_DEFAULTS.DB_POOL_SIZE,
      activeConnections: 0,
      waitingConnections: 0,
      averageQueryTime: 0,
      slowQueries: 0,
  ...
  private getDefaultRedisMetrics(): RedisMetricsDto {
    return {
      memoryUsage: COLLECT_METRIC_PERFORMANCE_DEFAULTS.REDIS_MEMORY_USAGE,
      connectedClients: 0,
      opsPerSecond: 0,
      hitRate: COLLECT_METRIC_PERFORMANCE_DEFAULTS.CACHE_HIT_RATE,
      evictedKeys: 0,
      expiredKeys: 0,
    };
  }
  ...
  private getDefaultSystemMetrics(): SystemMetricsDto {
    return {
      cpuUsage: COLLECT_METRIC_PERFORMANCE_DEFAULTS.SYSTEM_CPU_USAGE,
      memoryUsage: COLLECT_METRIC_PERFORMANCE_DEFAULTS.SYSTEM_MEMORY_USAGE,
      heapUsed: 0,
      heapTotal: 0,
      uptime: 0,
      eventLoopLag: 0,
  ...
  private addMetric(
    ...
    this.eventEmitter.emit(COLLECT_METRIC_PERFORMANCE_EVENTS.METRIC_RECORDED, {
      metric: name,
      value,
    });
  }
  ...
  private startSystemMetricsCollection(): void {
    ...
    this.addMetric(
      COLLECT_METRIC_NAMES.SYSTEM_CPU_USAGE,
      metrics.cpuUsage,
      COLLECT_METRIC_UNITS.PERCENT,
      {},
    );
    this.addMetric(
      COLLECT_METRIC_NAMES.SYSTEM_MEMORY_USAGE,
      metrics.memoryUsage,
      COLLECT_METRIC_UNITS.BYTES,
      {},
    );
    this.addMetric(
      COLLECT_METRIC_NAMES.SYSTEM_HEAP_USED,
      metrics.heapUsed,
      COLLECT_METRIC_UNITS.BYTES,
      {},
    );
    this.addMetric(
      COLLECT_METRIC_NAMES.SYSTEM_UPTIME,
      metrics.uptime,
      COLLECT_METRIC_UNITS.SECONDS,
      {},
    );
  ...
  private getEventLoopLag(): void {
    ...
    setImmediate(() => {
      ...
      this.addMetric(
        COLLECT_METRIC_NAMES.SYSTEM_EVENT_LOOP_LAG,
        lag,
        COLLECT_METRIC_UNITS.MILLISECONDS,
        {},
      );
  ...
  // ✅ calculateHealthScore方法已移至Analytics层
  // Metrics层只负责数据收集和基础聚合，不包含业务逻辑和分析算法
