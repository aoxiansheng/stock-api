...
export class LongportSgStreamContextService implements OnModuleDestroy {
  private readonly logger = createLogger(LongportSgStreamContextService.name);

  // === 单例模式实现 ===
  private static instance: LongportSgStreamContextService | null = null;
  ...
  constructor(private readonly configService: ConfigService) {
    this.logger.log("LongportSgStreamContextService 构造函数调用");
  }
  ...
  public static async getInstance(
  ...
  ): Promise<LongportSgStreamContextService> {
    if (LongportSgStreamContextService.instance) {
      return LongportSgStreamContextService.instance;
    }

    // 实现线程安全的单例初始化
    if (LongportSgStreamContextService.initializationLock) {
      // 等待其他线程完成初始化
      await LongportSgStreamContextService.waitForInitialization();
      if (LongportSgStreamContextService.instance) {
        return LongportSgStreamContextService.instance;
      }
    }

    LongportSgStreamContextService.initializationLock = true;
    ...
    try {
      ...
      LongportSgStreamContextService.initializationLock = false;
    }
  }
  ...
  private static async waitForInitialization(): Promise<void> {
    ...
    while (LongportSgStreamContextService.initializationLock) {
      if (Date.now() - startTime > LongportSgStreamContextService.lockTimeout) {
        throw new Error("LongportSgStreamContextService 初始化超时");
      }
  ...
  public static async resetInstance(): Promise<void> {
    if (LongportSgStreamContextService.instance) {
      await LongportSgStreamContextService.instance.cleanup();
      LongportSgStreamContextService.instance = null;
    }
    LongportSgStreamContextService.initializationLock = false;
  }
