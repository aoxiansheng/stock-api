...
import { InfrastructureMetricsRegistryService } from '../../infrastructure/metrics/infrastructure-metrics-registry.service';
...
export class CollectorService implements ICollector {
  ...
  recordRequest(
    ...
    this.InfrastructureMetricsRegistryService.receiverProcessingDuration.observe(
  ...
  recordDatabaseOperation(
    ...
    // 使用统一的 Prometheus 指标 
    this.InfrastructureMetricsRegistryService.storageOperationsTotal.inc({
      operation: operation,
      storage_type: 'database'
    });
    
    this.InfrastructureMetricsRegistryService.storageQueryDuration.observe(
  ...
  recordCacheOperation(
    ...
    this.InfrastructureMetricsRegistryService.storageCacheEfficiency.set(
      { cache_layer: 'redis' },
      hit ? 1 : 0
    );
  ...
  recordSystemMetrics(metrics: SystemMetricsDto): void {
    ...
    // 使用统一的 Prometheus 指标
    this.InfrastructureMetricsRegistryService.systemCpuUsagePercent.set(metrics.cpu.usage * 100);
    this.InfrastructureMetricsRegistryService.storageDataVolume.set(
