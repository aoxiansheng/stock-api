...
/**
 * 获取InfrastructureMetricsRegistryService实例
 * 尝试从应用上下文中获取
 */
function getMetricsRegistry(): InfrastructureMetricsRegistryService | null {
  try {
    // 这里可以通过依赖注入容器获取实例
...
function recordToPrometheusMetrics(
  metricsRegistry: InfrastructureMetricsRegistryService,
  data: {
    metricType: 'database' | 'cache' | 'auth';
    operation: string;
    duration: number;
