...
/**
 * 展示层错误处理服务
 * 职责：集中处理展示层组件的所有错误，提供统一的错误处理和日志记录
 */
@Injectable()
export class PresenterErrorHandlerService implements ISystemStatusErrorHandler {
  private readonly logger = createLogger(PresenterErrorHandlerService.name);

  constructor(private readonly eventBus: EventEmitter2) {
    this.logger.log(
      "PresenterErrorHandlerService initialized - 展示层错误处理器已启动",
    );
  }
  ...
  handleBusinessError(
    ...
    try {
      const errorMessage = this.formatBusinessErrorMessage(
        error,
        context,
        businessData,
      );
   ...
   * 格式化业务错误消息
   */
  private formatBusinessErrorMessage(
    error: Error,
    context: ErrorContext,
    businessData?: any,
  ): string {
    const dataInfo = businessData
      ? ` [数据: ${JSON.stringify(businessData).substring(0, 100)}]`
      : "";
