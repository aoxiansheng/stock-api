...
class MonitoringConfigValidator {
   ...
   * @returns 验证结果
   */
  static validateTtlConfig(
    config: MonitoringUnifiedTtlConfig,
   ...
   * @returns 验证结果
   */
  static validateLimitsConfig(
    config: MonitoringUnifiedLimitsConfig,
  ): ConfigValidationResult {
    const errors = validateSync(config);
    const warnings: string[] = [];
    let validatedFields = 0;
    ...
    if (config.systemLimits) {
      ...
      if (
        config.systemLimits.maxBufferSize <
        config.systemLimits.maxRetryAttempts * 10
      ) {
   ...
   * @returns 验证结果
   */
  static validateCoreEnvConfig(
    config: MonitoringCoreEnvConfig,
  ): ConfigValidationResult {
    const errors = validateSync(config);
    const warnings: string[] = [];
  ...
  static validateEnvironmentVariables(): EnvironmentValidationResult {
    ...
    for (const envVar of coreEnvVars) {
      ...
      if (!envValue) {
        ...
        if (
          envVar.includes("TTL") ||
          envVar.includes("BATCH_SIZE") ||
          envVar.includes("RESPONSE") ||
          envVar.includes("RETRY")
        ) {
  ...
  static validateCompleteConfiguration(): {
    ...
    results: {
      ...
      overlaps: ReturnType<
        typeof MonitoringConfigValidator.detectConfigurationOverlaps
      >;
    };
    summary: {
      totalErrors: number;
    ...
    const overallScore = Math.max(
      0,
      Math.min(100, 100 - (actualIssues / maxPossibleIssues) * 100),
    );
  ...
  static generateValidationReport(
    result: ReturnType<
      typeof MonitoringConfigValidator.validateCompleteConfiguration
    >,
  ): string {
