...
import { IsNumber, IsBoolean, IsString, IsEnum, Min, Max } from 'class-validator';
...
export class MonitoringEventsConfig {
  ...
  validateConfiguration(): { isValid: boolean; errors: string[] } {
    ...
    // 验证告警频率配置
    if (this.alertFrequency.maxAlertsPerMinute * 60 > this.alertFrequency.maxAlertsPerHour) {
      errors.push('每分钟最大告警数与每小时最大告警数不一致');
    }

    // 验证冷却时间递增
    if (this.alertFrequency.cooldownEmergencySeconds >= this.alertFrequency.cooldownCriticalSeconds) {
      errors.push('紧急告警冷却时间应小于严重告警冷却时间');
    }

    // 验证重试配置
    if (this.eventRetry.initialRetryDelayMs >= this.eventRetry.maxRetryDelayMs) {
      errors.push('初始重试延迟应小于最大重试延迟');
    }

    // 验证升级时间递增
    if (this.alertEscalation.level1EscalationMinutes >= this.alertEscalation.level2EscalationMinutes) {
      errors.push('第一级升级时间应小于第二级升级时间');
    }

    // 验证工作时间
    if (this.alertEscalation.businessHoursStart >= this.alertEscalation.businessHoursEnd) {
      errors.push('工作时间开始应早于工作时间结束');
    }

    // 验证静默时间
    if (this.eventNotification.quietHoursStart === this.eventNotification.quietHoursEnd) {
   ...
   * 获取事件数据保留时间
   */
  getDataRetention(dataType: 'realtime' | 'hourly' | 'daily' | 'monthly'): number {
    switch (dataType) {
      case 'realtime':
   ...
   * 判断是否可以发送告警
   */
  canSendAlert(level: AlertLevel, recentAlertCount: number, timeWindowMinutes: number = 1): boolean {
  ...
  isBusinessHours(): boolean {
    ...
    return currentHour >= this.alertEscalation.businessHoursStart && 
 ...
 * ```
 */
export const monitoringEventsConfig = registerAs('monitoringEvents', (): MonitoringEventsConfig => {
  ...
  if (env.MONITORING_ALERT_COOLDOWN_EMERGENCY) {
    ...
    if (!isNaN(parsed)) config.alertFrequency.cooldownEmergencySeconds = parsed;
  }
  
  if (env.MONITORING_ALERT_COOLDOWN_CRITICAL) {
    const parsed = parseInt(env.MONITORING_ALERT_COOLDOWN_CRITICAL, 10);
    if (!isNaN(parsed)) config.alertFrequency.cooldownCriticalSeconds = parsed;
  }
  ...
  if (env.MONITORING_EVENT_REALTIME_INTERVAL_SEC) {
    ...
    if (!isNaN(parsed)) config.eventCollection.realtimeIntervalSeconds = parsed;
  }
  ...
  if (env.MONITORING_NOTIFICATION_EMAIL_ENABLED) {
    config.eventNotification.emailEnabled = env.MONITORING_NOTIFICATION_EMAIL_ENABLED !== 'false';
  }
  
  if (env.MONITORING_NOTIFICATION_SMS_ENABLED) {
    config.eventNotification.smsEnabled = env.MONITORING_NOTIFICATION_SMS_ENABLED === 'true';
  }
  
  if (env.MONITORING_NOTIFICATION_TIMEOUT_MS) {
    const parsed = parseInt(env.MONITORING_NOTIFICATION_TIMEOUT_MS, 10);
    if (!isNaN(parsed)) config.eventNotification.notificationTimeoutMs = parsed;
  }

  // 事件存储配置
  if (env.MONITORING_EVENT_REALTIME_RETENTION_HOURS) {
    const parsed = parseInt(env.MONITORING_EVENT_REALTIME_RETENTION_HOURS, 10);
    if (!isNaN(parsed)) config.eventStorage.realtimeRetentionHours = parsed;
  }
  
  if (env.MONITORING_EVENT_COMPRESSION_ENABLED) {
    config.eventStorage.compressionEnabled = env.MONITORING_EVENT_COMPRESSION_ENABLED !== 'false';
  }

  // 告警升级配置
  if (env.MONITORING_ALERT_ESCALATION_ENABLED) {
    config.alertEscalation.escalationEnabled = env.MONITORING_ALERT_ESCALATION_ENABLED !== 'false';
  }
  
  if (env.MONITORING_ALERT_ESCALATION_LEVEL1_MINUTES) {
    const parsed = parseInt(env.MONITORING_ALERT_ESCALATION_LEVEL1_MINUTES, 10);
    if (!isNaN(parsed)) config.alertEscalation.level1EscalationMinutes = parsed;
  }
  ...
  if (env.MONITORING_AUTO_ANALYSIS_ENABLED) {
    config.enableAutoAnalysis = env.MONITORING_AUTO_ANALYSIS_ENABLED !== 'false';
    console.warn('[DEPRECATED] MONITORING_AUTO_ANALYSIS_ENABLED is deprecated. Use MONITORING_AUTO_ANALYSIS instead.');
  }
...
export class MonitoringEventsUtils {
  ...
  static calculateRetryDelay(
    ...
    let delay = config.initialRetryDelayMs * Math.pow(config.backoffMultiplier, attempt);
  ...
  static shouldAggregateEvent(
    ...
    return event1.type === event2.type && 
           ...
           Math.abs(event1.timestamp - event2.timestamp) <= config.aggregationWindowSeconds * 1000;
  }

  /**
   * 获取通知渠道列表
   */
  static getEnabledNotificationChannels(config: EventNotificationConfig): NotificationChannel[] {
   ...
   * 获取告警级别的升级时间
   */
  static getEscalationTime(level: number, config: AlertEscalationConfig): number {
    switch (level) {
      case 1: return config.level1EscalationMinutes * 60 * 1000; // 转换为毫秒
      case 2: return config.level2EscalationMinutes * 60 * 1000;
      case 3: return config.level3EscalationMinutes * 60 * 1000;
      default: return config.level3EscalationMinutes * 60 * 1000;
    }
  }
  ...
  static shouldSendNotification(
    ...
    // 静默时间内，只发送紧急通知
    if (config.isQuietHours() && !config.eventNotification.emergencyDuringQuietHours) {
      return false;
    }
  ...
  static getEnvironmentVariableMapping(): Record<string, string> {
    return {
      ...
      'alertFrequency.cooldownEmergencySeconds': 'MONITORING_ALERT_COOLDOWN_EMERGENCY',
      'alertFrequency.cooldownCriticalSeconds': 'MONITORING_ALERT_COOLDOWN_CRITICAL',
      'alertFrequency.cooldownWarningSeconds': 'MONITORING_ALERT_COOLDOWN_WARNING',
      'alertFrequency.cooldownInfoSeconds': 'MONITORING_ALERT_COOLDOWN_INFO',
      'alertFrequency.consecutiveThreshold': 'MONITORING_ALERT_CONSECUTIVE_THRESHOLD',
      
      // 事件重试
      'eventRetry.maxRetryAttempts': 'MONITORING_EVENT_MAX_RETRY_ATTEMPTS',
      'eventRetry.initialRetryDelayMs': 'MONITORING_EVENT_INITIAL_RETRY_DELAY_MS',
      'eventRetry.backoffMultiplier': 'MONITORING_EVENT_RETRY_BACKOFF_MULTIPLIER',
      ...
      // 事件收集
      'eventCollection.realtimeIntervalSeconds': 'MONITORING_EVENT_REALTIME_INTERVAL_SEC',
      'eventCollection.highFrequencyIntervalSeconds': 'MONITORING_EVENT_HIGH_FREQUENCY_INTERVAL_SEC',
      'eventCollection.normalIntervalSeconds': 'MONITORING_EVENT_NORMAL_INTERVAL_SEC',
      'eventCollection.lowFrequencyIntervalSeconds': 'MONITORING_EVENT_LOW_FREQUENCY_INTERVAL_SEC',
      'eventCollection.bufferSize': 'MONITORING_EVENT_BUFFER_SIZE',
      'eventCollection.aggregationEnabled': 'MONITORING_EVENT_AGGREGATION_ENABLED',
      ...
      'eventNotification.webhookEnabled': 'MONITORING_NOTIFICATION_WEBHOOK_ENABLED',
      'eventNotification.slackEnabled': 'MONITORING_NOTIFICATION_SLACK_ENABLED',
      'eventNotification.dingtalkEnabled': 'MONITORING_NOTIFICATION_DINGTALK_ENABLED',
      'eventNotification.notificationTimeoutMs': 'MONITORING_NOTIFICATION_TIMEOUT_MS',
      'eventNotification.quietHoursStart': 'MONITORING_NOTIFICATION_QUIET_HOURS_START',
      'eventNotification.quietHoursEnd': 'MONITORING_NOTIFICATION_QUIET_HOURS_END',
      
      // 事件存储
      'eventStorage.realtimeRetentionHours': 'MONITORING_EVENT_REALTIME_RETENTION_HOURS',
      'eventStorage.hourlyRetentionHours': 'MONITORING_EVENT_HOURLY_RETENTION_HOURS',
      'eventStorage.dailyRetentionHours': 'MONITORING_EVENT_DAILY_RETENTION_HOURS',
      'eventStorage.monthlyRetentionHours': 'MONITORING_EVENT_MONTHLY_RETENTION_HOURS',
      ...
      // 告警升级
      'alertEscalation.escalationEnabled': 'MONITORING_ALERT_ESCALATION_ENABLED',
      'alertEscalation.level1EscalationMinutes': 'MONITORING_ALERT_ESCALATION_LEVEL1_MINUTES',
      'alertEscalation.level2EscalationMinutes': 'MONITORING_ALERT_ESCALATION_LEVEL2_MINUTES',
      'alertEscalation.level3EscalationMinutes': 'MONITORING_ALERT_ESCALATION_LEVEL3_MINUTES',
      'alertEscalation.autoResolveHours': 'MONITORING_ALERT_AUTO_RESOLVE_HOURS',
      'alertEscalation.businessHoursOnly': 'MONITORING_ALERT_ESCALATION_BUSINESS_HOURS_ONLY',
