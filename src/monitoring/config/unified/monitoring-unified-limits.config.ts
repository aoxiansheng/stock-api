...
export class MonitoringUnifiedLimitsConfig {
  ...
  dataProcessingBatch: DataProcessingBatchConfig = new DataProcessingBatchConfig();
 ...
 * ```
 */
export const monitoringUnifiedLimitsConfig = registerAs('monitoringUnifiedLimits', (): MonitoringUnifiedLimitsConfig => {
  ...
  // 1. 优先检查新的核心环境变量
  const defaultBatchSize = process.env.MONITORING_DEFAULT_BATCH_SIZE ? parseInt(process.env.MONITORING_DEFAULT_BATCH_SIZE, 10) : 10;
  ...
  if (env.MONITORING_ALERT_BATCH_SMALL) {
    ...
    if (!isNaN(parsed)) {
      ...
      console.warn('[DEPRECATED] MONITORING_ALERT_BATCH_SMALL is deprecated. Use MONITORING_DEFAULT_BATCH_SIZE instead.');
    }
  }
  ...
  if (env.MONITORING_ALERT_BATCH_MEDIUM) {
    ...
    if (!isNaN(parsed)) {
      ...
      console.warn('[DEPRECATED] MONITORING_ALERT_BATCH_MEDIUM is deprecated. Use MONITORING_DEFAULT_BATCH_SIZE instead.');
    }
  }
  ...
  if (env.MONITORING_ALERT_BATCH_LARGE) {
    ...
    if (!isNaN(parsed)) {
      ...
      console.warn('[DEPRECATED] MONITORING_ALERT_BATCH_LARGE is deprecated. Use MONITORING_DEFAULT_BATCH_SIZE instead.');
    }
  }
  ...
  if (env.MONITORING_ALERT_BATCH_MAX) {
    ...
    if (!isNaN(parsed)) {
      ...
      console.warn('[DEPRECATED] MONITORING_ALERT_BATCH_MAX is deprecated. Use MONITORING_DEFAULT_BATCH_SIZE instead.');
    }
  }
  ...
  if (env.MONITORING_DATA_BATCH_STANDARD) {
    ...
    if (!isNaN(parsed)) {
      ...
      console.warn('[DEPRECATED] MONITORING_DATA_BATCH_STANDARD is deprecated. Use MONITORING_DEFAULT_BATCH_SIZE instead.');
    }
  }
  ...
  if (env.MONITORING_DATA_BATCH_HIGH_FREQUENCY) {
    ...
    if (!isNaN(parsed)) {
      ...
      console.warn('[DEPRECATED] MONITORING_DATA_BATCH_HIGH_FREQUENCY is deprecated. Use MONITORING_DEFAULT_BATCH_SIZE instead.');
    }
  }
  ...
  if (env.MONITORING_DATA_BATCH_ANALYSIS) {
    ...
    if (!isNaN(parsed)) {
      ...
      console.warn('[DEPRECATED] MONITORING_DATA_BATCH_ANALYSIS is deprecated. Use MONITORING_DEFAULT_BATCH_SIZE instead.');
    }
  }
...
export class MonitoringLimitsUtils {
   ...
   * 验证批量大小是否在合理范围内
   */
  static isValidBatchSize(batchSize: number, min: number = 1, max: number = 1000): boolean {
