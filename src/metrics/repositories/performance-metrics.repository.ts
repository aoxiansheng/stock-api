...
export class PerformanceMetricsRepository {
  ...
  async recordRequest(
    ...
    try {
      ...
      pipeline.ltrim(responseTimeKey, 0, PERFORMANCE_LIMITS.MAX_RESPONSE_TIMES_PER_ENDPOINT - 1);
  ...
  async recordDatabaseQuery(duration: number): Promise<void> {
    ...
    try {
      ...
      // 保留最新的 N 条记录
      pipeline.zremrangebyrank(key, 0, -PERFORMANCE_LIMITS.MAX_DB_QUERY_TIMES -1);
  ...
  async getEndpointStats(): Promise<{ key: string; stats: Record<string, string>; responseTimes: string[] }[]> {
    ...
    try {
      ...
      do {
        const scanResult = await this.redis.scan(cursor, 'MATCH', scanPattern, 'COUNT', 100);
        cursor = scanResult[0];
      ...
      for (let i = 0; i < endpointKeys.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (key) => {
          try {
            ...
            const [statsData, responseTimesStrings] = await Promise.race([dataPromise, timeoutPromise]);
            
            if (statsData && Object.keys(statsData).length > 0) {
              return { key, stats: statsData, responseTimes: responseTimesStrings };
            }
            return null;
          } catch (error) {
      ...
      this.logger.error({ operation, error: error.stack }, '从Redis获取端点统计信息失败');
      return [];
    }
  }

  async getDatabaseQueryTimes(startDate?: string, endDate?: string): Promise<string[]> {
    const operation = 'getDatabaseQueryTimes';
      try {
          ...
          const results = await this.redis.zrangebyscore(PERFORMANCE_REDIS_KEYS.DB_QUERY_TIMES_KEY, min, max);
          // 从 member 中解析出 duration
          return results.map(member => member.split(':')[1]);
      } catch (error) {
          this.logger.error({ operation, error: error.stack }, '从Redis获取数据库查询时间失败');
          return [];
      }
  }

  async getRedisInfoPayload(): Promise<{info: string, stats: string, clients: string}> {
      try {
          // 检查Redis连接状态
          if (!this.redis || this.redis.status !== 'ready') {
          ...
          const [info, stats, clients] = await Promise.race([infoPromise, timeoutPromise]);
          ...
          if (error.message.includes('ECONNRESET') || error.message.includes('Connection is closed')) {
              ...
              this.logger.error({ operation: 'getRedisInfoPayload', error: error.stack }, '从Redis获取Info失败');
          }
          return null;
      }
  }
  ...
  async flushMetrics(metrics: PerformanceMetric[]): Promise<void> {
    ...
    try {
      ...
      this.logger.error({ operation, error: error.stack }, '刷新指标到Redis失败');
    }
  }
  ...
  async cleanupOldMetrics(): Promise<void> {
    ...
    try {
      ...
      const keys = await this.redis.keys(`${PERFORMANCE_REDIS_KEYS.METRICS_PREFIX}:*`);
