...
export class PerformanceInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    return next.handle().pipe(
      tap(() => {
        ...
        this.handleRequestComplete(endpoint, method, duration, true, requestId, response, monitoringConfig);
      }),
      catchError((error) => {
        const duration = Date.now() - startTime;
        this.handleRequestComplete(endpoint, method, duration, false, requestId, response, monitoringConfig);
  ...
  private handleRequestComplete(
    ...
    // 记录慢请求
    if (monitoringConfig.trackSlowRequests && duration > monitoringConfig.slowRequestThreshold) {
      this.logger.warn(
        `慢请求检测: ${method} ${endpoint} - ${duration}ms`,
        { requestId, duration, endpoint, method }
      );
    }
   ...
   * 获取性能监控配置
   */
  private getMonitoringConfig(context: ExecutionContext): PerformanceMonitoringConfig {
  ...
  private setResponseHeader(response: Response, name: string, value: string): void {
    try {
  ...
  private normalizeEndpoint(path: string): string {
    ...
    return path
      .replace(/\/[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}/g, "/:uuid") // UUID (最具体的模式)
