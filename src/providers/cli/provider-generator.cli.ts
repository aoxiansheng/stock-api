...
export class ProviderGeneratorCLI {
  ...
  static registerCommands(program: Command): void {
    ...
    program
      ...
      .option('-c, --capabilities <capabilities>', '能力列表（逗号分隔）', 'get-stock-quote')
  ...
  private static async generateProvider(
    ...
    try {
      ...
      // 生成提供商模板
      const generatedFiles = await SmartErrorHandler.generateProviderTemplate(name, {
        capabilities,
      ...
      console.log('\n📖 接下来的步骤:');
      console.log(`1. 查看生成的文档: ${SmartPathResolver.getRelativePath(providerPath)}/README.md`);
      console.log(`2. 实现能力逻辑: ${SmartPathResolver.getRelativePath(providerPath)}/capabilities/`);
  ...
  private static async validateProvider(
    ...
    try {
      ...
      if (options.verbose) {
        console.log(`\n🔍 详细信息:`);
        console.log(`   - 提供商路径: ${SmartPathResolver.getProviderPath(name)}`);
        console.log(`   - 能力目录: ${SmartPathResolver.getProviderCapabilitiesPath(name)}`);
      }
   ...
   * 列出所有提供商
   */
  private static async listProviders(options: { detailed: boolean }): Promise<void> {
    try {
      console.log(`📋 扫描提供商...`);
      ...
      providers.forEach((provider, index) => {
        ...
        console.log(`   📁 路径: ${SmartPathResolver.getRelativePath(SmartPathResolver.getProviderPath(provider.name))}`);
        console.log(`   🔧 能力数: ${provider.capabilities.length}`);
        
        if (options.detailed) {
          if (provider.capabilities.length > 0) {
            console.log(`   ⚡ 能力列表:`);
            provider.capabilities.forEach(cap => console.log(`      - ${cap}`));
          }
          console.log(`   🤖 自动发现: ${provider.autoDiscovered ? '是' : '否'}`);
        }
      });
   ...
   * 扫描提供商
   */
  private static async scanProviders(options: { validate: boolean; autoFix: boolean }): Promise<void> {
    try {
      console.log(`🔍 开始扫描提供商...`);
      ...
      if (providers.length > 0) {
        ...
        providers.forEach(provider => {
          console.log(`   - ${provider.name} (${provider.capabilities.length} 个能力)`);
        });
      }
  ...
  private static async generateCapability(
    ...
    try {
      ...
      console.log(`📁 文件位置: ${SmartPathResolver.getRelativePath(capabilityPath)}`);
      console.log(`\n📖 接下来的步骤:`);
  ...
  private static async generateCapabilityFile(
    ...
    const className = this.toPascalCase(capability.replace(/-/g, ' ')) + 'Capability';
    const markets = options.markets.split(',').map((m: string) => `'${m.trim()}'`).join(', ');
