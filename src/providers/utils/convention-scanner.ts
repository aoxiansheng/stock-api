...
export class ConventionScanner {
  ...
  async scanProviders(options: ScanOptions = {}): Promise<ScanResult> {
    const finalOptions = {
      ...ConventionScanner.getDefaultOptions(),
      ...options,
    };
   ...
   * 扫描特定提供商的能力
   */
  async scanProviderCapabilities(
    providerName: string,
    options: ScanOptions = {},
  ): Promise<{
    capabilities: string[];
  ...
  private async analyzeProvider(
    ...
    try {
      ...
      // 扫描能力
      const { capabilities } = await this.scanProviderCapabilities(
        name,
        options,
      );
   ...
   * 验证提供商结构
   */
  private async validateProviderStructure(
    providerName: string,
  ): Promise<boolean> {
    ...
    try {
      ...
      const hasMainFile = possibleMainFiles.some((file) =>
        SmartPathResolver.pathExists(file),
      );
    ...
    for (const file of possibleMainFiles) {
      if (SmartPathResolver.pathExists(file)) {
        try {
          ...
          ConventionScanner.logger.debug(
            `加载提供商主文件失败: ${file}, ${error.message}`,
          );
        }
      }
    }
