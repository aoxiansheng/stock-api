...
export const streamStockQuote: IStreamCapability = {
  ...
  supportedSymbolFormats: ['700.HK', '00700.HK', '09618.HK', '00700', '09618', '000001.SZ', '600000.SH', 'AAPL.US'],
  rateLimit: {
    maxConnections: 100,
   ...
   * 初始化 WebSocket 连接
   */
  async initialize(contextService: LongportStreamContextService): Promise<void> {
   ...
   * 订阅符号数据流
   */
  async subscribe(symbols: string[], contextService: LongportStreamContextService): Promise<void> {
    const logger = createLogger('LongportStreamStockQuote');
    ...
    try {
      ...
      // 使用统一的符号验证逻辑
      const { valid: validSymbols, invalid: invalidSymbols } = SymbolValidationUtils.validateSymbols(symbols);
      ...
      if (SymbolValidationUtils.isSymbolCountExceeded(symbols, 500)) {
        throw new Error(`符号数量超过LongPort限制，最多支持500个，当前：${symbols.length}`);
      }
   ...
   * 取消订阅符号数据流
   */
  async unsubscribe(symbols: string[], contextService: LongportStreamContextService): Promise<void> {
    const logger = createLogger('LongportStreamStockQuote');
