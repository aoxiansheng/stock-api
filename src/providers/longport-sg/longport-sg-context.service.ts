  ...
  async onModuleInit() {
    ...
    this.initialize().catch(error => {
      this.logger.warn(
        ...
        '启动时 LongPort SG SDK 异步初始化失败，将在首次使用时重试',
      );
    });
  }
  ...
  private initialize(): Promise<void> {
    ...
    this.initializationPromise = (async () => {
      try {
        ...
        process.env.LONGPORT_ACCESS_TOKEN = process.env.LONGPORT_SG_ACCESS_TOKEN;
  ...
  async close(): Promise<void> {
    if (this.quoteContext) {
      ...
      try {
        ...
        this.logger.error({ error: error.stack }, '关闭 LongPort SDK 连接时发生错误');
      }
    }
  }
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      if (error.message?.includes('101004')) {
        this.logger.log('LongPort 连接测试成功（API返回标的未找到，但连接正常）');
        return true;
      }
