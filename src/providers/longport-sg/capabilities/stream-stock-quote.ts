...
export const streamStockQuote: IStreamCapability = {
   ...
   */
  async initialize(
    contextService: LongportSgStreamContextService,
  ): Promise<void> {
    ...
    try {
      ...
      if (!contextService) {
        throw new Error("LongportSgStreamContextService 未提供");
      }
  ...
  async cleanup(): Promise<void> {
    ...
    try {
      ...
      // 清理操作由 LongportSgStreamContextService 处理
      // 这里只需要清理能力层面的资源
   ...
   * 检查 WebSocket 连接状态
   */
  isConnected(contextService?: LongportSgStreamContextService): boolean {
