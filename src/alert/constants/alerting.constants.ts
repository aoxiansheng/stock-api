...
export const ALERTING_VALIDATION_RULES = Object.freeze({
  RULE_NAME_PATTERN: /^[\u4e00-\u9fff\u3400-\u4dbf\uff00-\uffef\u3000-\u303fa-zA-Z0-9\s\-_\.]+$/,
...
export class AlertingTemplateUtil {
  ...
  static generateRuleId(): string {
    const timestamp = Date.now().toString(ALERTING_CONFIG.RULE_ID_TIMESTAMP_BASE);
    const random = Math.random()
      .toString(ALERTING_CONFIG.RULE_ID_TIMESTAMP_BASE)
      .substring(ALERTING_CONFIG.RULE_ID_RANDOM_START, ALERTING_CONFIG.RULE_ID_RANDOM_START + ALERTING_CONFIG.RULE_ID_RANDOM_LENGTH);
   ...
   * @returns 格式化后的消息
   */
  static formatAlertMessage(template: string, context: Record<string, any>): string {
   ...
   * @returns 优先级分数
   */
  static calculatePriorityScore(severity: string, value: number, threshold: number): number {
