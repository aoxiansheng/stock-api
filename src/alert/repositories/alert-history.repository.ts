...
export class AlertHistoryRepository {
  ...
  async create(alertData: AlertCreateData & { id: string; startTime: Date; status: AlertStatus }): Promise<IAlert> {
    const alert = new this.alertHistoryModel(alertData);
    const savedAlert = await alert.save();
    return savedAlert.toObject();
  }

  async update(alertId: string, updateData: AlertUpdateData): Promise<IAlert | null> {
  ...
  async getStatistics(): Promise<{
    ...
    const [
      ...
      this.alertHistoryModel.aggregate([
        { $match: { status: { $in: [AlertStatus.FIRING, AlertStatus.ACKNOWLEDGED] } } },
        { $group: { _id: '$severity', count: { $sum: 1 } } },
      ]),
      this.alertHistoryModel.countDocuments({ startTime: { $gte: today } }),
      this.alertHistoryModel.countDocuments({ startTime: { $gte: today }, status: AlertStatus.RESOLVED }),
      this.alertHistoryModel.aggregate([
        {
          $match: {
        ...
        { $project: { resolutionTime: { $subtract: ['$resolvedAt', '$startTime'] } } },
