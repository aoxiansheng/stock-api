...
export class AlertRuleRepository {
  ...
  async create(createRuleDto: CreateAlertRuleDto & { id: string }): Promise<IAlertRule> {
  ...
  async update(ruleId: string, updateRuleDto: UpdateAlertRuleDto): Promise<IAlertRule> {
  ...
  async findById(ruleId: string): Promise<IAlertRule | null> {
    const rule = await this.alertRuleModel.findOne({ id: ruleId }).lean().exec();
    if (!rule) {
