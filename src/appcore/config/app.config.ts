...
export const createAppConfig = (): Partial<AppConfig> => ({
  ...
  security: {
    ...
    session: {
      ...
      lockoutDuration:
        parseInt(process.env.ACCOUNT_LOCKOUT_DURATION, 10) || 1800,
    },
    rateLimit: {
  ...
  notification: {
    ...
    defaultChannels: process.env.NOTIFICATION_DEFAULT_CHANNELS?.split(",") || [
      "email",
      "log",
    ],
    timeouts: {
      ...
      webhook: parseInt(
        process.env.NOTIFICATION_WEBHOOK_TIMEOUT || "10000",
        10,
      ),
      slack: parseInt(process.env.NOTIFICATION_SLACK_TIMEOUT || "15000", 10),
      dingtalk: parseInt(
        process.env.NOTIFICATION_DINGTALK_TIMEOUT || "10000",
        10,
      ),
      default: parseInt(
        process.env.NOTIFICATION_DEFAULT_TIMEOUT || "15000",
        10,
      ),
    },
    retry: {
      maxAttempts: parseInt(
        process.env.NOTIFICATION_MAX_RETRY_ATTEMPTS || "3",
        10,
      ),
      initialDelay: parseInt(
        process.env.NOTIFICATION_INITIAL_RETRY_DELAY || "1000",
        10,
      ),
      backoffMultiplier: parseFloat(
        process.env.NOTIFICATION_RETRY_BACKOFF_MULTIPLIER || "2",
      ),
      maxDelay: parseInt(
        process.env.NOTIFICATION_MAX_RETRY_DELAY || "30000",
        10,
      ),
    },
    batch: {
