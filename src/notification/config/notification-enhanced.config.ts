...
export default registerAs("notification", () => {
  const rawConfig = {
    ...
    dingtalkTimeout:
      parseInt(process.env.NOTIFICATION_DINGTALK_TIMEOUT, 10) || 10000,
    defaultTimeout:
    ...
    criticalPriorityWeight:
      parseInt(process.env.NOTIFICATION_CRITICAL_PRIORITY_WEIGHT, 10) || 100,
    urgentPriorityWeight:
      parseInt(process.env.NOTIFICATION_URGENT_PRIORITY_WEIGHT, 10) || 80,
    highPriorityWeight:
      parseInt(process.env.NOTIFICATION_HIGH_PRIORITY_WEIGHT, 10) || 60,
    normalPriorityWeight:
      parseInt(process.env.NOTIFICATION_NORMAL_PRIORITY_WEIGHT, 10) || 40,
    lowPriorityWeight:
      parseInt(process.env.NOTIFICATION_LOW_PRIORITY_WEIGHT, 10) || 20,
    variableNameMinLength:
      parseInt(process.env.NOTIFICATION_VARIABLE_NAME_MIN_LENGTH, 10) || 1,
    variableNameMaxLength:
      parseInt(process.env.NOTIFICATION_VARIABLE_NAME_MAX_LENGTH, 10) || 50,
    minTemplateLength:
      parseInt(process.env.NOTIFICATION_MIN_TEMPLATE_LENGTH, 10) || 1,
    maxTemplateLength:
      parseInt(process.env.NOTIFICATION_MAX_TEMPLATE_LENGTH, 10) || 10000,
    titleMaxLength:
      parseInt(process.env.NOTIFICATION_TITLE_MAX_LENGTH, 10) || 200,
    contentMaxLength:
      parseInt(process.env.NOTIFICATION_CONTENT_MAX_LENGTH, 10) || 2000,
    ...
    defaultTemplate:
      process.env.NOTIFICATION_DEFAULT_TEMPLATE ||
      "Default notification: {message}",
    emailSubjectTemplate:
      process.env.NOTIFICATION_EMAIL_SUBJECT_TEMPLATE || "[Alert] {title}",
    ...
    getChannelTimeout(channelType: string): number {
      ...
      return (
        channelTimeouts[channelType.toLowerCase()] || rawConfig.defaultTimeout
      );
    },
    ...
    getPriorityWeight(priority: string): number {
      ...
      return (
        priorityWeights[priority.toLowerCase()] ||
        rawConfig.normalPriorityWeight
      );
    },
  };
  ...
  if (
    !rawConfig.getChannelTimeout ||
    typeof rawConfig.getChannelTimeout !== "function"
  ) {
