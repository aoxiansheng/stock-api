...
export class AlertToNotificationAdapter {
  ...
  private mapSeverityToPriority(
    alertSeverity: GenericAlertSeverity,
  ): NotificationPriority {
  ...
  private buildMessage(alertEvent: GenericAlertEvent): string {
    ...
    messageParts.push(
      `- 时间: ${alertEvent.timestamp.toLocaleString("zh-CN")}`,
    );
    ...
    if (alertEvent.eventData?.resolutionComment) {
      messageParts.push(
        `- 解决备注: ${alertEvent.eventData.resolutionComment}`,
      );
    }
    if (alertEvent.eventData?.acknowledgmentComment) {
      messageParts.push(
        `- 确认备注: ${alertEvent.eventData.acknowledgmentComment}`,
      );
    }
    if (alertEvent.eventData?.suppressionReason) {
      messageParts.push(
        `- 抑制原因: ${alertEvent.eventData.suppressionReason}`,
      );
    }
   ...
   * 提取通知渠道类型
   */
  private extractChannelTypes(
    alertEvent: GenericAlertEvent,
  ...
  private getDefaultChannelTypes(
    ...
    const severityChannelMap: Record<
      GenericAlertSeverity,
      NotificationChannelType[]
    > = {
      [GenericAlertSeverity.LOW]: [NotificationChannelType.LOG],
      [GenericAlertSeverity.MEDIUM]: [
  ...
  private extractRecipients(
    ...
    // 从事件数据中提取接收者信息
    if (
      alertEvent.eventData?.recipients &&
      Array.isArray(alertEvent.eventData.recipients)
    ) {
      return alertEvent.eventData.recipients.filter(
        (recipient: any) =>
    ...
    // 从metadata中提取
    if (
      alertEvent.context?.metadata?.recipients &&
      Array.isArray(alertEvent.context.metadata.recipients)
    ) {
      return alertEvent.context.metadata.recipients.filter(
        (recipient: any) =>
          typeof recipient === "string" && recipient.trim().length > 0,
      );
    }
   ...
   * 验证Alert事件有效性
   */
  validateAlertEvent(alertEvent: GenericAlertEvent): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
