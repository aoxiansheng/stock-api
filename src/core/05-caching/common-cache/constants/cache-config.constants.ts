...
export const CACHE_CONFIG = {
  ...
  DECOMPRESSION: {
    ...
    MAX_CONCURRENT: parseInt(
      process.env.CACHE_DECOMPRESSION_MAX_CONCURRENT || "10",
      10,
    ), // 10个 - 最大并发解压数
    MAX_RETRY_ATTEMPTS: parseInt(
      process.env.CACHE_DECOMPRESSION_MAX_RETRY || "3",
      10,
    ), // 3次 - 解压失败重试次数
    TIMEOUT_MS: parseInt(
      process.env.CACHE_DECOMPRESSION_TIMEOUT_MS || "5000",
      10,
    ), // 5000ms - 解压操作超时时间
  ...
  DECOMPRESSION_MONITORING: {
    ...
    PERFORMANCE_ALERT_THRESHOLD_MS: parseInt(
      process.env.CACHE_PERF_ALERT_THRESHOLD || "50",
      10,
    ), // 50ms - 性能告警阈值
    ERROR_RATE_ALERT_THRESHOLD: parseFloat(
      process.env.CACHE_ERROR_ALERT_THRESHOLD || "0.01",
    ), // 1% - 错误率告警阈值
  ...
  METRICS: {
    HISTOGRAM_BUCKETS: {
      ...
      // TTL剩余时间分布（s秒）
      TTL_REMAINING: [
        30, 60, 120, 300, 600, 1200, 1800, 3600, 7200, 14400, 28800, 86400,
      ],
      // 批量操作大小分布（条数）
