...
export class CacheConfigValidator {
  ...
  validateConfig(): CacheConfigValidationResult {
    ...
    // 验证Redis配置
    const redisConfig = this.validateRedisConfig(errors, warnings, recommendations);
    
    // 验证压缩配置
    const compressionConfig = this.validateCompressionConfig(errors, warnings, recommendations);
    
    // 验证解压缩配置
    const decompressionConfig = this.validateDecompressionConfig(errors, warnings, recommendations);
    
    // 验证批处理配置
    const batchConfig = this.validateBatchConfig(errors, warnings, recommendations);
    
    // 验证TTL配置
    const ttlConfig = this.validateTTLConfig(errors, warnings, recommendations);
    
    // 验证安全配置
    const securityConfig = this.validateSecurityConfig(errors, warnings, recommendations);
    ...
    if (!valid) {
      ...
      this.logger.warn('缓存配置验证通过但有建议', { warnings, recommendations });
    } else {
  ...
  private validateRedisConfig(
    ...
    const commandTimeout = this.configService.get<number>('redis.commandTimeout', 5000);
    const maxRetriesPerRequest = this.configService.get<number>('redis.maxRetriesPerRequest', 3);
    const retryDelayOnFailover = this.configService.get<number>('redis.retryDelayOnFailover', 100);
  ...
  private validateCompressionConfig(
    ...
    const enabled = this.configService.get<boolean>('cache.compression.enabled', true);
    const thresholdBytes = this.configService.get<number>('cache.compression.thresholdBytes', 1024);
    const algorithm = this.configService.get<string>('cache.compression.algorithm', 'gzip');
    ...
    if (!supportedAlgorithms.includes(algorithm)) {
      errors.push(`不支持的压缩算法: ${algorithm}，支持的算法: ${supportedAlgorithms.join(', ')}`);
    }

    // 性能建议
    if (enabled && algorithm === 'br') {
      recommendations.push('Brotli 压缩比更高但 CPU 消耗更大，适合低频访问的数据');
  ...
  private validateDecompressionConfig(
    ...
    const maxConcurrent = this.configService.get<number>('cache.decompression.maxConcurrent', 10);
    const timeoutMs = this.configService.get<number>('cache.decompression.timeoutMs', 5000);
    ...
    if (maxConcurrent > cpuCores * 2) {
      recommendations.push(`当前 CPU 核心数: ${cpuCores}，建议解压缩并发数不超过 ${cpuCores * 2}`);
    }
  ...
  private validateBatchConfig(
    ...
    const maxBatchSize = this.configService.get<number>('cache.batch.maxBatchSize', 100);
    const timeoutMs = this.configService.get<number>('cache.batch.timeoutMs', 10000);
  ...
  private validateTTLConfig(
    ...
    const defaultSeconds = this.configService.get<number>('cache.ttl.defaultSeconds', 3600);
    const maxSeconds = this.configService.get<number>('cache.ttl.maxSeconds', 86400);
    const minSeconds = this.configService.get<number>('cache.ttl.minSeconds', 60);
    ...
    // 性能建议
    if (defaultSeconds < 300) { // 5分钟
      recommendations.push('默认TTL较短，适合实时性要求高的数据');
    } else if (defaultSeconds > 7200) { // 2小时
  ...
  private validateSecurityConfig(
    ...
    const enableMetrics = this.configService.get<boolean>('cache.security.enableMetrics', true);
    const sanitizeKeys = this.configService.get<boolean>('cache.security.sanitizeKeys', true);
   ...
   * @returns 是否适合生产环境
   */
  isProductionReady(config: CacheValidatedConfig): { ready: boolean; issues: string[] } {
    const issues: string[] = [];
