...
export class CacheCompressionService {
  ...
  async compress(data: any): Promise<{ 
    ...
    try {
      ...
      if (compressionRatio > CACHE_CONFIG.COMPRESSION.SAVING_RATIO) {
        this.logger.debug(`Compression ratio ${compressionRatio.toFixed(3)} too high, using original data`);
      ...
      this.logger.debug(`Compressed data from ${originalSize} to ${compressedSize} bytes (ratio: ${compressionRatio.toFixed(3)})`);
   ...
   * @returns 是否有效
   */
  async validateCompressedData(compressedData: string, metadata: CacheMetadata): Promise<boolean> {
    try {
   ...
   * @returns 统计信息
   */
  getCompressionStats(originalData: any, compressedData: string, metadata: CacheMetadata) {
    const originalSize = this.getDataSize(originalData);
    const compressedSize = metadata.compressedSize || Buffer.byteLength(compressedData, 'utf8');
