...
export class SmartCacheConfigValidator {
  ...
  static validateTTL(ttl: number, strategyName: string): string[] {
    ...
    if (ttl > SMART_CACHE_CONSTANTS.TTL_SECONDS.ADAPTIVE_MAX_S) {
      errors.push(`${strategyName}: TTL过大，最大值: ${SMART_CACHE_CONSTANTS.TTL_SECONDS.ADAPTIVE_MAX_S}秒`);
    }
  ...
  static validateConcurrency(concurrency: number): string[] {
    ...
    if (concurrency < MIN_CONCURRENT_UPDATES_COUNT || concurrency > MAX_CONCURRENT_UPDATES_COUNT) {
      errors.push(
  ...
  static validateInterval(interval: number, intervalName: string): string[] {
    ...
    if (interval <= 0) {
      errors.push(`${intervalName}: 时间间隔必须为正数，当前值: ${interval}毫秒`);
    }
    
    // 最小间隔不能小于1秒
    if (interval < 1000) {
      errors.push(`${intervalName}: 时间间隔不能小于1秒，当前值: ${interval}毫秒`);
    }
    
    // 最大间隔不能超过1天
    if (interval > 24 * 60 * 60 * 1000) {
      errors.push(`${intervalName}: 时间间隔不能超过1天，当前值: ${interval}毫秒`);
    }
   ...
   * @returns 验证错误数组
   */
  static validateAdaptiveTtlRange(minTtl: number, maxTtl: number, baseTtl: number): string[] {
    const errors: string[] = [];
    ...
    if (baseTtl < minTtl || baseTtl > maxTtl) {
      errors.push(`自适应策略: 基础TTL(${baseTtl})必须在最小TTL(${minTtl})和最大TTL(${maxTtl})之间`);
    }
   ...
   * @returns 验证错误数组
   */
  static validateMarketAwareConfig(openTtl: number, closedTtl: number, checkInterval: number): string[] {
    const errors: string[] = [];
