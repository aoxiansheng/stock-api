...
export class StreamCacheModule implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    console.log(
      `📡 StreamCache Redis configuration: ${redisHost}:${redisPort} (DB: ${redisDb})`,
    );
    console.log(
      `⚙️  StreamCache config: Hot TTL=${STREAM_CACHE_CONFIG.TTL.HOT_CACHE_MS}ms, Warm TTL=${STREAM_CACHE_CONFIG.TTL.WARM_CACHE_SECONDS}s`,
    );
