...
export class StreamConnectionImpl implements StreamConnection {
  ...
  private async initializeConnection(): Promise<void> {
    try {
      ...
      // 检查能力实例是否已连接
      if (this.capabilityInstance.isConnected && this.capabilityInstance.isConnected(this.contextService)) {
        ...
        if (this.capabilityInstance.isConnected(this.contextService)) {
          ...
          throw new Error(`流连接初始化失败: ${this.provider}/${this.capability}`);
        }
      }
  ...
  private handleConnectionError(error: any): void {
    ...
    this.logger.error('流连接错误', sanitizeLogData({
      connectionId: this.id,
      provider: this.provider,
      capability: this.capability,
      error: error.message,
    ...
    // 自动重连逻辑
    if (this.options.autoReconnect && this.stats.reconnectCount < this.options.maxReconnectAttempts) {
  ...
  async sendHeartbeat(): Promise<boolean> {
    try {
      if (this.isConnected && typeof this.capabilityInstance.ping === 'function') {
  ...
  getStats(): StreamConnectionStats {
    const avgProcessingLatency = this.stats.processingTimes.length > 0
