...
export class ConnectionPoolManager {
  ...
  unregisterConnection(key: string, clientIP?: string): void {
    ...
    this.logger.debug('连接已注销', {
      ...
      ipCount: clientIP ? Math.max(0, (this.connectionsByIP.get(clientIP) || 0)) : 0,
    });
  }
  ...
  getStats() {
    ...
    return {
      ...
      byKey: Array.from(this.connectionsByKey.entries()).map(([key, count]) => ({
        key,
        current: count,
        max: config.maxPerKey,
   ...
   * 获取连接池利用率告警信息
   */
  getAlerts(): Array<{ type: string; message: string; level: 'warning' | 'critical' }> {
    const config = this.streamConfig.getConnectionConfig();
    const alerts = [];
    
    // 全局连接数告警
    const globalUtilization = (this.globalConnectionCount / config.maxGlobal) * 100;
