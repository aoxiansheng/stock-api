...
export class StreamRecoveryWorkerService implements OnModuleInit, OnModuleDestroy {
   ...
   * @param data 事件数据
   */
  private emitRecoveryEvent(metricName: string, data: {
    operation?: string;
    duration?: number;
    count?: number;
    ...
    setImmediate(() => {
      this.eventBus.emit(SYSTEM_STATUS_EVENTS.METRIC_COLLECTED, {
        ...
        metricType: 'business',
        metricName,
        metricValue: data.duration || data.count || data.data_points || data.batch_size || data.bytes || data.tokens || data.qps || 1,
        tags: {
   ...
   * @param data 事件数据
   */
  private emitPerformanceEvent(metricName: string, data: {
    operation?: string;
    duration?: number;
    qps?: number;
