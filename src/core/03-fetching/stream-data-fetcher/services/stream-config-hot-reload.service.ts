 ...
 */
@Injectable()
export class StreamConfigHotReloadService implements OnModuleInit, OnModuleDestroy {
  ...
  private currentConfig = {
    ...
    adaptiveConcurrency: {
      ...
      adjustmentFactor: parseFloat(process.env.CONCURRENCY_ADJUSTMENT_FACTOR || '0.2'),
      stabilizationPeriod: parseInt(process.env.STABILIZATION_PERIOD || '30000'),
    },
  ...
  private validateConfiguration(config: any): { isValid: boolean; errors: string[] } {
    ...
    if (config.adaptiveConcurrency.maxConcurrency < config.adaptiveConcurrency.minConcurrency) {
      errors.push('最大并发数必须大于等于最小并发数');
    }
    
    if (config.adaptiveConcurrency.adjustmentFactor <= 0 || config.adaptiveConcurrency.adjustmentFactor >= 1) {
    ...
    if (config.performanceThresholds.good <= config.performanceThresholds.excellent) {
      errors.push('良好性能阈值必须大于优秀阈值');
    }
    
    if (config.performanceThresholds.poor <= config.performanceThresholds.good) {
  ...
  private hasConfigChanged(section: string, newConfig: any): boolean {
    return JSON.stringify(this.currentConfig[section]) !== JSON.stringify(newConfig[section]);
  }
  ...
  private getConfigChanges(): any {
    // 简化实现：返回主要配置项的变化
    return {
      concurrencyChanged: this.currentConfig.adaptiveConcurrency.minConcurrency !== 
                         parseInt(process.env.MIN_CONCURRENCY || '2'),
      healthCheckChanged: this.currentConfig.healthCheck.timeoutMs !== 
  ...
  getServiceStatus() {
    return {
      ...
      signalHandlersEnabled: this.currentConfig.reloadSettings.enableSignalReload,
