...
export class StreamRecoveryConfigService {
  ...
  private loadConfig(): StreamRecoveryConfig {
    return {
      ...
      recovery: {
        maxRecoveryWindow: parseInt(process.env.RECOVERY_MAX_WINDOW || '300000'), // 5分钟
        batchSize: parseInt(process.env.RECOVERY_BATCH_SIZE || '100'),
        maxDataPoints: parseInt(process.env.RECOVERY_MAX_DATA_POINTS || '10000'),
      },
      ...
      reconnect: {
        ...
        autoRestoreSubscriptions: process.env.RECONNECT_AUTO_RESTORE !== 'false',
        autoRecoverData: process.env.RECONNECT_AUTO_RECOVER !== 'false',
        recoveryPriority: (process.env.RECONNECT_RECOVERY_PRIORITY as any) || 'normal',
        heartbeatTimeout: parseInt(process.env.RECONNECT_HEARTBEAT_TIMEOUT || '60000'),
      },
      
      cleanup: {
        removeOnComplete: parseInt(process.env.RECOVERY_CLEANUP_COMPLETE || '1000'),
  ...
  getRateLimitConfig(provider: string): RateLimitConfig {
    return this.config.rateLimit.providers[provider] || this.config.rateLimit.default;
  }
  ...
  getReconnectDelay(attempt: number): number {
    ...
    switch (delayStrategy.type) {
      ...
      default:
        return Math.min(
          delayStrategy.initialDelay * Math.pow(delayStrategy.factor || 2, attempt - 1),
          delayStrategy.maxDelay
        );
    }
  }
