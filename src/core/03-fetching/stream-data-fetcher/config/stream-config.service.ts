...
export class StreamConfigService {
  ...
  private loadConfiguration(): StreamDataFetcherConfig {
    return {
      ...
      healthCheck: {
        ...
        skipUnresponsive: this.getEnvBoolean('HEALTHCHECK_SKIP_UNRESPONSIVE', true),
        healthRateThreshold: this.getEnvNumber('HEALTHCHECK_RATE_THRESHOLD', 50),
      },
      
      performance: {
        slowResponseMs: this.getEnvNumber('STREAM_SLOW_RESPONSE_MS', 2000),
        maxTimePerSymbolMs: this.getEnvNumber('STREAM_MAX_TIME_PER_SYMBOL_MS', 500),
        maxSymbolsPerBatch: this.getEnvNumber('STREAM_MAX_SYMBOLS_PER_BATCH', 50),
      ...
      polling: {
        ...
        connectionIntervalMs: this.getEnvNumber('STREAM_CONNECTION_POLLING_INTERVAL_MS', 100),
      },
      
      security: {
        http: {
          rateLimitTtl: this.getEnvNumber('STREAM_HTTP_RATE_LIMIT_TTL', 60),
          rateLimitCount: this.getEnvNumber('STREAM_HTTP_RATE_LIMIT_COUNT', 100),
          burstLimit: this.getEnvNumber('STREAM_HTTP_BURST_LIMIT', 20),
        },
        websocket: {
          maxConnectionsPerIP: this.getEnvNumber('WS_MAX_CONNECTIONS_PER_IP', 10),
          maxConnectionsPerUser: this.getEnvNumber('WS_MAX_CONNECTIONS_PER_USER', 5),
          messagesPerMinute: this.getEnvNumber('WS_MESSAGES_PER_MINUTE', 120),
          maxSubscriptionsPerConnection: this.getEnvNumber('WS_MAX_SUBSCRIPTIONS_PER_CONNECTION', 50),
          burstMessages: this.getEnvNumber('WS_BURST_MESSAGES', 20),
        },
      },
      
      monitoring: {
        enableVerboseLogging: this.getEnvBoolean('STREAM_VERBOSE_LOGGING', false),
        metricsCollectionInterval: this.getEnvNumber('STREAM_METRICS_COLLECTION_INTERVAL', 30000),
        poolStatsReportInterval: this.getEnvNumber('STREAM_POOL_STATS_REPORT_INTERVAL', 300000),
      },
    };
  }
  ...
  private validateConfiguration(): void {
    ...
    if (errors.length > 0) {
      throw new Error(`Stream configuration validation failed: ${errors.join(', ')}`);
    }
  }
  ...
  private logConfiguration(): void {
    this.logger.log('流数据获取器配置已加载', this.sanitizeConfigForLogging(this.config));
  }
   ...
   * @private
   */
  private getUpdatedFields(oldConfig: any, newConfig: any, path = ''): string[] {
    ...
    for (const key in newConfig) {
      ...
      if (typeof newConfig[key] === 'object' && newConfig[key] !== null && !Array.isArray(newConfig[key])) {
        updated.push(...this.getUpdatedFields(oldConfig[key] || {}, newConfig[key], currentPath));
