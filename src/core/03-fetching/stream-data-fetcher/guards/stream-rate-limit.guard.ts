...
export class StreamRateLimitGuard implements CanActivate {
  ...
  // 请求计数器 - IP级别
  private readonly ipRequestCounts = new Map<string, { count: number; lastReset: number; burstCount: number; lastBurst: number }>();
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // 获取装饰器配置
    const config = this.reflector.get(STREAM_RATE_LIMIT_KEY, context.getHandler()) || this.defaultConfig;
    ...
    try {
      ...
      // 用户级别限制检查
      if (config.perUser && userId && !this.checkRateLimit('user', userId, config)) {
  ...
  getStats() {
    ...
    return {
      ipCounters: Array.from(this.ipRequestCounts.entries()).map(([ip, counter]) => ({
        ip,
        count: counter.count,
      ...
      userCounters: Array.from(this.userRequestCounts.entries()).map(([user, counter]) => ({
        user,
        count: counter.count,
        burstCount: counter.burstCount,
        lastReset: new Date(counter.lastReset).toISOString(),
