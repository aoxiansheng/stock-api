...
export class WebSocketRateLimitGuard {
  ...
  private readonly config: WebSocketRateLimitConfig = {
    ...
    maxSubscriptionsPerConnection: parseInt(process.env.WS_MAX_SUBSCRIPTIONS_PER_CONNECTION || '50'),
  ...
  // 消息计数
  private readonly messageCounters = new Map<string, { count: number; lastReset: number; burstCount: number; lastBurst: number }>();
  ...
  canConnect(socket: Socket): boolean {
    ...
    try {
      ...
      this.logger.debug('WebSocket连接已允许', {
        ...
        totalUserConnections: userId ? (this.userConnections.get(userId)?.size || 0) : 0,
      });
   ...
   * 注册连接
   */
  private registerConnection(clientIP: string, userId: string | null, socketId: string): void {
    // 注册IP连接
  ...
  getStats() {
    return {
      ...
      connections: {
        ...
        byUser: Array.from(this.userConnections.entries()).map(([user, sockets]) => ({
          user,
          count: sockets.size,
          sockets: Array.from(sockets),
        })),
      },
      messages: Array.from(this.messageCounters.entries()).map(([socketId, counter]) => ({
        socketId,
        count: counter.count,
      ...
      subscriptions: Array.from(this.subscriptionCounters.entries()).map(([socketId, subs]) => ({
        socketId,
        count: subs.size,
