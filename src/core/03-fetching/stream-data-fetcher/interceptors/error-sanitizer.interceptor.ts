import { Injectable, NestInterceptor, ExecutionContext, CallHandler, HttpException, BadRequestException, ServiceUnavailableException } from '@nestjs/common';
...
export class ErrorSanitizerInterceptor implements NestInterceptor {
  ...
  private sanitizeError(error: any): HttpException {
    ...
    if (error instanceof ServiceUnavailableException) {
      return new ServiceUnavailableException(this.getGenericMessage('service_unavailable'));
    }
    ...
    // 连接相关错误
    if (sanitizedMessage.includes('connection') || sanitizedMessage.includes('连接')) {
      return new ServiceUnavailableException(this.getGenericMessage('connection_error'));
    }
    
    // 超时错误
    if (sanitizedMessage.includes('timeout') || sanitizedMessage.includes('超时')) {
      return new ServiceUnavailableException(this.getGenericMessage('timeout'));
    }
    
    // 权限相关错误
    if (sanitizedMessage.includes('permission') || sanitizedMessage.includes('unauthorized') || sanitizedMessage.includes('权限')) {
      return new BadRequestException(this.getGenericMessage('permission_denied'));
    }
    
    // 配置错误
    if (sanitizedMessage.includes('config') || sanitizedMessage.includes('配置')) {
      return new ServiceUnavailableException(this.getGenericMessage('configuration_error'));
    }
    
    // Provider相关错误
    if (sanitizedMessage.includes('provider') || sanitizedMessage.includes('提供商')) {
      return new BadRequestException(this.getGenericMessage('provider_error'));
    }
    
    // 默认处理：返回通用错误信息
    return new ServiceUnavailableException(this.getGenericMessage('internal_error'));
  }
