...
export class CoreLimitsUtil {
  ...
  static validateStringLength(
    value: string,
    type: "short" | "medium" | "long" | "extra_long" = "medium",
  ): boolean {
    ...
    return (
      value.length >= limits.MIN_LENGTH && value.length <= maxLengths[type]
    );
  }
  ...
  static getRecommendedBatchSize(
    operation: "tiny" | "small" | "medium" | "large" | "default",
  ): number {
    const limits = CORE_LIMITS.BATCH_LIMITS;
    switch (operation) {
      case "tiny":
        return limits.TINY_BATCH_SIZE;
      case "small":
        return limits.SMALL_BATCH_SIZE;
      case "medium":
        return limits.MEDIUM_BATCH_SIZE;
      case "large":
        return limits.LARGE_BATCH_SIZE;
      default:
        return limits.DEFAULT_BATCH_SIZE;
    }
  }
  ...
  static validateId(
    id: string,
    type: "uuid" | "shortid" | "mongodb" | "custom",
  ): boolean {
    ...
    switch (type) {
      ...
      case "shortid":
        return (
          id.length === limits.SHORTID_LENGTH && /^[A-Za-z0-9_-]+$/.test(id)
        );
      case "mongodb":
        return (
          id.length === limits.MONGODB_ID_LENGTH && /^[0-9a-f]{24}$/i.test(id)
        );
      case "custom":
        return (
          id.length >= limits.CUSTOM_ID_MIN && id.length <= limits.CUSTOM_ID_MAX
        );
      default:
        return false;
    }
  ...
  static checkStorageLimit(
    ...
    switch (type) {
      ...
      case "json":
        return sizeBytes <= limits.MAX_JSON_SIZE_BYTES;
      case "log":
        return sizeBytes <= limits.MAX_LOG_SIZE_BYTES;
      default:
        return false;
    }
  }
  ...
  static safeSlice<T>(array: T[], start: number, end?: number): T[] {
    ...
    const actualEnd =
        ...
        ? Math.min(end, start + maxLength)
        : Math.min(array.length, start + maxLength);
