...
export class UserJsonPersistenceController {
  ...
  @ApiOperation({
    summary: "分析数据源结构",
    description:
      "分析提供的JSON数据，自动提取字段结构和类型信息。如果saveAsTemplate=true，则同时保存为模板",
  })
  @ApiResponse({
    status: 201,
    description: "分析成功，模板已创建",
    type: DataSourceAnalysisResponseDto,
  })
  @ApiStandardResponses()
  @ApiKeyAuthResponses()
  async analyzeDataSource(
    @Body() dto: AnalyzeDataSourceDto,
    ...
    if (dto.saveAsTemplate) {
      const templateData: CreateDataSourceTemplateDto = {
        ...
        description:
          dto.description || `${dto.provider} ${dto.dataType} 数据模板`,
