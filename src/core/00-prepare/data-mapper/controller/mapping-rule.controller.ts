...
export class MappingRuleController {
  ...
  @ApiResponse({ status: 201, description: '创建成功', type: FlexibleMappingRuleResponseDto })
  @ApiStandardResponses()
  @JwtAuthResponses()
  async createFlexibleRule(@Body() dto: CreateFlexibleMappingRuleDto): Promise<FlexibleMappingRuleResponseDto> {
  ...
  async getFlexibleRules(
    ...
    return await this.ruleService.findRules(page, limit, provider, apiType, transDataRuleListType);
  }
  ...
  @ApiResponse({ status: 200, description: '查询成功', type: FlexibleMappingRuleResponseDto })
  @ApiStandardResponses()
  @ApiKeyAuthResponses()
  async getRuleById(@Param('id') id: string): Promise<FlexibleMappingRuleResponseDto> {
  ...
  @ApiResponse({ status: 200, description: '更新成功', type: FlexibleMappingRuleResponseDto })
  @ApiStandardResponses()
  @JwtAuthResponses()
  async updateRule(
    @Param('id') id: string,
  ...
  async realignExistingRule(@Param('id') dataMapperRuleId: string) {
    return await this.ruleAlignmentService.realignExistingRule(dataMapperRuleId);
  }
  ...
  async manualAdjustFieldMapping(
    ...
    return await this.ruleAlignmentService.manualAdjustFieldMapping(dataMapperRuleId, adjustments);
  }
  ...
  @ApiResponse({ status: 200, description: '测试成功', type: FlexibleMappingTestResultDto })
  @ApiStandardResponses()
  @ApiKeyAuthResponses()
  async testMappingRule(@Body() dto: TestFlexibleMappingRuleDto): Promise<FlexibleMappingTestResultDto> {
    ...
    const mappingStats = (result.debugInfo && result.debugInfo.length > 0)
      ? {
          totalMappings: result.debugInfo.length,
          successfulMappings: result.debugInfo.filter(item => item.success).length,
          failedMappings: result.debugInfo.length - result.debugInfo.filter(item => item.success).length,
          successRate: result.debugInfo.length > 0
            ? (result.debugInfo.filter(item => item.success).length / result.debugInfo.length) * 100
            : 0,
        }
      : result.mappingStats;
