...
export const StreamReceiverConfigKeys = {
  ...
  MAX_RETRY_ATTEMPTS: 'STREAM_RECEIVER_MAX_RETRY',
  RETRY_DELAY_BASE: 'STREAM_RECEIVER_RETRY_DELAY',
  CIRCUIT_BREAKER_THRESHOLD: 'STREAM_RECEIVER_CB_THRESHOLD',
  CIRCUIT_BREAKER_RESET_TIMEOUT: 'STREAM_RECEIVER_CB_RESET',
  BATCH_PROCESSING_INTERVAL: 'STREAM_RECEIVER_BATCH_INTERVAL',
  
  // 动态批处理配置
  DYNAMIC_BATCHING_ENABLED: 'STREAM_RECEIVER_DYNAMIC_BATCHING_ENABLED',
  DYNAMIC_BATCHING_MIN_INTERVAL: 'STREAM_RECEIVER_DYNAMIC_MIN_INTERVAL',
  DYNAMIC_BATCHING_MAX_INTERVAL: 'STREAM_RECEIVER_DYNAMIC_MAX_INTERVAL',
  DYNAMIC_BATCHING_HIGH_LOAD_INTERVAL: 'STREAM_RECEIVER_DYNAMIC_HIGH_LOAD_INTERVAL',
  DYNAMIC_BATCHING_LOW_LOAD_INTERVAL: 'STREAM_RECEIVER_DYNAMIC_LOW_LOAD_INTERVAL',
  DYNAMIC_BATCHING_SAMPLE_WINDOW: 'STREAM_RECEIVER_DYNAMIC_SAMPLE_WINDOW',
  DYNAMIC_BATCHING_HIGH_LOAD_THRESHOLD: 'STREAM_RECEIVER_DYNAMIC_HIGH_LOAD_THRESHOLD',
  DYNAMIC_BATCHING_LOW_LOAD_THRESHOLD: 'STREAM_RECEIVER_DYNAMIC_LOW_LOAD_THRESHOLD',
  DYNAMIC_BATCHING_ADJUSTMENT_STEP: 'STREAM_RECEIVER_DYNAMIC_ADJUSTMENT_STEP',
  DYNAMIC_BATCHING_ADJUSTMENT_FREQUENCY: 'STREAM_RECEIVER_DYNAMIC_ADJUSTMENT_FREQUENCY',
  
  MEMORY_CHECK_INTERVAL: 'STREAM_RECEIVER_MEMORY_CHECK_INTERVAL',
  MEMORY_WARNING_THRESHOLD: 'STREAM_RECEIVER_MEMORY_WARNING_MB',
  MEMORY_CRITICAL_THRESHOLD: 'STREAM_RECEIVER_MEMORY_CRITICAL_MB',
  
  RATE_LIMIT_MAX_CONNECTIONS: 'STREAM_RECEIVER_RATE_LIMIT_CONNECTIONS',
  RATE_LIMIT_WINDOW_SIZE: 'STREAM_RECEIVER_RATE_LIMIT_WINDOW',
} as const;

/**
 * 配置验证函数
 */
export function validateStreamReceiverConfig(config: Partial<StreamReceiverConfig>): string[] {
  const errors: string[] = [];
  
  if (config.connectionCleanupInterval !== undefined && config.connectionCleanupInterval < 10000) {
  ...
  if (config.connectionStaleTimeout !== undefined && config.connectionStaleTimeout < 30000) {
  ...
  if (config.batchProcessingInterval !== undefined && config.batchProcessingInterval < 10) {
    errors.push('批处理间隔不能少于10ms');
  }
  
  // 动态批处理配置验证
  if (config.dynamicBatching?.minInterval !== undefined && config.dynamicBatching.minInterval < 5) {
    errors.push('动态批处理最小间隔不能少于5ms');
  }
  
  if (config.dynamicBatching?.maxInterval !== undefined && config.dynamicBatching.maxInterval > 1000) {
    errors.push('动态批处理最大间隔不能超过1000ms');
  }
  
  if (config.dynamicBatching?.minInterval !== undefined && 
  ...
  if (config.dynamicBatching?.highLoadInterval !== undefined && 
  ...
  if (config.dynamicBatching?.lowLoadInterval !== undefined && 
      config.dynamicBatching?.maxInterval !== undefined &&
      config.dynamicBatching.lowLoadInterval > config.dynamicBatching.maxInterval) {
    errors.push('低负载间隔不能大于最大间隔');
  }
  
  if (config.dynamicBatching?.loadDetection?.sampleWindow !== undefined && 
      config.dynamicBatching.loadDetection.sampleWindow < 5) {
    errors.push('负载检测采样窗口不能少于5个批次');
  }
  
  if (config.dynamicBatching?.loadDetection?.highLoadThreshold !== undefined && 
      config.dynamicBatching?.loadDetection?.lowLoadThreshold !== undefined &&
      config.dynamicBatching.loadDetection.highLoadThreshold <= config.dynamicBatching.loadDetection.lowLoadThreshold) {
    errors.push('高负载阈值必须大于低负载阈值');
  }
  
  if (config.dynamicBatching?.loadDetection?.adjustmentStep !== undefined && 
      config.dynamicBatching.loadDetection.adjustmentStep < 1) {
    errors.push('间隔调整步长不能少于1ms');
  }
  
  if (config.dynamicBatching?.loadDetection?.adjustmentFrequency !== undefined && 
      config.dynamicBatching.loadDetection.adjustmentFrequency < 1000) {
    errors.push('调整频率不能少于1秒');
  }
  
  // 内存监控配置验证
  if (config.memoryMonitoring?.checkInterval !== undefined && 
      config.memoryMonitoring.checkInterval < 5000) {
    errors.push('内存检查间隔不能少于5秒');
  }
  
  if (config.memoryMonitoring?.warningThreshold !== undefined && 
      config.memoryMonitoring.warningThreshold < 100 * 1024 * 1024) {
    errors.push('内存警告阈值不能少于100MB');
  }
  
  if (config.memoryMonitoring?.criticalThreshold !== undefined && 
      config.memoryMonitoring?.warningThreshold !== undefined &&
      config.memoryMonitoring.criticalThreshold <= config.memoryMonitoring.warningThreshold) {
    errors.push('内存临界阈值必须大于警告阈值');
  }
  
  // 频率限制配置验证
  if (config.rateLimit?.maxConnectionsPerMinute !== undefined && 
      config.rateLimit.maxConnectionsPerMinute < 1) {
    errors.push('频率限制不能少于每分钟1次连接');
  }
  
  if (config.rateLimit?.windowSize !== undefined && config.rateLimit.windowSize < 1000) {
