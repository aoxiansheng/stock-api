...
export class QueryService implements OnModuleInit {
  ...
  async executeQuery(request: QueryRequestDto): Promise<QueryResponseDto> {
    ...
    try {
      ...
      this.logger.log(
        QUERY_SUCCESS_MESSAGES.QUERY_EXECUTION_SUCCESS,
        sanitizeLogData({ queryId, executionTime }),
      );
  ...
  private async executeSymbolBasedQuery(
    ...
    const results = (
      await Promise.all(request.symbols.map(processSymbol))
  ...
  private async fetchSymbolData(
    ...
    if (request.useCache) {
      ...
      if (cacheResult)
        return { data: cacheResult.data, source: DataSourceType.CACHE };
    }
    const realtimeResult = await this.fetchFromRealtimeAndCache(
      symbol,
      request,
      queryId,
    );
  ...
  private async fetchFromRealtimeAndCache(
    symbol: string,
    request: QueryRequestDto,
    queryId: string,
