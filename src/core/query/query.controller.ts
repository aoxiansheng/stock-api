...
import { ApiKeyAuth, Public } from "../../auth/decorators/auth.decorator";
...
- 大批量查询建议使用 \`/query/bulk\` 端点
  ...
  @ApiOperation({
    summary: "查询服务健康检查",
    description:
      "测试查询服务功能和数据源连接状态，检查缓存、持久化存储和实时数据源的可用性",
  })
  @ApiHealthResponse()
  async healthCheck() {
    ...
    try {
      ...
      const healthResult = {
        ...
        dataSources: {
          ...
          persistent: true, // Would check actual DB connectivity
          realtime: true, // Would check actual data provider connectivity
        },
        overallHealth: {
