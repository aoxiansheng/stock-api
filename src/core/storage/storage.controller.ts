...
- 删除缓存数据可能影响系统性能
  ...
  @ApiKeyAuth([Permission.SYSTEM_MONITOR])
  @Get("stats")
  @ApiOperation({
    summary: "📈 获取存储统计信息",
    description: `
...
- **⚠️ 错误统计**: 错误率和失败原因分析
  ...
  @ApiKeyAuth([Permission.SYSTEM_HEALTH])
  @Post("health-check")
  @ApiOperation({
    summary: "🏥 执行存储健康检查",
    description: `
...
### 权限要求
需要 SYSTEM_HEALTH 权限（系统健康检查）

### 检查内容
- **📡 Redis缓存连接**: 测试缓存服务可用性和响应时间
- **💾 MongoDB连接**: 测试数据库服务可用性和响应时间
- **⚡ 读写性能**: 测试实际的存储和检索操作
- **🧹 自动清理**: 测试完成后自动清理测试数据
...
- 性能基准测试
  ...
  @ApiStandardResponses()
  async healthCheck() {
    this.logger.log(`API Request: Storage health check`);
    ...
    try {
      ...
      let persistentHealthy = false;
      let persistentLatency = 0;
      ...
      try {
        ...
        persistentHealthy = !!retrieved.data;
        persistentLatency = Date.now() - persistentStartTime;

        // Cleanup
        await this.storageService.deleteData(
          `${testKey}-persistent`,
          StorageType.PERSISTENT,
        );
      } catch (persistentError) {
        this.logger.warn(
          "Persistent storage health check failed",
          persistentError,
        );
      }
      ...
      const result = {
        ...
        overall: {
          healthy: cacheHealthy && persistentHealthy,
        },
      };

      this.logger.log(`API Success: Storage health check completed`, {
        cacheHealthy,
        persistentHealthy,
        cacheLatency,
        persistentLatency,
        overallHealthy: result.overall.healthy,
      });

      // 遵循控制器编写规范：让拦截器自动处理响应格式化
      return result;
    } catch (error: any) {
      this.logger.error(`API Error: Storage health check failed`, {
        error: error.message,
        errorType: error.constructor.name,
      });
