...
import { STORAGE_ERROR_MESSAGES, STORAGE_KEY_PATTERNS } from '../constants/storage.constants';
...
export class StorageRepository {
  ...
  async storeInCache(key: string, data: string, ttl: number, compressed: boolean): Promise<void> {
    if (!this.redis) {
      throw new ServiceUnavailableException(STORAGE_ERROR_MESSAGES.REDIS_NOT_AVAILABLE);
    }
    const cacheKey = this.getCacheKey(key);
    const metadata = JSON.stringify({ compressed, storedAt: new Date().toISOString() });

    await this.redis.pipeline()
      .setex(cacheKey, ttl, data)
      .setex(`${cacheKey}${STORAGE_KEY_PATTERNS.METADATA_SUFFIX}`, ttl, metadata)
      .exec();
  }

  async retrieveFromCache(key: string): Promise<{ data: string | null; metadata: string | null; ttl: number }> {
    if (!this.redis) {
  ...
  async deleteFromCache(key: string): Promise<boolean> {
    ...
    const cacheKey = this.getCacheKey(key);
    const deletedCount = await this.redis.del(cacheKey, `${cacheKey}${STORAGE_KEY_PATTERNS.METADATA_SUFFIX}`);
  ...
  async getAverageTtl(): Promise<number> {
    ...
    try {
        ...
        const ttls = results.map(res => res[1] as number).filter(ttl => ttl > 0);
  ...
  async upsert(document: Partial<StoredDataDocument>): Promise<StoredDataDocument> {
