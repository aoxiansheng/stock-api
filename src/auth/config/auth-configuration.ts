...
const authConfig = registerAs('auth', () => ({
  ...
  security: {
    ...
    maxPayloadSizeBytes: parseInt(process.env.AUTH_MAX_PAYLOAD_BYTES || '10485760'),
    
    // 字符串长度限制
    maxStringLengthSanitize: parseInt(process.env.AUTH_MAX_STRING_LENGTH_SANITIZE || '10000'),
    maxStringLengthComplexity: parseInt(process.env.AUTH_MAX_STRING_LENGTH_COMPLEXITY || '1000'),
    findLongStringThreshold: parseInt(process.env.AUTH_FIND_LONG_STRING_THRESHOLD || '1000'),
    
    // 对象复杂度限制
    maxObjectDepthComplexity: parseInt(process.env.AUTH_MAX_OBJECT_DEPTH || '10'),
    maxObjectFieldsComplexity: parseInt(process.env.AUTH_MAX_OBJECT_FIELDS || '50'),
  ...
  rateLimit: {
    ...
    redis: {
      ...
      connectionTimeout: parseInt(process.env.AUTH_REDIS_CONNECTION_TIMEOUT || '5000'),
    ...
    ipRateLimit: {
      ...
      maxRequests: parseInt(process.env.AUTH_IP_RATE_LIMIT_MAX_REQUESTS || '100'),
