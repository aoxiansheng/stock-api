...
import authUnifiedConfig, { type AuthUnifiedConfigInterface } from './auth-unified.config';
...
export class AuthConfigCompatibilityWrapper {
  ...
  constructor(
    ...
    this.config = this.configService.get<AuthUnifiedConfigInterface>('authUnified')!;
  }
  ...
  get API_KEY_OPERATIONS() {
    return {
      ...
      CREATE_PER_MINUTE: Math.floor(this.config.limits.apiKeyCreatePerDay / 24 / 60), // 转换为每分钟
  ...
  get USER_LOGIN() {
    return {
      ...
      TOKEN_REFRESH_HOURS: Math.floor(this.config.cache.sessionCacheTtl / 3600 * 7), // 7倍会话时间
    };
  }
  ...
  get SESSION_CONFIG() {
    return {
      ...
      REFRESH_TOKEN_DAYS: Math.floor(this.config.cache.sessionCacheTtl / 86400 * 7), // 7倍会话时间，转为天
      SESSION_TIMEOUT_MINUTES: this.config.limits.sessionTimeoutMinutes,
      REMEMBER_ME_DAYS: Math.floor(this.config.cache.sessionCacheTtl / 86400 * 30), // 30倍会话时间
  ...
  validateCompatibility(): { isValid: boolean; errors: string[] } {
    ...
    try {
      ...
      if (!permissionCheck.CHECK_TIMEOUT_MS || permissionCheck.CHECK_TIMEOUT_MS <= 0) {
        errors.push('PERMISSION_CHECK.CHECK_TIMEOUT_MS 无效');
      }
      
      const validationLimits = this.VALIDATION_LIMITS;
      if (!validationLimits.MAX_STRING_LENGTH || validationLimits.MAX_STRING_LENGTH <= 0) {
