...
export class AuthCacheConfigValidation {
  ...
  statisticsCacheTtl: number = parseInt(process.env.AUTH_STATISTICS_CACHE_TTL || '300');
  ...
  sessionCacheTtl: number = parseInt(process.env.AUTH_SESSION_CACHE_TTL || '3600');
}
...
const createAuthCacheConfig = (): AuthCacheConfigValidation => {
  ...
  if (errors.length > 0) {
    ...
    throw new Error(`Auth Cache configuration validation failed: ${errorMessages}`);
  }
