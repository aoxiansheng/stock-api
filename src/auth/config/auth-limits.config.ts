...
export class AuthLimitsConfigValidation {
  ...
  apiKeyValidatePerSecond: number = parseInt(process.env.AUTH_API_KEY_VALIDATE_RATE || '100');
  ...
  loginRatePerMinute: number = parseInt(process.env.AUTH_LOGIN_RATE_LIMIT || '5');
  ...
  maxPayloadSizeBytes: number = parseInt(process.env.AUTH_MAX_PAYLOAD_BYTES || '10485760'); // 10MB
  ...
  redisConnectionTimeout: number = parseInt(process.env.AUTH_REDIS_CONNECTION_TIMEOUT || '5000');
  ...
  redisCommandTimeout: number = parseInt(process.env.AUTH_REDIS_COMMAND_TIMEOUT || '5000');
  ...
  apiKeyCreatePerDay: number = parseInt(process.env.AUTH_API_KEY_CREATE_LIMIT || '10');
  ...
  maxObjectFields: number = parseInt(process.env.AUTH_MAX_OBJECT_FIELDS || '50');
  ...
  maxRecursionDepth: number = parseInt(process.env.AUTH_MAX_RECURSION_DEPTH || '100');
  ...
  maxLoginAttempts: number = parseInt(process.env.AUTH_MAX_LOGIN_ATTEMPTS || '5');
  ...
  loginLockoutMinutes: number = parseInt(process.env.AUTH_LOGIN_LOCKOUT_MINUTES || '15');
  ...
  passwordMinLength: number = parseInt(process.env.AUTH_PASSWORD_MIN_LENGTH || '8');
  ...
  passwordMaxLength: number = parseInt(process.env.AUTH_PASSWORD_MAX_LENGTH || '128');
  ...
  maxConcurrentSessions: number = parseInt(process.env.AUTH_MAX_CONCURRENT_SESSIONS || '5');
  ...
  sessionTimeoutMinutes: number = parseInt(process.env.AUTH_SESSION_TIMEOUT_MINUTES || '60');
}
...
const createAuthLimitsConfig = (): AuthLimitsConfigValidation => {
  ...
  if (errors.length > 0) {
    ...
    throw new Error(`Auth Limits configuration validation failed: ${errorMessages}`);
  }
