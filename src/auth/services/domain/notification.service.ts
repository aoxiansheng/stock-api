...
export class AuthEventNotificationService {
  ...
  async sendRegistrationSuccessEvent(user: User): Promise<void> {
    this.logger.debug('发送用户注册成功通知', { userId: user.id, username: user.username });
    ...
    try {
      ...
      setImmediate(() => {
        this.sendWelcomeEmail(user).catch(error =>
          this.logger.error('发送欢迎邮件失败', { userId: user.id, error: error.message })
        );
      });
   ...
   * 发送用户注册失败事件
   */
  async sendRegistrationFailureEvent(createUserDto: CreateUserDto, error: any): Promise<void> {
    this.logger.debug('发送用户注册失败通知', { username: createUserDto.username });
  ...
  async sendLoginSuccessEvent(user: User): Promise<void> {
    this.logger.debug('发送用户登录成功通知', { userId: user.id, username: user.username });
    ...
    try {
      ...
      setImmediate(() => {
        this.checkAndSendLoginReminder(user).catch(error =>
          this.logger.error('检查登录提醒失败', { userId: user.id, error: error.message })
        );
      });
   ...
   * 发送API密钥撤销事件
   */
  async sendApiKeyRevocationEvent(appKey: string, userId: string): Promise<void> {
   ...
   * 发送管理员操作事件
   */
  async sendAdminOperationEvent(operation: string, details: any): Promise<void> {
   ...
   * 发送业务监控指标事件
   */
  private async emitBusinessMetric(metricName: string, success: boolean, metadata?: any): Promise<void> {
    try {
   ...
   * 发送技术监控指标事件
   */
  private async emitTechnicalMetric(metricName: string, success: boolean, metadata?: any): Promise<void> {
    try {
      this.eventBus.emit(SYSTEM_STATUS_EVENTS.METRIC_COLLECTED, {
        timestamp: new Date(),
        source: "auth",
   ...
   * 发送用户通知事件
   */
  private async emitUserNotification(eventType: string, data: any): Promise<void> {
    try {
   ...
   * 发送管理员通知事件
   */
  private async emitAdminNotification(eventType: string, data: any): Promise<void> {
    try {
   ...
   * 发送安全通知
   */
  private async sendSecurityNotification(eventType: string, data: any): Promise<void> {
    try {
   ...
   * 检查并发送安全警报
   */
  private async checkAndSendSecurityAlert(alertType: string, data: any): Promise<void> {
    try {
      // 实际实现中应该包含安全规则引擎
  ...
  private extractErrorReason(error: any): string {
    if (error.message.includes('用户已存在') || error.message.includes('USER_EXISTS')) {
      return 'user_exists';
    }
    if (error.message.includes('无效凭据') || error.message.includes('INVALID_CREDENTIALS')) {
      return 'invalid_credentials';
    }
    if (error.message.includes('用户不活跃') || error.message.includes('user_inactive')) {
