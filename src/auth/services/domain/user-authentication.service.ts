...
export class UserAuthenticationService {
  ...
  async validateUserStatus(userId: string): Promise<boolean> {
    ...
    try {
      const user = await this.userRepository.findById(userId);
      return user !== null && user.status === CommonStatus.ACTIVE;
    } catch (error) {
  ...
  async getAllUsers(
    ...
    this.logger.log('获取用户列表', { page, limit, includeInactive, includeStats });
    ...
    try {
      ...
      this.logger.error('获取用户列表失败', { page, limit, error: error.message });
      throw error;
    }
  }
   ...
   * 用于注册前的可用性检查
   */
  async checkUserAvailability(username?: string, email?: string): Promise<{
  ...
  async updateLastLoginTime(userId: string): Promise<void> {
    ...
    try {
      ...
      // 这是非关键操作，记录错误但不抛出异常
      this.logger.error('更新最后登录时间失败', { userId, error: error.message });
    }
  }
}