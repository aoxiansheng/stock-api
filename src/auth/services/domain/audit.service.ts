...
export class AuditService {
   ...
   * 记录用户注册失败
   */
  async logUserRegistrationFailure(createUserDto: CreateUserDto, error: any): Promise<void> {
    const auditEvent = {
   ...
   * 记录API密钥创建失败
   */
  async logApiKeyCreationFailure(userId: string, createApiKeyDto: CreateApiKeyDto, error: any): Promise<void> {
    const auditEvent = {
   ...
   * 记录API密钥撤销失败
   */
  async logApiKeyRevocationFailure(appKey: string, userId: string, error: any): Promise<void> {
    const auditEvent = {
   ...
   * 记录API密钥频率限制重置失败
   */
  async logApiKeyRateLimitResetFailure(appKey: string, userId: string, error: any): Promise<void> {
    const auditEvent = {
   ...
   * 记录管理员用户列表访问
   */
  async logAdminUserListAccess(page: number, recordCount: number): Promise<void> {
    const auditEvent = {
   ...
   * 记录安全违规事件
   */
  async logSecurityViolation(violationType: string, details: any): Promise<void> {
    const auditEvent = {
