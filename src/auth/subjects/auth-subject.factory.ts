...
export class AuthSubjectFactory {
  ...
  static createFromRequest(request: any): AuthSubject {
    ...
    } else if (user.permissions !== undefined && Array.isArray(user.permissions)) {
  ...
  static isValidSubject(subject: AuthSubject): boolean {
    ...
    // API Key需要额外检查有效性
    if (subject.type === AuthSubjectType.API_KEY && subject instanceof ApiKeySubject) {
