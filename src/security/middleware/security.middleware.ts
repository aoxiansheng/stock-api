...
export class SecurityMiddleware implements NestMiddleware {
   ...
   * 内容类型安全验证
   */
  private validateContentTypeSecurity(req: Request): { isValid: boolean; reason?: string; details?: any } {
   ...
   * 高级输入安全验证
   */
  private validateAdvancedInputSecurity(req: Request): { isValid: boolean; reason?: string; details?: any } {
    try {
      // 检查URL中的恶意模式
   ...
   * 检查恶意URL模式
   */
  private checkMaliciousUrl(url: string): { isValid: boolean; reason?: string; details?: any } {
   ...
   * 检查请求体复杂度
   */
  private checkRequestBodyComplexity(body: any): { isValid: boolean; reason?: string; details?: any } {
    // 检查对象深度
   ...
   * 检查Unicode安全问题
   */
  private checkUnicodeSecurity(obj: any): { isValid: boolean; reason?: string; details?: any } {
      ...
      if (typeof value === 'string') {
        ...
        if (Array.isArray(value)) {
          ...
          return Object.entries(value).some(([key, val]) => 
            checkString(key) || checkValue(val)
          );
        }
      }
      return false;
    ...
    if (checkValue(obj)) {
      return {
        ...
        details: { patterns: dangerousUnicodePatterns.map(p => p.toString()) }
      };
    }
   ...
   * 检查查询参数复杂度
   */
  private checkQueryComplexity(query: any): { isValid: boolean; reason?: string; details?: any } {
  ...
  private getObjectDepth(obj: any, currentDepth: number = 0): number {
    if (currentDepth > SECURITY_LIMITS.MAX_RECURSION_DEPTH) { // 防止无限递归
      return currentDepth;
    }
    ...
    if (Array.isArray(obj)) {
      return Math.max(currentDepth, ...obj.map(item => this.getObjectDepth(item, currentDepth + 1)));
    }
   ...
   * 查找过长的字符串
   */
  private findLongString(obj: any, visited: Set<any> = new Set()): string | null {
    ...
    for (const value of values) {
      ...
      if (longString && longString.length > SECURITY_LIMITS.FIND_LONG_STRING_THRESHOLD) { // 临时阈值
        return longString;
      }
    }
...
export class RateLimitByIPMiddleware implements NestMiddleware {
  ...
  private readonly maxRequestsPerMinute = RATE_LIMIT_CONFIG.IP_RATE_LIMIT.MAX_REQUESTS;
