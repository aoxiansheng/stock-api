...
export const SECURITY_VULNERABILITY_TEMPLATES = Object.freeze({
  ...
  WEAK_JWT_SECRET: {
    ...
    recommendation:
      "在.env文件中设置一个至少{minLength}个字符的随机字符串作为JWT_SECRET",
  },
  ...
  WEAK_PASSWORD_HASHING: {
    ...
    title: "密码哈希强度不足",
    description:
      "Bcrypt轮数设置为 {currentRounds}，低于建议的 {recommendedRounds} 轮",
  ...
  POTENTIAL_DATA_EXPOSURE: {
    ...
    recommendation:
      "审查所有 API 响应，确保只返回必要的数据，并对敏感信息进行脱敏处理",
  },
  ...
  NOSQL_INJECTION_RISK: {
    ...
    recommendation:
      "对所有用户输入进行严格的验证和清理，使用参数化查询或 ORM/ODM 来防止注入攻击",
  },
});
