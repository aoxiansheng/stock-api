...
export class CacheSemanticsUtil {
  ...
  static isFastResponse(responseTimeMs: number): boolean {
    return (
      responseTimeMs <=
      CACHE_PERFORMANCE_SEMANTICS.RESPONSE_TIME_THRESHOLDS.FAST_MS
    );
  }
...
export class EnhancedCacheSemanticsUtil {
  ...
  static shouldCompress(valueSize: number): boolean {
    return (
      CACHE_ADVANCED_STRATEGY_SEMANTICS.COMPRESSION.ENABLE_COMPRESSION &&
   ...
   * 根据缓存键解析模块类型
   */
  static parseModuleFromKey(
    cacheKey: string,
  ): { moduleType: string; module: string } | null {
    ...
    // 查找匹配的模块类型
    for (const [moduleType, modules] of Object.entries(
   ...
   * 获取监控阈值
   */
  static getMonitoringThreshold(
    metric: "hit_rate" | "response_time" | "memory_usage",
  ): number {
...
export type CacheAdvancedStrategySemantics =
  typeof CACHE_ADVANCED_STRATEGY_SEMANTICS;
