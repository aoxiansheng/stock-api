...
export const RETRY_CONFIG_TEMPLATES = Object.freeze({
  ...
  NETWORK_OPERATION: {
    ...
    backoffStrategy:
      RETRY_STRATEGY_SEMANTICS.BACKOFF_STRATEGIES.EXPONENTIAL_JITTER,
...
export class RetrySemanticsUtil {
  ...
  static isRetryableHttpStatus(statusCode: number): boolean {
    return RETRYABLE_ERROR_SEMANTICS.HTTP_STATUS_CODES.RETRYABLE.includes(
      statusCode,
    );
  }
  ...
  static isRetryableError(error: Error): boolean {
    return RETRYABLE_ERROR_SEMANTICS.ERROR_TYPES.RETRYABLE.includes(
   ...
   * 计算指数退避延迟
   */
  static calculateExponentialDelay(
    attempt: number,
    baseDelayMs: number = RETRY_DELAY_SEMANTICS.BASIC.BASE_MS,
  ): number {
   ...
   * 计算带抖动的指数退避延迟
   */
  static calculateExponentialJitterDelay(
    attempt: number,
    baseDelayMs: number = RETRY_DELAY_SEMANTICS.BASIC.BASE_MS,
  ): number {
    const baseDelay = this.calculateExponentialDelay(attempt, baseDelayMs);
    const jitter =
      Math.random() * RETRY_DELAY_SEMANTICS.BACKOFF.RANDOM_JITTER_MAX_MS;
  ...
  static getRecommendedConfig(
    operationType: keyof typeof RETRY_CONFIG_TEMPLATES,
  ) {
