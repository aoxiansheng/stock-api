...
export type DataState = typeof SYSTEM_CONSTANTS.DATA_STATES[keyof typeof SYSTEM_CONSTANTS.DATA_STATES];
...
export function isValidOperationStatus(status: string): status is OperationStatus {
  return Object.values(SYSTEM_CONSTANTS.OPERATION_STATUS).includes(status as OperationStatus);
}
