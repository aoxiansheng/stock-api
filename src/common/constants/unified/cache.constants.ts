 ...
 * @param suffix 可选后缀
 */
export function buildCacheKey(prefix: CacheKeyPrefix, identifier: string, suffix?: string): string {
 ...
 * @param cacheKey 缓存键
 */
export function parseCacheKey(cacheKey: string): { prefix: string; identifier: string; suffix?: string } | null {
 ...
 * @param dataType 数据类型
 */
export function getRecommendedTTL(dataType: 'realtime' | 'static' | 'config' | 'session' | 'metrics'): number {
  const ttlMap = {
...
export function shouldCompress(valueSize: number): boolean {
  const thresholdBytes = CACHE_CONSTANTS.SIZE_LIMITS.COMPRESSION_THRESHOLD_KB * 1024;
  return CACHE_CONSTANTS.STRATEGY_CONFIG.ENABLE_COMPRESSION && valueSize > thresholdBytes;
}