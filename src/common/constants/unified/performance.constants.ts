...
// 导出类型定义
export type ResponseTimeThreshold = keyof typeof PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS;
 ...
 */
export function getTimeoutFromEnv(key: TimeoutSetting, defaultValue?: number): number {
...
export function calculateRetryDelay(attempt: number): number {
  const { RETRY_DELAY_MS, EXPONENTIAL_BACKOFF_BASE, MAX_RETRY_DELAY_MS, JITTER_FACTOR } = 
    PERFORMANCE_CONSTANTS.RETRY_SETTINGS;
  
  // 指数退避计算
  const baseDelay = RETRY_DELAY_MS * Math.pow(EXPONENTIAL_BACKOFF_BASE, attempt);
 ...
 */
export function isSlowResponse(responseTime: number, threshold: ResponseTimeThreshold = 'SLOW_REQUEST_MS'): boolean {
  return responseTime > PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS[threshold];
}
 ...
 * @param responseTime 响应时间（毫秒）
 */
export function getResponseTimeLevel(responseTime: number): 'fast' | 'normal' | 'slow' | 'critical' {
