...
import { IsNumber, IsString, Min, Max, validateSync, IsOptional } from 'class-validator';
...
export class CommonConfigHelper {
   ...
   * 根据操作类型获取推荐超时时间
   */
  static getRecommendedTimeout(operationType: 'quick' | 'normal' | 'long' | 'connection'): number {
    const config = new CommonConstantsConfigValidation();
   ...
   * 根据内存使用量获取建议
   */
  static getMemoryUsageAdvice(currentMemoryMb: number): { level: string; advice: string } {
    const config = new CommonConstantsConfigValidation();
