...
export class AuthPermissionConstants implements PermissionConstants {
  ...
  get PERMISSION_LEVELS(): {
    ...
    return this._constants?.PERMISSION_LEVELS || {
      HIGH: [],
      MEDIUM: [],
      LOW: []
    };
  ...
  private async initializeConstants() {
    if (!authConstants) {
      try {
        // 动态导入认证模块常量
        const { REQUIRE_API_KEY } = await import("../../../../auth/decorators/require-apikey.decorator");
        const { PERMISSIONS_KEY } = await import("../../../../auth/decorators/permissions.decorator");
        const { Permission } = await import("../../../../auth/enums/user-role.enum");
    ...
    // 设置常量值
    this._constants = {
      REQUIRE_API_KEY_METADATA_KEY: authConstants?.REQUIRE_API_KEY || "require_api_key",
      PERMISSIONS_METADATA_KEY: authConstants?.PERMISSIONS_KEY || "permissions",
      AVAILABLE_PERMISSIONS: availablePermissions,
      PERMISSION_LEVELS: {
        HIGH: availablePermissions.filter(p => 
          p.includes("ADMIN") || p.includes("WRITE") || p.includes("DELETE")
        ),
        MEDIUM: availablePermissions.filter(p => 
          p.includes("UPDATE") || p.includes("MODIFY")
        ),
        LOW: availablePermissions.filter(p => 
          p.includes("READ") || p.includes("VIEW")
        )
      }
    };
  }
 ...
 */
@Injectable() 
export class AuthPermissionMetadataExtractor implements PermissionMetadataExtractor {
  constructor(
  ...
  extractApiKeyAuthFlag(handler: (...args: any[]) => any): boolean {
    return this.reflector.get(this.constants.REQUIRE_API_KEY_METADATA_KEY, handler) === true;
  }
 ...
 */
@Injectable()
export class AuthPermissionValidationService implements PermissionValidationContract {
  constructor(
  ...
  validatePermissionLevel(permissions: string[]): { isValid: boolean; issues: string[] } {
    const issues: string[] = [];
  ...
  validatePermissionCombination(permissions: string[]): { isValid: boolean; issues: string[] } {
    const issues: string[] = [];
