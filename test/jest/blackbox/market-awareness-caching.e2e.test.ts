...
describe("Real Environment Black-_box: Market Awareness & Caching E2E", () => {
  ...
  describe("🌍 多市场智能推断测试", () => {
    ...
    marketTestCases.forEach((testCase) => {
      it(`应该正确识别 ${testCase.description}: ${testCase.symbol}`, async () => {
        ...
        if (metadata.cacheKey) {
          expect(metadata.cacheKey).toContain(
            testCase.expectedMarket.toLowerCase(),
          );
        }
      });
    });
    ...
    it("应该支持混合市场批量查询", async () => {
      ...
      if (metadata.detectedMarkets) {
        ...
        console.log(
          `✅ 混合市场查询检测到市场: ${metadata.detectedMarkets.join(", ")}`,
        );
      }
  ...
  describe("🔍 37字段变化检测测试", () => {
    it("应该检测股票价格字段变化", async () => {
      ...
      const secondResponse = await httpClient.post(
      ...
      if (metadata.changeDetection) {
        ...
        console.log(`变化检测结果: ${metadata.changeDetection.reason}`);
