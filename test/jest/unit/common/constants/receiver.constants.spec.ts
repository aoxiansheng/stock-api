...
describe('Receiver Constants', () => {
  describe('RECEIVER_ERROR_MESSAGES', () => {
    it('should define all error messages', () => {
      ...
      expect(RECEIVER_ERROR_MESSAGES.UNSUPPORTED_DATA_TYPE).toContain('{dataType}');
  ...
  describe('DATA_TYPE_TO_CAPABILITY_MAP', () => {
    it('should define all data type mappings', () => {
      expect(DATA_TYPE_TO_CAPABILITY_MAP['stock-quote']).toBe('get-stock-quote');
      expect(DATA_TYPE_TO_CAPABILITY_MAP['stock-basic-info']).toBe('get-stock-basic-info');
      expect(DATA_TYPE_TO_CAPABILITY_MAP['index-quote']).toBe('get-index-quote');
      expect(DATA_TYPE_TO_CAPABILITY_MAP['market-status']).toBe('get-market-status');
      expect(DATA_TYPE_TO_CAPABILITY_MAP['trading-days']).toBe('get-trading-days');
      expect(DATA_TYPE_TO_CAPABILITY_MAP['global-state']).toBe('get-global-state');
      expect(DATA_TYPE_TO_CAPABILITY_MAP['crypto-quote']).toBe('get-crypto-quote');
  ...
  describe('RECEIVER_PERFORMANCE_THRESHOLDS', () => {
    it('should define all performance thresholds', () => {
      ...
      expect(RECEIVER_PERFORMANCE_THRESHOLDS.LARGE_SYMBOL_COUNT_WARNING).toBe(50);
      expect(RECEIVER_PERFORMANCE_THRESHOLDS.PROVIDER_SELECTION_TIMEOUT_MS).toBe(5000);
      expect(RECEIVER_PERFORMANCE_THRESHOLDS.SYMBOL_TRANSFORMATION_TIMEOUT_MS).toBe(10000);
      expect(RECEIVER_PERFORMANCE_THRESHOLDS.DATA_FETCHING_TIMEOUT_MS).toBe(30000);
    });

    it('should have reasonable threshold values', () => {
      expect(RECEIVER_PERFORMANCE_THRESHOLDS.SLOW_REQUEST_MS).toBeGreaterThan(0);
      expect(RECEIVER_PERFORMANCE_THRESHOLDS.MAX_SYMBOLS_PER_REQUEST).toBeGreaterThan(0);
      expect(RECEIVER_PERFORMANCE_THRESHOLDS.LOG_SYMBOLS_LIMIT).toBeGreaterThan(0);
      expect(RECEIVER_PERFORMANCE_THRESHOLDS.LARGE_SYMBOL_COUNT_WARNING).toBeLessThan(RECEIVER_PERFORMANCE_THRESHOLDS.MAX_SYMBOLS_PER_REQUEST);
    });
  ...
  describe('RECEIVER_VALIDATION_RULES', () => {
    ...
    it('should have reasonable validation values', () => {
      ...
      expect(RECEIVER_VALIDATION_RULES.MAX_SYMBOL_LENGTH).toBeGreaterThan(RECEIVER_VALIDATION_RULES.MIN_SYMBOL_LENGTH);
      ...
      expect(RECEIVER_VALIDATION_RULES.MAX_DATA_TYPE_LENGTH).toBeGreaterThan(RECEIVER_VALIDATION_RULES.MIN_DATA_TYPE_LENGTH);
    });
  ...
  describe('MARKET_RECOGNITION_RULES', () => {
    it('should define all market recognition rules', () => {
      ...
      expect(MARKET_RECOGNITION_RULES.HK_PATTERNS.NUMERIC_PATTERN).toBeInstanceOf(RegExp);
      ...
      expect(MARKET_RECOGNITION_RULES.US_PATTERNS.ALPHA_PATTERN).toBeInstanceOf(RegExp);
      ...
      expect(MARKET_RECOGNITION_RULES.SZ_PATTERNS.PREFIX_PATTERNS).toEqual(['00', '30']);
      ...
      expect(MARKET_RECOGNITION_RULES.SH_PATTERNS.PREFIX_PATTERNS).toEqual(['60', '68']);
  ...
  describe('RECEIVER_METRICS', () => {
    it('should define all metric names', () => {
      ...
      expect(RECEIVER_METRICS.VALIDATION_ERRORS).toBe('receiver_validation_errors');
      expect(RECEIVER_METRICS.PROVIDER_SELECTION_TIME).toBe('receiver_provider_selection_time');
      expect(RECEIVER_METRICS.SYMBOL_TRANSFORMATION_TIME).toBe('receiver_symbol_transformation_time');
      expect(RECEIVER_METRICS.DATA_FETCHING_TIME).toBe('receiver_data_fetching_time');
      ...
      expect(RECEIVER_METRICS.SYMBOLS_PROCESSED).toBe('receiver_symbols_processed');
  ...
  describe('Integration with receiver service', () => {
    it('should support error message templating', () => {
      const maxLength = '20';
      const errorMessage = RECEIVER_ERROR_MESSAGES.INVALID_SYMBOL_FORMAT.replace('{maxLength}', maxLength);
    ...
    it('should support performance threshold checking', () => {
      ...
      const isSlowRequest = processingTime > RECEIVER_PERFORMANCE_THRESHOLDS.SLOW_REQUEST_MS;
    ...
    it('should support symbol validation', () => {
      const symbol = 'AAPL';
      const isValidFormat = RECEIVER_VALIDATION_RULES.SYMBOL_PATTERN.test(symbol);
      expect(isValidFormat).toBe(true);

      const invalidSymbol = 'AAPL@';
      const isInvalidFormat = RECEIVER_VALIDATION_RULES.SYMBOL_PATTERN.test(invalidSymbol);
      expect(isInvalidFormat).toBe(false);
    });

    it('should support market recognition', () => {
      const hkSymbol = '00700';
      const isHkPattern = MARKET_RECOGNITION_RULES.HK_PATTERNS.NUMERIC_PATTERN.test(hkSymbol);
      expect(isHkPattern).toBe(true);

      const usSymbol = 'AAPL';
      const isUsPattern = MARKET_RECOGNITION_RULES.US_PATTERNS.ALPHA_PATTERN.test(usSymbol);
