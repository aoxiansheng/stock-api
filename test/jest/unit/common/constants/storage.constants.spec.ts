...
describe('Storage Constants', () => {
  describe('STORAGE_ERROR_MESSAGES', () => {
    it('should define all error messages', () => {
      ...
      expect(STORAGE_ERROR_MESSAGES.REDIS_NOT_AVAILABLE).toBe('Redis连接不可用');
      ...
      expect(STORAGE_ERROR_MESSAGES.SERIALIZATION_FAILED).toBe('数据序列化失败');
      expect(STORAGE_ERROR_MESSAGES.DESERIALIZATION_FAILED).toBe('数据反序列化失败');
      expect(STORAGE_ERROR_MESSAGES.CACHE_UPDATE_FAILED).toBe('缓存更新失败');
      expect(STORAGE_ERROR_MESSAGES.PERSISTENT_STORAGE_FAILED).toBe('持久化存储失败');
      expect(STORAGE_ERROR_MESSAGES.DELETE_FAILED).toBe('删除失败');
      expect(STORAGE_ERROR_MESSAGES.STATS_GENERATION_FAILED).toBe('统计信息生成失败');
      expect(STORAGE_ERROR_MESSAGES.INVALID_STORAGE_TYPE).toBe('无效的存储类型');
  ...
  describe('STORAGE_WARNING_MESSAGES', () => {
    it('should define all warning messages', () => {
      expect(STORAGE_WARNING_MESSAGES.REDIS_CONNECTION_UNAVAILABLE).toBe('Redis连接不可用');
      expect(STORAGE_WARNING_MESSAGES.COMPRESSION_SKIPPED).toBe('跳过数据压缩');
      expect(STORAGE_WARNING_MESSAGES.CACHE_MISS).toBe('缓存未命中');
      expect(STORAGE_WARNING_MESSAGES.PERSISTENT_FALLBACK).toBe('回退到持久化存储');
      ...
      expect(STORAGE_WARNING_MESSAGES.TTL_CALCULATION_FAILED).toBe('TTL计算失败');
      expect(STORAGE_WARNING_MESSAGES.METADATA_PARSING_FAILED).toBe('元数据解析失败');
    });
  ...
  describe('STORAGE_PERFORMANCE_THRESHOLDS', () => {
    ...
    it('should have reasonable threshold values', () => {
      ...
      expect(STORAGE_PERFORMANCE_THRESHOLDS.SLOW_RETRIEVAL_MS).toBeGreaterThan(0);
      ...
      expect(STORAGE_PERFORMANCE_THRESHOLDS.HIGH_MEMORY_USAGE_MB).toBeGreaterThan(0);
      expect(STORAGE_PERFORMANCE_THRESHOLDS.LARGE_DATA_SIZE_KB).toBeGreaterThan(0);
    });
  ...
  describe('STORAGE_METRICS', () => {
    it('should define all metric names', () => {
      expect(STORAGE_METRICS.STORAGE_OPERATIONS_TOTAL).toBe('storage_operations_total');
      ...
      expect(STORAGE_METRICS.OPERATIONS_PER_SECOND).toBe('operations_per_second');
      expect(STORAGE_METRICS.MEMORY_USAGE_BYTES).toBe('memory_usage_bytes');
      expect(STORAGE_METRICS.TTL_REMAINING).toBe('ttl_remaining');
      expect(STORAGE_METRICS.PERSISTENT_STORAGE_SIZE).toBe('persistent_storage_size');
    });
  ...
  describe('Integration with storage service', () => {
    ...
    it('should support performance threshold checking', () => {
      ...
      const isSlowStorage = processingTime > STORAGE_PERFORMANCE_THRESHOLDS.SLOW_STORAGE_MS;
    ...
    it('should support data size validation', () => {
      ...
      const isLargeData = dataSize > STORAGE_PERFORMANCE_THRESHOLDS.LARGE_DATA_SIZE_KB * 1024;
