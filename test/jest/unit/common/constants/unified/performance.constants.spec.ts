...
describe('PERFORMANCE_CONSTANTS', () => {
  describe('RESPONSE_TIME_THRESHOLDS', () => {
    it('should have all required response time thresholds', () => {
      expect(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.FAST_REQUEST_MS).toBe(100);
      expect(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.NORMAL_REQUEST_MS).toBe(500);
      expect(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.SLOW_REQUEST_MS).toBe(1000);
      expect(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.CRITICAL_SLOW_MS).toBe(5000);
      // 特定业务操作阈值
      expect(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.SLOW_QUERY_MS).toBe(1000);
      expect(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.SLOW_STORAGE_MS).toBe(1000);
      expect(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.SLOW_MAPPING_MS).toBe(100);
      expect(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.SLOW_TRANSFORMATION_MS).toBe(5000);
      expect(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.SLOW_AUTHENTICATION_MS).toBe(300);
      expect(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.SLOW_CACHE_MS).toBe(50);
    });
  ...
  describe('TIMEOUTS', () => {
    it('should have all required timeout configurations', () => {
      ...
      expect(PERFORMANCE_CONSTANTS.TIMEOUTS.HTTP_REQUEST_TIMEOUT_MS).toBe(15000);
      expect(PERFORMANCE_CONSTANTS.TIMEOUTS.AUTHENTICATION_TIMEOUT_MS).toBe(5000);
      expect(PERFORMANCE_CONSTANTS.TIMEOUTS.FILE_UPLOAD_TIMEOUT_MS).toBe(120000);
    });
  ...
  describe('RETRY_SETTINGS', () => {
    it('should have retry configuration', () => {
      ...
      expect(PERFORMANCE_CONSTANTS.RETRY_SETTINGS.EXPONENTIAL_BACKOFF_BASE).toBe(2);
      expect(PERFORMANCE_CONSTANTS.RETRY_SETTINGS.MAX_RETRY_DELAY_MS).toBe(10000);
  ...
  describe('BATCH_LIMITS', () => {
    it('should have batch processing limits', () => {
      ...
      expect(PERFORMANCE_CONSTANTS.BATCH_LIMITS.MAX_CONCURRENT_OPERATIONS).toBe(10);
  ...
  describe('MEMORY_THRESHOLDS', () => {
    it('should have memory usage thresholds', () => {
      expect(PERFORMANCE_CONSTANTS.MEMORY_THRESHOLDS.LOW_MEMORY_USAGE_MB).toBe(50);
      expect(PERFORMANCE_CONSTANTS.MEMORY_THRESHOLDS.NORMAL_MEMORY_USAGE_MB).toBe(100);
      expect(PERFORMANCE_CONSTANTS.MEMORY_THRESHOLDS.HIGH_MEMORY_USAGE_MB).toBe(200);
      expect(PERFORMANCE_CONSTANTS.MEMORY_THRESHOLDS.CRITICAL_MEMORY_USAGE_MB).toBe(500);
      expect(PERFORMANCE_CONSTANTS.MEMORY_THRESHOLDS.MAX_OBJECT_SIZE_MB).toBe(10);
      expect(PERFORMANCE_CONSTANTS.MEMORY_THRESHOLDS.MAX_REQUEST_SIZE_MB).toBe(50);
    });
  });
  ...
  describe('CONNECTION_POOLS', () => {
    it('should have connection pool configurations', () => {
      ...
      expect(PERFORMANCE_CONSTANTS.CONNECTION_POOLS.IDLE_TIMEOUT_MS).toBe(300000);
      expect(PERFORMANCE_CONSTANTS.CONNECTION_POOLS.CONNECTION_TIMEOUT_MS).toBe(5000);
      expect(PERFORMANCE_CONSTANTS.CONNECTION_POOLS.ACQUIRE_TIMEOUT_MS).toBe(10000);
    });
  });

  describe('MONITORING', () => {
    it('should have monitoring configurations', () => {
      expect(PERFORMANCE_CONSTANTS.MONITORING.METRICS_COLLECTION_INTERVAL_MS).toBe(10000);
      expect(PERFORMANCE_CONSTANTS.MONITORING.HEALTH_CHECK_INTERVAL_MS).toBe(30000);
      ...
      expect(PERFORMANCE_CONSTANTS.MONITORING.SLOW_REQUEST_SAMPLE_RATE).toBe(1.0);
    });
  });
});
...
describe('calculateRetryDelay', () => {
  ...
  it('should cap delay at maximum retry delay', () => {
    ...
    expect(delay).toBeLessThanOrEqual(PERFORMANCE_CONSTANTS.RETRY_SETTINGS.MAX_RETRY_DELAY_MS);
  });
...
describe('isSlowResponse', () => {
  ...
  it('should return false when response time equals threshold', () => {
    const threshold = PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.SLOW_REQUEST_MS;
...
describe('getResponseTimeLevel', () => {
  ...
  it('should return "fast" for responses at fast threshold', () => {
    const level = getResponseTimeLevel(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.FAST_REQUEST_MS);
    expect(level).toBe('fast');
  });
  ...
  it('should return "normal" for responses at normal threshold', () => {
    const level = getResponseTimeLevel(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.NORMAL_REQUEST_MS);
    expect(level).toBe('normal');
  });
  ...
  it('should return "slow" for responses at slow threshold', () => {
    const level = getResponseTimeLevel(PERFORMANCE_CONSTANTS.RESPONSE_TIME_THRESHOLDS.SLOW_REQUEST_MS);
    expect(level).toBe('slow');
  });
