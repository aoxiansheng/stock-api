...
describe('Error Messages Constants - Branch Coverage Enhancement', () => {
  describe('Error message structure validation', () => {
    ...
    it('should contain Chinese error messages', () => {
      ...
      expect(chineseRegex.test(AUTH_ERROR_MESSAGES.INVALID_CREDENTIALS)).toBe(true);
      expect(chineseRegex.test(BUSINESS_ERROR_MESSAGES.VALIDATION_FAILED)).toBe(true);
      expect(chineseRegex.test(SYSTEM_ERROR_MESSAGES.INTERNAL_SERVER_ERROR)).toBe(true);
      ...
      Object.entries(AUTH_ERROR_MESSAGES).forEach(([key, message]) => {
        if (typeof message === 'string') {
          if (message.length < 4) {
            ...
            console.error(`发现短消息: key="${key}", value="${message}", 长度=${message.length}`);
          }
        }
      });
      ...
      const compositeErrorScenarios = [
        {
          ...
          messages: [AUTH_ERROR_MESSAGES.INVALID_CREDENTIALS, ERROR_MESSAGES.RATE_LIMIT_EXCEEDED]
        },
        {
          condition: 'business_validation_with_system_error',
          messages: [BUSINESS_ERROR_MESSAGES.VALIDATION_FAILED, SYSTEM_ERROR_MESSAGES.INTERNAL_SERVER_ERROR]
        }
      ];
    ...
    it('should handle concurrent access patterns', () => {
      ...
      return Promise.all(promises).then(results => {
        results.forEach((result, index) => {
          ...
          expect(result.business).toBe(BUSINESS_ERROR_MESSAGES.VALIDATION_FAILED);
          expect(result.system).toBe(SYSTEM_ERROR_MESSAGES.INTERNAL_SERVER_ERROR);
        });
      });
    });
  });
