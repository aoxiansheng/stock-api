...
describe('Transformer Constants', () => {
  describe('TRANSFORM_ERROR_MESSAGES', () => {
    it('should define all error messages', () => {
      expect(TRANSFORM_ERROR_MESSAGES.NO_MAPPING_RULE).toBe('未找到匹配的映射规则');
      expect(TRANSFORM_ERROR_MESSAGES.TRANSFORMATION_FAILED).toBe('数据转换失败');
      expect(TRANSFORM_ERROR_MESSAGES.VALIDATION_FAILED).toBe('转换后数据验证失败');
      expect(TRANSFORM_ERROR_MESSAGES.INVALID_RAW_DATA).toBe('原始数据格式无效');
      expect(TRANSFORM_ERROR_MESSAGES.MISSING_REQUIRED_FIELDS).toBe('缺少必需字段');
      expect(TRANSFORM_ERROR_MESSAGES.RULE_NOT_FOUND).toBe('指定的映射规则不存在');
      expect(TRANSFORM_ERROR_MESSAGES.BATCH_TRANSFORMATION_FAILED).toBe('批量转换失败');
      expect(TRANSFORM_ERROR_MESSAGES.PREVIEW_GENERATION_FAILED).toBe('预览生成失败');
      expect(TRANSFORM_ERROR_MESSAGES.SAMPLE_DATA_EXTRACTION_FAILED).toBe('样本数据提取失败');
  ...
  describe('TRANSFORM_PERFORMANCE_THRESHOLDS', () => {
    ...
    it('should have reasonable threshold values', () => {
      expect(TRANSFORM_PERFORMANCE_THRESHOLDS.SLOW_TRANSFORMATION_MS).toBeGreaterThan(0);
      expect(TRANSFORM_PERFORMANCE_THRESHOLDS.LARGE_DATASET_SIZE).toBeGreaterThan(0);
      expect(TRANSFORM_PERFORMANCE_THRESHOLDS.HIGH_MEMORY_USAGE_MB).toBeGreaterThan(0);
      expect(TRANSFORM_PERFORMANCE_THRESHOLDS.MAX_PROCESSING_TIME_MS).toBeGreaterThan(TRANSFORM_PERFORMANCE_THRESHOLDS.SLOW_TRANSFORMATION_MS);
    });
  ...
  describe('Integration with transformer service', () => {
    it('should support error message construction', () => {
      ...
      const errorMessage = `${TRANSFORM_ERROR_MESSAGES.NO_MAPPING_RULE}: provider '${provider}', dataType '${dataType}'`;
      expect(errorMessage).toBe("未找到匹配的映射规则: provider 'test-provider', dataType 'test-type'");
    });

    it('should support performance threshold checking', () => {
      const processingTime = 6000;
      const isSlowTransformation = processingTime > TRANSFORM_PERFORMANCE_THRESHOLDS.SLOW_TRANSFORMATION_MS;
