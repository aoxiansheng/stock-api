...
describe('GlobalExceptionFilter', () => {
  ...
  describe('catch', () => {
    ...
    it('should handle JSON parsing errors', () => {
      const jsonError = new SyntaxError('Unexpected token at position 5 in JSON');
    ...
    it('should handle ValidationError array', () => {
      ...
      expect(mockResponse.json).toHaveBeenCalledWith(
        expect.objectContaining({
          statusCode: 400,
          message: 'username should not be empty, username must be longer than 3 characters, email must be an email',
          data: null,
          timestamp: expect.any(String),
          error: {
            code: 'VALIDATION_ERROR',
            details: {
              ...
              fields: [
                ...
                { field: 'username', code: 'minLength', message: '长度不能少于 3 characters' },
                { field: 'email', code: 'isEmail', message: '必须是有效的邮箱地址' },
                { field: 'email.domain', code: 'isString', message: '必须是字符串' },
              ],
              path: '/api/v1/test',
              requestId: 'req-123',
            },
    ...
    it('should handle 401 errors with API key context', () => {
      ...
      const unauthorizedError = new HttpException('无效的API凭证', HttpStatus.UNAUTHORIZED);
    ...
    it('should handle different unauthorized message translations', () => {
      ...
      unauthorizedMessages.forEach(({ input, expected }) => {
        const unauthorizedError = new HttpException(input, HttpStatus.UNAUTHORIZED);
    ...
    it('should handle HTTP standard error message translations', () => {
      const standardErrorMappings = [
        { input: 'Bad Request', expected: '请求参数错误', status: HttpStatus.BAD_REQUEST },
        { input: 'Unauthorized', expected: '未授权访问', status: HttpStatus.UNAUTHORIZED },
        { input: 'Forbidden', expected: '访问被禁止', status: HttpStatus.FORBIDDEN },
        { input: 'Not Found', expected: '资源不存在', status: HttpStatus.NOT_FOUND },
        { input: 'Method Not Allowed', expected: '请求方法不允许', status: HttpStatus.METHOD_NOT_ALLOWED },
        { input: 'Internal Server Error', expected: '服务器内部错误', status: HttpStatus.INTERNAL_SERVER_ERROR },
        { input: 'Service Unavailable', expected: '服务暂时不可用', status: HttpStatus.SERVICE_UNAVAILABLE },
        { input: 'Gateway Timeout', expected: '网关超时', status: HttpStatus.GATEWAY_TIMEOUT },
        { input: 'Too Many Requests', expected: '请求频率超出限制', status: HttpStatus.TOO_MANY_REQUESTS },
      ];
          ...
          error: {
            ...
            details: expect.objectContaining({
              fields: [
                ...
                { field: 'dataType', message: '数据类型不支持的数据类型', code: 'INVALID_TYPE' },
                { field: 'provider', message: '提供商必须是有效的提供商', code: 'INVALID_FORMAT' },
              ],
            }),
          },
        })
          ...
          error: expect.objectContaining({
            details: expect.objectContaining({
              fields: expect.arrayContaining([
                expect.objectContaining({ field: 'user.profile.name', message: '不能为空' }),
              ]),
            }),
          }),
        }),
    ...
    it('should return correct error code for PAYLOAD_TOO_LARGE status', () => {
        const payloadTooLargeException = new HttpException('Payload Too Large', HttpStatus.PAYLOAD_TOO_LARGE);
     ...
     it('should return correct error code for GATEWAY_TIMEOUT status', () => {
        const gatewayTimeoutException = new HttpException('Gateway Timeout', HttpStatus.GATEWAY_TIMEOUT);
