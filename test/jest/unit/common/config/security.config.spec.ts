...
describe('SecurityConfig', () => {
  ...
  describe('rate limit configuration', () => {
    ...
    it('should have consistent Redis configuration', () => {
      ...
      expect(rateLimit.luaExpireBufferSeconds).toBe(RATE_LIMIT_CONFIG.REDIS.EXPIRE_BUFFER_SECONDS);
    });

    it('should support performance test mode', () => {
      const { rateLimit } = securityConfig;
      
      expect(rateLimit.performanceTestMode).toBe(RATE_LIMIT_CONFIG.PERFORMANCE.TEST_MODE);
      expect(rateLimit.multiplier).toBe(RATE_LIMIT_CONFIG.PERFORMANCE.MULTIPLIER);
    });
  ...
  describe('configuration consistency', () => {
    ...
    it('should have numeric values where expected', () => {
      const { passwordPolicy, session, data, permission, audit } = securityConfig;
