...
describe('ResponseInterceptor', () => {
  ...
  describe('intercept', () => {
    ...
    it('should handle different HTTP methods', (done) => {
      ...
      interceptor.intercept(mockExecutionContext, mockCallHandler).subscribe(result => {
        ...
        expect(console.time).toHaveBeenCalledWith('ResponseInterceptor耗时-POST /api/users');
        expect(console.timeEnd).toHaveBeenCalledWith('ResponseInterceptor耗时-POST /api/users');
        done();
      });
    });
    ...
    it('should measure execution time', (done) => {
      ...
      interceptor.intercept(mockExecutionContext, mockCallHandler).subscribe(() => {
        expect(console.time).toHaveBeenCalledWith('ResponseInterceptor耗时-GET /api/test');
        expect(console.timeEnd).toHaveBeenCalledWith('ResponseInterceptor耗时-GET /api/test');
        done();
      });
    });
  });
  ...
  describe('timing functionality', () => {
    it('should call console.time and console.timeEnd with correct labels', (done) => {
      ...
      interceptor.intercept(mockExecutionContext, mockCallHandler).subscribe(() => {
        expect(console.time).toHaveBeenCalledWith('ResponseInterceptor耗时-DELETE /api/items/123');
        expect(console.timeEnd).toHaveBeenCalledWith('ResponseInterceptor耗时-DELETE /api/items/123');
        done();
      });
    });
    ...
    it('should handle URL with query parameters', (done) => {
      ...
      interceptor.intercept(mockExecutionContext, mockCallHandler).subscribe(() => {
        expect(console.time).toHaveBeenCalledWith('ResponseInterceptor耗时-GET /api/users?page=1&limit=10');
        expect(console.timeEnd).toHaveBeenCalledWith('ResponseInterceptor耗时-GET /api/users?page=1&limit=10');
        done();
      });
    });
  });
