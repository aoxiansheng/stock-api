...
describe('URLSecurityValidator', () => {
  describe('validateURL', () => {
    describe('valid URLs', () => {
      ...
      it('should accept allowed external domains with HTTP', () => {
        const result = URLSecurityValidator.validateURL('http://hooks.slack.com/webhook');
        expect(result.valid).toBe(true);
        expect(result.error).toBeUndefined();
      });

      it('should accept subdomains of allowed domains', () => {
        const result = URLSecurityValidator.validateURL('https://api.hooks.slack.com/webhook');
        expect(result.valid).toBe(true);
        expect(result.error).toBeUndefined();
      });
      ...
      it('should handle case insensitive protocols and hostnames', () => {
        const result1 = URLSecurityValidator.validateURL('HTTPS://hooks.slack.com');
        const result2 = URLSecurityValidator.validateURL('https://HOOKS.SLACK.COM');
    ...
    describe('hostname validation', () => {
      ...
      it('should specifically reject AWS metadata service', () => {
        const result = URLSecurityValidator.validateURL('https://169.254.169.254/meta-data');
        expect(result.valid).toBe(false);
    ...
    describe('port validation', () => {
      it('should reject invalid port ranges', () => {
        ...
        invalidPorts.forEach(port => {
          const result = URLSecurityValidator.validateURL(`https://hooks.slack.com:${port}/webhook`);
          expect(result.valid).toBe(false);
          expect(result.error).toContain(port < 80 ? '端口范围无效' : '不允许的端口');
        });
      });
      ...
      it('should handle port edge cases', () => {
        ...
        // Test port 0
        const result2 = URLSecurityValidator.validateURL('https://hooks.slack.com:0/webhook');
    ...
    describe('path validation', () => {
      it('should reject suspicious path patterns', () => {
        ...
        const etcPath = URLSecurityValidator.validateURL('https://hooks.slack.com/etc/passwd');
        expect(etcPath.valid).toBe(false);
        expect(etcPath.error).toBe('URL路径包含可疑模式');

        const procPath = URLSecurityValidator.validateURL('https://hooks.slack.com/proc/self');
        expect(procPath.valid).toBe(false);
        expect(procPath.error).toBe('URL路径包含可疑模式');

        const devPath = URLSecurityValidator.validateURL('https://hooks.slack.com/dev/null');
        expect(devPath.valid).toBe(false);
        expect(devPath.error).toBe('URL路径包含可疑模式');

        const varPath = URLSecurityValidator.validateURL('https://hooks.slack.com/var/log');
        expect(varPath.valid).toBe(false);
        expect(varPath.error).toBe('URL路径包含可疑模式');

        const tmpPath = URLSecurityValidator.validateURL('https://hooks.slack.com/tmp/secret');
        expect(tmpPath.valid).toBe(false);
        expect(tmpPath.error).toBe('URL路径包含可疑模式');

        const metaDataPath = URLSecurityValidator.validateURL('https://hooks.slack.com/meta-data/instance');
        expect(metaDataPath.valid).toBe(false);
        expect(metaDataPath.error).toBe('URL路径包含可疑模式');

        const userDataPath = URLSecurityValidator.validateURL('https://hooks.slack.com/user-data/config');
        expect(userDataPath.valid).toBe(false);
        expect(userDataPath.error).toBe('URL路径包含可疑模式');

        const latestPath = URLSecurityValidator.validateURL('https://hooks.slack.com/latest/something');
      ...
      it('should test URL normalization behavior', () => {
        ...
        const result = URLSecurityValidator.validateURL('https://hooks.slack.com/path/../config');
      ...
      it('should accept normal paths', () => {
        ...
        normalPaths.forEach(path => {
          const result = URLSecurityValidator.validateURL(`https://hooks.slack.com${path}`);
          expect(result.valid).toBe(true);
          expect(result.error).toBeUndefined();
        });
      });
  ...
  describe('validateURLOrThrow', () => {
    ...
    it('should throw with correct error messages for different validation failures', () => {
      ...
      testCases.forEach(({ url, expectedError }) => {
        expect(() => URLSecurityValidator.validateURLOrThrow(url)).toThrow(expectedError);
      });
    });
  });
  ...
  describe('comprehensive security scenarios', () => {
    ...
    it('should validate whitelist matching correctly', () => {
      // Exact match
      expect(URLSecurityValidator.validateURL('https://hooks.slack.com').valid).toBe(true);
      
      // Subdomain match
      expect(URLSecurityValidator.validateURL('https://api.hooks.slack.com').valid).toBe(true);
      
      // Partial match should not work
      expect(URLSecurityValidator.validateURL('https://fakehooks.slack.com.evil.com').valid).toBe(false);
      
      // Similar but different domain
      expect(URLSecurityValidator.validateURL('https://hooks-slack.com').valid).toBe(false);
    });
    ...
    it('should test all IP pattern ranges', () => {
      ...
      ipRangeTests.forEach(({ ip, shouldReject }) => {
        ...
        if (shouldReject) {
          expect(result.valid).toBe(false);
          expect(result.error).toMatch(/不允许访问内部IP地址|不允许直接访问IP地址/);
        } else {
