...
describe('LogLevelController', () => {
  ...
  describe('Configuration Loading', () => {
    ...
    it('should handle invalid JSON configuration file gracefully', async () => {
      const invalidConfigPath = path.join(process.cwd(), 'test-invalid-config.json');
  ...
  describe('shouldLog Core Logic', () => {
    ...
    it('should handle invalid log levels gracefully', () => {
      // 应该允许未知的日志级别（返回 true）
      expect(controller.shouldLog('TestService', 'unknown' as LogLevel)).toBe(true);
    });
  ...
  describe('Configuration Validation', () => {
    it('should validate and fix invalid global log level', async () => {
      ...
      // 模拟加载无效配置
      jest.spyOn(controller as any, 'loadFromConfigFile').mockReturnValue(invalidConfig);
