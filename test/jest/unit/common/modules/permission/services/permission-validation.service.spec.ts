...
describe("PermissionValidationService", () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<PermissionValidationService>(
      PermissionValidationService,
    );
  });
  ...
  it("should call validatePermissions on module init", async () => {
    ...
    (
      mockPermissionValidator.validateAllControllers as jest.Mock
    ).mockResolvedValue([]);
  ...
  it("should log warnings and report if violations are found", async () => {
    ...
    (mockPermissionValidator.generateReport as jest.Mock).mockReturnValue(
      "Test Report",
    );
