...
import { PermissionDecoratorValidator, PermissionViolation } from '../../../../../../../src/common/modules/permission/validators/permission-decorator.validator';
...
describe('PermissionDecoratorValidator', () => {
  ...
  describe('validateAllControllers', () => {
    ...
    it('should validate all controllers and aggregate results', async () => {
      ...
      const mockControllers: InstanceWrapper[] = [
        { name: 'TestController1', metatype: TestController1 } as InstanceWrapper,
        { name: 'TestController2', metatype: TestController2 } as InstanceWrapper,
      ];
      mockDiscoveryService.getControllers.mockReturnValue(mockControllers);

      // 模拟 getRoutes 方法
      jest.spyOn(validator as any, 'getRoutes').mockReturnValueOnce([
      ...
      jest.spyOn(validator as any, 'hasRequirePermissions').mockReturnValue(false);
      jest.spyOn(validator as any, 'getRequiredPermissions').mockReturnValue([]);
      jest.spyOn(validator as any, 'validatePermissionLevel').mockReturnValue(null);
      jest.spyOn(validator as any, 'validatePermissionCombination').mockReturnValue(null);
  ...
  describe('validateController', () => {
    ...
    beforeEach(() => {
      ...
      mockMetadataScanner.getAllMethodNames.mockReturnValue(['method1', 'method2', 'method3']);
    ...
    it('should identify missing @RequirePermissions when @ApiKeyAuth is present', async () => {
      ...
      const result = await (validator as any).validateController(mockControllerInstance);
  ...
  describe('generateReport', () => {
    ...
    it('should generate a report with violations', () => {
      ...
      expect(report).toContain('问题: @ApiKeyAuth装饰器必须配合@RequirePermissions使用');
      expect(report).toContain('建议: 添加 @RequirePermissions(Permission.XXX) 装饰器');
    });
  });
});
