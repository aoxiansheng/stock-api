...
describe('NotificationService Optimization Features', () => {
  ...
  describe('Enhanced Single Notification', () => {
    ...
    it('should use template utility for unsupported type error', async () => {
      ...
      try {
        await service.sendNotification(mockAlert as any, mockRule, mockChannelConfig);
      } catch (error) {
  ...
  describe('Enhanced Template Generation', () => {
    it('should use constants and utility for template generation', async () => {
      ...
      const template = service.generateTemplate(mockAlert as any, mockRuleForTemplate as any);
      ...
      expect(template.variables).toHaveProperty(NOTIFICATION_TEMPLATE_VARIABLES.ALERT_ID, 'alert_123');
      expect(template.variables).toHaveProperty(NOTIFICATION_TEMPLATE_VARIABLES.RULE_NAME, 'CPU Alert Rule');
    });
  });

  describe('Utility Functions', () => {
    it('should generate error messages using templates', () => {
      const message = NotificationTemplateUtil.generateErrorMessage('UNSUPPORTED_TYPE', {
    ...
    it('should format templates correctly', () => {
      const template = 'Alert {{alertId}} for rule {{ruleName}} has value {{value}}';
      const variables = {
        alertId: 'alert_123',
        ruleName: 'Test Rule',
        value: 85,
    ...
    it('should validate email addresses correctly', () => {
      expect(NotificationTemplateUtil.isValidEmail('test@example.com')).toBe(true);
      expect(NotificationTemplateUtil.isValidEmail('user.name+tag@domain.co.uk')).toBe(true);
    ...
    it('should validate URLs correctly', () => {
      ...
      expect(NotificationTemplateUtil.isValidUrl('http://localhost:3000')).toBe(true);
    ...
    it('should generate template variables correctly', () => {
      ...
      const variables = NotificationTemplateUtil.generateTemplateVariables(mockAlert, mockRule);

      expect(variables[NOTIFICATION_TEMPLATE_VARIABLES.ALERT_ID]).toBe('alert_123');
      expect(variables[NOTIFICATION_TEMPLATE_VARIABLES.RULE_NAME]).toBe('CPU Alert Rule');
      ...
      expect(variables[NOTIFICATION_TEMPLATE_VARIABLES.SEVERITY]).toBe('critical');
    });
  });
});
