...
describe('WebhookSender', () => {
  ...
  const validConfig = { url: 'https://example.com/webhook', headers: { 'X-Test': 'true' }, timeout: 5000 };
  ...
  describe('send', () => {
    it('should send a notification successfully', async () => {
      ...
      expect(httpService.post).toHaveBeenCalledWith(validConfig.url, expect.any(Object), {
  ...
  describe('test', () => {
    ...
    it('should return false for a failed test connection', async () => {
        (httpService.get as jest.Mock).mockReturnValue(throwError(() => new Error()));
    ...
    it('should throw BadRequestException if Authorization header is present', async () => {
        const configWithAuth = { ...validConfig, headers: { 'Authorization': 'Bearer token' } };
        await expect(sender.test(configWithAuth)).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('validateConfig', () => {
    ...
    it('should be invalid if Authorization header is present', () => {
        const { valid, errors } = sender.validateConfig({ ...validConfig, headers: { 'Authorization': 'Bearer token' } });
        expect(valid).toBe(false);
        expect(errors).toContain('出于安全原因，配置中不允许包含 Authorization 头部');
    });
  });
}); 