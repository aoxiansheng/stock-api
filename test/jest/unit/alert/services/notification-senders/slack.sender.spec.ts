...
describe('SlackSender', () => {
  ...
  describe('send', () => {
    ...
    it('should use default username and icon_emoji if not provided', async () => {
      ...
      const configWithoutDefaults = { ...mockConfig, username: undefined, icon_emoji: undefined };
  ...
  describe('validateConfig', () => {
    ...
    it('should return valid: false if webhook_url is missing', () => {
      ...
      expect(result.errors).toContain('Webhook URL is required for Slack notification');
    });
    ...
    it('should return valid: false if webhook_url is not a valid Slack URL', () => {
      const config = { webhook_url: 'https://example.com', channel: '#general' };
      const result = sender.validateConfig(config);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Webhook URL must be a valid Slack webhook URL');
    });
    ...
    it('should return valid: false if channel is missing', () => {
      const config = { webhook_url: 'https://hooks.slack.com/services/T00000000/B00000000/XXXXXXXXXXXXXXXXXXXXXXXX' };
      const result = sender.validateConfig(config);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Channel is required for Slack notification');
    });

    it('should return valid: false if channel is not a string', () => {
      const config = { webhook_url: 'https://hooks.slack.com/services/T00000000/B00000000/XXXXXXXXXXXXXXXXXXXXXXXX', channel: 123 };
      const result = sender.validateConfig(config);
      expect(result.valid).toBe(false);
    ...
    it('should return valid: false if channel does not start with # or @', () => {
      const config = { webhook_url: 'https://hooks.slack.com/services/T00000000/B00000000/XXXXXXXXXXXXXXXXXXXXXXXX', channel: 'general' };
      const result = sender.validateConfig(config);
      expect(result.valid).toBe(false);
  ...
  describe('getSeverityColor', () => {
    ...
    it('should return warning for unknown severity', () => {
      expect(sender['getSeverityColor']('UNKNOWN' as AlertSeverity)).toBe('warning');
    });
  });
});