...
import { Alert, AlertRule, AlertSeverity, NotificationChannelType } from '../../../../../../src/alert/types/alert.types';
...
describe('SlackSender', () => {
  ...
  const mockAlert: Alert = { id: 'alert-1', ruleId: 'rule-1', ruleName: 'Test Rule', message: 'Test alert message', metric: 'cpu', value: 90, threshold: 80, severity: AlertSeverity.CRITICAL, status: 'firing', startTime: new Date(), createdAt: new Date(), updatedAt: new Date() };
  const validConfig = { webhook_url: 'https://hooks.slack.com/services/test', channel: '#test' };
  ...
  describe('send', () => {
    ...
    it('should return a failed result for invalid webhook URL', async () => {
        ...
        expect(result.error).toContain('Slack Webhook URL安全检查失败: Invalid URL');
    });
  ...
  describe('test', () => {
    ...
    it('should return false for a failed test connection', async () => {
        (httpService.post as jest.Mock).mockReturnValue(throwError(() => new Error()));
  ...
  describe('validateConfig', () => {
    ...
    it('should be invalid if webhook_url is missing', () => {
        ...
        expect(errors).toContain('Webhook URL is required for Slack notification');
    });
    ...
    it('should be invalid if channel does not start with # or @', () => {
        const { valid, errors } = sender.validateConfig({ ...validConfig, channel: 'test' });
        expect(valid).toBe(false);
