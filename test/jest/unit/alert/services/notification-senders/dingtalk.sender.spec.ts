...
describe('DingTalkSender', () => {
  ...
  const mockRule: AlertRule = { id: 'rule-1', name: 'Test Rule', metric: 'cpu', operator: 'gt', threshold: 80, duration: 60, severity: 'critical', enabled: true, channels: [], cooldown: 300, createdAt: new Date(), updatedAt: new Date() };
  const mockAlert: Alert = { id: 'alert-1', ruleId: 'rule-1', ruleName: 'Test Rule', message: 'Test alert message', metric: 'cpu', value: 90, threshold: 80, severity: 'critical', status: 'firing', startTime: new Date(), createdAt: new Date(), updatedAt: new Date() };
  const validConfig = { webhook_url: 'https://oapi.dingtalk.com/robot/send?access_token=test', secret: 'test-secret' };
  ...
  describe('send', () => {
    it('should send a notification successfully', async () => {
      const mockResponse = { data: { errcode: 0 }, status: 200, statusText: 'OK', headers: {}, config: {} } as AxiosResponse;
    ...
    it('should return a failed result for invalid webhook URL', async () => {
        ...
        expect(result.error).toContain('DingTalk Webhook URL安全检查失败: Invalid URL');
    });

    it('should handle API errors from DingTalk', async () => {
      const mockResponse = { data: { errcode: 1, errmsg: 'error' }, status: 200, statusText: 'OK', headers: {}, config: {} } as AxiosResponse;
      jest.spyOn(httpService, 'post').mockReturnValue(of(mockResponse));
  ...
  describe('test', () => {
    ...
    it('should return false for a failed test connection', async () => {
        jest.spyOn(httpService, 'post').mockReturnValue(throwError(() => new Error()));
    ...
    it('should throw BadRequestException for invalid URL in test', async () => {
        (URLSecurityValidator.validateURL as jest.Mock).mockReturnValue({ valid: false, error: 'Invalid URL' });
        await expect(sender.test(validConfig)).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('validateConfig', () => {
    ...
    it('should be invalid if webhook_url is missing', () => {
        ...
        expect(errors).toContain('Webhook URL is required for DingTalk notification');
    });
    ...
    it('should be invalid for a non-dingtalk URL', () => {
        const { valid, errors } = sender.validateConfig({ webhook_url: 'https://example.com', secret: 'secret' });
        expect(valid).toBe(false);
        expect(errors).toContain('Webhook URL must be a valid DingTalk webhook URL');
    });

    it('should be invalid if security validation fails', () => {
        (URLSecurityValidator.validateURL as jest.Mock).mockReturnValue({ valid: false, error: 'Security fail' });
        const { valid, errors } = sender.validateConfig(validConfig);
        expect(valid).toBe(false);
