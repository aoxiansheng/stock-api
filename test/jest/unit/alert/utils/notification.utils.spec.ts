...
jest.mock("../../../../../src/alert/constants/notification.constants", () => {
  const original = jest.requireActual(
    "../../../../../src/alert/constants/notification.constants",
  );
  return {
    ...original,
    NOTIFICATION_ERROR_TEMPLATES: {
...
describe("NotificationTemplateUtil", () => {
  describe("replaceErrorTemplate", () => {
    ...
    it("should replace multiple placeholders in template", () => {
      ...
      expect(result).toBe(
        "User user-456 action login failed: Invalid password",
      );
    });
  });
  ...
  describe("formatTemplate", () => {
    ...
    it("should remove comments from template", () => {
      const template =
        "Hello {userName},/* this is a comment */ your order is ready.";
    ...
    it("should handle if blocks correctly", () => {
      const template =
        "Hello {userName},{#if hasDiscount} you have a discount.{/if}";
    ...
    it("should handle unless blocks correctly", () => {
      const template =
        "Hello {userName},{#unless isPaid} please pay your invoice.{/unless}";
  ...
  describe("isValidVariableName", () => {
    it("should validate correct variable names", () => {
      expect(NotificationTemplateUtil.isValidVariableName("userName")).toBe(
        true,
      );
      expect(NotificationTemplateUtil.isValidVariableName("userVar123")).toBe(
        true,
      );
    });

    it("should reject invalid variable names", () => {
      expect(NotificationTemplateUtil.isValidVariableName("123var")).toBe(
        false,
      );
      expect(NotificationTemplateUtil.isValidVariableName("_private")).toBe(
        false,
      );
      expect(NotificationTemplateUtil.isValidVariableName("user-name")).toBe(
        false,
      );
    });
  });
  ...
  describe("isValidTemplateLength", () => {
    it("should validate template length within limits", () => {
      ...
      expect(
        NotificationTemplateUtil.isValidTemplateLength(shortTemplate),
      ).toBe(true);
  ...
  describe("calculateRetryDelay", () => {
    it("should calculate exponential backoff delay", () => {
      ...
      expect(delay1).toBeGreaterThanOrEqual(
        NOTIFICATION_RETRY_CONFIG.INITIAL_DELAY_MS *
          (1 - NOTIFICATION_RETRY_CONFIG.JITTER_FACTOR),
      );
    });

    it("should not exceed maximum delay", () => {
      const largeAttemptDelay =
        NotificationTemplateUtil.calculateRetryDelay(10);
      expect(largeAttemptDelay).toBeLessThanOrEqual(
        NOTIFICATION_RETRY_CONFIG.MAX_DELAY_MS,
      );
    });
  });
  ...
  describe("generateTemplateVariables", () => {
    it("should generate template variables from alert and rule", () => {
      ...
      expect(variables.tags).toBe(
        JSON.stringify({ host: "server-1", env: "production" }, null, 2),
      );
