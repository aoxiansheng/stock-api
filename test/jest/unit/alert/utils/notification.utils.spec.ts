...
describe('NotificationTemplateUtil', () => {
  ...
  describe('generateTemplateVariables', () => {
    it('should generate template variables from alert and rule', () => {
      ...
      expect(variables.alertId).toBe('alert-123');
      expect(variables.ruleName).toBe('High CPU Rule');
      ...
      expect(variables.duration).toBe(300); // 5 mins = 300s
      expect(variables.tags).toBe(JSON.stringify({ host: 'server-1', env: 'production' }, null, 2));
      expect(variables.ruleId).toBe('rule-456');
      expect(variables.ruleDescription).toBe('Rule for high CPU usage');
