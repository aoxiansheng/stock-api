...
describe("AlertController", () => {
  ...
  let alertHistoryService: jest.Mocked<
    ReturnType<typeof mockAlertHistoryService>
  >;
  let notificationService: jest.Mocked<
    ReturnType<typeof mockNotificationService>
  >;
  ...
  describe("getRules", () => {
    it("should return all alert rules", async () => {
      const mockRules: IAlertRule[] = [
        { id: "1", name: "Test Rule" } as IAlertRule,
      ];
  ...
  describe("toggleRule", () => {
    it("should toggle an alert rule to enabled", async () => {
      ...
      await expect(
        controller.toggleRule("1", { enabled: true }),
    ...
    it("should toggle an alert rule to disabled", async () => {
      alertingService.toggleRule.mockResolvedValue(undefined);
      await expect(
        controller.toggleRule("1", { enabled: false }),
      ).resolves.toBeUndefined();
  ...
  describe("getAlertHistory", () => {
    it("should return paginated alert history", async () => {
      ...
      paginationService._createPaginatedResponse.mockReturnValue(
        mockPaginatedResult,
      );

      const result = await controller.getAlertHistory(query);
      expect(result).toEqual(mockPaginatedResult);
      expect(alertHistoryService._queryAlerts).toHaveBeenCalledWith(
  ...
  describe("resolveAlert", () => {
    ...
    it("should resolve an alert", async () => {
      ...
      await expect(
        controller.resolveAlert("1", resolveDto),
      ).resolves.toBeUndefined();
    ...
    it("should throw NotFoundException if alert not found", async () => {
      ...
      await expect(
        controller.resolveAlert("nonexistent", resolveDto),
  ...
  describe("testNotificationChannel", () => {
    it("should test a notification channel successfully", async () => {
      const testDto: TestNotificationChannelDto & {
  ...
  describe("triggerEvaluation", () => {
    ...
    it("should throw BadRequestException if specified rule does not exist", async () => {
      ...
      await expect(
        controller.triggerEvaluation(triggerDto, mockReq),
      ).rejects.toThrow(new BadRequestException("指定的告警规则不存在"));
    });
    ...
    it("should apply rate limiting", async () => {
      ...
      await expect(
        controller.triggerEvaluation({ metrics: [] }, mockReq),
      ).rejects.toThrow(
