...
describe('NotificationChannelDTOs', () => {
  ...
  describe('Config DTOs', () => {
    it('EmailConfigDto should pass validation', async () => {
      const dto = plainToClass(EmailConfigDto, { to: 'test@example.com', subject: 'Test' });
      const errors = await validate(dto);
      expect(errors.length).toBe(0);
    });

    it('WebhookConfigDto should pass validation', async () => {
      const dto = plainToClass(WebhookConfigDto, { url: 'http://example.com/webhook' });
      const errors = await validate(dto);
      expect(errors.length).toBe(0);
    });

    it('SlackConfigDto should pass validation', async () => {
        const dto = plainToClass(SlackConfigDto, { webhook_url: 'http://slack.com/webhook', channel: 'general' });
        const errors = await validate(dto);
        expect(errors.length).toBe(0);
    });

    it('DingTalkConfigDto should pass validation', async () => {
        const dto = plainToClass(DingTalkConfigDto, { webhook_url: 'http://dingtalk.com/webhook', secret: 'secret' });
        const errors = await validate(dto);
        expect(errors.length).toBe(0);
    });

    it('SmsConfigDto should pass validation', async () => {
        const dto = plainToClass(SmsConfigDto, { phone: '1234567890', template: 'template-id' });
        const errors = await validate(dto);
        expect(errors.length).toBe(0);
    });
  ...
  describe('Other DTOs', () => {
    ...
    it('UpdateNotificationChannelDto should pass validation', async () => {
        const dto = plainToClass(UpdateNotificationChannelDto, { name: 'updated-name' });
        const errors = await validate(dto);
        expect(errors.length).toBe(0);
    });

    it('TestNotificationChannelDto should pass validation', async () => {
        const dto = plainToClass(TestNotificationChannelDto, { message: 'Test message' });
        const errors = await validate(dto);
        expect(errors.length).toBe(0);
    });
  });
