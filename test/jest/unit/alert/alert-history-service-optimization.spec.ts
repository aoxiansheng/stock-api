...
describe('AlertHistoryService Optimization Features', () => {
  ...
  describe('Enhanced Alert Status Update', () => {
    it('should use constants for status update start logging', async () => {
      ...
      const updateDto = { status: AlertStatus.ACKNOWLEDGED, acknowledgedBy: 'user_123' };
      await service.updateAlertStatus('alrt_123', updateDto.status, updateDto.acknowledgedBy);
    ...
    it('should use constants for successful update logging', async () => {
      ...
      const updateDto = { status: AlertStatus.RESOLVED, resolvedBy: 'user_123' };
      await service.updateAlertStatus('alrt_123', updateDto.status, updateDto.resolvedBy);
  ...
  describe('Enhanced Alert Query', () => {
    it('should use constants and utility for query processing', async () => {
      ...
      const mockAlerts: IAlert[] = [
        { id: 'alrt_1', severity: AlertSeverity.CRITICAL, ruleId: 'r1', ruleName: 'rule1', metric: 'cpu', value: 90, threshold: 80, status: AlertStatus.FIRING, startTime: new Date(), message: 'high cpu' },
        { id: 'alrt_2', severity: AlertSeverity.WARNING, ruleId: 'r2', ruleName: 'rule2', metric: 'mem', value: 85, threshold: 80, status: AlertStatus.FIRING, startTime: new Date(), message: 'high mem' },
      ];
  ...
  describe('Enhanced Batch Operations', () => {
    it('should use constants and validation for batch updates', async () => {
      ...
      const result = await service.batchUpdateAlertStatus(alertIds, AlertStatus.RESOLVED, 'user_123');
  ...
  describe('Enhanced Service Stats', () => {
    it('should use constants for service stats', () => {
      ...
      expect(stats.defaultCleanupDays).toBe(ALERT_HISTORY_CONFIG.DEFAULT_CLEANUP_DAYS);
      expect(stats.idPrefixFormat).toBe(ALERT_HISTORY_CONFIG.ID_FORMAT_TEMPLATE);
      expect(stats.maxBatchUpdateSize).toBe(ALERT_HISTORY_CONFIG.BATCH_UPDATE_LIMIT);
    });
  });
  ...
  describe('Utility Functions', () => {
    ...
    it('should generate batch result summary correctly', () => {
      const summary = AlertHistoryUtil.generateBatchResultSummary(8, 2, ['error1', 'error2']);
