...
describe('Notification Constants', () => {
  ...
  describe('NOTIFICATION_MESSAGES', () => {
    ...
    it('should have warning messages', () => {
      expect(NOTIFICATION_MESSAGES.NO_ENABLED_CHANNELS).toBe('没有启用的通知渠道');
      expect(NOTIFICATION_MESSAGES.PARTIAL_BATCH_SUCCESS).toBe('批量通知部分成功');
    });

    it('should have info messages', () => {
      expect(NOTIFICATION_MESSAGES.NOTIFICATION_QUEUED).toBe('通知已加入队列');
      expect(NOTIFICATION_MESSAGES.BATCH_PROCESSING_PROGRESS).toBe('批量处理进度更新');
    });
  });

  describe('NOTIFICATION_ERROR_TEMPLATES', () => {
    it('should have all error template keys', () => {
      expect(NOTIFICATION_ERROR_TEMPLATES.UNSUPPORTED_TYPE).toBe('不支持的通知类型: {channelType}');
      expect(NOTIFICATION_ERROR_TEMPLATES.SEND_FAILED_WITH_REASON).toBe('发送失败: {error}');
  ...
  describe('NOTIFICATION_TEMPLATE_PATTERNS', () => {
    it('should have all regex pattern sources and flags', () => {
      ...
      expect(NOTIFICATION_TEMPLATE_PATTERNS.IF_BLOCK_PATTERN_SOURCE).toBeTruthy();
      expect(NOTIFICATION_TEMPLATE_PATTERNS.UNLESS_BLOCK_PATTERN_SOURCE).toBeTruthy();
      expect(NOTIFICATION_TEMPLATE_PATTERNS.EACH_BLOCK_PATTERN_SOURCE).toBeTruthy();
    });
  ...
  describe('NOTIFICATION_VALIDATION_RULES', () => {
    it('should have validation pattern sources and flags', () => {
      expect(NOTIFICATION_VALIDATION_RULES.VARIABLE_NAME_PATTERN_SOURCE).toBeTruthy();
...
describe('NotificationTemplateUtil', () => {
  ...
  describe('generateErrorMessage', () => {
    it('should generate error message from template', () => {
      const result = NotificationTemplateUtil.generateErrorMessage('UNSUPPORTED_TYPE', {
        channelType: 'invalid',
      });
    ...
    it('should generate batch processing error message', () => {
      const result = NotificationTemplateUtil.generateErrorMessage('BATCH_PROCESSING_ERROR', {
        successful: 8,
        total: 10,
        failed: 2,
      });
  ...
  describe('isValidVariableName', () => {
    it('should return true for valid variable names', () => {
      expect(NotificationTemplateUtil.isValidVariableName('alertId')).toBe(true);
      expect(NotificationTemplateUtil.isValidVariableName('rule_name')).toBe(true);
      expect(NotificationTemplateUtil.isValidVariableName('a')).toBe(true);
    });

    it('should return false for invalid variable names', () => {
      expect(NotificationTemplateUtil.isValidVariableName('123invalid')).toBe(false);
      expect(NotificationTemplateUtil.isValidVariableName('invalid-name')).toBe(false);
      expect(NotificationTemplateUtil.isValidVariableName('')).toBe(false);
    });

    it('should return false for names exceeding max length', () => {
      const longName = 'a'.repeat(51);
      expect(NotificationTemplateUtil.isValidVariableName(longName)).toBe(false);
    });
  });
  ...
  describe('isValidTemplateLength', () => {
    it('should return true for valid template lengths', () => {
      ...
      expect(NotificationTemplateUtil.isValidTemplateLength('Hello {{name}}')).toBe(true);
    });
    ...
    it('should return false for templates exceeding max length', () => {
      ...
      expect(NotificationTemplateUtil.isValidTemplateLength(longTemplate)).toBe(false);
    });
  });
  ...
  describe('isValidEmail', () => {
    it('should return true for valid emails', () => {
      ...
      expect(NotificationTemplateUtil.isValidEmail('test.user@domain.org')).toBe(true);
    });
  ...
  describe('isValidUrl', () => {
    it('should return true for valid URLs', () => {
      expect(NotificationTemplateUtil.isValidUrl('https://example.com')).toBe(true);
      expect(NotificationTemplateUtil.isValidUrl('http://api.example.com/webhook')).toBe(true);
    });

    it('should return false for invalid URLs', () => {
      expect(NotificationTemplateUtil.isValidUrl('ftp://example.com')).toBe(false);
      expect(NotificationTemplateUtil.isValidUrl('invalid-url')).toBe(false);
      expect(NotificationTemplateUtil.isValidUrl('mailto:test@example.com')).toBe(false);
    });
  });
  ...
  describe('generateTemplateVariables', () => {
    it('should generate template variables from alert and rule', () => {
      ...
      expect(variables[NOTIFICATION_TEMPLATE_VARIABLES.ALERT_ID]).toBe('ALERT-001');
      expect(variables[NOTIFICATION_TEMPLATE_VARIABLES.RULE_NAME]).toBe('CPU High Usage');
      expect(variables[NOTIFICATION_TEMPLATE_VARIABLES.METRIC]).toBe('cpu_usage');
