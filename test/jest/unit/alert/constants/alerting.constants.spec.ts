...
describe("Alerting Constants", () => {
  describe("ALERTING_OPERATIONS", () => {
    it("应包含所有必需的操作常量", () => {
      ...
      expect(ALERTING_OPERATIONS.EVALUATE_RULES_SCHEDULED).toBe(
        "evaluateRulesScheduled",
      );
      ...
      expect(ALERTING_OPERATIONS.HANDLE_RULE_EVALUATION).toBe(
        "handleRuleEvaluation",
      );
      expect(ALERTING_OPERATIONS.CONVERT_EVENT_TO_METRIC).toBe(
        "convertEventToMetric",
      );
  ...
  describe("ALERTING_MESSAGES", () => {
    it("应包含所有成功消息", () => {
      ...
      expect(ALERTING_MESSAGES.RULE_STATUS_TOGGLED).toBe(
        "切换告警规则状态成功",
      );
    ...
    it("应包含所有错误消息", () => {
      expect(ALERTING_MESSAGES.INITIALIZATION_FAILED).toBe(
        "初始化加载活跃告警失败",
      );
      expect(ALERTING_MESSAGES.CREATE_RULE_DB_FAILED).toBe(
        "创建告警规则数据库操作失败",
      );
  ...
  describe("AlertingTemplateUtil", () => {
    describe("replaceErrorTemplate", () => {
      it("应正确替换模板中的占位符", () => {
        ...
        const result = AlertingTemplateUtil.replaceErrorTemplate(
          template,
          params,
        );
    ...
    describe("generateErrorMessage", () => {
      it("应生成规则未找到错误消息", () => {
        const result = AlertingTemplateUtil.generateErrorMessage(
          "RULE_NOT_FOUND",
          { ruleId: "rule_123" },
        );
      ...
      it("应生成指标处理错误消息", () => {
        ...
        expect(result).toBe(
          "处理指标 cpu_usage 时发生错误: Connection timeout",
        );
      });
    ...
    describe("isValidRuleName", () => {
      it("应接受有效的规则名称", () => {
        expect(AlertingTemplateUtil.isValidRuleName("CPU使用率告警")).toBe(
          true,
        );
        expect(AlertingTemplateUtil.isValidRuleName("Memory Alert")).toBe(true);
        expect(
          AlertingTemplateUtil.isValidRuleName("Database-Connection_Alert.v1"),
        ).toBe(true);
      ...
      it("应拒绝无效的规则名称", () => {
        ...
        expect(AlertingTemplateUtil.isValidRuleName(undefined as any)).toBe(
          false,
        );
    ...
    describe("isValidRuleId", () => {
      it("应接受有效的规则ID", () => {
        expect(AlertingTemplateUtil.isValidRuleId("rule_abc123_def456")).toBe(
          true,
        );
        expect(AlertingTemplateUtil.isValidRuleId("rule_123abc_456def")).toBe(
          true,
        );
      });
      ...
      it("应拒绝无效的规则ID", () => {
        ...
        expect(AlertingTemplateUtil.isValidRuleId("rule_abc_defgh")).toBe(
          false,
        ); // 超过6位
        expect(AlertingTemplateUtil.isValidRuleId("rule_ABC_def123")).toBe(
          false,
        ); // 大写字母
    ...
    describe("isValidMetricName", () => {
      it("应接受有效的指标名称", () => {
        ...
        expect(AlertingTemplateUtil.isValidMetricName("memory.usage")).toBe(
          true,
        );
        expect(AlertingTemplateUtil.isValidMetricName("Database123")).toBe(
          true,
        );
        expect(AlertingTemplateUtil.isValidMetricName("system_load_1m")).toBe(
          true,
        );
      });
      ...
      it("应拒绝无效的指标名称", () => {
        ...
        expect(AlertingTemplateUtil.isValidMetricName(undefined as any)).toBe(
          false,
        );
    ...
    describe("isValidThreshold", () => {
      it("应接受有效的阈值", () => {
        ...
        expect(
          AlertingTemplateUtil.isValidThreshold(Number.MAX_SAFE_INTEGER),
        ).toBe(true);
      });

      it("应拒绝无效的阈值", () => {
        expect(AlertingTemplateUtil.isValidThreshold(null as any)).toBe(false);
        expect(AlertingTemplateUtil.isValidThreshold(undefined as any)).toBe(
          false,
        );
        ...
        expect(
          AlertingTemplateUtil.isValidThreshold(Number.MAX_SAFE_INTEGER + 1),
        ).toBe(false);
      });
    });
    ...
    describe("formatAlertMessage", () => {
      it("应格式化告警消息", () => {
        ...
        const result = AlertingTemplateUtil.formatAlertMessage(
          template,
          context,
        );
      ...
      it("应处理复杂的上下文数据", () => {
        const template =
          "服务器 {server} 在 {timestamp} 发生告警，详情: {details}";
        const context = {
          server: "web-01",
        ...
        expect(result).toBe(
          "服务器 web-01 在 2023-12-01 10:30:00 发生告警，详情: CPU过高, 内存不足, 磁盘空间告警",
        );
      });
    });
