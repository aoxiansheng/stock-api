...
describe("Alert History Constants", () => {
  describe("ALERT_HISTORY_OPERATIONS", () => {
    it("应包含所有必需的操作常量", () => {
      ...
      expect(ALERT_HISTORY_OPERATIONS.CLEANUP_EXPIRED_ALERTS).toBe(
        "cleanupExpiredAlerts",
      );
      expect(ALERT_HISTORY_OPERATIONS.BATCH_UPDATE_ALERT_STATUS).toBe(
        "batchUpdateAlertStatus",
      );
      expect(ALERT_HISTORY_OPERATIONS.GET_ALERT_COUNT_BY_STATUS).toBe(
        "getAlertCountByStatus",
      );
      expect(ALERT_HISTORY_OPERATIONS.GET_RECENT_ALERTS).toBe(
        "getRecentAlerts",
      );
      expect(ALERT_HISTORY_OPERATIONS.GET_SERVICE_STATS).toBe(
        "getServiceStats",
      );
      expect(ALERT_HISTORY_OPERATIONS.GENERATE_ALERT_ID).toBe(
        "generateAlertId",
      );
      expect(ALERT_HISTORY_OPERATIONS.CALCULATE_STATISTICS).toBe(
        "calculateStatistics",
      );
      expect(ALERT_HISTORY_OPERATIONS.VALIDATE_QUERY_PARAMS).toBe(
        "validateQueryParams",
      );
      expect(ALERT_HISTORY_OPERATIONS.PROCESS_BATCH_RESULTS).toBe(
        "processBatchResults",
      );
    });
  ...
  describe("ALERT_HISTORY_MESSAGES", () => {
    it("应包含所有成功消息", () => {
      ...
      expect(ALERT_HISTORY_MESSAGES.ACTIVE_ALERTS_RETRIEVED).toBe(
        "获取活跃告警成功",
      );
      expect(ALERT_HISTORY_MESSAGES.ALERT_STATS_RETRIEVED).toBe(
        "获取告警统计成功",
      );
    ...
    it("应包含所有错误消息", () => {
      expect(ALERT_HISTORY_MESSAGES.CREATE_ALERT_FAILED).toBe(
        "创建告警记录失败",
      );
      expect(ALERT_HISTORY_MESSAGES.UPDATE_ALERT_STATUS_FAILED).toBe(
        "更新告警状态失败",
      );
      expect(ALERT_HISTORY_MESSAGES.QUERY_ALERTS_FAILED).toBe(
        "查询告警记录失败",
      );
      expect(ALERT_HISTORY_MESSAGES.GET_ACTIVE_ALERTS_FAILED).toBe(
        "获取活跃告警失败",
      );
      expect(ALERT_HISTORY_MESSAGES.GET_ALERT_STATS_FAILED).toBe(
        "获取告警统计失败",
      );
      expect(ALERT_HISTORY_MESSAGES.GET_ALERT_FAILED).toBe("获取告警失败");
      expect(ALERT_HISTORY_MESSAGES.CLEANUP_FAILED).toBe("清理过期告警失败");
      expect(ALERT_HISTORY_MESSAGES.BATCH_UPDATE_FAILED).toBe(
        "批量更新告警状态失败",
      );
    });
    ...
    it("应包含所有信息消息", () => {
      ...
      expect(ALERT_HISTORY_MESSAGES.BATCH_UPDATE_STARTED).toBe(
        "开始批量更新告警状态",
      );
      expect(ALERT_HISTORY_MESSAGES.ALERT_CREATION_STARTED).toBe(
        "开始创建告警记录",
      );
      expect(ALERT_HISTORY_MESSAGES.ALERT_STATUS_UPDATE_STARTED).toBe(
        "开始更新告警状态",
      );
    });
    ...
    it("应包含所有警告消息", () => {
      ...
      expect(ALERT_HISTORY_MESSAGES.PARTIAL_BATCH_UPDATE_SUCCESS).toBe(
        "批量更新部分成功",
      );
      expect(ALERT_HISTORY_MESSAGES.CLEANUP_NO_EXPIRED_ALERTS).toBe(
        "没有过期告警需要清理",
      );
      expect(ALERT_HISTORY_MESSAGES.STATISTICS_INCOMPLETE).toBe(
        "统计数据不完整",
      );
      expect(ALERT_HISTORY_MESSAGES.QUERY_LIMIT_EXCEEDED).toBe(
        "查询限制超出范围",
      );
    });
  ...
  describe("ALERT_HISTORY_CONFIG", () => {
    it("应包含正确的配置值", () => {
      ...
      expect(ALERT_HISTORY_CONFIG.ID_FORMAT_TEMPLATE).toBe(
        "alrt_{timestamp}_{random}",
      );
    ...
    it("分页限制应合理", () => {
      expect(ALERT_HISTORY_CONFIG.MIN_PAGE_LIMIT).toBeLessThan(
        ALERT_HISTORY_CONFIG.MAX_PAGE_LIMIT,
      );
      expect(ALERT_HISTORY_CONFIG.DEFAULT_PAGE_LIMIT).toBeGreaterThanOrEqual(
        ALERT_HISTORY_CONFIG.MIN_PAGE_LIMIT,
      );
      expect(ALERT_HISTORY_CONFIG.DEFAULT_PAGE_LIMIT).toBeLessThanOrEqual(
        ALERT_HISTORY_CONFIG.MAX_PAGE_LIMIT,
      );
    });

    it("清理天数应合理", () => {
      expect(ALERT_HISTORY_CONFIG.MIN_CLEANUP_DAYS).toBeLessThan(
        ALERT_HISTORY_CONFIG.MAX_CLEANUP_DAYS,
      );
      expect(ALERT_HISTORY_CONFIG.DEFAULT_CLEANUP_DAYS).toBeGreaterThanOrEqual(
        ALERT_HISTORY_CONFIG.MIN_CLEANUP_DAYS,
      );
      expect(ALERT_HISTORY_CONFIG.DEFAULT_CLEANUP_DAYS).toBeLessThanOrEqual(
        ALERT_HISTORY_CONFIG.MAX_CLEANUP_DAYS,
      );
    });
  ...
  describe("ALERT_HISTORY_METRICS", () => {
    it("应包含所有指标名称", () => {
      expect(ALERT_HISTORY_METRICS.ALERT_CREATION_COUNT).toBe(
        "alert_history_creation_count",
      );
      expect(ALERT_HISTORY_METRICS.ALERT_UPDATE_COUNT).toBe(
        "alert_history_update_count",
      );
      expect(ALERT_HISTORY_METRICS.ALERT_QUERY_COUNT).toBe(
        "alert_history_query_count",
      );
      expect(ALERT_HISTORY_METRICS.CLEANUP_EXECUTION_COUNT).toBe(
        "alert_history_cleanup_count",
      );
      expect(ALERT_HISTORY_METRICS.BATCH_UPDATE_COUNT).toBe(
        "alert_history_batch_update_count",
      );
      expect(ALERT_HISTORY_METRICS.AVERAGE_QUERY_TIME).toBe(
        "alert_history_avg_query_time",
      );
      expect(ALERT_HISTORY_METRICS.AVERAGE_UPDATE_TIME).toBe(
        "alert_history_avg_update_time",
      );
      expect(ALERT_HISTORY_METRICS.AVERAGE_CLEANUP_TIME).toBe(
        "alert_history_avg_cleanup_time",
      );
      expect(ALERT_HISTORY_METRICS.ACTIVE_ALERTS_COUNT).toBe(
        "alert_history_active_alerts_count",
      );
      expect(ALERT_HISTORY_METRICS.TOTAL_ALERTS_COUNT).toBe(
        "alert_history_total_alerts_count",
      );
    });
  ...
  describe("ALERT_HISTORY_VALIDATION_RULES", () => {
    it("应包含正确的验证规则", () => {
      expect(ALERT_HISTORY_VALIDATION_RULES.ALERT_ID_PATTERN).toBeInstanceOf(
        RegExp,
      );
    ...
    it("告警ID模式应验证正确的格式", () => {
      expect(
        ALERT_HISTORY_VALIDATION_RULES.ALERT_ID_PATTERN.test(
          "alrt_abc123_def456",
        ),
      ).toBe(true);
      expect(
      ...
      expect(
        ALERT_HISTORY_VALIDATION_RULES.ALERT_ID_PATTERN.test("invalid"),
      ).toBe(false);
      expect(
        ALERT_HISTORY_VALIDATION_RULES.ALERT_ID_PATTERN.test("alrt_"),
      ).toBe(false);
      expect(
        ALERT_HISTORY_VALIDATION_RULES.ALERT_ID_PATTERN.test("alrt_abc"),
      ).toBe(false);
    });

    it("长度限制应合理", () => {
      expect(ALERT_HISTORY_VALIDATION_RULES.MIN_ALERT_ID_LENGTH).toBeLessThan(
        ALERT_HISTORY_VALIDATION_RULES.MAX_ALERT_ID_LENGTH,
      );
      expect(ALERT_HISTORY_VALIDATION_RULES.MIN_RULE_ID_LENGTH).toBeLessThan(
        ALERT_HISTORY_VALIDATION_RULES.MAX_RULE_ID_LENGTH,
      );
      expect(ALERT_HISTORY_VALIDATION_RULES.MIN_MESSAGE_LENGTH).toBeLessThan(
        ALERT_HISTORY_VALIDATION_RULES.MAX_MESSAGE_LENGTH,
      );
    });
  ...
  describe("ALERT_HISTORY_TIME_CONFIG", () => {
    it("应包含合理的时间配置", () => {
      ...
      expect(ALERT_HISTORY_TIME_CONFIG.STATISTICS_CALCULATION_TIMEOUT_MS).toBe(
        60000,
      );
    ...
    it("清理超时应大于其他操作超时", () => {
      expect(ALERT_HISTORY_TIME_CONFIG.CLEANUP_TIMEOUT_MS).toBeGreaterThan(
        ALERT_HISTORY_TIME_CONFIG.DEFAULT_QUERY_TIMEOUT_MS,
      );
      expect(ALERT_HISTORY_TIME_CONFIG.CLEANUP_TIMEOUT_MS).toBeGreaterThan(
        ALERT_HISTORY_TIME_CONFIG.BATCH_UPDATE_TIMEOUT_MS,
      );
    });
  ...
  describe("ALERT_HISTORY_THRESHOLDS", () => {
    it("应包含合理的阈值", () => {
      ...
      expect(ALERT_HISTORY_THRESHOLDS.STATISTICS_REFRESH_INTERVAL_MS).toBe(
        300000,
      );
    });

    it("最大活跃告警数应大于单规则告警数", () => {
      expect(ALERT_HISTORY_THRESHOLDS.MAX_ACTIVE_ALERTS).toBeGreaterThan(
        ALERT_HISTORY_THRESHOLDS.MAX_ALERTS_PER_RULE,
      );
    });
  ...
  describe("AlertHistoryUtil", () => {
    ...
    describe("isValidAlertId", () => {
      it("应接受有效的告警ID", () => {
        expect(AlertHistoryUtil.isValidAlertId("alrt_abc123_def456")).toBe(
          true,
        );
        expect(AlertHistoryUtil.isValidAlertId("alrt_123abc_456def")).toBe(
          true,
        );
      });
    ...
    describe("isValidAlertMessage", () => {
      it("应接受有效的告警消息", () => {
        ...
        expect(AlertHistoryUtil.isValidAlertMessage("a".repeat(1000))).toBe(
          true,
        );
      });
      ...
      it("应拒绝无效的告警消息", () => {
        ...
        expect(AlertHistoryUtil.isValidAlertMessage("a".repeat(1001))).toBe(
          false,
        );
      });
    });
    ...
    describe("generateBatchResultSummary", () => {
      it("应生成正确的批量操作结果摘要", () => {
        const summary = AlertHistoryUtil.generateBatchResultSummary(80, 20, [
          "错误1",
          "错误2",
        ]);
