...
describe('AlertingService Optimization Features', () => {
  ...
  describe('Enhanced Rule Management', () => {
    ...
    it('should use template utility for rule not found error', async () => {
      ...
      await expect(service.getRuleById('nonexistent')).rejects.toThrow(NotFoundException);
  ...
  describe('Enhanced Alert Management', () => {
    ...
    it('should use template utility for alert not found error', async () => {
      ...
      try {
        ...
        expect(error.message).toContain('确认后未能找到ID为 nonexistent 的告警'); // 修复：更新断言
      }
    });
  });
  ...
  describe('Statistics and Health Checks', () => {
    it('should use constants and call history service for stats', async () => {
      ...
      cacheService.get.mockResolvedValue(JSON.stringify({ critical: 2, warning: 3 }));
  ...
  describe('Utility Functions', () => {
    ...
    it('should generate error messages using templates', () => {
      const message = AlertingTemplateUtil.generateErrorMessage('RULE_NOT_FOUND', {
    ...
    it('should validate rule names correctly', () => {
      expect(AlertingTemplateUtil.isValidRuleName('Valid Rule Name')).toBe(true);
      expect(AlertingTemplateUtil.isValidRuleName('Valid-Rule_Name.123')).toBe(true);
      expect(AlertingTemplateUtil.isValidRuleName('Invalid@Rule#Name')).toBe(false);
    });
    ...
    it('should calculate priority scores correctly', () => {
      ...
      const lowScore = AlertingTemplateUtil.calculatePriorityScore('info', 50, 100);
