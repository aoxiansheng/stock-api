...
describe("StreamStockQuote Capability", () => {
  ...
  describe("Basic Properties", () => {
    ...
    it("should have correct description", () => {
      expect(streamStockQuote.description).toBe(
        "获取股票实时报价数据流（WebSocket）",
      );
    });
  ...
  describe("initialize()", () => {
    it("should initialize WebSocket connection successfully", async () => {
      ...
      await expect(
        streamStockQuote.initialize(mockContextService),
      ).resolves.toBeUndefined();

      expect(mockContextService.initializeWebSocket).toHaveBeenCalledTimes(1);
      expect(mockLogger.debug).toHaveBeenCalledWith(
        "初始化 LongPort WebSocket 流连接",
      );
      expect(mockLogger.log).toHaveBeenCalledWith(
        "LongPort WebSocket 流连接初始化成功",
      );
    });

    it("should throw error when contextService is not provided", async () => {
      await expect(streamStockQuote.initialize(null)).rejects.toThrow(
        "LongPort WebSocket 流初始化失败: LongportStreamContextService 未提供",
      );
    });
  ...
  describe("subscribe()", () => {
    ...
    it("should subscribe to symbols successfully", async () => {
      ...
      await expect(
        streamStockQuote.subscribe(testSymbols, mockContextService),
      ).resolves.toBeUndefined();

      expect(mockContextService.subscribe).toHaveBeenCalledWith(testSymbols);
      expect(mockLogger.debug).toHaveBeenCalledWith(
        "订阅 LongPort WebSocket 股票报价流",
        { symbols: testSymbols },
      );
      expect(mockLogger.log).toHaveBeenCalledWith({
    ...
    it("should throw error when contextService is not provided", async () => {
      await expect(
        streamStockQuote.subscribe(testSymbols, null),
      ).rejects.toThrow(
        "LongPort 流订阅失败: LongportStreamContextService 未提供",
      );
    });

    it("should throw error when symbols array is empty", async () => {
      await expect(
        streamStockQuote.subscribe([], mockContextService),
      ).rejects.toThrow("LongPort 流订阅失败: 订阅符号列表不能为空");
    });

    it("should throw error when symbols is null", async () => {
      await expect(
        streamStockQuote.subscribe(null, mockContextService),
      ).rejects.toThrow("LongPort 流订阅失败: 订阅符号列表不能为空");
    });
    ...
    it("should reject subscription when invalid symbols are present", async () => {
      ...
      await expect(
        streamStockQuote.subscribe(symbolsWithInvalid, mockContextService),
      ).rejects.toThrow(
        "LongPort 流订阅失败: 无效的股票符号格式，拒绝订阅: INVALID_SYMBOL",
      );
  ...
  describe("unsubscribe()", () => {
    ...
    it("should unsubscribe from symbols successfully", async () => {
      ...
      await expect(
        streamStockQuote.unsubscribe(testSymbols, mockContextService),
      ).resolves.toBeUndefined();

      expect(mockContextService.unsubscribe).toHaveBeenCalledWith(testSymbols);
      expect(mockLogger.debug).toHaveBeenCalledWith(
        "取消订阅 LongPort WebSocket 股票报价流",
        { symbols: testSymbols },
      );
      expect(mockLogger.log).toHaveBeenCalledWith({
    ...
    it("should throw error when contextService is not provided", async () => {
      await expect(
        streamStockQuote.unsubscribe(testSymbols, null),
      ).rejects.toThrow(
        "LongPort 流取消订阅失败: LongportStreamContextService 未提供",
      );
    });
  ...
  describe("cleanup()", () => {
    it("should cleanup resources successfully", async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        "清理 LongPort WebSocket 流资源",
      );
      expect(mockLogger.log).toHaveBeenCalledWith(
        "LongPort WebSocket 流资源清理完成",
      );
    });
...
describe("Symbol Validation Helper", () => {
  ...
  it("should reject subscription with all invalid symbols", async () => {
    ...
    // 期望抛出错误，因为所有符号都无效
    await expect(
      streamStockQuote.subscribe(invalidSymbols, mockContextService),
    ).rejects.toThrow(
      "LongPort 流订阅失败: 无效的股票符号格式，拒绝订阅: INVALID, 123, ABC.XY",
    );
