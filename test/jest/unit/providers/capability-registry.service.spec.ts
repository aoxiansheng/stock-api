...
describe('CapabilityRegistryService', () => {
  ...
  describe('loadProviderCapabilities', () => {
    it('应该处理缺少capabilities目录的提供商', async () => {
      ...
      // Act
      await (service as any).loadProviderCapabilities('provider-without-capabilities');

      // Assert
      expect(loggerWarnSpy).toHaveBeenCalledWith({
        message: '提供商 provider-without-capabilities 缺少 \'capabilities\' 目录',
    ...
    it('应该成功加载提供商的所有能力文件', async () => {
      ...
      mockedFs.readdir.mockResolvedValue(['get-stock-quote.ts', 'get-stock-info.ts', 'README.md'] as any);
      ...
      expect(loadCapabilitySpy).toHaveBeenCalledWith('test-provider', 'get-stock-quote');
      expect(loadCapabilitySpy).toHaveBeenCalledWith('test-provider', 'get-stock-info');
      expect(loadCapabilitySpy).not.toHaveBeenCalledWith('test-provider', 'README'); // 不加载非.ts文件
  ...
  describe('registerCapability edge cases', () => {
    it('应该能覆盖同一提供商的同名能力', () => {
      ...
      expect(providerCapabilities?.get('get-stock-quote')?.capability).toBe(capability2);
    });
  });
  ...
  describe('onModuleInit', () => {
    it('应该在模块初始化时调用discoverCapabilities', async () => {
      // Arrange
      const discoverCapabilitiesSpy = jest.spyOn(service, 'discoverCapabilities');
  ...
  describe('complex integration scenarios', () => {
    it('应该处理完整的能力发现和注册流程', async () => {
      ...
      mockDynamicImport.mockImplementation(async (providerName: string, capabilityName: string) => {
      ...
      // 验证能力被正确注册
      expect(service.getCapability('longport', 'get-stock-quote')).toBe(mockCapability1);
      expect(service.getCapability('longport', 'get-market-info')).toBe(mockCapability2);
      expect(service.getCapability('test-provider', 'get-stock-quote')).toBe(mockCapability1);
      expect(service.getCapability('test-provider', 'get-market-info')).toBe(mockCapability2);
    ...
    it('应该正确处理混合的成功和失败场景', async () => {
      ...
      // Assert
      // 成功的提供商应该被正确注册
      expect(service.getCapability('working-provider', 'working-capability')).toBe(workingCapability);
      
      // 失败的提供商不应该有能力被注册
      expect(service.getCapability('failing-provider', 'failing-capability')).toBeNull();
