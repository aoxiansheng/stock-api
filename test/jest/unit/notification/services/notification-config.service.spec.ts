...
describe('NotificationConfigService', () => {
  ...
  describe('Template Configuration Access Methods', () => {
    ...
    it('应该正确获取默认邮件主题模板', () => {
      const defaultEmailSubjectTemplate = service.getDefaultEmailSubjectTemplate();
  ...
  describe('Business Logic Helper Methods', () => {
    it('应该根据渠道类型返回正确的超时时间', () => {
      const channelTimeouts = [
        { channel: 'email' as NotificationChannelType, expected: service.getEmailTimeout() },
        { channel: 'sms' as NotificationChannelType, expected: service.getSmsTimeout() },
        { channel: 'webhook' as NotificationChannelType, expected: service.getWebhookTimeout() },
        { channel: 'slack' as NotificationChannelType, expected: service.getDefaultTimeout() },
        { channel: 'dingtalk' as NotificationChannelType, expected: service.getDefaultTimeout() },
        { channel: 'log' as NotificationChannelType, expected: service.getDefaultTimeout() },
      ];
    ...
    it('应该验证重试延迟计算的一致性', () => {
      ...
      // 验证所有延迟都在合理范围内
      const expectedBase = service.getInitialRetryDelay() * Math.pow(service.getRetryBackoffMultiplier(), attempt - 1);
      const maxJitter = expectedBase * service.getJitterFactor();
      
      delays.forEach(delay => {
        expect(delay).toBeGreaterThanOrEqual(expectedBase - maxJitter);
        expect(delay).toBeLessThanOrEqual(Math.min(expectedBase + maxJitter, service.getMaxRetryDelay()));
      });
  ...
  describe('Complete Configuration Access', () => {
    ...
    it('应该确保配置对象的不变性', () => {
      ...
      // 验证返回的是相同的配置对象
      expect(config1.batch.defaultBatchSize).toBe(config2.batch.defaultBatchSize);
      expect(config1.timeouts.defaultTimeout).toBe(config2.timeouts.defaultTimeout);
  ...
  describe('Error Handling and Edge Cases', () => {
    it('应该处理无效的渠道类型', () => {
      // 测试未知的渠道类型
      const unknownChannelTimeout = service.getChannelTimeout('unknown' as NotificationChannelType);
    ...
    it('应该处理边界值验证', () => {
      ...
      expect(service.isValidBatchSize(service.getMaxBatchSize() + 1)).toBe(false);
      
      expect(service.isValidRetryCount(0)).toBe(true);
      expect(service.isValidRetryCount(service.getMaxRetryAttempts())).toBe(true);
      expect(service.isValidRetryCount(service.getMaxRetryAttempts() + 1)).toBe(false);
    });
  });
  ...
  describe('Performance Tests', () => {
    it('应该在高频访问下保持性能', () => {
      ...
      console.log(`配置服务性能测试: ${iterations}次操作耗时 ${durationMs.toFixed(2)}ms`);
    ...
    it('应该验证重试延迟计算的性能', () => {
      ...
      console.log(`重试延迟计算性能测试: ${iterations}次计算耗时 ${durationMs.toFixed(2)}ms`);
