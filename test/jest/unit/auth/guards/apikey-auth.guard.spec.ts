...
describe('ApiKeyAuthGuard', () => {
  ...
  describe('canActivate', () => {
    beforeEach(() => {
      ...
      jest.spyOn(Object.getPrototypeOf(ApiKeyAuthGuard.prototype), 'canActivate')
    ...
    describe('Non-public endpoints without API key requirement', () => {
      it('should allow access when API key is not required', () => {
        ...
        expect(reflector.getAllAndOverride).toHaveBeenNthCalledWith(1, IS_PUBLIC_KEY, [
          mockContext.getHandler(),
          mockContext.getClass(),
        ]);
        expect(reflector.getAllAndOverride).toHaveBeenNthCalledWith(2, REQUIRE_API_KEY, [
          mockContext.getHandler(),
          mockContext.getClass(),
        ]);
        expect(result).toBe(true);
    ...
    describe('API key required endpoints', () => {
      it('should delegate to parent guard when API key is required', () => {
        const mockSuperCanActivate = jest.spyOn(Object.getPrototypeOf(ApiKeyAuthGuard.prototype), 'canActivate')
          .mockReturnValue(true);
    ...
    describe('Reflector interaction', () => {
      ...
      it('should call reflector with correct metadata keys', () => {
        ...
        expect(reflector.getAllAndOverride).toHaveBeenCalledWith(REQUIRE_API_KEY, expect.any(Array));
      });
  ...
  describe('handleRequest', () => {
    ...
    describe('Authentication failures', () => {
      it('should throw UnauthorizedException when error is present', () => {
        ...
        expect(() => guard.handleRequest(error, mockApiKey))
          .toThrow(error);
      });
      ...
      it('should throw original error when both error and apiKey are present', () => {
        ...
        expect(() => guard.handleRequest(customError, mockApiKey))
          .toThrow(customError);
      });
      ...
      it('should throw UnauthorizedException when api key is false', () => {
        expect(() => guard.handleRequest(null, false))
          .toThrow(UnauthorizedException);
      });

      it('should throw UnauthorizedException when api key is 0', () => {
        expect(() => guard.handleRequest(null, 0))
          .toThrow(UnauthorizedException);
      });
    ...
    describe('Error priority', () => {
      ...
      it('should throw original error even when api key is undefined', () => {
        ...
        expect(() => guard.handleRequest(error, undefined))
          .toThrow(error);
      });
      ...
      it('should prioritize specific errors over generic ones', () => {
        ...
        expect(() => guard.handleRequest(specificError, null))
