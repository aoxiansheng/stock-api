...
describe('Configuration Overlap Elimination', () => {
  ...
  describe('Cache Layer Configuration Isolation', () => {
    it('should have no TTL configuration overlaps between cache and limits layers', () => {
      ...
      const ttlProperties = cacheProperties.filter(prop => prop.includes('Ttl') || prop.includes('TTL'));
      
      expect(cacheProperties.length).toBe(ttlProperties.length);

      // 验证限制层不包含TTL配置
      const limitsProperties = Object.keys(limitsConfig);
      const limitsTtlProperties = limitsProperties.filter(prop => prop.includes('Ttl') || prop.includes('TTL'));
  ...
  describe('Limits Layer Configuration Isolation', () => {
    it('should contain only non-TTL configuration parameters', () => {
      ...
      // 限制层不应该包含TTL配置
      const ttlProperties = properties.filter(prop => 
    ...
    it('should have no configuration value overlaps with cache layer', () => {
      ...
      expect(cacheConfig.permissionCacheTtl).not.toBe(limitsConfig.globalRateLimit);
    });
  });
  ...
  describe('Cross-Layer Configuration Dependencies', () => {
    it('should maintain logical relationships between cache and limits configurations', () => {
      ...
      // 权限缓存TTL应该小于会话超时时间
      expect(cacheConfig.permissionCacheTtl).toBeLessThan(limitsConfig.sessionTimeoutMinutes * 60);
    });
    ...
    it('should validate consistency between related configurations', () => {
      ...
      expect(limitsConfig.globalRateLimit).toBeGreaterThanOrEqual(expectedGlobalLimit);
    });
  });
  ...
  describe('Backward Compatibility Layer Verification', () => {
    ...
    it('should provide different TTL values for different cache types through compatibility wrapper', () => {
      ...
      expect(apiKeyOps.CACHE_TTL_SECONDS).not.toBe(permissionCheck.CACHE_TTL_SECONDS);
      expect(permissionCheck.CACHE_TTL_SECONDS).not.toBe(rateLimits.TTL_SECONDS);
  ...
  describe('Four-Layer Configuration System Compliance', () => {
    it('should separate semantic constants from configurable parameters', () => {
      ...
      const { PERMISSION_LEVELS } = require('../../../../../src/auth/constants/permission-control.constants');
    ...
    it('should have clear layer separation with no cross-layer contamination', () => {
      ...
      expect(cacheKeys.every(key => key.toLowerCase().includes('ttl'))).toBe(true);
      
      // 限制层：不包含TTL配置，只包含数值限制和超时
      const limitsKeys = Object.keys(limitsConfig);
      expect(limitsKeys.some(key => key.toLowerCase().includes('limit'))).toBe(true);
      expect(limitsKeys.some(key => key.toLowerCase().includes('max'))).toBe(true);
      expect(limitsKeys.some(key => key.toLowerCase().includes('timeout'))).toBe(true);
      expect(limitsKeys.every(key => !key.toLowerCase().includes('cachettl'))).toBe(true);
    });
