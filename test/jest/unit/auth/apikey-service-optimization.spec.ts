...
describe("ApiKeyService Optimization Features", () => {
  ...
  describe("Enhanced API Key Validation", () => {
    ...
    it("should use constants for invalid credentials error", async () => {
      ...
      await expect(
        ...
        new UnauthorizedException(ERROR_MESSAGES.API_CREDENTIALS_INVALID),
      );
    ...
    it("should use constants for expired credentials error", async () => {
      ...
      await expect(
        ...
        new UnauthorizedException(ERROR_MESSAGES.API_CREDENTIALS_EXPIRED),
      );
  ...
  describe("Enhanced API Key Creation", () => {
    ...
    it("should use constants for creation failure error", async () => {
      ...
      await expect(
        ...
        new InternalServerErrorException(ERROR_MESSAGES.CREATE_API_KEY_FAILED),
      );
  ...
  describe("Enhanced Get User API Keys", () => {
    ...
    it("should use constants for retrieval failure error", async () => {
      ...
      await expect(service.getUserApiKeys("user123")).rejects.toThrow(
  ...
  describe("Enhanced API Key Revocation", () => {
    ...
    it("should use constants for revocation failure error", async () => {
      ...
      await expect(
        ...
        new InternalServerErrorException(ERROR_MESSAGES.REVOKE_API_KEY_FAILED),
      );
  ...
  describe("Utility Functions", () => {
    ...
    it("should validate app key format", () => {
      expect(
        ApiKeyUtil.isValidAppKey("sk-12345678-1234-1234-1234-123456789012"),
      ).toBe(true);
    ...
    it("should validate access token format", () => {
      expect(
        ApiKeyUtil.isValidAccessToken("abcdefghijklmnopqrstuvwxyz123456"),
      ).toBe(true);
      expect(ApiKeyUtil.isValidAccessToken("short")).toBe(false);
      expect(
        ApiKeyUtil.isValidAccessToken("toolongaccesstokenstring123456789"),
      ).toBe(false);
    });
