...
describe('API Key Constants - Enhanced Branch Coverage', () => {
  describe('ApiKeyUtil', () => {
    ...
    describe('isNearExpiry', () => {
      ...
      it('should return true for dates within warning period', () => {
        const warningDays = 7;
        const withinWarningDate = new Date(Date.now() + 3 * 24 * 60 * 60 * 1000); // 3 days
        
        expect(ApiKeyUtil.isNearExpiry(withinWarningDate, warningDays)).toBe(true);
      });

      it('should return false for dates beyond warning period', () => {
        const warningDays = 7;
        const beyondWarningDate = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000); // 30 days
        
        expect(ApiKeyUtil.isNearExpiry(beyondWarningDate, warningDays)).toBe(false);
      });
    });
    ...
    describe('sanitizeAccessToken', () => {
      it('should return *** for short tokens', () => {
        const shortTokens = ['', 'a', 'ab', 'abc', 'abcd', 'abcde', 'abcdef', 'abcdefgh'];
      ...
      it('should sanitize normal tokens correctly', () => {
        expect(ApiKeyUtil.sanitizeAccessToken('123456789012345678901234')).toBe('1234***1234');
        expect(ApiKeyUtil.sanitizeAccessToken('abcdefghijklmnopqrstuvwxyz')).toBe('abcd***wxyz');
      });
  ...
  describe('Validation patterns', () => {
    it('should have correct APP_KEY_PATTERN', () => {
      ...
      expect(pattern.test('sk-12345678-1234-1234-1234-123456789012')).toBe(true);
  ...
  describe('Status and type constants', () => {
    it('should have all required status values', () => {
      const expectedStatuses = ['active', 'inactive', 'expired', 'revoked', 'suspended', 'pending'];
    ...
    it('should have all required type values', () => {
      const expectedTypes = ['standard', 'premium', 'enterprise', 'trial', 'development', 'production'];
