...
describe("RateLimitExceptionFilter - 异常增强模式", () => {
  ...
  beforeEach(async () => {
    ...
    mockIsSecurityException = isSecurityException as jest.MockedFunction<typeof isSecurityException>;
  ...
  describe("非429异常处理", () => {
    it("应该重新抛出非429异常", () => {
      const nonRateLimitException = new HttpException("Not Found", HttpStatus.NOT_FOUND);
  ...
  describe("已增强的安全异常处理", () => {
    it("应该直接重新抛出已增强的安全异常", () => {
      ...
      expect(() => {
        filter.catch(enhancedRateLimitException, mockArgumentsHost as ArgumentsHost);
    ...
    it("应该处理没有retryAfter的增强异常", () => {
      const enhancedRateLimitException = new EnhancedRateLimitException(
        100, 101, 0, Date.now() + 60000, 0, // retryAfter = 0
        "/api/test", "192.168.1.1", "Test Browser"
      );
  ...
  describe("标准429异常转换为增强异常", () => {
    it("应该将标准429异常转换为增强异常", () => {
      ...
      mockSecurityExceptionFactory.createRateLimitException.mockReturnValue(mockEnhancedException);
      ...
      expect(mockSecurityExceptionFactory.createRateLimitException).toHaveBeenCalledWith(
        100, 101, 0, expect.any(Number), 60,
        mockRequest,
        "test-api-key",
        rateLimitException
      );
  ...
  describe("日志记录功能", () => {
    it("应该记录频率限制触发日志", () => {
      ...
      expect(mockHttpHeadersUtil.getUserAgent).toHaveBeenCalledWith(mockRequest);
      expect(mockHttpHeadersUtil.getHeader).toHaveBeenCalledWith(mockRequest, "x-app-key");
    });
  ...
  describe("向后兼容性", () => {
    it("应该与旧的异常格式兼容", () => {
      ...
      expect(() => {
        filter.catch(legacyRateLimitException, mockArgumentsHost as ArgumentsHost);
      }).toThrow(mockEnhancedException);

      expect(mockSecurityExceptionFactory.createRateLimitException).toHaveBeenCalledWith(
        50, 51, 0, expect.any(Number), 30,
        mockRequest,
        "test-api-key",
  ...
  describe("边缘情况处理", () => {
    it("应该处理null/undefined的异常响应", () => {
      const rateLimitException = new HttpException(null, HttpStatus.TOO_MANY_REQUESTS);
