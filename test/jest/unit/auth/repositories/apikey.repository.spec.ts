...
describe('ApiKeyRepository', () => {
  ...
  describe('findAllActive', () => {
    it('应该返回所有活跃的API Key', async () => {
      // Arrange
      const mockActiveKeys = [mockApiKey, { ...mockApiKey, id: '507f1f77bcf86cd799439013' }];
    ...
    it('应该返回不同权限配置的API Key', async () => {
      ...
      // Assert
      expect(result).toHaveLength(2);
      expect(result[0].permissions).toEqual([Permission.DATA_READ, Permission.QUERY_EXECUTE]);
      expect(result[1].permissions).toEqual([Permission.DATA_READ, Permission.PROVIDERS_READ]);
    });
