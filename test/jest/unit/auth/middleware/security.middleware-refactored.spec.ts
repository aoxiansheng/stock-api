...
describe("SecurityMiddleware - 异常增强模式", () => {
  ...
  describe("正常请求处理", () => {
    ...
    it("应该设置安全响应头", () => {
      ...
      expect(HttpHeadersUtil.setSecurityHeaders).toHaveBeenCalledWith(
        mockResponse,
      );
    });
  });
  ...
  describe("PAYLOAD_TOO_LARGE异常处理", () => {
    it("应该抛出EnhancedPayloadTooLargeException而非直接构造响应", () => {
      ...
      mockSecurityExceptionFactory.createPayloadTooLargeException.mockReturnValue(
        mockException,
      );
      ...
      expect(
        mockSecurityExceptionFactory.createPayloadTooLargeException,
      ).toHaveBeenCalledWith(2097152, 1048576, "1MB", mockRequest);
  ...
  describe("UNSUPPORTED_MEDIA_TYPE异常处理", () => {
    it("应该抛出EnhancedUnsupportedMediaTypeException而非直接构造响应", () => {
      ...
      mockSecurityExceptionFactory.createUnsupportedMediaTypeException.mockReturnValue(
        mockException,
      );
      ...
      expect(
        mockSecurityExceptionFactory.createUnsupportedMediaTypeException,
      ).toHaveBeenCalledWith(
        "text/javascript",
        "DANGEROUS_CONTENT_TYPE",
        mockRequest,
    ...
    it("应该处理缺少Content-Type的POST请求", () => {
      ...
      expect(
        mockSecurityExceptionFactory.createUnsupportedMediaTypeException,
      ).toHaveBeenCalledWith("unknown", "MISSING_CONTENT_TYPE", mockRequest);
    });
  ...
  describe("BAD_REQUEST异常处理", () => {
    it("应该抛出InputSecurityViolationException而非直接构造响应", () => {
      ...
      mockSecurityExceptionFactory.createInputSecurityViolationException.mockReturnValue(
        mockException,
      );
      ...
      expect(
        mockSecurityExceptionFactory.createInputSecurityViolationException,
      ).toHaveBeenCalledWith(
        "MALICIOUS_URL_PATTERN",
        expect.any(Object),
        mockRequest,
    ...
    it("应该拒绝JSON炸弹攻击", () => {
      const mockException = new InputSecurityViolationException(
        "JSON_BOMB",
        { actualFields: 1500, maxFields: 1000 },
        "/test",
        "127.0.0.1",
        "Test Agent",
      );
  ...
  describe("INTERNAL_SERVER_ERROR异常处理", () => {
    it("应该抛出SecurityMiddlewareException而非直接构造响应", () => {
      ...
      mockSecurityExceptionFactory.createSecurityMiddlewareException.mockReturnValue(
        mockException,
      );
      ...
      expect(
        mockSecurityExceptionFactory.createSecurityMiddlewareException,
      ).toHaveBeenCalledWith(
        "security-middleware",
        "安全中间件处理失败: Test error",
        mockRequest,
    ...
    it("应该在异常处理前记录错误日志", () => {
      ...
      const mockException = new SecurityMiddlewareException(
        "security-middleware",
        "安全中间件处理失败: Input sanitization failed",
        "/test",
        "127.0.0.1",
        "Test Agent",
        originalError,
