...
describe("PermissionTemplateUtil", () => {
  describe("replaceTemplate", () => {
    ...
    it("应该处理数组值", () => {
      ...
      expect(result).toBe(
        `Required permissions: read${PERMISSION_CONFIG.PERMISSION_LIST_SEPARATOR} write${PERMISSION_CONFIG.PERMISSION_LIST_SEPARATOR} delete`,
      );
    });
  ...
  describe("generateDetails", () => {
    it("应该使用模板生成详情", () => {
      const templateKey =
        "CHECK_PASSED" as keyof typeof PERMISSION_DETAIL_TEMPLATES;
      const params = { subjectName: "TestUser" };

      const result = PermissionTemplateUtil.generateDetails(
        templateKey,
        params,
      );
    ...
    it("应该处理带有多个参数的复杂模板", () => {
      const templateKey =
        "PERMISSION_SUMMARY" as keyof typeof PERMISSION_DETAIL_TEMPLATES;
    ...
    it("应该处理数组值", () => {
      const templateKey =
        "MISSING_PERMISSIONS" as keyof typeof PERMISSION_DETAIL_TEMPLATES;
      ...
      expect(result).toBe(
        `缺失权限: [read${PERMISSION_CONFIG.PERMISSION_LIST_SEPARATOR} write]`,
      );
    });

    it("应该处理未定义的参数", () => {
      const templateKey =
        "SUBJECT_INFO" as keyof typeof PERMISSION_DETAIL_TEMPLATES;
