...
describe('ApiKeyStrategy', () => {
  ...
  describe('validate', () => {
    ...
    it('应该在缺少App Key时抛出UnauthorizedException', async () => {
      ...
      await expect(strategy.validate(mockRequest)).rejects.toThrow('缺少API凭证');
    ...
    it('应该在AuthService验证失败时抛出UnauthorizedException', async () => {
      ...
      authService.validateApiKey = jest.fn().mockRejectedValue(
        new UnauthorizedException('API凭证无效'),
      );
      ...
      await expect(strategy.validate(mockRequest)).rejects.toThrow('API凭证无效');
      expect(authService.validateApiKey).toHaveBeenCalledWith(
        'invalid-key',
        'invalid-token',
      );
    ...
    it('应该拒绝包含空白字符的头部值', async () => {
      ...
      await expect(strategy.validate(mockRequest)).rejects.toThrow('API凭证格式无效');
      expect(authService.validateApiKey).not.toHaveBeenCalled();
    });
