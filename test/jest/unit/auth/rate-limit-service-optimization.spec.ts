...
describe('RateLimitService Optimization Features', () => {
  ...
  describe('Constants Usage', () => {
    it('should use operation constants for all methods', () => {
      ...
      expect(RATE_LIMIT_OPERATIONS.CHECK_SLIDING_WINDOW).toBe('checkSlidingWindow');
    ...
    it('should use message constants for logging', () => {
      ...
      expect(RATE_LIMIT_MESSAGES.RATE_LIMIT_RESET).toBe('重置API Key的频率限制计数器');
    });

    it('should use Lua script constants', () => {
      expect(RATE_LIMIT_LUA_SCRIPTS.SLIDING_WINDOW).toContain('local key = KEYS[1]');
      expect(RATE_LIMIT_LUA_SCRIPTS.SLIDING_WINDOW).toContain('ZREMRANGEBYSCORE');
  ...
  describe('Enhanced Rate Limit Checking', () => {
    ...
    it('should use constants for rate limit check start logging', async () => {
      await service.checkRateLimit(mockApiKey as any, RateLimitStrategy.FIXED_WINDOW);
    ...
    it('should use template utility for unsupported strategy error', async () => {
      await expect(
        service.checkRateLimit(mockApiKey as any, 'UNSUPPORTED_STRATEGY' as any)
      ).rejects.toThrow(BadRequestException);

      // The error message should be generated using the template utility
      try {
        await service.checkRateLimit(mockApiKey as any, 'UNSUPPORTED_STRATEGY' as any);
      } catch (error) {
  ...
  describe('Enhanced Fixed Window Algorithm', () => {
    it('should use constants for exceeded limit warning', async () => {
      ...
      mockRedis.pipeline.mockReturnValue({
        incr: jest.fn(),
        expire: jest.fn(),
        exec: jest.fn().mockResolvedValue([[null, 6], [null, 'OK']]), // 6 > 5 limit
      });
  ...
  describe('Enhanced Rate Limit Reset', () => {
    ...
    it('should use constants for unsupported strategy warning', async () => {
      ...
      await service.resetRateLimit(mockApiKey as any, 'UNSUPPORTED_STRATEGY' as any);
  ...
  describe('Template Utility Functions', () => {
    it('should generate error messages using templates', () => {
      const message = RateLimitTemplateUtil.generateErrorMessage('UNSUPPORTED_STRATEGY', {
    ...
    it('should generate error messages for invalid window format', () => {
      const message = RateLimitTemplateUtil.generateErrorMessage('INVALID_WINDOW_FORMAT', {
    ...
    it('should generate error messages for unsupported time unit', () => {
      const message = RateLimitTemplateUtil.generateErrorMessage('UNSUPPORTED_TIME_UNIT', {
        unit: 'x',
      });
