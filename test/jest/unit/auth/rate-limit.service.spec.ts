...
import { ApiKey, ApiKeyDocument } from '../../../../src/auth/schemas/apikey.schema';
...
describe('RateLimitService', () => {
  ...
  describe('generateRedisKey', () => {
    it('应该生成正确的Redis键', () => {
      ...
      expect(generateKey('test-app-key', '1h')).toBe('rate_limit:test-app-key:1h');
      expect(generateKey('another-key', '5m')).toBe('rate_limit:another-key:5m');
    });
  });
  ...
  describe('checkRateLimit - Fixed Window', () => {
    it('应该在未超过限制时允许请求', async () => {
      ...
      // Act
      const result = await service.checkRateLimit(mockApiKey as any, RateLimitStrategy.FIXED_WINDOW);
  ...
  describe('checkRateLimit - Sliding Window', () => {
    it('应该在未超过限制时允许请求', async () => {
      ...
      // Act
      const result = await service.checkRateLimit(mockApiKey as any, RateLimitStrategy.SLIDING_WINDOW);
    ...
    it('应该使用正确的Lua脚本参数', async () => {
      ...
      // Act
      await service.checkRateLimit(mockApiKey as any, RateLimitStrategy.SLIDING_WINDOW);
  ...
  describe('checkRateLimit - Error Handling', () => {
    ...
    it('应该在不支持的策略时抛出错误', async () => {
      // Arrange
      const invalidStrategy = 'invalid_strategy' as unknown as RateLimitStrategy;
  ...
  describe('getCurrentUsage', () => {
    it('应该返回当前使用统计 - 固定窗口', async () => {
      ...
      // Act
      const result = await service.getCurrentUsage(mockApiKey as any, RateLimitStrategy.FIXED_WINDOW);
    ...
    it('应该返回当前使用统计 - 滑动窗口', async () => {
      ...
      // Act
      const result = await service.getCurrentUsage(mockApiKey as any, RateLimitStrategy.SLIDING_WINDOW);
  ...
  describe('resetRateLimit', () => {
    it('应该删除固定窗口策略的Redis键', async () => {
      ...
      // Act
      await service.resetRateLimit(mockApiKey as any, RateLimitStrategy.FIXED_WINDOW);
    ...
    it('应该删除滑动窗口策略的Redis键', async () => {
      ...
      // Act
      await service.resetRateLimit(mockApiKey as any, RateLimitStrategy.SLIDING_WINDOW);

      // Assert
      expect(mockRedis.del).toHaveBeenCalledWith('rate_limit:test-app-key:1h:sliding');
    });
  });
  ...
  describe('getUsageStatistics', () => {
    it('应该返回详细的使用统计', async () => {
      ...
      // Act
      const result = await service.getUsageStatistics(mockApiKeyWithTime as any, RateLimitStrategy.FIXED_WINDOW);
    ...
    it('应该处理新创建的API Key', async () => {
      ...
      // Act
      const result = await service.getUsageStatistics(newApiKey as any, RateLimitStrategy.FIXED_WINDOW);
    ...
    it('应该确保至少1小时的时间计算', async () => {
      ...
      // Act
      const result = await service.getUsageStatistics(veryNewApiKey as any, RateLimitStrategy.FIXED_WINDOW);
  ...
  describe('Integration Scenarios', () => {
    it('应该处理分钟级别的窗口', async () => {
      ...
      // Act
      const result = await service.checkRateLimit(apiKeyWithMinuteWindow as any);
