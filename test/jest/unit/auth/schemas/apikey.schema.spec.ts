...
describe("ApiKey Schema", () => {
  ...
  beforeAll(async () => {
    ...
    apiKeyModel = moduleRef.get<Model<ApiKeyDocument>>(
  ...
  it("应该支持完整的API Key生命周期", async () => {
    ...
    expect(updatedApiKey.lastUsedAt?.getTime()).toBeGreaterThan(
  ...
  it("应该创建索引", async () => {
    ...
    // 查找appKey的唯一索引
    const appKeyIndex = indexes.find(
      (index) =>
    ...
    // 查找accessToken的唯一索引
    const accessTokenIndex = indexes.find(
      (index) =>
        index.key &&
