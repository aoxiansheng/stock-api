...
describe('JwtStrategy', () => {
  ...
  describe('validate', () => {
    it('应该成功验证有效的JWT载荷', async () => {
      ...
      tokenService.validateUserFromPayload = jest.fn().mockResolvedValue(mockUser);

      // Act
      const result = await strategy.validate(payload);

      // Assert
      expect(tokenService.validateUserFromPayload).toHaveBeenCalledWith(payload);
    ...
    it('应该在AuthService验证失败时抛出UnauthorizedException', async () => {
      ...
      tokenService.validateUserFromPayload = jest.fn().mockRejectedValue(
