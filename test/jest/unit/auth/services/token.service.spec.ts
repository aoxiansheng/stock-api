...
import { TokenService, JwtPayload } from '../../../../../src/auth/services/token.service';
...
describe('TokenService', () => {
  ...
  describe('generateTokens', () => {
    it('should generate access and refresh tokens successfully', async () => {
      ...
      const payload: JwtPayload = { sub: mockUserDocument.id, username: mockUserDocument.username, role: mockUserDocument.role };
  ...
  describe('validateUserFromPayload', () => {
    const payload: JwtPayload = { sub: 'user-id-123', username: 'testuser', role: UserRole.DEVELOPER };

    it('should return the user if found and active', async () => {
      jest.spyOn(userRepository, 'findById').mockResolvedValue(mockUserDocument);
    ...
    it('should throw UnauthorizedException if user is not found', async () => {
      ...
      await expect(service.validateUserFromPayload(payload)).rejects.toThrow(UnauthorizedException);
    });

    it('should throw UnauthorizedException if user is inactive', async () => {
      const inactiveUserDocument = { ...mockUserDocument, isActive: false } as UserDocument;
      jest.spyOn(userRepository, 'findById').mockResolvedValue(inactiveUserDocument);
