...
describe('PerformanceMonitorService - Comprehensive Coverage', () => {
  ...
  describe('Request Recording', () => {
    it('should record API request metrics', async () => {
      ...
      expect(mockPerformanceMetricsRepository.recordRequest).toHaveBeenCalledWith(
        '/api/test',
        'GET',
        150,
        true
    ...
    it('should record failed API request', async () => {
      ...
      const requestMetric = metricBuffer.find(m => m.name === METRIC_NAMES.API_REQUEST_DURATION);
  ...
  describe('Database Query Recording', () => {
    it('should record successful database query', async () => {
      ...
      expect(mockPerformanceMetricsRepository.recordDatabaseQuery).toHaveBeenCalledWith(75);

      const metricBuffer = (service as any).metricBuffer;
      const dbMetric = metricBuffer.find(m => m.name === METRIC_NAMES.DB_QUERY_DURATION);
      expect(dbMetric).toEqual(expect.objectContaining({
  ...
  describe('Cache Operation Recording', () => {
    it('should record cache hit with duration', () => {
      ...
      const cacheOpMetric = metricBuffer.find(m => m.name === METRIC_NAMES.CACHE_OPERATION_TOTAL);
      expect(cacheOpMetric).toEqual(expect.objectContaining({
      ...
      const cacheDurationMetric = metricBuffer.find(m => m.name === METRIC_NAMES.CACHE_OPERATION_DURATION);
      expect(cacheDurationMetric).toEqual(expect.objectContaining({
  ...
  describe('Authentication Recording', () => {
    it('should record successful authentication', () => {
      ...
      const authDurationMetric = metricBuffer.find(m => m.name === METRIC_NAMES.AUTH_DURATION);
      expect(authDurationMetric).toEqual(expect.objectContaining({
      ...
      const authTotalMetric = metricBuffer.find(m => m.name === METRIC_NAMES.AUTH_TOTAL);
      expect(authTotalMetric).toEqual(expect.objectContaining({
  ...
  describe('Rate Limit Recording', () => {
    it('should record rate limit allowed', () => {
      ...
      const rateLimitCheckMetric = metricBuffer.find(m => m.name === METRIC_NAMES.RATE_LIMIT_CHECK);
      expect(rateLimitCheckMetric).toEqual(expect.objectContaining({
      ...
      const rateLimitRemainingMetric = metricBuffer.find(m => m.name === METRIC_NAMES.RATE_LIMIT_REMAINING);
      expect(rateLimitRemainingMetric).toEqual(expect.objectContaining({
  ...
  describe('Timing Wrapper', () => {
    ...
    it('should wrap failed async operations', async () => {
      ...
      await expect(service.wrapWithTiming(operation, onComplete)).rejects.toThrow(error);
      expect(onComplete).toHaveBeenCalledWith(
        expect.any(Number),
        false
      );
    ...
    it('should wrap failed synchronous operations', () => {
      ...
      expect(() => service.wrapWithTiming(operation, onComplete)).toThrow(error);
      expect(onComplete).toHaveBeenCalledWith(
        expect.any(Number),
        false
      );
    ...
    it('should warn about slow operations', async () => {
      const slowOperation = jest.fn().mockImplementation(() => 
        new Promise(resolve => 
          setTimeout(() => resolve('slow result'), PERFORMANCE_THRESHOLDS.SLOW_REQUEST_MS + 50)
        )
      );
      const onComplete = jest.fn();
  ...
  describe('Endpoint Metrics', () => {
    it('should get endpoint metrics', async () => {
      ...
      mockPerformanceMetricsRepository.getEndpointStats.mockResolvedValue(mockRawStats);
  ...
  describe('Database Metrics', () => {
    it('should get database metrics', async () => {
      ...
      const mockQueryTimes = [normalQuery, normalQuery, normalQuery, slowQuery1, slowQuery2];
      mockPerformanceMetricsRepository.getDatabaseQueryTimes.mockResolvedValue(mockQueryTimes);

      const result = await service.getDatabaseMetrics();

      // 计算期望的averageQueryTime
      const avgTime = mockQueryTimes.reduce((sum, time) => sum + parseInt(time), 0) / mockQueryTimes.length;
    ...
    it('should return default metrics when no query times', async () => {
      mockPerformanceMetricsRepository.getDatabaseQueryTimes.mockResolvedValue(null);
    ...
    it('should handle date range parameters', async () => {
      mockPerformanceMetricsRepository.getDatabaseQueryTimes.mockResolvedValue(['50']);

      await service.getDatabaseMetrics('2023-01-01', '2023-01-02');

      expect(mockPerformanceMetricsRepository.getDatabaseQueryTimes).toHaveBeenCalledWith(
        '2023-01-01',
        '2023-01-02'
      );
    });
  ...
  describe('Redis Metrics', () => {
    it('should get Redis metrics', async () => {
      ...
      mockPerformanceMetricsRepository.getRedisInfoPayload.mockResolvedValue(mockRedisInfo);
    ...
    it('should return default metrics when Redis info unavailable', async () => {
      mockPerformanceMetricsRepository.getRedisInfoPayload.mockResolvedValue(null);
  ...
  describe('Performance Summary', () => {
    beforeEach(() => {
      ...
      mockPerformanceMetricsRepository.getEndpointStats.mockResolvedValue([
        {
          key: 'endpoint_stats:GET:/api/test',
          stats: { totalRequests: '100', successfulRequests: '95', failedRequests: '5' },
          responseTimes: ['50', '100'],
        },
      ]);

      mockPerformanceMetricsRepository.getDatabaseQueryTimes.mockResolvedValue(['25', '50']);
    ...
    it('should handle performance summary errors', async () => {
      mockPerformanceMetricsRepository.getEndpointStats.mockRejectedValue(new Error('Failed'));
    ...
    it('should limit endpoints to top 10', async () => {
      const manyEndpoints = Array.from({ length: 15 }, (_, i) => ({
        ...
        stats: { totalRequests: `${15 - i}`, successfulRequests: `${15 - i}`, failedRequests: '0' },
        responseTimes: ['100'],
      }));

      mockPerformanceMetricsRepository.getEndpointStats.mockResolvedValue(manyEndpoints);
  ...
  describe('Health Score Calculation', () => {
    it('should calculate health score correctly', () => {
      ...
      const calculateHealthScore = (service as any).calculateHealthScore.bind(service);
      // 手动实现需要用到的辅助方法
      ...
      (service as any).calculateOverallAverageResponseTime = function(metrics) {
        if (metrics.length === 0) return 0;
        let totalTime = 0;
        let totalRequests = 0;
        for (const metric of metrics) {
      ...
      const score = calculateHealthScore(endpointMetrics, dbMetrics, systemMetrics);
    ...
    it('should calculate overall average response time', () => {
      ...
      // 使用bind绑定服务实例
      const calculateOverallAverageResponseTime = (service as any).calculateOverallAverageResponseTime.bind(service);
    ...
    it('should calculate overall error rate', () => {
      ...
      // 使用bind绑定服务实例
      const calculateOverallErrorRate = (service as any).calculateOverallErrorRate.bind(service);
  ...
  describe('Metric Buffer Management', () => {
    it('should add metrics to buffer', () => {
      ...
      expect(metricBuffer[0]).toEqual(expect.objectContaining({
        name: 'test_metric',
        value: 100,
        unit: 'count',
        tags: { tag: 'value' },
    ...
    it('should limit buffer size', () => {
      ...
      const metricBuffer = (service as any).metricBuffer;
      expect(metricBuffer).toHaveLength(PERFORMANCE_LIMITS.MAX_METRIC_BUFFER_SIZE);
  ...
  describe('Periodic Tasks', () => {
    it('should flush metrics periodically', async () => {
      ...
      expect(mockPerformanceMetricsRepository.flushMetrics).toHaveBeenCalledWith(
    ...
    it('should skip flush when already flushing', async () => {
      ...
      expect(mockPerformanceMetricsRepository.flushMetrics).not.toHaveBeenCalled();
    ...
    it('should collect system metrics periodically', () => {
      ...
      // 使用bind绑定服务实例
      const startSystemMetricsCollection = (service as any).startSystemMetricsCollection.bind(service);
      ...
      expect(systemMetrics.some(m => m.name === METRIC_NAMES.SYSTEM_CPU_USAGE)).toBe(true);
      expect(systemMetrics.some(m => m.name === METRIC_NAMES.SYSTEM_MEMORY_USAGE)).toBe(true);
    });
    ...
    it('should measure event loop lag', () => {
      ...
      const metricBuffer = (service as any).metricBuffer;
      const eventLoopLagMetric = metricBuffer.find(m => m.name === METRIC_NAMES.SYSTEM_EVENT_LOOP_LAG);
  ...
  describe('Default Metrics', () => {
    it('should provide default performance summary', () => {
      ...
      // 使用bind绑定服务实例
      const getDefaultPerformanceSummary = (service as any).getDefaultPerformanceSummary.bind(service);
    ...
    it('should provide default database metrics', () => {
      // 使用bind绑定服务实例
      const getDefaultDatabaseMetrics = (service as any).getDefaultDatabaseMetrics.bind(service);
    ...
    it('should provide default Redis metrics', () => {
      // 使用bind绑定服务实例
      const getDefaultRedisMetrics = (service as any).getDefaultRedisMetrics.bind(service);
    ...
    it('should provide default system metrics', () => {
      // 使用bind绑定服务实例
      const getDefaultSystemMetrics = (service as any).getDefaultSystemMetrics.bind(service);
