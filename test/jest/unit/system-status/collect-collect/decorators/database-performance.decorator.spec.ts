...
describe("Performance Decorators", () => {
  ...
  beforeEach(() => {
    // 创建模拟的MetricsPerformanceService
    ...
    // 设置全局模拟对象
    global["MetricsPerformanceService"] = mockPerformanceMonitor;
  });

  afterEach(() => {
    jest.clearAllMocks();
    delete global["MetricsPerformanceService"];
  });
  ...
  describe("DatabasePerformance Decorator", () => {
    ...
    it("应该包装同步方法并测量性能", () => {
      ...
      expect(mockPerformanceMonitor.recordDatabaseQuery).toHaveBeenCalledWith(
        "find_query",
        5,
        true,
      );
  ...
  describe("AuthPerformance Decorator", () => {
    ...
    it("应该包装JWT认证方法并测量性能", async () => {
      ...
      expect(mockPerformanceMonitor.wrapWithTiming).toHaveBeenCalled();
      expect(mockPerformanceMonitor.recordAuthentication).toHaveBeenCalledWith(
        "jwt",
        true,
        100,
      );
    ...
    it("应该处理认证错误并记录失败", async () => {
      ...
      await expect(instance.verifyTokenWithError()).rejects.toThrow(
        "Invalid token",
      );
      expect(mockPerformanceMonitor.recordAuthentication).toHaveBeenCalledWith(
        "jwt",
  ...
  describe("CachePerformance Decorator", () => {
    ...
    it("应该包装缓存获取方法并记录命中情况", async () => {
      ...
      expect(mockPerformanceMonitor.wrapWithTiming).toHaveBeenCalled();
      expect(mockPerformanceMonitor.recordCacheOperation).toHaveBeenCalledWith(
        "get",
        true,
        100,
      );
