...
describe("MonitoringCacheService", () => {
  ...
  describe("Health Data Operations", () => {
    ...
    describe("setHealthData", () => {
      ...
      it("should handle set errors gracefully", async () => {
        ...
        await expect(
          service.setHealthData("test-key", { data: "test" }),
  ...
  describe("Cache Invalidation", () => {
    describe("invalidateHealthCache", () => {
      it("should invalidate health cache using key index", async () => {
        ...
        expect(mockCacheService.setMembers).toHaveBeenCalledWith(
          "monitoring:index:health",
        );
  ...
  describe("Private Methods", () => {
    describe("buildKey", () => {
      ...
      it("should validate input parameters", () => {
        // 测试空值验证
        expect(() => service["buildKey"]("", "test")).toThrow(
          "监控缓存键类别无效",
        );
        expect(() => service["buildKey"]("health", "")).toThrow(
          "监控缓存键名称无效",
        );
        expect(() => service["buildKey"](null as any, "test")).toThrow(
          "监控缓存键类别无效",
        );
        expect(() => service["buildKey"]("health", null as any)).toThrow(
          "监控缓存键名称无效",
        );
      });

      it("should reject forbidden characters", () => {
        expect(() => service["buildKey"]("health:invalid", "test")).toThrow(
          "禁止字符",
        );
        expect(() => service["buildKey"]("health", "test*key")).toThrow(
          "禁止字符",
        );
        expect(() => service["buildKey"]("health", "test key")).toThrow(
          "禁止字符",
        );
        expect(() => service["buildKey"]("health[0]", "test")).toThrow(
          "禁止字符",
        );
      });
  ...
  describe("Error Handling and Resilience", () => {
    it("should gracefully handle all cache service method failures", async () => {
      ...
      await expect(
        service.setHealthData("key", { data: "test" }),
      ).resolves.not.toThrow();

      const factory = jest.fn().mockResolvedValue({ data: "test" });
      await expect(service.getOrSetHealthData("key", factory)).resolves.toEqual(
        { data: "test" },
      );
    });
