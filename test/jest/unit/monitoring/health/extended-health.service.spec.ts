...
describe("ExtendedHealthService", () => {
  ...
  describe("getFullHealthStatus", () => {
    ...
    it("should return degraded status when there are warnings", async () => {
      ...
      expect(result.recommendations).toContain(
        "Review configuration warnings for optimal setup",
      );
    });
    ...
    it("should handle configuration validation failure gracefully", async () => {
      ...
      configValidatorService.validateAll.mockRejectedValue(
        new Error("验证失败"),
      );
      ...
      expect(result.recommendations).toContain(
        "Fix configuration validation system",
      );
    });
  });
  ...
  describe("getConfigHealthStatus", () => {
    ...
    it("should handle validation failure", async () => {
      ...
      expect(result.errors).toContain(
        "Configuration validation failed: 验证失败",
      );
    });
  });
  ...
  describe("performStartupCheck", () => {
    it("should perform startup check successfully", async () => {
      ...
      startupCheckerService.performQuickCheck.mockResolvedValue(
        mockStartupResult,
      );
    ...
    it("should handle startup check failure", async () => {
      startupCheckerService.performQuickCheck.mockRejectedValue(
        new Error("启动检查失败"),
      );

      await expect(service.performStartupCheck()).rejects.toThrow(
        "启动检查失败",
      );
    });
  });
