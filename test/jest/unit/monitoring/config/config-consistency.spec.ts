  ...
  describe("TTL配置一致性测试", () => {
    describe("TTL配置重复检测", () => {
      ...
      it("应该消除原有的TTL常量重复", () => {
        ...
        expectedTtlTypes.forEach((type) => {
          expect(MONITORING_UNIFIED_TTL_CONSTANTS.DEFAULTS).toHaveProperty(
            type.toUpperCase(),
          );
          expect(MONITORING_UNIFIED_TTL_CONSTANTS.PRODUCTION).toHaveProperty(
            type.toUpperCase(),
          );
          expect(MONITORING_UNIFIED_TTL_CONSTANTS.TEST).toHaveProperty(
            type.toUpperCase(),
          );
        });
      });
      ...
      it("应该支持旧环境变量的向后兼容", () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
          expect.stringContaining(
            "[DEPRECATED] MONITORING_TTL_HEALTH is deprecated",
          ),
        );
    ...
    describe("TTL逻辑一致性验证", () => {
      it("应该验证TTL配置的业务逻辑一致性", () => {
        ...
        expect(validationResult.summary.configurationName).toBe(
          "MonitoringUnifiedTtlConfig",
        );
      });
  ...
  describe("批量配置一致性测试", () => {
    describe("批量大小配置重复检测", () => {
      it("应该无重复的批量大小配置定义", () => {
        ...
        // 验证告警批量配置的层次关系
        expect(limitsConfig.alertBatch.small).toBeLessThanOrEqual(
          limitsConfig.alertBatch.medium,
        );
        expect(limitsConfig.alertBatch.medium).toBeLessThanOrEqual(
          limitsConfig.alertBatch.large,
        );
        expect(limitsConfig.alertBatch.large).toBeLessThanOrEqual(
          limitsConfig.alertBatch.max,
        );
      });
      ...
      it("应该消除原有的批量配置重复", () => {
        ...
        alertBatchSizes.forEach((size) => {
          expect(
            MONITORING_UNIFIED_LIMITS_CONSTANTS.ALERT_BATCH,
          ).toHaveProperty(size);
        });
      });
    ...
    describe("批量配置逻辑关系验证", () => {
      ...
      it("应该验证系统限制配置的合理性", () => {
        ...
        // 应该有关于队列大小的警告
        expect(
          validationResult.warnings.some((w) => w.includes("系统队列大小过小")),
        ).toBe(true);
      });
    });
  ...
  describe("向后兼容性测试", () => {
    ...
    describe("弃用警告验证", () => {
      it("应该为旧环境变量显示弃用警告", () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
          expect.stringContaining(
            "[DEPRECATED] MONITORING_TTL_TREND is deprecated",
          ),
        );
  ...
  describe("配置验证器完整性测试", () => {
    ...
    it("应该生成详细的验证报告", () => {
      const completeValidation =
        MonitoringConfigValidator.validateCompleteConfiguration();
      const report =
        MonitoringConfigValidator.generateValidationReport(completeValidation);
    ...
    it("应该计算合理的配置质量分数", () => {
      ...
      expect(Number.isInteger(completeValidation.summary.overallScore)).toBe(
        true,
      );
    });
  });
