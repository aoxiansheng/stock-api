...
import { MonitoringTtlUtils } from "../../../../../src/monitoring/config/unified/monitoring-unified-ttl.config";

import { MonitoringPerformanceThresholdsUtils } from "../../../../../src/monitoring/config/unified/monitoring-performance-thresholds.config";
...
describe("监控增强配置系统集成测试", () => {
  ...
  describe("配置系统整体集成", () => {
    ...
    it("应该支持配置导出和导入", () => {
      ...
      // 验证导出的配置可以重新创建
      const reimportedConfig =
        MonitoringConfigFactory.createFromObject(exported);
  ...
  describe("环境变量支持", () => {
    ...
    it("应该支持通过环境变量配置性能阈值", () => {
      ...
      expect(envConfig.performanceThresholds.apiResponse.apiExcellentMs).toBe(
        50,
      );
      expect(envConfig.performanceThresholds.systemResource.cpuUsageHigh).toBe(
        0.6,
      );
    });
    ...
    it("应该显示弃用警告对于旧环境变量", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "[DEPRECATED] MONITORING_ALERT_BATCH_SMALL is deprecated",
        ),
      );
  ...
  describe("与旧系统的兼容性", () => {
    ...
    it("应该支持配置迁移路径", () => {
      ...
      expect(MONITORING_ENHANCED_CONFIG_ENV_MAPPING["base.namespace"]).toBe(
        "MONITORING_CACHE_NAMESPACE",
      );
      expect(MONITORING_ENHANCED_CONFIG_ENV_MAPPING["ttl.health"]).toBe(
  ...
  describe("配置服务功能", () => {
    ...
    it("应该提供性能级别判断", () => {
      const responseLevel = configService.getPerformanceLevel(
        "response_time",
        50,
      );
      expect(["excellent", "good", "warning", "poor", "critical"]).toContain(
        responseLevel,
      );
  ...
  describe("工具类集成", () => {
    it("应该集成限制工具类", () => {
      ...
      const selectedSize = MonitoringLimitsUtils.selectBatchSize(
        dataCount,
        batchConfig,
      );
    ...
    it("应该集成TTL工具类", () => {
      ...
      const smartTtl = MonitoringTtlUtils.getSmartTtl(
        "health",
        "high_frequency",
      );
  ...
  describe("环境特定配置", () => {
    it("应该为不同环境应用不同的配置", () => {
      ...
      // 生产环境应该有更严格的阈值
      expect(
        prodConfig.performanceThresholds.apiResponse.apiExcellentMs,
      ).toBeLessThanOrEqual(
        devConfig.performanceThresholds.apiResponse.apiExcellentMs,
      );

      // 生产环境应该有更大的批量大小
      expect(prodConfig.limits.alertBatch.large).toBeGreaterThanOrEqual(
        devConfig.limits.alertBatch.large,
      );
    });
  ...
  describe("配置完整性检查", () => {
    ...
    it("应该提供完整的环境变量映射", () => {
      ...
      expect(
        mapping["performanceThresholds.apiResponse.apiExcellentMs"],
      ).toBeDefined();
    ...
    it("应该支持所有配置的序列化", () => {
      ...
      expect(deserialized.performanceThresholds).toEqual(
        exported.performanceThresholds,
      );
