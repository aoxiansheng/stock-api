    ...
    describe("环境变量类型转换和验证", () => {
      ...
      it("应该验证环境变量值的范围", () => {
        ...
        testCases.forEach(({ var: envVar, value, expectValid }) => {
          ...
          const config = monitoringCoreEnvConfig();
          const validation =
            MonitoringConfigValidator.validateCoreEnvConfig(config);
  ...
  describe("环境特定默认值测试", () => {
    describe("开发环境默认值", () => {
      ...
      it("应该在开发环境应用合理的系统限制", () => {
        ...
        // 开发环境应该有适中的限制
        expect(limitsConfig.systemLimits.maxQueueSize).toBeGreaterThanOrEqual(
          1000,
        );
        expect(limitsConfig.systemLimits.maxQueueSize).toBeLessThanOrEqual(
          5000,
        );
    ...
    describe("测试环境默认值", () => {
      ...
      it("应该在测试环境使用较小的批量大小", () => {
        ...
        expect(limitsConfig.dataProcessingBatch.standard).toBeLessThanOrEqual(
          10,
        );
    ...
    describe("环境切换行为验证", () => {
      it("应该在环境切换时正确调整配置", () => {
        ...
        expect(results.development.health).toBeLessThanOrEqual(
          results.production.health,
        );

        expect(results.test.alert).toBeLessThan(results.development.alert);
        expect(results.development.alert).toBeLessThanOrEqual(
          results.production.alert,
        );
  ...
  describe("环境变量完整性验证", () => {
    ...
    describe("无效值处理", () => {
      it("应该检测和处理无效的环境变量值", () => {
        ...
        // 无效值应该在数组中
        expect(
          validation.invalid.some((invalid) =>
