    ...
    describe("常量与配置分离验证", () => {
      it("应该验证算法常量与配置参数正确分离", async () => {
        ...
        try {
          ...
          const algorithmicConstantsPattern =
            /ALGORITHM|CALCULATION|THRESHOLD|WEIGHT/g;
          const configurationConstantsPattern =
            /TTL|BATCH_SIZE|CACHE_SIZE|QUEUE_SIZE/g;
  ...
  describe("静态重复检测机制", () => {
    describe("代码扫描重复检测", () => {
      it("应该能够检测代码中的配置重复定义", async () => {
        ...
        const findings: {
          [key: string]: { file: string; matches: string[] }[];
        } = {};
      ...
      it("应该检测环境变量重复定义", () => {
        // 使用配置验证器检测环境变量重复
        const overlapResult =
          MonitoringConfigValidator.detectConfigurationOverlaps();
    ...
    describe("运行时配置值重复检测", () => {
      it("应该检测运行时配置值的逻辑重复", () => {
        ...
        if (validationResult.results.overlaps.hasOverlaps) {
          console.warn(
            "检测到配置重复:",
            validationResult.results.overlaps.overlaps,
          );
        }
      });
      ...
      it("应该检测配置实例的数据重复", () => {
        ...
        const instances = Array.from(
          { length: 5 },
          () => new MonitoringUnifiedTtlConfig(),
        );
    ...
    describe("配置新增重复检测", () => {
      it("应该能够检测新配置的重复定义", () => {
        ...
        if (duplicateFields.length > 0) {
          ...
          console.warn(
            "建议：使用现有的MonitoringUnifiedTtlConfig而不是创建新的配置类",
          );
        }
