...
import { GetDbPerformanceQueryDto, MonitoringDateRangeValidator } from '../../../../../src/monitoring/dto/monitoring-query.dto';
...
describe('MonitoringQueryDto', () => {
  describe('GetDbPerformanceQueryDto', () => {
    ...
    it('应该拒绝无效的日期格式', async () => {
      ...
      expect(endDateErrors?.constraints?.MonitoringDateRangeValidator).toBeUndefined();
    });
    ...
    it('应该验证日期范围不超过31天', async () => {
      ...
      expect(endDateError?.constraints).toHaveProperty('MonitoringDateRangeValidator');
    });
  ...
  describe('MonitoringDateRangeValidator', () => {
    ...
    describe('defaultMessage', () => {
      it('应该返回正确的错误消息', () => {
        ...
        // Assert
        expect(message).toBe('The date range cannot exceed 31 days, and the start date must be before the end date.');
      });
    });
    ...
    describe('边界条件测试', () => {
      ...
      it('应该处理时区差异', () => {
        ...
        // Act
        const result = validator.validate('2024-01-15T16:00:00-05:00', mockArgs); // 美国东部时间
