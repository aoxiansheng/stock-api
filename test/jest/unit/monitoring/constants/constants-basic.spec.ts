...
describe('监控常量基础验证', () => {
  ...
  describe('新统一配置系统 - 错误率阈值', () => {
    it('应该定义错误率阈值', () => {
      ...
      expect(testConfig.performanceThresholds.errorRateAvailability.errorRateAcceptable).toBeGreaterThan(0);
      expect(testConfig.performanceThresholds.errorRateAvailability.errorRateWarning).toBeGreaterThan(testConfig.performanceThresholds.errorRateAvailability.errorRateAcceptable);
      // 在测试环境中critical可能小于warning，因为配置不同，所以我们检查它们都是正数
      expect(testConfig.performanceThresholds.errorRateAvailability.errorRateCritical).toBeGreaterThan(0);
    });

    it('应该定义健康阈值', () => {
      expect(testConfig.performanceThresholds.systemResource).toBeDefined();
      expect(testConfig.performanceThresholds.systemResource.cpuUsageHigh).toBeGreaterThan(0);
      expect(testConfig.performanceThresholds.systemResource.cpuUsageCritical).toBeGreaterThan(testConfig.performanceThresholds.systemResource.cpuUsageHigh);
      expect(testConfig.performanceThresholds.apiResponse.apiWarningMs).toBeGreaterThan(0);
    });
  });
  ...
  describe('新统一配置系统 - 系统限制', () => {
    it('应该定义HTTP性能阈值', () => {
      ...
      expect(testConfig.performanceThresholds.apiResponse.apiExcellentMs).toBeGreaterThan(0);
      expect(testConfig.performanceThresholds.apiResponse.apiGoodMs).toBeGreaterThan(testConfig.performanceThresholds.apiResponse.apiExcellentMs);
      expect(testConfig.performanceThresholds.apiResponse.apiWarningMs).toBeGreaterThan(testConfig.performanceThresholds.apiResponse.apiGoodMs);
    });

    it('应该定义性能阈值', () => {
      expect(testConfig.performanceThresholds.databasePerformance).toBeDefined();
      expect(testConfig.performanceThresholds.cachePerformance).toBeDefined();
      expect(testConfig.performanceThresholds.databasePerformance.mongoQueryExcellentMs).toBeGreaterThan(0);
      expect(testConfig.performanceThresholds.cachePerformance.redisResponseExcellentMs).toBeGreaterThan(0);
    });
  ...
  describe('常量一致性验证', () => {
    it('错误率阈值应该合理配置', () => {
      const errorRateConfig = testConfig.performanceThresholds.errorRateAvailability;
      expect(errorRateConfig.errorRateAcceptable).toBeLessThan(errorRateConfig.errorRateWarning);
    ...
    it('CPU使用率阈值应该是递增的', () => {
      ...
      expect(systemResource.cpuUsageLow).toBeLessThan(systemResource.cpuUsageMedium);
      expect(systemResource.cpuUsageMedium).toBeLessThan(systemResource.cpuUsageHigh);
      expect(systemResource.cpuUsageHigh).toBeLessThan(systemResource.cpuUsageCritical);
    });
  ...
  describe('类型验证', () => {
    ...
    it('系统限制应该是数字类型', () => {
      expect(typeof testConfig.limits.systemLimits.maxBufferSize).toBe('number');
    ...
    it('采样配置应该是数字类型', () => {
      expect(typeof MONITORING_BUSINESS.SAMPLING_CONFIG.RECENT_METRICS_COUNT).toBe('number');
      expect(typeof MONITORING_BUSINESS.SAMPLING_CONFIG.MIN_DATA_POINTS).toBe('number');
      expect(typeof MONITORING_BUSINESS.SAMPLING_CONFIG.SAMPLE_SIZE_MEDIUM).toBe('number');
    });
  });
  ...
  describe('常量冻结验证', () => {
    ...
    it('统一限制常量应该是冻结的', () => {
      ...
      expect(Object.isFrozen(MONITORING_UNIFIED_LIMITS_CONSTANTS.ALERT_BATCH)).toBe(true);
      expect(Object.isFrozen(MONITORING_UNIFIED_LIMITS_CONSTANTS.SYSTEM_LIMITS)).toBe(true);
    });
  });
  ...
  describe('业务逻辑验证', () => {
    ...
    it('系统限制应该是正整数', () => {
      expect(testConfig.limits.systemLimits.maxBufferSize).toBeGreaterThan(0);
      expect(testConfig.limits.alertBatch.medium).toBeGreaterThan(0);
      expect(Number.isInteger(testConfig.limits.systemLimits.maxBufferSize)).toBe(true);
      expect(Number.isInteger(testConfig.limits.alertBatch.medium)).toBe(true);
    });

    it('采样配置应该是正整数', () => {
      expect(MONITORING_BUSINESS.SAMPLING_CONFIG.RECENT_METRICS_COUNT).toBeGreaterThan(0);
      expect(MONITORING_BUSINESS.SAMPLING_CONFIG.MIN_DATA_POINTS).toBeGreaterThan(0);
      expect(Number.isInteger(MONITORING_BUSINESS.SAMPLING_CONFIG.RECENT_METRICS_COUNT)).toBe(true);
      expect(Number.isInteger(MONITORING_BUSINESS.SAMPLING_CONFIG.MIN_DATA_POINTS)).toBe(true);
    });
  });
