...
describe("MONITORING_METRICS Constants", () => {
  ...
  describe("MONITORING_METRIC_CATEGORIES", () => {
    it("应该正确分类性能指标", () => {
      expect(MONITORING_METRIC_CATEGORIES.PERFORMANCE).toContain(
    ...
    it("应该正确分类系统资源指标", () => {
      expect(MONITORING_METRIC_CATEGORIES.SYSTEM).toContain(
    ...
    it("应该正确分类错误指标", () => {
      expect(MONITORING_METRIC_CATEGORIES.ERROR).toContain(
    ...
    it("应该正确分类缓存指标", () => {
      expect(MONITORING_METRIC_CATEGORIES.CACHE).toContain(
    ...
    it("应该正确分类数据库指标", () => {
      expect(MONITORING_METRIC_CATEGORIES.DATABASE).toContain(
    ...
    it("每个指标都应该属于某个分类", () => {
      ...
      const categorizedMetrics = Object.values(
        MONITORING_METRIC_CATEGORIES,
      ).flat();
  ...
  describe("MONITORING_METRIC_UNITS", () => {
    it("应该为所有指标定义正确的单位", () => {
      expect(MONITORING_METRIC_UNITS[MONITORING_METRICS.RESPONSE_TIME]).toBe(
        "ms",
      );
      expect(MONITORING_METRIC_UNITS[MONITORING_METRICS.THROUGHPUT]).toBe(
        "rps",
      );
      expect(MONITORING_METRIC_UNITS[MONITORING_METRICS.CPU_USAGE]).toBe("%");
      expect(MONITORING_METRIC_UNITS[MONITORING_METRICS.MEMORY_USAGE]).toBe(
        "MB",
      );
  ...
  describe("MONITORING_METRIC_THRESHOLDS", () => {
    it("应该定义关键指标的阈值", () => {
      expect(
        MONITORING_METRIC_THRESHOLDS[MONITORING_METRICS.RESPONSE_TIME],
      ).toBeDefined();
      expect(
        MONITORING_METRIC_THRESHOLDS[MONITORING_METRICS.CPU_USAGE],
      ).toBeDefined();
      expect(
        MONITORING_METRIC_THRESHOLDS[MONITORING_METRICS.MEMORY_USAGE],
      ).toBeDefined();
      expect(
        MONITORING_METRIC_THRESHOLDS[MONITORING_METRICS.ERROR_RATE],
      ).toBeDefined();
      expect(
        MONITORING_METRIC_THRESHOLDS[MONITORING_METRICS.CACHE_HIT_RATE],
      ).toBeDefined();
    });
    ...
    it("critical阈值应该比warning更严重", () => {
      ...
      const responseThreshold =
        MONITORING_METRIC_THRESHOLDS[MONITORING_METRICS.RESPONSE_TIME];
      expect(responseThreshold.critical).toBeGreaterThan(
        responseThreshold.warning,
      );
  ...
  describe("MONITORING_METRIC_PRIORITIES", () => {
    ...
    it("优先级数字应该递增（数字越小优先级越高）", () => {
      expect(MONITORING_METRIC_PRIORITIES.HIGH).toBeLessThan(
        MONITORING_METRIC_PRIORITIES.MEDIUM,
      );
      expect(MONITORING_METRIC_PRIORITIES.MEDIUM).toBeLessThan(
        MONITORING_METRIC_PRIORITIES.LOW,
      );
    });
  });
});
...
describe("辅助函数测试", () => {
  describe("getMetricCategory", () => {
    it("应该返回正确的指标分类", () => {
      expect(getMetricCategory(MONITORING_METRICS.RESPONSE_TIME)).toBe(
        "performance",
      );
      ...
      expect(getMetricCategory(MONITORING_METRICS.CACHE_HIT_RATE)).toBe(
        "cache",
      );
      expect(getMetricCategory(MONITORING_METRICS.DB_CONNECTIONS)).toBe(
        "database",
      );
    });
    ...
    it("对未知指标应该返回null", () => {
      expect(
        getMetricCategory("unknown_metric" as PerformanceMetricType),
      ).toBeNull();
    });
  });
  ...
  describe("getMetricUnit", () => {
    ...
    it("对未知指标应该返回unknown", () => {
      expect(getMetricUnit("unknown_metric" as PerformanceMetricType)).toBe(
        "unknown",
      );
    });
  });
  ...
  describe("checkMetricThreshold", () => {
    ...
    it("对于没有定义阈值的指标应该返回正常状态", () => {
      expect(
        checkMetricThreshold(MONITORING_METRICS.REQUEST_COUNT, 1000),
      ).toEqual({
        status: "normal",
      });
    });
...
describe("类型安全性验证", () => {
  it("PerformanceMetricType应该包含所有定义的指标", () => {
    const allMetrics: PerformanceMetricType[] =
      Object.values(MONITORING_METRICS);
