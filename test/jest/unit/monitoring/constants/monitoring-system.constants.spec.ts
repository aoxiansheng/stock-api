...
describe("MONITORING_SYSTEM_LIMITS Constants", () => {
  describe("HTTP状态码阈值", () => {
    ...
    it("HTTP阈值应该是递增的", () => {
      expect(MONITORING_SYSTEM_LIMITS.HTTP_SUCCESS_THRESHOLD).toBeLessThan(
  ...
  describe("性能阈值（毫秒）", () => {
    ...
    it("性能阈值应该合理", () => {
      // 缓存应该比API响应快
      expect(
        MONITORING_SYSTEM_LIMITS.CACHE_RESPONSE_THRESHOLD_MS,
      ).toBeLessThanOrEqual(MONITORING_SYSTEM_LIMITS.API_RESPONSE_TIME_MS);

      // API响应应该比慢请求阈值快
      expect(MONITORING_SYSTEM_LIMITS.API_RESPONSE_TIME_MS).toBeLessThan(
  ...
  describe("系统限制", () => {
    ...
    it("系统限制应该合理", () => {
      // 批量大小应该小于缓冲区大小
      expect(MONITORING_SYSTEM_LIMITS.MAX_BATCH_SIZE).toBeLessThanOrEqual(
        MONITORING_SYSTEM_LIMITS.MAX_BUFFER_SIZE,
      );

      // 缓冲区大小应该小于队列大小
      expect(MONITORING_SYSTEM_LIMITS.MAX_BUFFER_SIZE).toBeLessThan(
  ...
  describe("时间窗口（毫秒）", () => {
    ...
    it("时间换算应该正确", () => {
      expect(MONITORING_SYSTEM_LIMITS.HOUR_IN_MS).toBe(
        MONITORING_SYSTEM_LIMITS.MINUTE_IN_MS * 60,
      );
      expect(MONITORING_SYSTEM_LIMITS.DAY_IN_MS).toBe(
        MONITORING_SYSTEM_LIMITS.HOUR_IN_MS * 24,
      );
    });
  });
  ...
  describe("批处理配置", () => {
    ...
    it("批处理配置应该合理", () => {
      // 强制刷新间隔应该大于默认刷新间隔
      expect(MONITORING_SYSTEM_LIMITS.FORCE_FLUSH_INTERVAL_MS).toBeGreaterThan(
        MONITORING_SYSTEM_LIMITS.DEFAULT_FLUSH_INTERVAL_MS,
      );

      // 事件计数器阈值应该大于默认批处理大小
      expect(MONITORING_SYSTEM_LIMITS.EVENT_COUNTER_THRESHOLD).toBeGreaterThan(
  ...
  describe("监控指标配置", () => {
    it("应该定义监控缓存过期时间", () => {
      expect(MONITORING_SYSTEM_LIMITS.MONITORING_CACHE_STALE_TIME_MS).toBe(
        300000,
      ); // 5分钟
    });
    ...
    it("缓存过期时间应该合理", () => {
      expect(
        MONITORING_SYSTEM_LIMITS.MONITORING_CACHE_STALE_TIME_MS,
...
describe("MonitoringSystemLimitUtils", () => {
  ...
  describe("百分比计算", () => {
    it("应该正确计算百分比", () => {
      ...
      expect(MonitoringSystemLimitUtils.calculatePercentage(1, 3)).toBeCloseTo(
        33.33,
        2,
      );
    ...
    it("应该处理边界情况", () => {
      expect(MonitoringSystemLimitUtils.calculatePercentage(100, 100)).toBe(
        100,
      );
      expect(MonitoringSystemLimitUtils.calculatePercentage(200, 100)).toBe(
        200,
      ); // 超过100%
    });
    ...
    it("应该处理除零情况", () => {
      expect(MonitoringSystemLimitUtils.calculatePercentage(10, 0)).toBe(
        Infinity,
      );
    });
  });
...
describe("边界值和异常情况", () => {
  ...
  it("应该正确处理极大数值", () => {
    expect(
      MonitoringSystemLimitUtils.isSlowQuery(Number.MAX_SAFE_INTEGER),
    ).toBe(true);
    expect(
      MonitoringSystemLimitUtils.calculatePercentage(
        1,
