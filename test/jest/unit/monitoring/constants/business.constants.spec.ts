...
describe('MONITORING_BUSINESS Constants - Phase 3 精简版本', () => {
  ...
  describe('迁移通知 - 错误率阈值已迁移', () => {
    it('错误率阈值现在在新配置系统中', () => {
      ...
      expect(testConfig.performanceThresholds.errorRateAvailability.errorRateWarning).toBeGreaterThan(0);
  ...
  describe('迁移通知 - 健康阈值已迁移', () => {
    it('健康阈值现在在新配置系统中', () => {
      ...
      expect(testConfig.performanceThresholds.systemResource.cpuUsageLow).toBeGreaterThan(0);
      expect(testConfig.performanceThresholds.systemResource.cpuUsageMedium).toBeGreaterThan(0);
      expect(testConfig.performanceThresholds.systemResource.cpuUsageHigh).toBeGreaterThan(0);
      expect(testConfig.performanceThresholds.systemResource.cpuUsageCritical).toBeGreaterThan(0);
      ...
      expect(testConfig.performanceThresholds.apiResponse.apiGoodMs).toBeGreaterThan(0);
      expect(testConfig.performanceThresholds.apiResponse.apiWarningMs).toBeGreaterThan(0);
      expect(testConfig.performanceThresholds.apiResponse.apiPoorMs).toBeGreaterThan(0);
    });
  });
...
describe('常量不变性验证 - Phase 3 精简版本', () => {
  ...
  it('不应该允许修改采样配置值', () => {
    const originalValue = MONITORING_BUSINESS.SAMPLING_CONFIG.RECENT_METRICS_COUNT;
    ...
    expect(MONITORING_BUSINESS.SAMPLING_CONFIG.RECENT_METRICS_COUNT).toBe(originalValue);
  });
