...
import { CacheUnifiedConfig, CacheUnifiedConfigValidation } from '../../../../../src/cache/config/cache-unified.config';
...
describe('Cache Configuration Consistency', () => {
  ...
  describe('🎯 配置重叠消除验证', () => {
    ...
    it('should have unified performance configuration', () => {
      ...
      // 验证与旧配置一致性（除已废弃的字段）
      expect(unifiedConfig.compressionThreshold).toBe(legacyCacheConfig?.compressionThreshold);
      expect(unifiedConfig.compressionEnabled).toBe(legacyCacheConfig?.compressionEnabled);
    });
  });
