...
describe('CacheService msgpack Serialization', () => {
  ...
  describe('msgpack Serialization', () => {
    ...
    it('should deserialize msgpack data correctly', async () => {
      ...
      const result = await service.get('test-key', CACHE_DATA_FORMATS.SERIALIZATION.MSGPACK);
    ...
    it('should handle complex nested objects with msgpack', async () => {
      ...
      // Get with msgpack
      const result = await service.get('complex-key', CACHE_DATA_FORMATS.SERIALIZATION.MSGPACK);
  ...
  describe('Performance Comparison', () => {
    it('should demonstrate msgpack vs JSON size difference', async () => {
      ...
      const jsonCall = redisClient.setex.mock.calls.find(call => call[0] === 'json-key');
      const msgpackCall = redisClient.setex.mock.calls.find(call => call[0] === 'msgpack-key');
      ...
      console.log(`JSON size: ${jsonSize} bytes, msgpack size: ${msgpackSize} bytes`);
