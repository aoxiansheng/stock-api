...
describe('CacheService - Error Handling Branch Coverage', () => {
  ...
  beforeEach(async () => {
    ...
    shouldCompressSpy = jest.spyOn(service as any, 'shouldCompress').mockReturnValue(false);
    jest.spyOn(service as any, 'isCompressed').mockImplementation((val: string) => val?.startsWith(COMPRESSION_PREFIX));
    serializeSpy = jest.spyOn(service as any, 'serialize').mockImplementation(val => JSON.stringify(val));
    jest.spyOn(service as any, 'deserialize').mockImplementation((val: string) => val === 'null' ? null : JSON.parse(val));
    compressSpy = jest.spyOn(service as any, 'compress').mockImplementation((val: string) => Promise.resolve(COMPRESSION_PREFIX + val));
    jest.spyOn(service as any, 'decompress').mockImplementation((val: string) => {
    ...
    jest.spyOn(service as any, 'validateKeyLength').mockImplementation(() => {});
  ...
  describe('Connection status dependent operations', () => {
    it('should throw ServiceUnavailableException when Redis is not ready for get operation', async () => {
      ...
      await expect(service.get('test-key')).rejects.toThrow(ServiceUnavailableException);
    ...
    it('should throw ServiceUnavailableException when Redis is connecting for set operation', async () => {
      ...
      await expect(service.set('test-key', 'test-value')).rejects.toThrow(ServiceUnavailableException);
      expect(loggerSpy).toHaveBeenCalledWith(
  ...
  describe('Redis operation errors', () => {
    ...
    it('should handle Redis delete operation errors', async () => {
      ...
      await expect(service.del('test-key')).rejects.toThrow(ServiceUnavailableException);
      expect(loggerSpy).toHaveBeenCalledWith(
  ...
  describe('Data compression error handling', () => {
    ...
    it('should handle compression errors during set operation', async () => {
      ...
      mockRedisInstance.setex.mockRejectedValue(new Error('Compression failed'));
      
      await expect(service.set('test-key', largeData)).rejects.toThrow(ServiceUnavailableException);
    });

    it('should handle decompression errors during get operation', async () => {
      // Mock Redis returning compressed data
      mockRedisInstance.get.mockResolvedValue(COMPRESSION_PREFIX + 'invalid-data');
      
      // Mock decompression to throw error by overriding the previous mock
      jest.spyOn(service as any, 'decompress').mockRejectedValue(new Error('Decompression failed'));
  ...
  describe('Bulk operations error handling', () => {
    ...
    it('should handle errors in mget operation', async () => {
      ...
      await expect(service.mget(['key1', 'key2', 'key3'])).rejects.toThrow(ServiceUnavailableException);
      expect(loggerSpy).toHaveBeenCalledWith(
    ...
    it('should handle errors in mset operation', async () => {
      ...
      const keyValuePairs = new Map([['key1', 'value1'], ['key2', 'value2']]);
      await expect(service.mset(keyValuePairs)).rejects.toThrow(ServiceUnavailableException);
      expect(loggerSpy).toHaveBeenCalledWith(
    ...
    it('should handle errors in delByPattern operation', async () => {
      ...
      await expect(service.delByPattern('test:*')).rejects.toThrow(ServiceUnavailableException);
      expect(loggerSpy).toHaveBeenCalledWith(
  ...
  describe('Hash operations error handling', () => {
    ...
    it('should handle errors in hashSet operation', async () => {
      ...
      await expect(service.hashSet('test-key', 'field', 'value')).rejects.toThrow(ServiceUnavailableException);
      expect(loggerSpy).toHaveBeenCalled();
    });
  ...
  describe('List operations error handling', () => {
    ...
    it('should handle errors in listPush operation', async () => {
      ...
      await expect(service.listPush('test-key', 'value')).rejects.toThrow(ServiceUnavailableException);
      expect(loggerSpy).toHaveBeenCalled();
    });
  ...
  describe('Set operations error handling', () => {
    ...
    it('should handle errors in setAdd operation', async () => {
      ...
      await expect(service.setAdd('test-key', 'member')).rejects.toThrow(ServiceUnavailableException);
      expect(loggerSpy).toHaveBeenCalled();
    });
    ...
    it('should handle errors in setRemove operation', async () => {
      ...
      await expect(service.setRemove('test-key', 'member')).rejects.toThrow(ServiceUnavailableException);
      expect(loggerSpy).toHaveBeenCalled();
    });
  });
  ...
  describe('Utility operations error handling', () => {
    ...
    it('should handle errors in expire operation', async () => {
      ...
      await expect(service.expire('test-key', 300)).rejects.toThrow(ServiceUnavailableException);
      expect(loggerSpy).toHaveBeenCalled();
    });
  });
  ...
  describe('Edge cases and boundary conditions', () => {
    ...
    it('should handle null and undefined values in set operation', async () => {
      ...
      await expect(service.set('undefined-key', undefined)).resolves.not.toThrow();
    });
