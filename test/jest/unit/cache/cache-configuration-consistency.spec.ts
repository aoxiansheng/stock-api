...
// 导入兼容性配置
import {
  CONFIGURATION_MIGRATION_MAP,
  ENVIRONMENT_VARIABLE_MAPPING,
} from "@cache/config/compatibility-registry";
...
describe("Cache Configuration Consistency", () => {
  ...
  describe("环境变量映射一致性验证", () => {
    ...
    it("应该正确映射环境变量到配置", () => {
      // 验证环境变量值正确映射到统一配置
      expect(unifiedConfig.defaultTtl).toBe(
        parseInt(process.env.CACHE_DEFAULT_TTL),
      );
      expect(unifiedConfig.strongTimelinessTtl).toBe(
        parseInt(process.env.CACHE_STRONG_TTL),
      );
      expect(unifiedConfig.realtimeTtl).toBe(
        parseInt(process.env.CACHE_REALTIME_TTL),
      );
      expect(unifiedConfig.authTtl).toBe(parseInt(process.env.CACHE_AUTH_TTL));
      expect(unifiedConfig.maxBatchSize).toBe(
        parseInt(process.env.CACHE_MAX_BATCH_SIZE),
      );
      expect(unifiedConfig.maxCacheSize).toBe(
        parseInt(process.env.CACHE_MAX_SIZE),
      );
      expect(unifiedConfig.smartCacheMaxBatch).toBe(
        parseInt(process.env.SMART_CACHE_MAX_BATCH),
      );
    });
  ...
  describe("向后兼容性验证", () => {
    ...
    it("应该维护配置迁移映射", () => {
      ...
      expect(CONFIGURATION_MIGRATION_MAP.cacheUnified.replaces).toContain(
        "cache",
      );
      expect(CONFIGURATION_MIGRATION_MAP.cacheUnified.replaces).toContain(
        "cacheLimits",
  ...
  describe("配置文件精简验证", () => {
    it("应该只有4个核心配置文件", () => {
      ...
      // 验证配置迁移映射反映了文件精简
      const activeMigrations = Object.values(
        CONFIGURATION_MIGRATION_MAP,
    ...
    it("应该移除冗余配置文件", () => {
      ...
      deprecatedConfigs.forEach((configName) => {
        expect(CONFIGURATION_MIGRATION_MAP[configName].status).toBe(
          "deprecated",
        );
  ...
  describe("业务规则一致性验证", () => {
    it("应该遵循缓存质量标准", () => {
      ...
      expect(qualityStandards.HIT_RATE.ACCEPTABLE).toBe(0.7);
