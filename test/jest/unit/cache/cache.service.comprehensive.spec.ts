...
jest.mock('zlib', () => {
  return {
    gzip: jest.fn((data, callback) => callback(null, Buffer.from(`compressed:${data}`))),
...
// 模拟 metrics 装饰器
jest.mock('../../../../src/metrics/decorators/database-performance.decorator', () => {
  return {
    CachePerformance: jest.fn().mockImplementation(() => (
      _target: any,
      _propertyKey: string,
...
describe('CacheService - Comprehensive Coverage', () => {
  ...
  describe('Compression Logic', () => {
    it('should decompress compressed values on get', async () => {
      ...
      jest.spyOn(service as any, 'decompress').mockResolvedValue(JSON.stringify('test data'));
  ...
  describe('Serialization/Deserialization', () => {
    it('should handle undefined values in serialization', async () => {
      ...
      expect(mockRedis.setex).toHaveBeenCalledWith('test:key', CACHE_TTL.DEFAULT, 'null');
    });
  ...
  describe('Batch Operations', () => {
    ...
    it('should handle compressed values in mget', async () => {
      ...
      jest.spyOn(service as any, 'decompress').mockImplementation(async (value) => {
        if (typeof value === 'string' && value.includes('COMPRESSED::Y29tcHJlc3NlZDpkYXRh')) {
          return 'data';
        }
        return value;
      });
  ...
  describe('Redis Data Structure Operations', () => {
    describe('List Operations', () => {
      it('should handle single and multiple values in listPush', async () => {
        ...
        expect(mockRedis.lpush).toHaveBeenCalledWith('test:list', 'multi1', 'multi2');
      });
    ...
    describe('Set Operations', () => {
      it('should handle single and multiple members in setAdd', async () => {
        ...
        expect(mockRedis.sadd).toHaveBeenCalledWith('test:set', 'multi1', 'multi2');
      });
      ...
      it('should handle single and multiple members in setRemove', async () => {
        ...
        expect(mockRedis.srem).toHaveBeenCalledWith('test:set', 'multi1', 'multi2');
      });
    });
    ...
    describe('Hash Operations', () => {
      it('should increment hash field correctly', async () => {
        ...
        expect(mockRedis.hincrby).toHaveBeenCalledWith('test:hash', 'counter', 2);
      });
  ...
  describe('Cache Statistics and Monitoring', () => {
    ...
    it('should extract key patterns correctly', () => {
      // 使用bind方法确保this上下文正确
      const extractKeyPattern = (service as any).extractKeyPattern.bind(service);
  ...
  describe('Health Check', () => {
    ...
    it('should return warning when memory usage is high', async () => {
      ...
      mockRedis.info.mockResolvedValue('used_memory:9100\r\nmaxmemory:10000\r\n');
  ...
  describe('Cache Metrics Update', () => {
    it('should warn about high miss rate', async () => {
      // 使用bind确保this上下文
      const updateCacheMetrics = (service as any).updateCacheMetrics.bind(service);
  ...
  describe('Background Tasks', () => {
    it('should start optimization tasks', () => {
      // 使用bind确保this上下文
      const startOptimizationTasks = (service as any).startOptimizationTasks.bind(service);
    ...
    it('should log health warnings during background checks', async () => {
      mockRedis.ping.mockResolvedValue('PONG');
      mockRedis.info.mockResolvedValue('used_memory:9500\r\nmaxmemory:10000\r\n');

      // 使用bind确保this上下文
      const checkAndLogHealth = (service as any).checkAndLogHealth.bind(service);
  ...
  describe('Key Validation', () => {
    it('should throw BadRequestException for oversized keys', () => {  
      ...
      // 使用bind确保this上下文
      const validateKeyLength = (service as any).validateKeyLength.bind(service);
  ...
  describe('Utility Methods', () => {
    ...
    it('should parse Redis keyspace correctly', () => {
      ...
      const keyspace = 'db0:keys=100,expires=50,avg_ttl=300\r\ndb1:keys=200,expires=100,avg_ttl=600\r\n';
