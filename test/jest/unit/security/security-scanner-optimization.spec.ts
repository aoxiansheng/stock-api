...
describe('SecurityScannerService Optimization Features', () => {
  ...
  describe('Constants Usage', () => {
    it('should use configuration constants for scan settings', () => {
      expect(SECURITY_SCANNER_CONFIG.SCAN_INTERVAL_MS).toBe(24 * 60 * 60 * 1000);
    ...
    it('should use operation constants for logging', () => {
      expect(SECURITY_SCANNER_OPERATIONS.PERFORM_SCAN).toBe('performSecurityScan');
      expect(SECURITY_SCANNER_OPERATIONS.GET_SCAN_HISTORY).toBe('getScanHistory');
      expect(SECURITY_SCANNER_OPERATIONS.CHECK_PASSWORD_SECURITY).toBe('checkPasswordSecurity');
    });
  ...
  describe('Vulnerability Template Usage', () => {
    it('should create default credentials vulnerability using template', () => {
      ...
      const vulnerability = VulnerabilityTemplateUtil.createDefaultCredentialsVulnerability(usernames);
    ...
    it('should create expired API key vulnerability using template', () => {
      const vulnerability = VulnerabilityTemplateUtil.createExpiredApiKeyVulnerability('123', 'abcd1234');
    ...
    it('should create weak JWT secret vulnerability using template', () => {
      const vulnerability = VulnerabilityTemplateUtil.createWeakJwtSecretVulnerability(32);
  ...
  describe('Enhanced Recommendations', () => {
    it('should generate context-aware recommendations', () => {
      ...
      expect(recommendations).toContain(SECURITY_SCANNER_RECOMMENDATIONS.GENERAL_AUDIT);
      expect(recommendations).toContain(SECURITY_SCANNER_RECOMMENDATIONS.UPDATE_DEPENDENCIES);

      // Should include context-specific recommendations
      expect(recommendations).toContain(SECURITY_SCANNER_RECOMMENDATIONS.IMPLEMENT_2FA);
      expect(recommendations).toContain(SECURITY_SCANNER_RECOMMENDATIONS.SECURITY_MONITORING);
    });
    ...
    it('should include penetration testing for critical vulnerabilities', () => {
      ...
      expect(recommendations).toContain(SECURITY_SCANNER_RECOMMENDATIONS.PENETRATION_TESTING);
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle password security check errors gracefully', async () => {
      ...
      userRepository.findByUsernames.mockRejectedValue(new Error('Database error'));

      await expect((service as any).checkPasswordSecurity()).rejects.toThrow('Database error');
    ...
    it('should handle API key security check errors gracefully', async () => {
      const errorSpy = jest.spyOn((service as any).logger, 'error').mockImplementation();
      apiKeyRepository.findAllActive.mockRejectedValue(new Error('Database error'));

      await expect((service as any).checkAPIKeySecurity()).rejects.toThrow('Database error');
  ...
  describe('Performance Monitoring', () => {
    it('should have performance monitoring constants defined', () => {
      ...
      expect(SECURITY_SCANNER_MESSAGES.SCAN_TIMEOUT_WARNING).toBe('扫描执行时间较长');
    });
  });
});
