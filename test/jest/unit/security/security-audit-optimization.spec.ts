...
describe('SecurityAuditService Optimization Features', () => {
  ...
  describe('Constants Usage', () => {
    ...
    it('should use operation constants for logging', () => {
      expect(SECURITY_AUDIT_OPERATIONS.LOG_SECURITY_EVENT).toBe('logSecurityEvent');
      expect(SECURITY_AUDIT_OPERATIONS.GENERATE_AUDIT_REPORT).toBe('generateAuditReport');
      expect(SECURITY_AUDIT_OPERATIONS.FLUSH_AUDIT_LOGS).toBe('flushAuditLogs');
    });

    it('should use message constants for logging', () => {
      expect(SECURITY_AUDIT_MESSAGES.EVENT_LOGGED).toBe('安全事件已记录到缓冲区');
      expect(SECURITY_AUDIT_MESSAGES.AUDIT_REPORT_GENERATED).toBe('审计报告生成成功');
      expect(SECURITY_AUDIT_MESSAGES.SUSPICIOUS_IP_CLEARED).toBe('清除可疑IP标记');
    });

    it('should use event source constants', () => {
      expect(SECURITY_AUDIT_EVENT_SOURCES.AUTH_SERVICE).toBe('AuthService');
      expect(SECURITY_AUDIT_EVENT_SOURCES.AUTHORIZATION_SERVICE).toBe('AuthorizationService');
      expect(SECURITY_AUDIT_EVENT_SOURCES.DATA_ACCESS_SERVICE).toBe('DataAccessService');
    });
  });
  ...
  describe('Enhanced Audit Report Generation', () => {
    ...
    it('should generate enhanced recommendations based on summary', async () => {
      ...
      auditLogRepository.findWithFilters.mockResolvedValue(manyFailedEvents as any);
      ...
      expect(report.recommendations).toContain(SECURITY_AUDIT_RECOMMENDATIONS.STRICT_ACCOUNT_LOCKOUT);
      expect(report.recommendations).toContain(SECURITY_AUDIT_RECOMMENDATIONS.ENABLE_MFA);
    });
  });
  ...
  describe('Enhanced Error Handling', () => {
    it('should handle audit log retrieval errors gracefully', async () => {
      ...
      auditLogRepository.findWithFilters.mockRejectedValue(new Error('Database error'));
  ...
  describe('Configuration Constants Usage', () => {
    ...
    it('should use event ID configuration for generation', async () => {
      ...
      const logCall = loggerSpy.mock.calls.find(call => 
        call[0] === SECURITY_AUDIT_MESSAGES.EVENT_LOGGED
      );
