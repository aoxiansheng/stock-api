...
describe("SecurityAuditService", () => {
  ...
  describe("logSecurityEvent", () => {
    ...
    it("should update IP analysis on each event", async () => {
      ...
      expect(mockCacheService.hashIncrementBy).toHaveBeenCalledWith(
      ...
      expect(mockCacheService.hashSet).toHaveBeenCalledWith(
        expect.stringContaining("192.168.1.100"),
        "lastSeen",
        expect.any(String),
      );
    ...
    it("should use hashGetAll for success events", async () => {
      ...
      expect(mockCacheService.hashGetAll).toHaveBeenCalledWith(
        expect.stringContaining("192.168.1.100"),
      );
      expect(mockCacheService.hashSet).toHaveBeenCalledWith(
        expect.stringContaining("192.168.1.100"),
  ...
  describe("logSystemEvent", () => {
    it("should log system event with system IP and user agent", async () => {
      await service.logSystemEvent("service_started", "info", {
        version: "1.0.0",
      });
      expect(mockCacheService.listPush).toHaveBeenCalled();
    });
  ...
  describe("error handling", () => {
    ...
    it("should handle logSecurityEvent cache errors", async () => {
      mockCacheService.listPush.mockRejectedValue(
        new Error("Cache unavailable"),
      );

      await expect(service.logSecurityEvent(mockSecurityEvent)).rejects.toThrow(
        "Cache unavailable",
      );
    });

    it("should handle getAuditLogs repository errors", async () => {
      mockAuditLogRepository.findWithFilters.mockRejectedValue(
    ...
    it("should handle generateAuditReport errors", async () => {
      ...
      await expect(
        service.generateAuditReport(new Date(), new Date()),
      ).rejects.toThrow("生成审计报告失败");
    });
  });
