...
describe('StorageRepository', () => {
  ...
  describe('Constructor', () => {
    ...
    it('should handle Redis connection failure gracefully', async () => {
      ...
      const repositoryWithFailedRedis = module.get<StorageRepository>(StorageRepository);
  ...
  describe('Cache Methods', () => {
    ...
    describe('storeInCache()', () => {
      ...
      it('should throw ServiceUnavailableException when Redis is not available', async () => {
        ...
        const repositoryWithoutRedis = module.get<StorageRepository>(StorageRepository);

        await expect(
          repositoryWithoutRedis.storeInCache('key', 'data', 3600, false),
        ).rejects.toThrow(
          new ServiceUnavailableException(STORAGE_ERROR_MESSAGES.REDIS_NOT_AVAILABLE),
        );
      });
      ...
      it('should handle pipeline execution errors', async () => {
        ...
        mockRedis.pipeline().exec.mockRejectedValue(new Error('Pipeline failed'));
    ...
    describe('retrieveFromCache()', () => {
      ...
      it('should return null values when Redis is not available', async () => {
        ...
        const result = await repositoryWithoutRedis.retrieveFromCache('test-key');
    ...
    describe('getAverageTtl()', () => {
      it('should calculate average TTL successfully', async () => {
        ...
        expect(result).toBe(Math.round(mockTtls.reduce((a, b) => a + b) / mockTtls.length));
  ...
  describe('Persistent Storage Methods', () => {
    describe('findByKey()', () => {
      ...
      it('should handle database errors gracefully', async () => {
        ...
        await expect(repository.findByKey(key)).rejects.toThrow('Database error');
      });
    ...
    describe('upsert()', () => {
      ...
      it('should handle database upsert errors', async () => {
        ...
        await expect(repository.upsert(document)).rejects.toThrow('Upsert failed');
      });
    ...
    describe('deleteByKey()', () => {
      it('should delete document by key successfully', async () => {
        ...
        mockStoredDataModel.deleteOne.mockResolvedValue(mockDeleteResult as any);
      ...
      it('should handle database delete errors', async () => {
        const key = 'error-key';
        mockStoredDataModel.deleteOne.mockRejectedValue(new Error('Delete failed'));

        await expect(repository.deleteByKey(key)).rejects.toThrow('Delete failed');
      });
    });
