...
describe("StorageService", () => {
  ...
  beforeEach(async () => {
    mockRepository = {
      ...
      upsert: jest
        .fn()
        .mockResolvedValue({ success: true, _id: "doc-id", key: "test-key" }),
      retrieveFromCache: jest.fn().mockResolvedValue({
        data: '{"test":"data"}',
        metadata: `{"compressed":false, "storedAt": "${new Date().toISOString()}"}`,
        ttl: 3600,
      }),
      ...
      getCacheStats: jest.fn().mockResolvedValue({
        info: "redis_version:6.2.5",
        dbSize: 100,
        totalMemoryUsage: 1024,
      }),
      countAll: jest.fn().mockResolvedValue(500),
      getDataTypeFilterStats: jest
        .fn()
        .mockResolvedValue([{ _id: "type1", count: 100 }]),
      getProviderStats: jest
        .fn()
        .mockResolvedValue([{ _id: "prov1", count: 200 }]),
  ...
  describe("storeData", () => {
    ...
    it("should throw an error if storage fails", async () => {
      ...
      const dto = { ...baseDto, storageType: StorageType.PERSISTENT };
      await expect(service.storeData(dto)).rejects.toThrow(
        "存储失败: DB connection lost",
      );
    });
  });
  ...
  describe("retrieveData", () => {
    ...
    it("should retrieve data from persistent storage", async () => {
      mockRepository.retrieveFromCache.mockResolvedValue({
        data: null,
        metadata: null,
        ttl: 0,
      });
    ...
    it("should throw NotFoundException if not found anywhere", async () => {
      ...
      await expect(service.retrieveData(dto)).rejects.toThrow(
        "数据未找到: not-found-key",
      );
    });
  });
