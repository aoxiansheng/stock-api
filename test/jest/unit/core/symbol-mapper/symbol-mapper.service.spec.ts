...
import { MappingRule, SymbolMappingRule } from '../../../../../src/core/symbol-mapper/schemas/symbol-mapping-rule.schema';
...
describe('SymbolMapperService', () => {
  ...
  describe('transformSymbols', () => {
    it('should transform symbols using provider mapping', async () => {
      mockRepository.findAllMappingsForSymbols.mockResolvedValue([mockMappingRule]);
      const result = await service.transformSymbols('longport', ['700.HK', 'AAPL']);
    ...
    it('should handle symbols when no mapping config exists', async () => {
      ...
      const result = await service.transformSymbols('unknown-provider', ['BABA']);
  ...
  describe('createDataSourceMapping', () => {
    it('should create new data source mapping', async () => {
      const createDto: CreateSymbolMappingDto = { dataSourceName: 'new-provider', mappingRules: [] };
    ...
    it('should throw ConflictException if mapping config exists', async () => {
      const createDto: CreateSymbolMappingDto = { dataSourceName: 'existing-provider', mappingRules: [] };
      mockRepository.exists.mockResolvedValue(true);
      await expect(service.createDataSourceMapping(createDto)).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('updateMappingRule', () => {
    it('should update a mapping rule', async () => {
      ...
      it('should throw NotFoundException if rule does not exist', async () => {
        ...
        await expect(service.updateMappingRule(updateDto)).rejects.toThrow(NotFoundException);
      });
  });
  ...
  describe('deleteMapping', () => {
    ...
    it('should throw NotFoundException if mapping ID does not exist', async () => {
      ...
      await expect(service.deleteMapping('non-existent-id')).rejects.toThrow(NotFoundException);
    });
  });
