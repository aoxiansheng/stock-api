...
describe('SymbolMappingRule Schema', () => {
  ...
  beforeAll(async () => {
    ...
    moduleRef = await Test.createTestingModule({
      imports: [
        ...
        MongooseModule.forFeature([{ name: SymbolMappingRule.name, schema: SymbolMappingRuleSchema }])
      ],
    }).compile();

    symbolMappingRuleModel = moduleRef.get<Model<SymbolMappingRuleDocument>>(getModelToken(SymbolMappingRule.name));
  });
  ...
  it('应该支持复杂的映射规则配置', async () => {
    ...
    // 验证港股映射
    const hkStocks = savedRule.mappingRules.filter(r => r.market === 'HK' && r.symbolType === 'stock');
    expect(hkStocks).toHaveLength(2);
    expect(hkStocks.find(s => s.inputSymbol === '700.HK')?.outputSymbol).toBe('00700');
    
    // 验证美股映射
    const usStocks = savedRule.mappingRules.filter(r => r.market === 'US' && r.symbolType === 'stock');
    expect(usStocks).toHaveLength(2);
    expect(usStocks.find(s => s.inputSymbol === 'AAPL.US')?.outputSymbol).toBe('AAPL');
    
    // 验证A股映射
    const cnStocks = savedRule.mappingRules.filter(r => r.market === 'CN');
    expect(cnStocks).toHaveLength(2);
    expect(cnStocks.find(s => s.inputSymbol === '000001.SZ')?.outputSymbol).toBe('000001');
    ...
    // 验证指数映射
    const indexes = savedRule.mappingRules.filter(r => r.symbolType === 'index');
  ...
  it('应该创建索引', async () => {
    ...
    const dataSourceNameIndex = indexes.find(index => 
      index.key && Object.keys(index.key).includes('dataSourceName') && index.unique);
    ...
    const inputSymbolIndex = indexes.find(index => 
      index.key && Object.keys(index.key).includes('mappingRules.inputSymbol'));
  ...
  it('应该支持不同符号类型', async () => {
    ...
    symbolTypes.forEach(type => {
      const matchingRule = savedRule.mappingRules.find(r => r.symbolType === type);
  ...
  it('应该支持映射规则的批量操作', async () => {
    ...
    const rules = await Promise.all(providers.map(async (provider, index) => {
      const ruleData = {
    ...
    rules.forEach((rule, index) => {
      expect(rule.dataSourceName).toBe(`provider-${String.fromCharCode(97 + index)}`);
    ...
    const providerARule = await symbolMappingRuleModel.findOne({ dataSourceName: 'provider-a' });
