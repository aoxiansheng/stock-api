...
describe('QueryStatisticsService', () => {
  ...
  describe('recordQueryPerformance', () => {
    ...
    it('should log a warning for slow queries', () => {
      ...
      const slowQueryTime = QUERY_PERFORMANCE_CONFIG.SLOW_QUERY_THRESHOLD_MS + 1;
      service.recordQueryPerformance('GET_HISTORICAL_DATA' as any, slowQueryTime, true, false);
      expect(loggerSpy).toHaveBeenCalled();
    });

    it('should initialize stats for a new query type', async () => {
      service.recordQueryPerformance('GET_MARKET_DATA' as any, 120, true, false);
      const stats = await service.getQueryStats();
  ...
  describe('getQueryStats', () => {
    ...
    it('should calculate stats correctly after multiple queries', async () => {
      ...
      service.recordQueryPerformance('GET_HISTORICAL_DATA' as any, 500, false, false);

      const stats = await service.getQueryStats();

      expect(stats.performance.totalQueries).toBe(3);
      expect(stats.performance.averageExecutionTime).toBeCloseTo((100 + 200 + 500) / 3);
