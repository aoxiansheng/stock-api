...
describe("FlexibleMappingRuleService", () => {
  ...
  beforeEach(async () => {
    ...
    templateService = module.get<DeepMocked<DataSourceTemplateService>>(DataSourceTemplateService);
    cacheService = module.get<DeepMocked<MappingRuleCacheService>>(MappingRuleCacheService);
  ...
  describe("createRule", () => {
    ...
    it("should throw BadRequestException when template not found", async () => {
      const dtoWithTemplate = { ...createRuleDto, sourceTemplateId: "nonexistent" };

      templateModel.findById.mockResolvedValue(null);

      await expect(service.createRule(dtoWithTemplate)).rejects.toThrow(BadRequestException);
  ...
  describe("findRules", () => {
    it("should find rules with pagination", async () => {
      ...
      expect(paginationService.createPaginatedResponse).toHaveBeenCalledWith([mockRule], 1, 10, 1);
  ...
  describe("findBestMatchingRule", () => {
    it("should find best matching rule from cache", async () => {
      ...
      expect(result).toEqual(mockRule);
      expect(cacheService.getCachedBestMatchingRule).toHaveBeenCalledWith("longport", "rest", "quote_fields");
    });
  ...
  describe("updateRule", () => {
    ...
    it("should throw NotFoundException when rule not found for update", async () => {
      ...
      await expect(service.updateRule("nonexistent", updateData)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe("toggleRuleStatus", () => {
    it("should toggle rule status successfully", async () => {
      ...
      jest.spyOn(FlexibleMappingRuleResponseDto, 'fromDocument').mockReturnValue({ ...mockRule, isActive: false });
      cacheService.cacheRuleById.mockResolvedValue(undefined);

      const result = await service.toggleRuleStatus("507f1f77bcf86cd799439011", false);
    ...
    it("should throw NotFoundException when rule not found for toggle", async () => {
      ...
      await expect(service.toggleRuleStatus("nonexistent", false)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe("applyFlexibleMappingRule", () => {
    ...
    it("should apply mapping rule successfully", async () => {
      ...
      expect(result.transformedData.lastPrice).toBe(100.50);
