  ...
  describe("TransformValidationDto", () => {
    ...
    describe("Validation Analysis", () => {
      ...
      it("should categorize validation issues", () => {
        ...
        // Act
        const criticalErrors = dto.errors.filter((error) =>
          error.includes("CRITICAL"),
        );
        const regularErrors = dto.errors.filter((error) =>
          error.includes("ERROR"),
        );
        const warnings = dto.warnings.filter((warning) =>
          warning.includes("WARNING"),
        );
        const infoMessages = dto.warnings.filter((warning) =>
  ...
  describe("DataTransformationStatsDto", () => {
    ...
    describe("Statistics Analysis", () => {
      it("should calculate performance metrics", () => {
        ...
        const transformationEfficiency =
          dto.transformationsApplied.length / dto.fieldsTransformed;

        // Assert
        expect(fieldsPerRecord).toBe(5);
        expect(transformationEfficiency).toBe(0.0006); // 3/5000
        expect(dto.transformationsApplied.length).toBeLessThan(
          dto.fieldsTransformed,
        );
      });
      ...
      it("should track transformation types", () => {
        ...
        // Act
        const transformTypes = dto.transformationsApplied.map(
          (t) => t.transformType,
        );
  ...
  describe("Real-world Usage Scenarios", () => {
    describe("Stock Quote Transformation", () => {
      it("should handle LongPort stock quote transformation", () => {
        ...
        stats.transformationsApplied = rule.sharedDataFieldMappings.map(
          (mapping) => ({
    ...
    describe("Market Data Transformation", () => {
      it("should handle market status transformation", () => {
        ...
        expect(marketRule.sharedDataFieldMappings[1].transform.type).toBe(
          "enum_mapping",
        );
        expect(marketRule.sharedDataFieldMappings[1].transform.value["1"]).toBe(
          "OPEN",
        );
      });
    });
    ...
    describe("Batch Transformation Processing", () => {
      it("should handle large batch transformation", () => {
        ...
        batchStats.transformationsApplied = [
          ...
          {
            sourceField: "secu_quote[0].turnover",
            targetField: "turnover",
            transformType: "currency",
          },
          {
            sourceField: "secu_quote[0].timestamp",
            targetField: "lastUpdated",
            transformType: "date_format",
          },
        ];
        ...
        expect(batchStats.fieldsTransformed / batchStats.recordsProcessed).toBe(
          7,
        );
    ...
    describe("Validation and Error Handling", () => {
      ...
      it("should handle transformation failures gracefully", () => {
        ...
        const transformationSuccessRate =
          failureStats.fieldsTransformed / expectedFields;
  ...
  describe("Edge Cases and Performance", () => {
    ...
    describe("Complex Field Paths", () => {
      it("should handle deeply nested field paths", () => {
        ...
        complexTransform.sourceField =
          "data.quotes[0].market_data.price_info.current.value";
