...
describe("DataTransformRequestDto", () => {
  ...
  it("当必填字段 transDataRuleListType 缺失时应无法通过验证", async () => {
    ...
    expect(errors.length).toBeGreaterThan(0); // 期望有验证错误
    expect(
      errors.some((error) => error.property === "transDataRuleListType"),
    ).toBe(true); // 验证是 transDataRuleListType 字段的错误
  });
  ...
  it("当嵌套的 options 对象无效时应无法通过验证", async () => {
    ...
    if (optionsError?.children) {
      expect(
        optionsError.children.some(
          (child) => child.property === "validateOutput",
        ),
      ).toBe(true); // 具体的错误字段
    }
  });
