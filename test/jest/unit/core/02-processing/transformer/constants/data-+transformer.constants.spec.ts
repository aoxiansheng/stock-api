...
} from "../../../../../../../src/core/02-processing/transformer/constants/data-transformer.constants";

describe("Transformer Constants", () => {
  describe("DATATRANSFORM_ERROR_MESSAGES", () => {
    it("should define all error messages", () => {
      expect(DATATRANSFORM_ERROR_MESSAGES.NO_MAPPING_RULE).toBe(
        "未找到匹配的映射规则",
      );
      expect(DATATRANSFORM_ERROR_MESSAGES.TRANSFORMATION_FAILED).toBe(
        "数据转换失败",
      );
      expect(DATATRANSFORM_ERROR_MESSAGES.VALIDATION_FAILED).toBe(
        "转换后数据验证失败",
      );
      expect(DATATRANSFORM_ERROR_MESSAGES.INVALID_RAW_DATA).toBe(
        "原始数据格式无效",
      );
      expect(DATATRANSFORM_ERROR_MESSAGES.MISSING_REQUIRED_FIELDS).toBe(
        "缺少必需字段",
      );
      expect(DATATRANSFORM_ERROR_MESSAGES.RULE_NOT_FOUND).toBe(
        "指定的映射规则不存在",
      );
      expect(DATATRANSFORM_ERROR_MESSAGES.BATCH_TRANSFORMATION_FAILED).toBe(
        "批量转换失败",
      );
      expect(DATATRANSFORM_ERROR_MESSAGES.PREVIEW_GENERATION_FAILED).toBe(
        "预览生成失败",
      );
    ...
    it("should be frozen", () => {
      expect(Object.isFrozen(DATATRANSFORM_ERROR_MESSAGES)).toBe(true);
    });

    it("should use Chinese messages", () => {
      const messages = Object.values(DATATRANSFORM_ERROR_MESSAGES);
  ...
  describe("DATATRANSFORM_CONFIG", () => {
    it("should define all configuration values", () => {
      expect(DATATRANSFORM_CONFIG.MAX_BATCH_SIZE).toBe(1000);
      expect(DATATRANSFORM_CONFIG.MAX_FIELD_MAPPINGS).toBe(100);
      expect(DATATRANSFORM_CONFIG.MAX_SAMPLE_SIZE).toBe(10);
      expect(DATATRANSFORM_CONFIG.DEFAULT_TIMEOUT_MS).toBe(30000);
    ...
    it("should have reasonable configuration values", () => {
      expect(DATATRANSFORM_CONFIG.MAX_BATCH_SIZE).toBeGreaterThan(0);
      expect(DATATRANSFORM_CONFIG.MAX_FIELD_MAPPINGS).toBeGreaterThan(0);
      expect(DATATRANSFORM_CONFIG.MAX_SAMPLE_SIZE).toBeGreaterThan(0);
      expect(DATATRANSFORM_CONFIG.DEFAULT_TIMEOUT_MS).toBeGreaterThan(0);
      expect(DATATRANSFORM_CONFIG.MAX_NESTED_DEPTH).toBeGreaterThan(0);
    });

    it("should be frozen", () => {
      expect(Object.isFrozen(DATATRANSFORM_CONFIG)).toBe(true);
    });
  });
  ...
  describe("DATATRANSFORM_PERFORMANCE_THRESHOLDS", () => {
    ...
    it("should have reasonable threshold values", () => {
      ...
      expect(
        DATATRANSFORM_PERFORMANCE_THRESHOLDS.LARGE_DATASET_SIZE,
      ).toBeGreaterThan(0);
      expect(
        DATATRANSFORM_PERFORMANCE_THRESHOLDS.HIGH_MEMORY_USAGE_MB,
      ).toBeGreaterThan(0);
      expect(
        DATATRANSFORM_PERFORMANCE_THRESHOLDS.MAX_PROCESSING_TIME_MS,
      ).toBeGreaterThan(
        DATATRANSFORM_PERFORMANCE_THRESHOLDS.SLOW_TRANSFORMATION_MS,
      );
    });
    ...
    it("should be frozen", () => {
      expect(Object.isFrozen(DATATRANSFORM_PERFORMANCE_THRESHOLDS)).toBe(true);
    });
  });
  ...
  describe("Integration with transformer service", () => {
    it("should support error message construction", () => {
      ...
      const errorMessage = `${DATATRANSFORM_ERROR_MESSAGES.NO_MAPPING_RULE}: provider '${provider}', transDataRuleListType '${transDataRuleListType}'`;
    ...
    it("should support batch size validation", () => {
      ...
      const exceedsLimit = batchSize > DATATRANSFORM_CONFIG.MAX_BATCH_SIZE;
      expect(exceedsLimit).toBe(true);
    });

    it("should support nested depth validation", () => {
      const pathDepth = 15;
      const exceedsDepth = pathDepth > DATATRANSFORM_CONFIG.MAX_NESTED_DEPTH;
