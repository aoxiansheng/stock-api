...
describe('TransformerService', () => {
  ...
  describe('transform - Basic Scenarios', () => {
    ...
    it('should transform data using specific mappingOutRuleId', async () => {
      ...
      dataMapperService.applyMappingRule.mockResolvedValue([{ lastPrice: 150.5 }]);
  ...
  describe('transform - Validation and Warnings', () => {
    ...
    it('should generate warnings for missing fields without throwing errors', async () => {
      ...
      const result = await service.transform({ ...validRequest, rawData: [incompleteData] });
    ...
    it('should generate performance warnings for slow transformations', async () => {
      ...
      dataMapperService.applyMappingRule.mockImplementation(async () => {
        await new Promise(resolve => setTimeout(resolve, TRANSFORM_PERFORMANCE_THRESHOLDS.SLOW_TRANSFORMATION_MS + 1));
        return [{ lastPrice: 150.5, volume: 100000, symbol: 'AAPL.US' }];
      });

      const transformPromise = service.transform(slowRequest);
      
      await jest.advanceTimersByTimeAsync(TRANSFORM_PERFORMANCE_THRESHOLDS.SLOW_TRANSFORMATION_MS + 1);
    ...
    it('should handle large datasets and generate warnings', async () => {
      const largeDataRequest = {
        ...validRequest,
        rawData: Array.from({ length: TRANSFORM_PERFORMANCE_THRESHOLDS.LARGE_DATASET_SIZE + 1 }, (_, i) => ({
          last_done: 150 + i,
          volume: 5000 + i,
          symbol: `STOCK${i}`,
        })),
      ...
      dataMapperService.findBestMatchingRule.mockResolvedValue(mockMappingRule);
      dataMapperService.applyMappingRule.mockResolvedValue(
        Array.from({ length: TRANSFORM_PERFORMANCE_THRESHOLDS.LARGE_DATASET_SIZE + 1 }, (_, i) => ({
          lastPrice: 150 + i,
          volume: 5000 + i,
          symbol: `STOCK${i}`,
        }))
      ...
      expect(result.transformedData).toBeDefined();
      expect(result.metadata.recordsProcessed).toBeGreaterThan(TRANSFORM_PERFORMANCE_THRESHOLDS.LARGE_DATASET_SIZE);
    });
  });
  ...
  describe('transform - Error Scenarios', () => {
    ...
    it('should throw NotFoundException when no mapping rule found using provider/dataType', async () => {
      ...
      await expect(service.transform(validRequest)).rejects.toThrow(InternalServerErrorException);
      await expect(service.transform(validRequest)).rejects.toThrow(/未找到匹配的映射规则/);
    });

    it('should throw NotFoundException when specific ruleId not found', async () => {
      const requestWithRuleId = { ...validRequest, mappingOutRuleId: 'non-existent-rule' };
      dataMapperService.findOne.mockResolvedValue(null);

      await expect(service.transform(requestWithRuleId)).rejects.toThrow(InternalServerErrorException);
    });
    ...
    it('should handle invalid raw data gracefully', async () => {
      const invalidRequest = { ...validRequest, rawData: null, options: { validateOutput: true } };
      dataMapperService.findBestMatchingRule.mockResolvedValue(mockMappingRule);
      dataMapperService.applyMappingRule.mockResolvedValue(null);

      await expect(service.transform(invalidRequest)).rejects.toThrow(InternalServerErrorException);
    });
  });
  ...
  describe('transformBatch - Advanced Scenarios', () => {
    ...
    it('should reject batches exceeding maximum size', async () => {
      const oversizedBatch = Array.from({ length: TRANSFORM_CONFIG.MAX_BATCH_SIZE + 1 }, (_, i) => ({
        provider: 'longport',
        dataType: 'stock-quote',
        rawData: { last_done: 150 + i, volume: 5000 },
      }));

      await expect(service.transformBatch({ requests: oversizedBatch }))
    ...
    it('should group requests by rule key efficiently', async () => {
      const mixedBatch: TransformRequestDto[] = [
        { provider: 'longport', dataType: 'stock-quote', rawData: { last_done: 150 } },
        { provider: 'longport', dataType: 'stock-quote', rawData: { last_done: 151 } },
        { provider: 'itick', dataType: 'stock-info', rawData: { name: 'Apple' } },
        { provider: 'itick', dataType: 'stock-info', rawData: { name: 'Google' } },
      ];
    ...
    it('should handle different rule IDs in batch requests', async () => {
      ...
      dataMapperService.applyMappingRule.mockResolvedValue([{ lastPrice: 150 }]);
  ...
  describe('previewTransformation', () => {
    ...
    it('should throw NotFoundException when no mapping rule found for preview', async () => {
      ...
      await expect(service.previewTransformation(previewRequest))
    ...
    it('should handle complex nested data in preview', async () => {
      dataMapperService.findBestMatchingRule.mockResolvedValue(mockComplexMappingRule);
      dataMapperService.applyMappingRule.mockResolvedValue([{
        price: 150.75,
  ...
  describe('validateTransformedData', () => {
    it('should return errors for null/undefined transformed data', () => {
      const validation = (service as any).validateTransformedData(null, mockMappingRule);

      expect(validation.errors).toContain(TRANSFORM_ERROR_MESSAGES.INVALID_RAW_DATA);
      expect(validation.warnings).toEqual([]);
    });

    it('should return warnings for empty transformed data', () => {
      const validation = (service as any).validateTransformedData([], mockMappingRule);

      expect(validation.errors).toEqual([]);
      expect(validation.warnings).toContain(TRANSFORM_WARNING_MESSAGES.EMPTY_TRANSFORMED_DATA);
    });
    ...
    it('should validate required fields and generate warnings for missing fields', () => {
      ...
      const validation = (service as any).validateTransformedData(transformedData, mockMappingRule);

      expect(validation.errors).toEqual([]);
      expect(validation.warnings.some(w => w.includes(TRANSFORM_WARNING_MESSAGES.MISSING_EXPECTED_FIELDS))).toBe(true);
    });
    ...
    it('should warn about null field values', () => {
      ...
      expect(validation.errors).toEqual([]);
      expect(validation.warnings.some(w => w.includes(TRANSFORM_WARNING_MESSAGES.NULL_FIELD_VALUES))).toBe(true);
    });
  ...
  describe('calculateTransformationStats', () => {
    it('should calculate stats for array data', () => {
      ...
      const stats = (service as any).calculateTransformationStats(transformedData, mockMappingRule);
  ...
  describe('extractSampleData', () => {
    ...
    it('should limit array properties to max sample size', () => {
      const rawData = {
        prices: Array.from({ length: TRANSFORM_CONFIG.MAX_SAMPLE_SIZE + 5 }, (_, i) => i),
        symbol: 'AAPL',
      };
      const sample = (service as any).extractSampleData(rawData);
  ...
  describe('_executeSingleTransform', () => {
    it('should execute single transformation successfully', async () => {
      ...
      const result = await (service as any)._executeSingleTransform(request, mockMappingRule);
    ...
    it('should handle validation errors in single transformation', async () => {
      ...
      await expect((service as any)._executeSingleTransform(request, mockMappingRule))
    ...
    it('should handle transformation errors in single execution', async () => {
      ...
      dataMapperService.applyMappingRule.mockRejectedValue(new Error('Transformation failed'));
    ...
    it('should generate performance warnings in single transformation', async () => {
      ...
      const transformPromise = (service as any)._executeSingleTransform(request, mockMappingRule);
      ...
      expect(result).toBeInstanceOf(TransformResponseDto);
      expect(result.metadata.processingTime).toBeGreaterThan(TRANSFORM_PERFORMANCE_THRESHOLDS.SLOW_TRANSFORMATION_MS);
      jest.useRealTimers();
    });
  });
  ...
  describe('Integration and Edge Cases', () => {
    it('should handle concurrent transformation requests', async () => {
      ...
      dataMapperService.findBestMatchingRule.mockResolvedValue(mockMappingRule);
      dataMapperService.applyMappingRule.mockImplementation(async (ruleId, data) => [
