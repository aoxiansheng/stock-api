...
describe("CacheKeyUtils", () => {
  describe("generateCacheKey", () => {
    ...
    it("should filter out falsy parts", () => {
      const result = CacheKeyUtils.generateCacheKey(
        "test",
        "part1",
        "",
        "part2",
    ...
    it("should throw error for keys that are too long", () => {
      ...
      expect(() => {
        ...
        `Cache key too long: ${5 + longPart.length} > ${CACHE_CONFIG.MEMORY.MAX_KEY_LENGTH}`,
      );
    });
  });
  ...
  describe("generateStockQuoteKey", () => {
    ...
    it("should generate stock quote key with market", () => {
      ...
      expect(result).toBe(
        `${CACHE_KEY_PREFIXES.STOCK_QUOTE}:700.HK:longport:HK`,
      );
    });
  });
  ...
  describe("generateProviderDataKey", () => {
    it("should generate provider data key", () => {
      ...
      expect(result).toBe(
        `${CACHE_KEY_PREFIXES.PROVIDER_DATA}:longport:quote:AAPL`,
      );
    });
  });
  ...
  describe("prepareBatchKeys", () => {
    ...
    it("should throw error when any key is too long", () => {
      ...
      expect(() => {
        ...
        `Cache key too long: ${longKey.length} > ${CACHE_CONFIG.MEMORY.MAX_KEY_LENGTH}`,
      );
    });
  });
  ...
  describe("normalizeKey", () => {
    ...
    it("should preserve allowed characters", () => {
      const result = CacheKeyUtils.normalizeKey(
        "test_key:with-allowed.chars123",
      );
