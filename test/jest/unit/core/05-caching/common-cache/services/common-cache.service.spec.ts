...
describe('CommonCacheService', () => {
  ...
  describe('getWithFallback', () => {
    it('应该从缓存返回数据当缓存命中时', async () => {
      ...
      // Act
      const result = await service.getWithFallback(key, fetchFn, { fallbackTTL: ttl });
    ...
    it('应该在fetchFn抛出错误时重新抛出错误', async () => {
      ...
      // Act & Assert
      await expect(service.getWithFallback(key, fetchFn, { fallbackTTL: ttl })).rejects.toThrow(fetchError);
    });
  });
