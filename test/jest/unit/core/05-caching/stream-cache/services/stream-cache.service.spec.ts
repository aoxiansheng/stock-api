...
describe('StreamCacheService', () => {
  ...
  const mockStreamData: StreamDataPoint[] = [
    ...
    { s: 'TSLA.US', p: 800.75, v: 2000, t: Date.now() + 1000, c: -5.25, cp: -0.65 },
  ];

  const mockRawData = [
    { symbol: 'AAPL.US', price: 150.25, volume: 1000, timestamp: Date.now() },
    { symbol: 'TSLA.US', lastPrice: 800.75, volume: 2000, timestamp: Date.now() + 1000 },
  ];
  ...
  describe('setData - 缓存设置', () => {
    ...
    it('数据压缩应该正确处理时间戳', async () => {
      const dataWithoutTimestamp = [
        { symbol: 'NVDA.US', price: 220.50, volume: 500 }, // 缺少timestamp
      ];
  ...
  describe('deleteData - 删除缓存', () => {
    ...
    it('应该删除Hot Cache和Warm Cache中的数据', async () => {
      ...
      // 验证Redis删除调用
      expect(mockRedisClient.del).toHaveBeenCalledWith('stream_cache:test:delete');
