...
describe("Cache Request Utils", () => {
  ...
  describe("buildUnifiedCacheKey", () => {
    ...
    it("should handle special characters in additional params", () => {
      ...
      expect(cacheKey).toBe(
        "get-stock:quote:long-port:BRK-A:queryId:test:query-789",
      );
    });
  ...
  describe("buildCacheOrchestratorRequest", () => {
    const mockExecuteOriginalDataFlow = jest
      .fn()
    ...
    it("should handle different strategies", () => {
      ...
      expect(request.cacheKey).toBe(
        "receiver:get-stock-quote:600000.SH:provider:longport",
      );
    });
    ...
    it("should handle request without queryId", () => {
      ...
      expect(request.cacheKey).toBe(
        "receiver:get-stock-candle:000001.SZ:provider:itick",
      );
    ...
    it("should handle single symbol correctly", () => {
      ...
      expect(request.cacheKey).toBe(
        "receiver:get-stock-quote:GOOGL:provider:longport",
      );
    });
    ...
    it("should handle multiple symbols with different markets", () => {
      ...
      expect(request.cacheKey).toBe(
        "receiver:get-stock-quote:000001.SZ|600000.SH|700.HK|AAPL:provider:longport",
      );
    });
  });
  ...
  describe("Integration Tests", () => {
    it("should work together to build complete cache request", () => {
      ...
      expect(cacheKey).toBe(
        `receiver:${receiverType}:000001.SZ|700.HK|AAPL:provider:${provider}`,
      );
    ...
    it("should handle edge cases consistently", () => {
      ...
      expect(() => buildUnifiedCacheKey(prefix, ["TEST"])).toThrow(
        "缓存键前缀不能为空",
      );

      // Empty symbols should throw error
      expect(() => buildUnifiedCacheKey("test:prefix", symbols)).toThrow(
        "符号列表不能为空",
      );
    });
  });
