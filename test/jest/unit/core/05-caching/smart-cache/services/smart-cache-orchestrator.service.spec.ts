...
describe("SmartCacheOrchestrator", () => {
      ...
      const hotQueries = [
        {
          ...
          request: {
            ...
            fetchFn: jest
              .fn()
              .mockResolvedValue({ symbol: "AAPL", price: 195.89 }),
        ...
        },
        {
          ...
          request: {
            ...
            fetchFn: jest
              .fn()
              .mockResolvedValue({ symbol: "TSLA", price: 248.42 }),
            strategy: CacheStrategy.STRONG_TIMELINESS,
          },
          priority: 5,
        },
    ...
    it("should handle batch data with optimized concurrency", async () => {
      ...
      const requests = [
        ...
        },
        {
          ...
          fetchFn: jest
            .fn()
            .mockResolvedValue({ symbol: "GOOGL", price: 2750.8 }),
          strategy: CacheStrategy.STRONG_TIMELINESS,
        },
      ];
  ...
  describe("Error Handling", () => {
    it("should handle fetchFn errors gracefully", async () => {
      ...
      const mockCommonCache = service["commonCacheService"] as jest.Mocked<any>;
      mockCommonCache.getWithFallback.mockRejectedValue(
        new Error("Fetch failed"),
      );

      // The method should throw the error from fetchFn since fallback also fails
      await expect(
        service.getDataWithSmartCache(failingRequest),
