...
describe("SmartCachePerformanceOptimizer", () => {
      ...
      providers: [
        {
          ...
          useFactory: () =>
            new SmartCachePerformanceOptimizer(mockCollectorService),
        },
      ],
    }).compile();
  ...
  describe("Error Handling", () => {
    it("should handle collector service failures gracefully", async () => {
      mockCollectorService.recordSystemMetrics.mockRejectedValueOnce(
        new Error("Collector error"),
      );

      // Should not crash when metrics recording fails
      await expect(
        service.calculateOptimalConcurrency(),
  ...
  describe("Integration", () => {
    it("should work without collector service", () => {
      ...
      expect(
        serviceWithoutCollector.getDynamicMaxConcurrency(),
      ).toBeGreaterThan(0);
    ...
    it("should provide consistent interface regardless of collector availability", async () => {
      const serviceWithCollector = new SmartCachePerformanceOptimizer(
        mockCollectorService,
      );
      const serviceWithoutCollector = new SmartCachePerformanceOptimizer();

      // Both should provide the same interface
      expect(typeof serviceWithCollector.calculateOptimalConcurrency).toBe(
        "function",
      );
      expect(typeof serviceWithoutCollector.calculateOptimalConcurrency).toBe(
        "function",
      );

      expect(typeof serviceWithCollector.getDynamicMaxConcurrency).toBe(
        "function",
      );
      expect(typeof serviceWithoutCollector.getDynamicMaxConcurrency).toBe(
        "function",
      );
    });
  });
