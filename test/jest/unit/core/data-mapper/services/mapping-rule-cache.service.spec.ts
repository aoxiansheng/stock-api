...
describe("MappingRuleCacheService", () => {
  ...
  describe("getCachedBestMatchingRule", () => {
    it("should retrieve cached best matching rule", async () => {
      ...
      expect(cacheService.get).toHaveBeenCalledWith(
        expect.stringContaining("best_rule:longport:rest:quote_fields")
      );
    });
  ...
  describe("cacheRuleById", () => {
    ...
    it("should handle missing rule id", async () => {
      const ruleWithoutId = { ...mockRule, id: undefined };

      await expect(service.cacheRuleById(ruleWithoutId as any))
        .resolves.not.toThrow();
  ...
  describe("getCachedRuleById", () => {
    it("should retrieve cached rule by id", async () => {
      cacheService.get.mockResolvedValue(JSON.stringify(mockRule));

      const result = await service.getCachedRuleById(mockRule.id);

      expect(result).toEqual(mockRule);
      expect(cacheService.get).toHaveBeenCalledWith(
        expect.stringContaining(`rule_by_id:${mockRule.id}`)
      );
    });
  ...
  describe("getCachedProviderRules", () => {
    it("should retrieve cached provider rules", async () => {
      ...
      expect(cacheService.get).toHaveBeenCalledWith(
        expect.stringContaining("provider_rules:longport")
      );
    });
  ...
  describe("warmupCache", () => {
    ...
    it("should skip rules without id during warmup", async () => {
      const ruleWithoutId = { ...mockRule, id: undefined };
      const rules = [mockRule, ruleWithoutId as any];
      cacheService.set.mockResolvedValue(undefined);
  ...
  describe("cache constants", () => {
    it("should have correct cache key prefixes", () => {
      ...
      expect(keys.RULE_BY_ID).toBe("rule_by_id");
      expect(keys.PROVIDER_RULES).toBe("provider_rules");
      expect(keys.STATS).toBe("stats");
    });
  ...
  describe("error resilience", () => {
    it("should handle JSON parse errors gracefully", async () => {
      cacheService.get.mockResolvedValue("malformed json {");
