...
describe('WebSocketServerProvider单元测试', () => {
  ...
  describe('Gateway集成测试', () => {
    ...
    test('Gateway服务器应该优先于Legacy服务器', () => {
      // 设置两个不同的服务器实例
      const legacyServer = { ...mockServer, path: jest.fn().mockReturnValue('/legacy') } as any;
      const gatewayServer = { ...mockServer, path: jest.fn().mockReturnValue('/gateway') } as any;
  ...
  describe('广播功能测试', () => {
    ...
    test('应该能向房间广播消息', async () => {
      const result = await provider.broadcastToRoom('test-room', 'test-event', { test: 'data' });
