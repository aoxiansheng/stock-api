...
describe('StreamReceiverService - Rate Limiting & Memory Protection Integration', () => {
  ...
  describe('Connection Rate Limiting', () => {
    ...
    it('should call checkConnectionRateLimit with correct parameters', async () => {
      // Arrange
      const checkRateLimitSpy = jest.spyOn(service as any, 'checkConnectionRateLimit')
      ...
      jest.spyOn(service as any, 'getOrCreateConnection').mockResolvedValue({ id: 'test-connection' });
      jest.spyOn(service as any, 'setupDataReceiving').mockImplementation(() => {});
    ...
    it('should handle rate limit check failures gracefully (fail-open strategy)', async () => {
      ...
      await expect(service.subscribeStream(subscribeDto, 'client1', '192.168.1.100'))
    ...
    it('should reject connections when rate limit is exceeded', async () => {
      // Arrange
      jest.spyOn(service as any, 'checkConnectionRateLimit')
    ...
    it('should reject connections when max connections limit is reached', async () => {
      ...
      for (let i = 0; i < 1000; i++) {  // MAX_CONNECTIONS = 1000
  ...
  describe('Memory Protection', () => {
    ...
    it('should initialize memory monitoring on service start', () => {
      // Arrange
      const initializeSpy = jest.spyOn(service as any, 'initializeMemoryMonitoring');
    ...
    it('should trigger memory critical threshold and force cleanup', () => {
      ...
      const forceCleanupSpy = jest.spyOn(service as any, 'forceConnectionCleanup');
    ...
    it('should record memory alerts through CollectorService', () => {
      ...
      // Assert
      expect((mockCollectorService as any).eventBus.emit).toHaveBeenCalledWith('ALERT_TRIGGERED', {
    ...
    it('should force cleanup least active connections first', () => {
      ...
      // 添加不同活跃度的连接
      activeConnections.set('active_recent', { id: 'recent', lastActivity: now - 1000 } as any);
      activeConnections.set('active_old', { id: 'old', lastActivity: now - 10000 } as any);
      activeConnections.set('active_oldest', { id: 'oldest', lastActivity: now - 20000 } as any);
