...
describe("StreamReceiverService - Dynamic Batching", () => {
  ...
  describe("Load Detection and Adjustment", () => {
    ...
    it("should have reinitializeBatchProcessingPipeline method", () => {
      ...
      expect(typeof service["reinitializeBatchProcessingPipeline"]).toBe(
        "function",
      );
    });
    ...
    it("should simulate load statistics update", () => {
      ...
      const updateLoadStatisticsSpy = jest.spyOn(
        service as any,
    ...
    it("should handle batch processing interval adjustment", () => {
      ...
      const adjustmentSpy = jest.spyOn(
        service as any,
        "adjustBatchProcessingInterval",
      );
      const reinitializeSpy = jest.spyOn(
        service as any,
        "reinitializeBatchProcessingPipeline",
      );
  ...
  describe("Performance Metrics Recording", () => {
    it("should record batch interval adjustment metrics", () => {
      ...
      service["recordBatchIntervalAdjustment"](
        oldInterval,
        newInterval,
        loadLevel,
      );
  ...
  describe("Resource Cleanup", () => {
    it("should cleanup dynamic batching timer in onModuleDestroy", () => {
      ...
      service["dynamicBatchingState"].adjustmentTimer = setInterval(
        () => {},
        1000,
      ) as NodeJS.Timeout;
    ...
    it("should handle disabled dynamic batching", async () => {
      ...
      const module: TestingModule = await Test.createTestingModule({
        providers: [
          ...
          { provide: CollectorService, useValue: mockCollectorService },
          {
            provide: StreamRecoveryWorkerService,
            useValue: mockRecoveryWorker,
          },
          { provide: RateLimitService, useValue: mockRateLimitService },
        ],
      }).compile();

      const disabledService = module.get<StreamReceiverService>(
        StreamReceiverService,
      );
  ...
  describe("Integration with Batch Processing", () => {
    it("should initialize batch processing with dynamic interval when enabled", () => {
      const initializeBatchProcessingSpy = jest.spyOn(
        service as any,
