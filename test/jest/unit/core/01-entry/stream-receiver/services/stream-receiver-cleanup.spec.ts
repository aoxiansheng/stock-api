...
describe("StreamReceiverService - RxJS Subject Cleanup", () => {
  ...
  beforeEach(async () => {
    ...
    // 创建 Subject 方法的 spy
    quoteBatchSubjectCompleteSpy = jest.spyOn(
      service["quoteBatchSubject"],
      "complete",
    );
    quoteBatchSubjectUnsubscribeSpy = jest.spyOn(
      service["quoteBatchSubject"],
      "unsubscribe",
    );
  });
  ...
  describe("onModuleDestroy - RxJS Subject Cleanup", () => {
    ...
    it("should clean up Subject before other resources", () => {
      ...
      const completeCallOrder =
        quoteBatchSubjectCompleteSpy.mock.invocationCallOrder[0];
      const unsubscribeCallOrder =
        quoteBatchSubjectUnsubscribeSpy.mock.invocationCallOrder[0];
    ...
    it("should clear active connections after Subject and timer cleanup", () => {
      ...
      service["activeConnections"].set(
        "provider1:capability1",
        mockConnection1,
      );
      service["activeConnections"].set(
        "provider2:capability2",
        mockConnection2,
      );
  ...
  describe("Resource Cleanup Integration", () => {
    it("should maintain correct cleanup order for all resources", () => {
      ...
      const mockTimer = setTimeout(() => {}, 1000);
      service["cleanupTimer"] = mockTimer;
      service["activeConnections"].set("test:connection", {
        id: "test",
      } as any);

      const clearIntervalSpy = jest.spyOn(global, "clearInterval");
      const connectionsClearSpy = jest.spyOn(
