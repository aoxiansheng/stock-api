...
import {
  StreamSubscribeDto,
  StreamUnsubscribeDto,
} from "../../../../../../src/core/01-entry/stream-receiver/dto";
...
describe("StreamReceiverGateway", () => {
  ...
  describe("_handleDisconnect()", () => {
    it("should handle client disconnection successfully", async () => {
      // Setup
      const client = createMockSocket();
      mockStreamReceiverService.cleanupClientSubscription.mockResolvedValue(
        undefined,
      );
      ...
      expect(
        mockStreamReceiverService.cleanupClientSubscription,
    ...
    it("should handle cleanup errors gracefully", async () => {
      ...
      mockStreamReceiverService.cleanupClientSubscription.mockRejectedValue(
        error,
      );
  ...
  describe("handleUnsubscribe()", () => {
    ...
    it("should handle unsubscription request successfully", async () => {
      // Setup
      const client = createMockSocket();
      mockStreamReceiverService._unsubscribeSymbols.mockResolvedValue(
        undefined,
      );
      ...
      expect(
        mockStreamReceiverService._unsubscribeSymbols,
      ).toHaveBeenCalledWith(client.id, unsubscribeDto);
  ...
  describe("handleGetSubscription()", () => {
    it("should return subscription status when client has active subscription", async () => {
      ...
      mockStreamReceiverService.getClientSubscription.mockReturnValue(
        mockSubscription,
      );
      ...
      // Verify
      expect(
        mockStreamReceiverService.getClientSubscription,
      ).toHaveBeenCalledWith(client.id);
  ...
  describe("handleGetInfo()", () => {
    ...
    it("should return connection info with API key permissions", async () => {
      // Setup
      const client = createMockSocket();
      client.data.apiKey.permissions = [
        "stream:read",
        "stream:subscribe",
        "data:read",
      ];
  ...
  describe("Message Body Validation", () => {
    ...
    it("should validate unsubscribe message structure", async () => {
      ...
      expect(
        mockStreamReceiverService._unsubscribeSymbols,
      ).toHaveBeenCalledWith(client.id, validDto);
    });
  });
