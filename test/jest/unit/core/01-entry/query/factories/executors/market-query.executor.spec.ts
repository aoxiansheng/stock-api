...
describe('MarketQueryExecutor', () => {
  ...
  describe('execute', () => {
    it('应该对BY_MARKET查询类型抛出NotImplementedException', async () => {
      ...
      // Act & Assert
      await expect(executor.execute(request)).rejects.toThrow(NotImplementedException);
    ...
    it('应该记录警告日志说明当前为占位实现', async () => {
      ...
      const logSpy = jest.spyOn((executor as any).logger, 'warn').mockImplementation();

      // Act & Assert
      await expect(executor.execute(request)).rejects.toThrow(NotImplementedException);
      
      expect(logSpy).toHaveBeenCalledWith(
        'MarketQueryExecutor当前为占位实现',
        {
