...
jest.mock(
  "../../../../../../../src/core/01-entry/query/utils/query.util",
  () => ({
...
describe("QueryService", () => {
  ...
  beforeEach(async () => {
    ...
    const cacheUtils = jest.requireMock(
      "../../../../../../../src/core/05-caching/smart-cache/utils/smart-cache-request.utils",
    );
    const queryUtils = jest.requireMock(
      "../../../../../../../src/core/01-entry/query/utils/query.util",
    );
  ...
  describe("fetchSymbolData", () => {
    ...
    it("should use NO_CACHE strategy when useCache is false", async () => {
      ...
      const { buildCacheOrchestratorRequest } = jest.requireMock(
        "../../../../../../../src/core/05-caching/smart-cache/utils/smart-cache-request.utils",
      );
  ...
  describe("executeOriginalDataFlow", () => {
    ...
    it("should build correct storage key", async () => {
      const { buildStorageKey } = jest.requireMock(
  ...
  describe("fetchFromRealtime", () => {
    ...
    it("should fetch realtime data successfully", async () => {
      const fetchFromRealtime = (service as any).fetchFromRealtime.bind(
        service,
      );
    ...
    it("should handle receiver service errors", async () => {
      receiverService.handleRequest.mockRejectedValueOnce(
        new Error("Receiver error"),
      );
  ...
  describe("Helper Methods", () => {
    ...
    it("should get symbols count range correctly", () => {
      const getSymbolsCountRange = (service as any).getSymbolsCountRange.bind(
        service,
      );
    ...
    it("should calculate cache TTL by market correctly", async () => {
      const calculateCacheTTLByMarket = (
        service as any
      ).calculateCacheTTLByMarket.bind(service);
  ...
  describe("Error Handling", () => {
    it("should handle market status service errors gracefully", async () => {
      marketStatusService.getMarketStatus.mockRejectedValueOnce(
  ...
  describe("Performance Metrics", () => {
    it("should track metrics during query execution", async () => {
      ...
      if (executeQuery) {
        ...
        expect(
          presenterRegistryService.queryConcurrentRequestsActive.inc,
        ).toHaveBeenCalled();
        expect(
          presenterRegistryService.queryPipelineDuration.observe,
        ).toHaveBeenCalled();
        expect(
          presenterRegistryService.querySymbolsProcessedTotal.inc,
        ).toHaveBeenCalled();
      }
    });
  });
