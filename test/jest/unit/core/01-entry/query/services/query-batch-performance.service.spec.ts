...
describe("QueryService - 批量处理性能基准测试", () => {
  ...
  describe("里程碑5.4: 批量查询性能基准测试", () => {
    ...
    it("应该验证10个符号的批量查询性能提升", async () => {
      ...
      console.log(
        `   Receiver调用次数: ${receiverService.handleRequest.mock.calls.length}`,
      );
    ...
    it("应该支持200个符号的跨市场查询", async () => {
      ...
      console.log(`   Receiver调用次数: ${receiverCalls}`);
      console.log(
        `   市场并行效率: ${(200 / receiverCalls).toFixed(1)}符号/批次`,
      );
    }, 35000);
    ...
    it("应该支持500个符号的高负载查询", async () => {
      ...
      console.log(`   Receiver调用次数: ${receiverCalls}`);
      console.log(
        `   平均批次大小: ${(500 / receiverCalls).toFixed(1)}符号/批次`,
      );
      console.log(
        `   吞吐量: ${((500 * 1000) / processingTime).toFixed(1)}符号/秒`,
      );
    }, 65000);
  });
  ...
  describe("性能分析和对比测试", () => {
    ...
    it("应该验证批量处理相比单个处理的性能提升", async () => {
      ...
      await service.executeQuery(batchRequest);
      const batchTime = Date.now() - batchStartTime;
      const batchReceiverCalls =
        receiverService.handleRequest.mock.calls.length;
      ...
      console.log(
        `   🎯 网络调用减少倍数: ${callReduction.toFixed(1)}x (目标>3x)`,
      );
    });
    ...
    it("应该验证分片策略的效率", async () => {
      ...
      for (const testCase of testCases) {
        ...
        console.log(
          `   平均批次大小: ${(testCase.count / receiverCalls).toFixed(1)}符号`,
        );
        console.log(
          `   每符号时间: ${(processingTime / testCase.count).toFixed(2)}ms`,
        );
    ...
    it("应该验证并发处理的效率", async () => {
      ...
      console.log(`   Receiver调用次数: ${receiverCalls}`);
      console.log(
        `   平均每个市场处理时间: ${(processingTime / 3).toFixed(2)}ms`,
      );
