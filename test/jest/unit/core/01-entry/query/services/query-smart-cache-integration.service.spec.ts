...
// Mock外部工具函数
jest.mock(
  "../../../../../../../src/core/public/smart-cache/utils/cache-request.utils",
  () => ({
...
jest.mock(
  "../../../../../../../src/core/restapi/query/utils/query.util",
  () => ({
...
describe("QueryService - Smart Cache Integration", () => {
  ...
  describe("Query批量流水线智能缓存集成", () => {
    ...
    it("应该使用SmartCacheOrchestrator进行批量查询", async () => {
      ...
      jest
        .spyOn(service as any, "executeQueryToReceiverFlow")
        .mockResolvedValue({
          symbol: "MSFT",
          lastPrice: 300.0,
        });
      ...
      // Assert
      expect(
        smartCacheOrchestrator.batchGetDataWithSmartCache,
    ...
    it("应该使用Query层弱时效策略(WEAK_TIMELINESS)", async () => {
      ...
      jest
        .spyOn(service as any, "getMarketStatusForSymbol")
        .mockResolvedValue({});
    ...
    it("应该正确处理缓存缺失情况并调用executeQueryToReceiverFlow", async () => {
      ...
      jest
        .spyOn(service as any, "executeQueryToReceiverFlow")
        .mockImplementation(mockExecuteQueryToReceiverFlow);
      jest
        .spyOn(service as any, "getMarketStatusForSymbol")
        .mockResolvedValue({});
    ...
    it("应该记录正确的监控指标", async () => {
      ...
      expect(metricsRegistry.queryReceiverCallsTotal.inc).toHaveBeenCalledWith({
        ...
        receiver_type: API_OPERATIONS.STOCK_DATA.GET_QUOTE,
      });

      expect(
        metricsRegistry.queryReceiverCallDuration.observe,
      ).toHaveBeenCalledWith(
        {
          market: Market.US,
  ...
  describe("executeQueryToReceiverFlow 方法", () => {
    it("应该调用完整的Receiver流向并允许Receiver层缓存", async () => {
      ...
      expect(result).toEqual({ symbol: "AAPL", lastPrice: 150.0 });
    });
  ...
  describe("getMarketStatusForSymbol 方法", () => {
    it("应该根据符号推断市场并获取市场状态", async () => {
      // Arrange
      const utils2 = jest.createMockFromModule(
        "../../../../../../../src/core/public/smart-cache/utils/cache-request.utils",
      ) as any;
      utils2.inferMarketFromSymbol.mockReturnValue(Market.HK);

      const createMarketStatusResult = (
        market: Market,
        timezone: string,
      ): MarketStatusResult => ({
        market,
      ...
      // Act
      const result = await (service as any).getMarketStatusForSymbol(REFERENCE_DATA.SAMPLE_SYMBOLS.HK_TENCENT);

      // Assert
      expect(utils2.inferMarketFromSymbol).toHaveBeenCalledWith(REFERENCE_DATA.SAMPLE_SYMBOLS.HK_TENCENT);
  ...
  describe("两层缓存协同工作验证", () => {
    it("应该实现Query层（300秒）+ Receiver层（5秒）协同缓存", async () => {
      ...
      jest.spyOn(service as any, "getSymbolsCountRange").mockReturnValue("1-5");
      jest
        .spyOn(service as any, "storeStandardizedData")
        .mockResolvedValue(undefined);

      // 模拟Query层缓存缺失，需要调用Receiver
      smartCacheOrchestrator.batchGetDataWithSmartCache.mockImplementation(
        async (requests) => {
