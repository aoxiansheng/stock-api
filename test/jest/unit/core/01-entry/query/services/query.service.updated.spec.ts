...
describe("QueryService - Updated Tests", () => {
  ...
  describe("executeQuery - Main Entry Point", () => {
    ...
    it("should handle empty symbols array", async () => {
      ...
      await expect(service.executeQuery(emptyRequest)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("TTL Strategy Verification", () => {
    it("should calculate correct cache TTL for HK market during trading hours", async () => {
      ...
      const ttl = await (service as any).calculateCacheTTLByMarket(
        Market.HK,
        symbols,
      );
  ...
  describe("Helper Methods", () => {
    ...
    it("should infer market from symbols correctly", () => {
      ...
      expect((service as any).inferMarketFromSymbols(["600000.SH"])).toBe(
        Market.SH,
      );
  ...
  describe("Error Handling", () => {
    it("should handle smart cache orchestrator errors", async () => {
      ...
      await expect(service.executeQuery(mockRequest)).rejects.toThrow(
        "Cache orchestrator error",
      );
    });

    it("should track error metrics", async () => {
      smartCacheOrchestrator.getDataWithSmartCache.mockRejectedValue(
      ...
      // expect(presenterRegistryService.queryErrors.inc).toHaveBeenCalled();
    });
  });
  ...
  describe("Performance Metrics", () => {
    ...
    it("should track batch size metrics for multi-symbol queries", async () => {
      ...
      // expect(presenterRegistryService.queryBatchSize.observe).toHaveBeenCalled();
    });
  });
