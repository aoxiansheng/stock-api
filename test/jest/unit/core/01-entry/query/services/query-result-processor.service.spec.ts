...
describe('QueryResultProcessorService', () => {
  ...
  describe('pagination metadata consistency', () => {
    it('should use pagination.total for totalResults when pagination exists', () => {
      const executionResult: QueryExecutionResultDto = {
        results: [{ symbol: 'AAPL', price: 150 }, { symbol: 'GOOG', price: 2800 }], // 2个结果
        cacheUsed: false,
        dataSources: { cache: { hits: 0, misses: 0 }, realtime: { hits: 2, misses: 0 } },
        errors: [],
        pagination: {
    ...
    it('should handle pagination.total as 0', () => {
      const executionResult: QueryExecutionResultDto = {
        results: [],
        cacheUsed: false,
        dataSources: { cache: { hits: 0, misses: 0 }, realtime: { hits: 0, misses: 1 } },
        errors: [],
        pagination: {
          page: 1,
          limit: 10,
      ...
      const result = service.process(executionResult, request, 'test-query-id', 50);
