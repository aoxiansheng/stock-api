...
describe("MarketStatusService", () => {
  ...
  describe("基础功能测试", () => {
    ...
    it("should get market status for HK market", async () => {
      ...
      expect(result.status).toMatch(
        /^(TRADING|CLOSED|PRE_MARKET|AFTER_HOURS|LUNCH_BREAK|WEEKEND|HOLIDAY)$/,
      );
  ...
  describe("缓存机制测试", () => {
    ...
    it("should handle cache TTL correctly based on market status", async () => {
      ...
      if (result.status === MarketStatus.TRADING) {
        ...
        result.status === MarketStatus.WEEKEND ||
        result.status === MarketStatus.HOLIDAY
      ) {
        expect(result.realtimeCacheTTL).toBeGreaterThanOrEqual(300); // Weekend/Holiday: >=5 minutes
        expect(result.analyticalCacheTTL).toBeGreaterThan(
          result.realtimeCacheTTL,
        );
      } else {
    ...
    it("should clear expired cache entries", async () => {
      ...
      // 时间戳应该不同
      expect(result1.currentTime.getTime()).not.toBe(
        result2.currentTime.getTime(),
      );
  ...
  describe("批量操作测试", () => {
    ...
    it("should handle partial failures in batch operations", async () => {
      ...
      jest
        .spyOn(service, "getMarketStatus")
        .mockImplementation((market: Market) => {
          if (market === Market.US) {
  ...
  describe("缓存TTL建议测试", () => {
    it("should provide appropriate cache TTL recommendations", async () => {
      ...
      const realtimeTTL = await service.getRecommendedCacheTTL(
        market,
        "REALTIME",
      );
      const analyticalTTL = await service.getRecommendedCacheTTL(
        market,
        "ANALYTICAL",
      );
  ...
  describe("性能测试", () => {
    it("should process single market status request efficiently", async () => {
      const benchmark = TestUtils.createPerformanceBenchmark(
        ...
        (global as any).testConfig.PERFORMANCE_THRESHOLDS
          .MARKET_STATUS_CACHE_MISS,
      );
  ...
  describe("边界条件测试", () => {
    it("should handle invalid market gracefully", async () => {
      ...
      await expect(service.getMarketStatus(invalidMarket)).rejects.toThrow(
