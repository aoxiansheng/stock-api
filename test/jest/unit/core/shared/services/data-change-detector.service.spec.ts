...
import { DataChangeDetectorService, ChangeDetectionResult } from '../../../../../../src/core/shared/services/data-change-detector.service';
...
describe('DataChangeDetectorService', () => {
  ...
  describe('detectSignificantChange', () => {
    ...
    it('should handle detection errors gracefully', async () => {
      ...
      jest.spyOn(service as any, 'calculateQuickChecksum').mockImplementation(() => {
  ...
  describe('calculateQuickChecksum', () => {
    ...
    it('should generate different checksum for different data', () => {
      const checksum1 = (service as any).calculateQuickChecksum(mockStockData);
      const checksum2 = (service as any).calculateQuickChecksum(modifiedStockData);
  ...
  describe('extractNumericValue', () => {
    ...
    it('should extract array indexed fields', () => {
      ...
      const result = (service as any).extractNumericValue(data, 'secu_quote[0].last_done');
    ...
    it('should handle complex nested paths', () => {
      ...
      const result = (service as any).extractNumericValue(data, 'market_data.quotes[0].bid.price');

      expect(result).toBe(150.70);
    });
    ...
    it('should handle invalid array indices', () => {
      ...
      const result = (service as any).extractNumericValue(data, 'quotes[5].price');
    ...
    it('should handle malformed paths gracefully', () => {
      ...
      const result = (service as any).extractNumericValue(data, 'price[invalid].test');
      expect(result).toBeNull();
    });
  });
  ...
  describe('getLastSnapshot and saveSnapshot', () => {
    ...
    it('should handle save snapshot errors gracefully', async () => {
      ...
      jest.spyOn(service as any, 'extractCriticalValues').mockImplementation(() => {
    ...
    it('should handle get snapshot errors gracefully', async () => {
      ...
      jest.spyOn((service as any).snapshotCache, 'get').mockImplementation(() => {
  ...
  describe('Edge Cases and Performance', () => {
    it('should handle extremely large data objects', async () => {
      const largeData = {
        ...mockStockData,
        metadata: Array.from({ length: 1000 }, (_, i) => ({ key: `value_${i}` })),
      };
