...
describe("DataChangeDetectorService", () => {
  ...
  describe("市场状态相关测试", () => {
    it("should use different thresholds for different market statuses", async () => {
      ...
      const tradingThreshold =
        CHANGE_DETECTION_THRESHOLDS.PRICE_CHANGE[MarketStatus.TRADING];
      expect(tradingThreshold).toBe(0.0001); // 0.01%

      // Test closed market (less sensitive thresholds)
      const closedThreshold =
        CHANGE_DETECTION_THRESHOLDS.PRICE_CHANGE[MarketStatus.CLOSED];
      expect(closedThreshold).toBe(0.01); // 1%

      // Test weekend (least sensitive thresholds)
      const weekendThreshold =
        CHANGE_DETECTION_THRESHOLDS.PRICE_CHANGE[MarketStatus.WEEKEND];
