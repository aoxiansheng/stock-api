  ...
  describe('getValueFromPath', () => {
    describe('基础功能测试', () => {
      ...
      it('should extract nested object values using dot notation', () => {
        ...
        expect(ObjectUtils.getValueFromPath(obj, 'user.profile.name')).toBe('John Doe');
        expect(ObjectUtils.getValueFromPath(obj, 'user.profile.details.email')).toBe('john@example.com');
      });
      ...
      it('should handle mixed dot and bracket notation', () => {
        ...
        expect(ObjectUtils.getValueFromPath(obj, 'data.records[0].name')).toBe('Record 1');
        expect(ObjectUtils.getValueFromPath(obj, 'data.records[1].values[2]')).toBe(60);
      });
    });
    ...
    describe('边界条件测试', () => {
      it('should return undefined for null or undefined objects', () => {
        ...
        expect(ObjectUtils.getValueFromPath(undefined, 'any.path')).toBeUndefined();
      });
      ...
      it('should return undefined for non-existent paths', () => {
        ...
        expect(ObjectUtils.getValueFromPath(obj, 'profile.name')).toBeUndefined();
        expect(ObjectUtils.getValueFromPath(obj, 'user.name.length')).toBeUndefined();
      });
      ...
      it('should handle empty objects and arrays', () => {
        ...
        expect(ObjectUtils.getValueFromPath({ items: [] }, 'items[0]')).toBeUndefined();
      });

      it('should handle primitive values in path', () => {
        const obj = { value: 'string' };
        
        expect(ObjectUtils.getValueFromPath(obj, 'value.length')).toBeUndefined();
    ...
    describe('高级特性测试', () => {
      it('should handle camelCase fallback matching', () => {
        ...
        expect(ObjectUtils.getValueFromPath(obj, 'user_email')).toBe('john@example.com');
      ...
      it('should prioritize exact matches over camelCase matches', () => {
        ...
        expect(ObjectUtils.getValueFromPath(obj, 'user_name')).toBe('camelCase');
      });
      ...
      it('should handle complex nested structures', () => {
              ...
              data: [
                {
                  stock_quote: [
                    ...
                    { symbol: 'GOOGL', last_done: 2800.50 }
                  ]
                }
              ]
            }
        ...
        expect(ObjectUtils.getValueFromPath(obj, 'api.response.data[0].stock_quote[1].symbol')).toBe('GOOGL');
        ...
        expect(ObjectUtils.getValueFromPath(obj, 'api.response.data[0].stock_quote[0].last_done')).toBe(150.25);
      });
      ...
      it('should handle special characters in property names', () => {
        ...
        expect(ObjectUtils.getValueFromPath(obj, 'with-dashes')).toBe('dash-value');
        expect(ObjectUtils.getValueFromPath(obj, 'with_underscores')).toBe('underscore-value');
        expect(ObjectUtils.getValueFromPath(obj, 'with spaces')).toBe('space-value');
      });
    });
    ...
    describe('数据类型测试', () => {
      it('should handle all JavaScript data types', () => {
        ...
        expect(ObjectUtils.getValueFromPath(obj, 'regexp')).toBeInstanceOf(RegExp);
        expect(ObjectUtils.getValueFromPath(obj, 'function')).toBeInstanceOf(Function);
      });
    ...
    describe('性能测试', () => {
      ...
      it('should handle deep nesting efficiently', async () => {
        ...
        const path = Array.from({ length: 9 }, (_, i) => `level${9-i}`).join('.') + '.value';
    ...
    describe('实际使用场景测试', () => {
      it('should work with API response structures', () => {
        ...
        expect(ObjectUtils.getValueFromPath(apiResponse, 'data.quotes[0].symbol')).toBe('700.HK');
        expect(ObjectUtils.getValueFromPath(apiResponse, 'data.quotes[1].last_done')).toBe(150.50);
        expect(ObjectUtils.getValueFromPath(apiResponse, 'data.quotes[0].market_info.exchange')).toBe('HKEX');
      });
      ...
      it('should work with configuration objects', () => {
        ...
        expect(ObjectUtils.getValueFromPath(config, 'database.mongodb.connection_string')).toBe('mongodb://localhost:27017');
        expect(ObjectUtils.getValueFromPath(config, 'database.mongodb.options.max_pool_size')).toBe(10);
        expect(ObjectUtils.getValueFromPath(config, 'database.redis.retry_policy.max_retries')).toBe(3);
      });
      ...
      it('should handle provider data transformation scenarios', () => {
        ...
        expect(ObjectUtils.getValueFromPath(providerData, 'secu_quote[0].last_done')).toBe(350.20);
        // The camelCase conversion doesn't work in reverse, so these tests need to use exact paths
        expect(ObjectUtils.getValueFromPath(providerData, 'basic_info.company_name')).toBe('Tencent Holdings Ltd');
        
        // Test the camelCase conversion that DOES work (underscore+letter to camelCase)
        const camelCaseData = {
          secuQuote: [{ lastDone: 350.20 }],
          basicInfo: { companyName: 'Tencent Holdings Ltd' }
        };
        expect(ObjectUtils.getValueFromPath(camelCaseData, 'secu_quote[0].last_done')).toBe(350.20);
        expect(ObjectUtils.getValueFromPath(camelCaseData, 'basic_info.company_name')).toBe('Tencent Holdings Ltd');
      });
    });
    ...
    describe('错误处理测试', () => {
      it('should handle circular references gracefully', () => {
        ...
        expect(() => ObjectUtils.getValueFromPath(obj, 'self.self.self.name')).not.toThrow();
      ...
      it('should handle malformed paths gracefully', () => {
        ...
        expect(() => ObjectUtils.getValueFromPath(obj, '[[[]]')).not.toThrow();
        expect(() => ObjectUtils.getValueFromPath(obj, 'test..value')).not.toThrow();
      ...
      it('should handle prototype pollution attempts', () => {
        ...
        expect(ObjectUtils.getValueFromPath(obj, '__proto__.polluted')).toBeUndefined();
        expect(ObjectUtils.getValueFromPath(obj, 'constructor.prototype.polluted')).toBeUndefined();
      });
    });
  });
});