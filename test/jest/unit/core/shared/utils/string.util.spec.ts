  ...
  describe('calculateSimilarity', () => {
    ...
    describe('实际使用场景测试', () => {
      it('should work correctly for stock symbols', () => {
        ...
        expect(StringUtils.calculateSimilarity('000001.SZ', '000001')).toBe(0.8);
      });

      it('should work correctly for company names', () => {
        expect(StringUtils.calculateSimilarity('Apple Inc', 'Apple Inc.')).toBeGreaterThan(0.8);
        expect(StringUtils.calculateSimilarity('Microsoft Corp', 'Microsoft Corporation')).toBeGreaterThan(0.5);
      });
    });
  });
  ...
  describe('generateSimpleHash', () => {
    ...
    describe('实际使用场景测试', () => {
      ...
      it('should work for data fingerprinting', () => {
        ...
        const fingerprint = StringUtils.generateSimpleHash(JSON.stringify(largeData));
        
        expect(fingerprint).toHaveLength(8);
        expect(fingerprint).toMatch(/^[0-9a-f]{8}$/);
        
        // 相同数据应该生成相同指纹
        const fingerprint2 = StringUtils.generateSimpleHash(JSON.stringify(largeData));
