...
describe('DataMappingRepository', () => {
  ...
  describe('findAllIncludingDeactivated', () => {
    it('should find all data mapping rules including deactivated ones', async () => {
      ...
      mockQuery.exec.mockResolvedValue([mockDataMappingDocument, deactivatedRule]);
  ...
  describe('getRuleListTypes', () => {
    it('should return unique rule types', async () => {
      mockQuery.exec.mockResolvedValue(['quote_fields', 'basic_info_fields', 'index_fields']);

      const result = await repository.getRuleListTypes();

      expect(model.distinct).toHaveBeenCalledWith("ruleListType");
      expect(result).toEqual(['quote_fields', 'basic_info_fields', 'index_fields']);
    });
  });
});
