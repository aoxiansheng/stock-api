...
describe("StoredData Schema", () => {
  ...
  beforeAll(async () => {
    ...
    storedDataModel = moduleRef.get<Model<StoredDataDocument>>(
      getModelToken(StoredData.name),
    );
  });
  ...
  it("应该成功创建存储数据记录", async () => {
    ...
    expect(savedData.storageClassification).toBe(storedDataInfo.storageClassification);
  ...
  it("应该正确应用默认值", async () => {
    const minimalDataInfo = {
      ...
      data: { test: "value" },
      storageClassification: "test-data",
  ...
  it("应该正确序列化对象（toJSON方法）", async () => {
    const storedDataInfo = {
      key: "json:test:key",
      data: { jsonTest: true },
      storageClassification: "json-test",
  ...
  it("应该验证必填字段", async () => {
    ...
    try {
      ...
      expect(error.errors.storageClassification).toBeDefined();
  ...
  it("应该支持复杂的数据结构存储", async () => {
    const complexData = {
      ...
      storageClassification: "stock-quote-detailed",
      provider: "longport",
      market: "HK",
      dataSize: 1024,
      compressed: true,
    ...
    expect(savedData.data.stockQuote.price.current).toBe(320.5);
  ...
  it("应该支持不同市场的数据", async () => {
    ...
    for (let i = 0; i < markets.length; i++) {
      ...
      const dataInfo = {
        ...
        storageClassification: "market-data",
  ...
  it("应该支持不同数据类型过滤器", async () => {
    ...
    for (let i = 0; i < dataTypes.length; i++) {
      const storageClassification = dataTypes[i];
      const dataInfo = {
        key: `datatype:${storageClassification}:${i}`,
        data: { type: storageClassification, index: i },
      ...
      expect(savedData.storageClassification).toBe(storageClassification);
    }
  });
  ...
  it("应该创建索引", async () => {
    ...
    const storedData = new storedDataModel({
      key: "index:test:key",
      data: { test: "index" },
      storageClassification: "index-test",
    ...
    // 查找querytransDataRuleListType索引
    const querytransDataRuleListTypeIndex = indexes.find(
      (index) => index.key && Object.keys(index.key).includes("storageClassification"),
    );
    expect(querytransDataRuleListTypeIndex).toBeDefined();

    // 查找provider索引
    const providerIndex = indexes.find(
      (index) => index.key && Object.keys(index.key).includes("provider"),
    );
    expect(providerIndex).toBeDefined();

    // 查找market索引
    const marketIndex = indexes.find(
      (index) => index.key && Object.keys(index.key).includes("market"),
    );
    ...
    // 查找storedAt索引
    const storedAtIndex = indexes.find(
      (index) => index.key && Object.keys(index.key).includes("storedAt"),
    );
    expect(storedAtIndex).toBeDefined();

    // 查找复合索引 (storageClassification, provider, market)
    const compoundIndex = indexes.find(
      (index) =>
        index.key &&
        Object.keys(index.key).includes("storageClassification") &&
  ...
  it("应该验证key的唯一性", async () => {
    const dataInfo = {
      ...
      data: { first: true },
      storageClassification: "duplicate-test",
  ...
  it("应该支持数据更新操作", async () => {
    const originalData = {
      ...
      storageClassification: "update-test",
    ...
    expect(updatedData.updatedAt.getTime()).toBeGreaterThan(
      updatedData.createdAt.getTime(),
    );
  });
  ...
  it("应该支持TTL过期功能", async () => {
    ...
    const dataInfo = {
      ...
      storageClassification: "expire-test",
