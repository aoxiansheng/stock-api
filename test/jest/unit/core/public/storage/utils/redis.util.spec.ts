  ...
  describe("extractValueFromInfo", () => {
    describe("valid Redis INFO format", () => {
      it("should extract simple key-value pairs", () => {
        ...
        expect(RedisUtils.extractValueFromInfo(info, "used_memory")).toBe(
          "1048576",
        );
        expect(RedisUtils.extractValueFromInfo(info, "expired_keys")).toBe("0");
        expect(RedisUtils.extractValueFromInfo(info, "total_commands")).toBe(
          "1000",
        );
      });
      ...
      it("should handle keys with underscores and numbers", () => {
        ...
        expect(RedisUtils.extractValueFromInfo(info, "db0")).toBe(
          "keys=100,expires=50,avg_ttl=3600",
        );
        expect(RedisUtils.extractValueFromInfo(info, "db1")).toBe(
          "keys=200,expires=100",
        );
      });
      ...
      it("should handle values with special characters", () => {
        ...
        expect(RedisUtils.extractValueFromInfo(info, "redis_version")).toBe(
          "6.2.7",
        );
        expect(RedisUtils.extractValueFromInfo(info, "config_file")).toBe(
          "/etc/redis/redis.conf",
        );
        expect(RedisUtils.extractValueFromInfo(info, "process_id")).toBe(
          "1234",
        );
      });
...
total_commands_processed:5000`;
        ...
        expect(RedisUtils.extractValueFromInfo(info, "used_memory_human")).toBe(
          "1.00M",
        );
        expect(RedisUtils.extractValueFromInfo(info, "used_memory_rss")).toBe(
          "2097152",
        );
        expect(
          RedisUtils.extractValueFromInfo(info, "total_connections_received"),
        ).toBe("1000");
        expect(
          RedisUtils.extractValueFromInfo(info, "total_commands_processed"),
        ).toBe("5000");
      });
      ...
      it("should handle values with spaces and colons", () => {
        ...
        expect(
          RedisUtils.extractValueFromInfo(info, "rdb_last_save_time"),
        ).toBe("1678901234");
      ...
      it("should handle empty values", () => {
        ...
        expect(RedisUtils.extractValueFromInfo(info, "valid_key")).toBe(
          "value",
        );
    ...
    describe("missing keys", () => {
      ...
      it("should return null for partial key matches", () => {
        ...
        expect(
          RedisUtils.extractValueFromInfo(info, "used_memory_"),
        ).toBeNull();
      });
      ...
      it("should be case sensitive", () => {
        ...
        expect(RedisUtils.extractValueFromInfo(info, "CaseSensitive")).toBe(
          "value",
        );
        expect(
          RedisUtils.extractValueFromInfo(info, "casesensitive"),
        ).toBeNull();
        expect(
          RedisUtils.extractValueFromInfo(info, "CASESENSITIVE"),
        ).toBeNull();
        expect(RedisUtils.extractValueFromInfo(info, "lowercase")).toBe(
          "value2",
        );
    ...
    describe("invalid/empty inputs", () => {
      ...
      it("should handle null/undefined inputs gracefully", () => {
        ...
        expect(
          RedisUtils.extractValueFromInfo(undefined as any, "key"),
        ).toBeNull();
        expect(RedisUtils.extractValueFromInfo("info", null as any)).toBeNull();
        expect(
          RedisUtils.extractValueFromInfo("info", undefined as any),
        ).toBeNull();
      });
      ...
      it("should handle malformed INFO strings", () => {
        ...
        expect(
          RedisUtils.extractValueFromInfo(malformedInfo, "no_colon_here"),
        ).toBeNull();
        expect(
          RedisUtils.extractValueFromInfo(malformedInfo, "valid_key"),
        ).toBe("value");
        expect(
          RedisUtils.extractValueFromInfo(
            malformedInfo,
            "invalid_line_without_colon",
          ),
        ).toBeNull();
    ...
    describe("edge cases with special characters", () => {
      it("should handle keys with special regex characters", () => {
        ...
        expect(RedisUtils.extractValueFromInfo(info, "key.with.dots")).toBe(
          "value1",
        );
        expect(RedisUtils.extractValueFromInfo(info, "key$with$dollars")).toBe(
          "value2",
        );
        expect(RedisUtils.extractValueFromInfo(info, "key[with]brackets")).toBe(
          "value3",
        );
      });
      ...
      it("should handle values with multiple colons", () => {
        const info =
          "time_stamp:2023:03:15:14:30:00\nurl:http://example.com:8080";

        expect(RedisUtils.extractValueFromInfo(info, "time_stamp")).toBe(
          "2023:03:15:14:30:00",
        );
        expect(RedisUtils.extractValueFromInfo(info, "url")).toBe(
      ...
      it("should handle Unicode characters in values", () => {
        ...
        expect(RedisUtils.extractValueFromInfo(info, "chinese_text")).toBe(
          "你好世界",
        );
        expect(RedisUtils.extractValueFromInfo(info, "emoji_value")).toBe(
          "🚀✨🎉",
        );
        expect(RedisUtils.extractValueFromInfo(info, "special_chars")).toBe(
          "áéíóú",
        );
      });
      ...
      it("should handle values with newlines and escapes", () => {
        ...
        expect(RedisUtils.extractValueFromInfo(info, "normal_key")).toBe(
          "normal_value",
        );
        expect(RedisUtils.extractValueFromInfo(info, "complex_key")).toBe(
          "value\\nwith\\tescapes",
        );
      });
    });
    ...
    describe("performance and large inputs", () => {
      it("should handle large INFO strings efficiently", () => {
        ...
        const largeInfo = Array.from(
          { length: 1000 },
          (_, i) => `key${i}:value${i}`,
        ).join("\n");

        // Should find keys at different positions efficiently
        expect(RedisUtils.extractValueFromInfo(largeInfo, "key0")).toBe(
          "value0",
        );
        expect(RedisUtils.extractValueFromInfo(largeInfo, "key500")).toBe(
          "value500",
        );
        expect(RedisUtils.extractValueFromInfo(largeInfo, "key999")).toBe(
          "value999",
        );
        expect(
          RedisUtils.extractValueFromInfo(largeInfo, "key1000"),
        ).toBeNull();
      });
      ...
      it("should handle repeated key searches without performance degradation", () => {
        ...
        for (let i = 0; i < 100; i++) {
          expect(RedisUtils.extractValueFromInfo(info, "target_key")).toBe(
            "target_value",
          );
        }
      });
...
mem_fragmentation_bytes:1048576`;
        ...
        expect(RedisUtils.extractValueFromInfo(memoryInfo, "used_memory")).toBe(
          "1048576",
        );
        expect(
          RedisUtils.extractValueFromInfo(memoryInfo, "used_memory_human"),
        ).toBe("1.00M");
        expect(
          RedisUtils.extractValueFromInfo(
            memoryInfo,
...
pubsub_patterns:0`;
        ...
        expect(
          RedisUtils.extractValueFromInfo(statsInfo, "keyspace_hits"),
        ).toBe("500");
        expect(RedisUtils.extractValueFromInfo(statsInfo, "expired_keys")).toBe(
          "0",
        );
      });
...
db1:keys=200,expires=100,avg_ttl=7200000`;
        ...
        expect(RedisUtils.extractValueFromInfo(keyspaceInfo, "db0")).toBe(
          "keys=100,expires=50,avg_ttl=3600000",
        );
        expect(RedisUtils.extractValueFromInfo(keyspaceInfo, "db1")).toBe(
          "keys=200,expires=100,avg_ttl=7200000",
        );
