...
describe('Analytics vs Direct Performance Benchmark', () => {
  ...
  it('should benchmark direct performance monitoring calls', async () => {
    ...
    const directDuration = await benchmarkFunction(directPerformanceCall, BENCHMARK_ITERATIONS);
  ...
  it('should benchmark Analytics service calls', async () => {
    ...
    const analyticsDuration = await benchmarkFunction(analyticsPerformanceCall, BENCHMARK_ITERATIONS);
  ...
  it('should compare performance overhead of Analytics layer', async () => {
    ...
    console.log(`├─────────────────┼──────────────┼────────────────┤`);
    console.log(`│ 直接调用        │ ${avgDirectTime.toFixed(3).padStart(12)} │ ${(1000/avgDirectTime).toFixed(0).padStart(14)} │`);
    console.log(`│ Analytics调用   │ ${avgAnalyticsTime.toFixed(3).padStart(12)} │ ${(1000/avgAnalyticsTime).toFixed(0).padStart(14)} │`);
    console.log(`└─────────────────┴──────────────┴────────────────┘`);
    console.log(`\n🔍 性能开销分析:`);
  ...
  it('should test batch processing performance', async () => {
    ...
    for (const batchSize of BATCH_SIZES) {
      ...
      const directBatchTime = await benchmarkFunction(async () => {
        const promises = Array(batchSize).fill(0).map(() => directPerformanceCall());
        await Promise.all(promises);
      }, 10);
      
      // Analytics批量调用  
      const analyticsBatchTime = await benchmarkFunction(async () => {
        const promises = Array(batchSize).fill(0).map(() => analyticsPerformanceCall());
        await Promise.all(promises);
      }, 10);
      
      const avgDirectBatch = directBatchTime / 10;
      const avgAnalyticsBatch = analyticsBatchTime / 10;
      const batchOverhead = ((avgAnalyticsBatch - avgDirectBatch) / avgDirectBatch) * 100;
    ...
    batchResults.forEach(result => {
      console.log(`│ ${result.batchSize.toString().padStart(10)} │ ${result.directTime.toFixed(2).padStart(11)} │ ${result.analyticsTime.toFixed(2).padStart(12)} │ ${result.overhead.toFixed(1).padStart(10)} │`);
    });
    console.log(`└────────────┴─────────────┴──────────────┴────────────┘`);
    ...
    const actualDirectScaling = largestBatch.directTime / smallestBatch.directTime;
    const actualAnalyticsScaling = largestBatch.analyticsTime / smallestBatch.analyticsTime;
  ...
  it('should measure memory usage impact', async () => {
    ...
    console.log(`📊 内存使用报告:`);
    console.log(`   运行前堆内存: ${(memoryBefore.heapUsed / 1024 / 1024).toFixed(2)} MB`);
    console.log(`   运行后堆内存: ${(memoryAfter.heapUsed / 1024 / 1024).toFixed(2)} MB`);
    console.log(`   堆内存变化: ${(heapDelta / 1024 / 1024).toFixed(2)} MB`);
    console.log(`   外部内存变化: ${(externalDelta / 1024 / 1024).toFixed(2)} MB`);
    console.log(`   平均每次调用内存: ${(heapDelta / BENCHMARK_ITERATIONS / 1024).toFixed(2)} KB`);
