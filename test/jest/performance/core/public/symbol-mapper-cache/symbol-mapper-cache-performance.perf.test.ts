...
describe.skip("SymbolMapperCache High-Performance Tests", () => {
  ...
  beforeAll(async () => {
      ...
      imports: [
        MongooseModule.forRoot(
          process.env.MONGODB_URI_TEST ||
            "mongodb://localhost:27017/symbol-mapper-perf-test",
        ),
        MongooseModule.forFeature([
          {
            name: SymbolMappingRuleDocument.name,
            schema: SymbolMappingRuleDocumentSchema,
          },
        ]),
      ],
      providers: [
    ...
    cacheService = moduleRef.get<SymbolMapperCacheService>(
      SymbolMapperCacheService,
    );
    repository = moduleRef.get<SymbolMappingRepository>(
      SymbolMappingRepository,
    );
    ...
    performanceMetrics.memoryUsage.initialHeap =
      initialMemUsage.heapUsed / 1024 / 1024;
  ...
  afterAll(async () => {
    ...
    performanceMetrics.memoryUsage.finalHeap =
  ...
  async function setupLargeScaleTestData() {
    ...
    try {
      ...
      console.log(
        `✅ Large scale test data setup completed: ${largeSymbolSet.length} symbols`,
      );
    } catch (error) {
  ...
  function generatePerformanceReport() {
    ...
    console.log(`🚀 高并发测试:`);
    console.log(
      `   - 并发用户数: ${performanceMetrics.highConcurrency.concurrentUsers}`,
    );
    console.log(
      `   - 每用户请求数: ${performanceMetrics.highConcurrency.requestsPerUser}`,
    );
    console.log(
      `   - 总请求数: ${performanceMetrics.highConcurrency.totalRequests}`,
    );
    console.log(
      `   - 成功请求数: ${performanceMetrics.highConcurrency.successfulRequests}`,
    );
    console.log(
      `   - 成功率: ${((performanceMetrics.highConcurrency.successfulRequests / performanceMetrics.highConcurrency.totalRequests) * 100).toFixed(2)}%`,
    );
    console.log(
      `   - 平均响应时间: ${performanceMetrics.highConcurrency.averageResponseTime.toFixed(2)}ms`,
    );
    console.log(
      `   - 最大响应时间: ${performanceMetrics.highConcurrency.maxResponseTime.toFixed(2)}ms`,
    );
    console.log(
      `   - 最小响应时间: ${performanceMetrics.highConcurrency.minResponseTime.toFixed(2)}ms`,
    );

    console.log(`\\n💾 内存使用:`);
    console.log(
      `   - 初始堆内存: ${performanceMetrics.memoryUsage.initialHeap.toFixed(2)}MB`,
    );
    console.log(
      `   - 峰值堆内存: ${performanceMetrics.memoryUsage.peakHeap.toFixed(2)}MB`,
    );
    console.log(
      `   - 最终堆内存: ${performanceMetrics.memoryUsage.finalHeap.toFixed(2)}MB`,
    );
    console.log(
      `   - 内存增长: ${(performanceMetrics.memoryUsage.finalHeap - performanceMetrics.memoryUsage.initialHeap).toFixed(2)}MB`,
    );

    console.log(`\\n🔄 缓存失效:`);
    console.log(
      `   - 失效时间: ${performanceMetrics.cacheInvalidation.invalidationTime.toFixed(2)}ms`,
    );
    console.log(
      `   - 受影响条目: ${performanceMetrics.cacheInvalidation.affectedEntries}`,
    );
    console.log(
      `   - 恢复时间: ${performanceMetrics.cacheInvalidation.recoveryTime.toFixed(2)}ms`,
    );
  }
  ...
  describe("🚀 High-Concurrency Stress Tests", () => {
    it("should handle high-concurrency batch queries efficiently", async () => {
      ...
      const concurrentUsers =
        performanceMetrics.highConcurrency.concurrentUsers;
      const requestsPerUser =
        performanceMetrics.highConcurrency.requestsPerUser;
          ...
          for (let req = 0; req < requestsPerUser; req++) {
            ...
            try {
              const { result, time } = await measureExecutionTime(async () => {
                return await cacheService.mapSymbols(
                  testDataSourceName,
                  symbolSet,
                  "to_standard",
                );
      ...
      performanceMetrics.highConcurrency.averageResponseTime =
        totalResponseTime / responseTimes.length;

      console.log(`✅ 高并发测试完成:`);
      console.log(
        `   - 总请求: ${performanceMetrics.highConcurrency.totalRequests}`,
      );
      console.log(
        `   - 成功: ${performanceMetrics.highConcurrency.successfulRequests}`,
      );
      console.log(
        `   - 平均响应时间: ${performanceMetrics.highConcurrency.averageResponseTime.toFixed(2)}ms`,
      );
      console.log(
        `   - 峰值内存: ${performanceMetrics.memoryUsage.peakHeap.toFixed(2)}MB`,
      );
      ...
      expect(
        ...
        performanceMetrics.highConcurrency.totalRequests * 0.95,
      ); // 95%成功率
      expect(
        performanceMetrics.highConcurrency.averageResponseTime,
      ).toBeLessThan(100); // 平均响应时间 < 100ms
      expect(performanceMetrics.memoryUsage.peakHeap).toBeLessThan(
        performanceMetrics.memoryUsage.initialHeap + 50,
      ); // 内存增长 < 50MB
    });
    ...
    it("should maintain performance under sustained load", async () => {
      ...
      const avgResponseTime =
        sustainedMetrics.responseTimes.reduce((a, b) => a + b, 0) /
        sustainedMetrics.responseTimes.length;
      const successRate =
        (sustainedMetrics.successfulRequests / sustainedMetrics.totalRequests) *
        100;
  ...
  describe("💾 Memory Usage and Leak Tests", () => {
    ...
    it("should release memory after cache operations", async () => {
      ...
      for (let i = 0; i < 100; i++) {
        const operation = cacheService.mapSymbols(
          testDataSourceName,
          [`temp_${i}`],
          "to_standard",
        );
  ...
  describe("🔄 Cache Invalidation Performance", () => {
    it("should handle cache invalidation efficiently", async () => {
      ...
      console.log(
        "失效前缓存状态:",
        JSON.stringify(preInvalidationStats, null, 2),
      );
      ...
      try {
        ...
        performanceMetrics.cacheInvalidation.invalidationTime =
          invalidationEnd - invalidationStart;
        ...
        const recoveryResult = await cacheService.mapSymbols(
          testDataSourceName,
          ["00001"],
          "to_standard",
        );
        ...
        performanceMetrics.cacheInvalidation.recoveryTime =
        ...
        expect(performanceMetrics.cacheInvalidation.recoveryTime).toBeLessThan(
          50,
        ); // 恢复时间 < 50ms
      } catch (error) {
        console.log(
          "⚠️ 缓存失效测试需要 Change Stream 支持，跳过:",
          error.message,
        );
    ...
    it("should handle massive cache invalidation scenarios", async () => {
      ...
      console.log(
        `大规模失效前缓存条目: L1=${beforeMassiveInvalidation.cacheSize.l1}, L2=${beforeMassiveInvalidation.cacheSize.l2}, L3=${beforeMassiveInvalidation.cacheSize.l3}`,
      );
      ...
      try {
        // 模拟删除部分映射规则
        await repository.removeSymbolMappingRule(
          testDataSourceName,
          "00001.HK",
        );
        await repository.removeSymbolMappingRule(
        ...
        const massiveInvalidationTime =
          massiveInvalidationEnd - massiveInvalidationStart;

        // 验证系统在大规模失效后仍能正常工作
        const postInvalidationQueries = [];
        for (let i = 0; i < 10; i++) {
          postInvalidationQueries.push(
            cacheService.mapSymbols(
              testDataSourceName,
              massiveSymbolBatches[i],
              "to_standard",
            ),
          );
        }

        const postInvalidationResults = await Promise.all(
          postInvalidationQueries,
        );
        const allSuccessful = postInvalidationResults.every(
          (result) => result.success,
        );

        console.log(`✅ 大规模缓存失效测试完成:`);
        console.log(
          `   - 失效处理时间: ${massiveInvalidationTime.toFixed(2)}ms`,
        );
        ...
        console.log(
          "⚠️ 大规模缓存失效测试需要 Change Stream 支持，跳过:",
          error.message,
        );
      }
    });
  ...
  describe("🎯 Cache Hit Ratio Optimization", () => {
    it("should achieve high cache hit ratios under realistic workloads", async () => {
      ...
      console.log(
        `   - L1命中率: ${(finalStats.l1HitRatio * 100).toFixed(1)}%`,
      );
      console.log(
        `   - L2命中率: ${(finalStats.l2HitRatio * 100).toFixed(1)}%`,
      );
      console.log(
        `   - L3命中率: ${(finalStats.l3HitRatio * 100).toFixed(1)}%`,
      );
      ...
      // 计算综合缓存效率
      const overallHitRatio = Math.max(
        finalStats.l1HitRatio,
        finalStats.l2HitRatio,
        finalStats.l3HitRatio,
      );
