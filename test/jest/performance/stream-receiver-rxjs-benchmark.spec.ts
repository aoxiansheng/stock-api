...
describe("StreamReceiver RxJS Batch Processing - Performance Benchmarks", () => {
  ...
  beforeAll(async () => {
    ...
    const batchEnabledModule: TestingModule = await Test.createTestingModule({
      providers: [
        StreamReceiverService,
        {
          ...
          useValue: mocks.mockCapabilityRegistry,
        },
        {
          provide: SymbolMapperService,
          useValue: mocks.mockSymbolMapperService,
        },
        {
          provide: FlexibleMappingRuleService,
          useValue: mocks.mockFlexibleMappingRuleService,
        },
        {
          provide: DataTransformerService,
          useValue: mocks.mockTransformerService,
        },
        {
        ...
        {
          provide: MockStreamPerformanceMetrics,
          useValue: mocks.mockPerformanceMetrics,
        },
      ],
    }).compile();
    ...
    batchEnabledService = batchEnabledModule.get<StreamReceiverService>(
      StreamReceiverService,
    );
    batchDisabledService = batchDisabledModule.get<StreamReceiverService>(
      StreamReceiverService,
    );
    performanceMetrics = batchEnabledModule.get<MockStreamPerformanceMetrics>(
      MockStreamPerformanceMetrics,
    );
  ...
  describe("小批量处理性能测试 (10条数据)", () => {
    it("应该在50ms内完成10条报价数据的批量处理", async () => {
      const quotes = generateQuoteData(
        PERFORMANCE_TEST_CONFIG.SMALL_BATCH_SIZE,
      );
      ...
      expect(processingTime).toBeLessThan(
        PERFORMANCE_TEST_CONFIG.PERFORMANCE_THRESHOLD_MS.SMALL,
      );

      console.log(
        `小批量处理 (${PERFORMANCE_TEST_CONFIG.SMALL_BATCH_SIZE}条): ${processingTime.toFixed(2)}ms`,
      );
    }, 10000);
    ...
    it("批量模式应该比传统模式快至少20%", async () => {
      ...
      for (const quote of quotes) {
        await (batchDisabledService as any).processAndCacheProviderData(
          quote.rawData,
          quote.providerName,
          quote.wsCapabilityType,
        );
      }
      const traditionalProcessingTime =
        performance.now() - traditionalStartTime;

      const improvementRatio =
        (traditionalProcessingTime - batchProcessingTime) /
        traditionalProcessingTime;

      expect(improvementRatio).toBeGreaterThan(0.2); // 至少20%提升

      console.log(
        `性能提升: ${(improvementRatio * 100).toFixed(1)}% (批量: ${batchProcessingTime.toFixed(2)}ms vs 传统: ${traditionalProcessingTime.toFixed(2)}ms)`,
      );
    }, 15000);
  });
  ...
  describe("中批量处理性能测试 (100条数据)", () => {
    it("应该在200ms内完成100条报价数据的批量处理", async () => {
      const quotes = generateQuoteData(
        PERFORMANCE_TEST_CONFIG.MEDIUM_BATCH_SIZE,
      );
      ...
      expect(processingTime).toBeLessThan(
        PERFORMANCE_TEST_CONFIG.PERFORMANCE_THRESHOLD_MS.MEDIUM,
      );

      console.log(
        `中批量处理 (${PERFORMANCE_TEST_CONFIG.MEDIUM_BATCH_SIZE}条): ${processingTime.toFixed(2)}ms`,
      );
    }, 15000);
    ...
    it("平均每条数据处理时间应少于2ms", async () => {
      ...
      const avgTimePerQuote =
        processingTime / PERFORMANCE_TEST_CONFIG.MEDIUM_BATCH_SIZE;
  ...
  describe("大批量处理性能测试 (1000条数据)", () => {
    it("应该在1s内完成1000条报价数据的批量处理", async () => {
      const quotes = generateQuoteData(
        PERFORMANCE_TEST_CONFIG.LARGE_BATCH_SIZE,
      );
      ...
      expect(processingTime).toBeLessThan(
        PERFORMANCE_TEST_CONFIG.PERFORMANCE_THRESHOLD_MS.LARGE,
      );

      console.log(
        `大批量处理 (${PERFORMANCE_TEST_CONFIG.LARGE_BATCH_SIZE}条): ${processingTime.toFixed(2)}ms`,
      );
    }, 20000);
    ...
    it("吞吐量应达到每秒1000+条数据", async () => {
      ...
      const throughputPerSecond =
        (PERFORMANCE_TEST_CONFIG.LARGE_BATCH_SIZE / processingTime) * 1000;
  ...
  describe("内存使用效率测试", () => {
    it("批量处理不应导致显著的内存泄漏", async () => {
      ...
      console.log(
        `内存使用变化: +${(memoryIncrease / 1024 / 1024).toFixed(2)}MB (${(memoryIncreaseRatio * 100).toFixed(1)}%)`,
      );
    }, 30000);
  });
  ...
  describe("并发性能测试", () => {
    it("应该能够处理并发的批量请求", async () => {
      ...
      console.log(
        `并发处理 ${concurrentBatches}个批次, 共${totalQuotes}条: ${processingTime.toFixed(2)}ms`,
      );
