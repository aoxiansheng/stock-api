...
describe('Cross Module Collaboration Integration', () => {
  ...
  beforeAll(async () => {
    ...
    performanceMetricsRepository = app.get<PerformanceMetricsRepository>(PerformanceMetricsRepository);
  });
  ...
  describe('Security-Alert-Cache 三模块协作', () => {
    it('应该在检测到安全威胁时触发告警并缓存', async () => {
      ...
      const threatEvent = events.find(e => e.type === SecurityEventType.THREAT_DETECTED);
      const authEvent = events.find(e => e.type === SecurityEventType.AUTHENTICATION_FAILURE);
      ...
      // 验证IP分析数据被更新
      const ipAnalysis = await securityAuditService.getIPAnalysis(threatData.ipAddress);
    ...
    it('应该在缓存性能异常时触发告警和安全审计', async () => {
      ...
      // 验证缓冲区中包含性能异常事件
      const events = cachedEvents.map(eventStr => JSON.parse(eventStr));
      const performanceEvents = events.filter(e => e.type === SecurityEventType.PERFORMANCE_ANOMALY);
      ...
      const actualDurations = performanceEvents.map(event => event.details.duration);
  ...
  describe('Metrics-Alert-Security 三模块协作', () => {
    it('应该基于性能指标触发安全告警', async () => {
      ...
      // 模拟1分钟内120次请求
      for (let i = 0; i < 120; i++) {
        await performanceMonitorService.recordRequest(endpoint, method, 50, true);
    ...
    it('应该在数据库性能异常时触发综合告警', async () => {
      ...
      // Assert - 验证数据库指标被正确计算
      const databaseMetrics = await performanceMonitorService.getDatabaseMetrics();
      ...
      // 验证数据库查询时间被记录到正确的键
      const cachedQueryTimes = await performanceMetricsRepository.getDatabaseQueryTimes();
      expect(cachedQueryTimes.length).toBeGreaterThan(0);

      // 验证记录的查询时间包含我们测试的慢查询
      const queryTimes = cachedQueryTimes.map(timeStr => parseInt(timeStr, 10));
      ...
      // 验证缓冲区中包含系统错误事件
      const events = cachedEvents.map(eventStr => JSON.parse(eventStr));
      const systemErrorEvents = events.filter(e => e.type === SecurityEventType.SYSTEM_ERROR);
      expect(systemErrorEvents.length).toBe(4);

      // 验证事件详情
      const expectedOperations = slowQueries.map(query => query.operation);
      const actualOperations = systemErrorEvents.map(event => event.details.operation);
      const expectedEventDurations = slowQueries.map(query => query.duration);
      const actualEventDurations = systemErrorEvents.map(event => event.details.duration);
  ...
  describe('四模块完整协作场景', () => {
    ...
    it('应该处理缓存故障情况下的多模块协作', async () => {
      ...
      // 1. 正常操作
      await performanceMonitorService.recordRequest('/api/v1/test', 'GET', 100, true);
      await securityAuditService.logSecurityEvent({
        type: SecurityEventType.SYSTEM_ERROR,
        severity: 'low',
      ...
      // 3. 故障期间的操作（应该降级处理）
      for (let i = 0; i < 5; i++) {
        await performanceMonitorService.recordRequest('/api/v1/degraded', 'GET', 200, true);
        await securityAuditService.logSecurityEvent({
          type: SecurityEventType.SYSTEM_ERROR,
          severity: 'medium',
      ...
      // 4. 缓存恢复后的操作
      await performanceMonitorService.recordRequest('/api/v1/recovered', 'GET', 100, true);
      await securityAuditService.logSecurityEvent({
      ...
      // 验证缓冲区中包含恢复事件
      const events = cachedEvents.map(eventStr => JSON.parse(eventStr));
      const recoveryEvents = events.filter(e => e.type === SecurityEventType.SYSTEM_RECOVERY);
  ...
  describe('模块间数据一致性测试', () => {
    it('应该保持跨模块数据的一致性', async () => {
      ...
      // 验证缓冲区中包含数据访问事件
      const events = cachedEvents.map(eventStr => JSON.parse(eventStr));
      const dataAccessEvents = events.filter(e => e.type === SecurityEventType.DATA_ACCESS);
    ...
    it('应该正确处理并发跨模块操作', async () => {
      ...
      // 验证缓冲区中包含并发API调用事件
      const events = cachedEvents.map(eventStr => JSON.parse(eventStr));
      const apiCallEvents = events.filter(e => e.type === SecurityEventType.API_CALL);
