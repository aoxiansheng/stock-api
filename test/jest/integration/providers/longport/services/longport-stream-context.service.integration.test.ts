...
describe('LongportStreamContextService Integration', () => {
  ...
  describe('Configuration Management', () => {
    ...
    it('should handle missing configuration gracefully', async () => {
      ...
      const testService = testModule.get<LongportStreamContextService>(LongportStreamContextService);

      if (!isLongPortAvailable) {
        // Mock环境下测试配置验证
        await expect(testService.initializeWebSocket()).rejects.toThrow(/配置不完整/);
      } else {
  ...
  describe('WebSocket Connection Management', () => {
    ...
    it('should handle connection initialization failure', async () => {
      if (!isLongPortAvailable) {
        ...
        await expect(service.initializeWebSocket()).rejects.toThrow(/初始化失败/);
  ...
  describe('Symbol Subscription Management', () => {
    ...
    it('should subscribe to symbols successfully', async () => {
      ...
      if (!isLongPortAvailable) {
        ...
        try {
          ...
          console.warn('真实订阅失败 - 可能需要有效的市场数据权限:', error.message);
        }
      }
    });
    ...
    it('should handle subscription limits', async () => {
      ...
      const manySymbols = Array.from({ length: 501 }, (_, i) => `${String(i).padStart(5, '0')}.HK`);

      await expect(service.subscribe(manySymbols)).rejects.toThrow(/符号数量超过限制/);
    });
  ...
  describe('Message Callback Management', () => {
    it('should add and trigger message callbacks', async () => {
      ...
      if (!isLongPortAvailable) {
        ...
        // 通过反射访问私有方法进行测试
        const handleQuoteUpdate = (service as any).handleQuoteUpdate.bind(service);
  ...
  describe('Error Handling and Recovery', () => {
    ...
    it('should parse error codes from LongPort errors', async () => {
      if (!isLongPortAvailable) {
        ...
        expect(parseErrorCode({ message: '301606: Rate limit exceeded' }))
          .toBe('301606: Rate limiting exceeded');
        
        expect(parseErrorCode({ message: '301605: Too many subscriptions' }))
          .toBe('301605: Subscription quantity exceeded');
        
        expect(parseErrorCode({ message: 'Unknown error' }))
          .toBeNull();
      }
    });
  });
