...
describe('EnhancedCapabilityRegistryService - Stream Capabilities Integration', () => {
  ...
  beforeAll(async () => {
    ...
    // 获取服务实例
    enhancedService = moduleRef.get<EnhancedCapabilityRegistryService>(EnhancedCapabilityRegistryService);
  ...
  describe('Stream Capability Registration', () => {
    it('should register stream capability correctly', async () => {
      // 执行
      // 注册流能力（模拟装饰器或手动注册）
      legacyService.registerStreamCapability('longport', mockStreamCapability, 1, true);
      
      // 验证
      const capability = legacyService.getStreamCapability('longport', 'stream-stock-quote');
    ...
    it('should handle multiple stream capabilities for same provider', async () => {
      ...
      // 验证
      expect(legacyService.getStreamCapability('longport', 'stream-stock-quote')).toBe(capability1);
      expect(legacyService.getStreamCapability('longport', 'stream-index-quote')).toBe(capability2);
    });
  });
  ...
  describe('Stream Capability Discovery', () => {
    it('should find best provider for a stream capability', async () => {
      // 设置
      legacyService.registerStreamCapability('longport', mockStreamCapability, 2, true);
      legacyService.registerStreamCapability('longport-sg', { ...mockStreamCapability }, 1, true);
      
      // 执行
      const bestProvider = legacyService.getBestStreamProvider('stream-stock-quote');
    ...
    it('should filter providers by market support', async () => {
      ...
      legacyService.registerStreamCapability('hk-provider', hkCapability, 1, true);
      legacyService.registerStreamCapability('us-provider', usCapability, 2, true);
      
      // 执行
      const hkProvider = legacyService.getBestStreamProvider('stream-stock-quote', MARKETS.HK);
      const usProvider = legacyService.getBestStreamProvider('stream-stock-quote', MARKETS.US);
  ...
  describe('Stream Capability Status Management', () => {
    it('should update and track stream capability status', async () => {
      ...
      // 执行 - 连接成功
      legacyService.updateStreamCapabilityStatus('longport', 'stream-stock-quote', 'connected');
    ...
    it('should track error counts and reset on successful connection', async () => {
      ...
      // 执行 - 发生错误
      legacyService.updateStreamCapabilityStatus('longport', 'stream-stock-quote', 'error');
      legacyService.updateStreamCapabilityStatus('longport', 'stream-stock-quote', 'error');
      
      // 验证
      let capabilities = legacyService.getAllStreamCapabilities();
      let registration = capabilities.get('longport')?.get('stream-stock-quote');
