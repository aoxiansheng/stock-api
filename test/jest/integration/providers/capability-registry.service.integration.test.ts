...
describe('CapabilityRegistryService - Integration', () => {
  ...
  const createMockCapabilityFile = (capabilityName: string, capability: ICapability): string => {
    return `
...
export default {
  ...
  describe('Manual Capability Registration', () => {
    ...
    it('should register multiple capabilities for same provider', () => {
      ...
      expect(service.getCapability('test-provider', 'get-stock-quote')).toBeTruthy();
      expect(service.getCapability('test-provider', 'get-market-data')).toBeTruthy();
    });
    ...
    it('should register capabilities for multiple providers', () => {
      ...
      expect(service.getCapability('provider-a', 'get-stock-quote')).toBeTruthy();
      expect(service.getCapability('provider-b', 'get-stock-quote')).toBeTruthy();
    });
  });
  ...
  describe('Best Provider Selection', () => {
    beforeEach(() => {
      ...
      service.registerCapability('low-priority-provider', mockCapability1, 5, true);
      service.registerCapability('high-priority-provider', mockCapability1, 1, true);
      service.registerCapability('medium-priority-provider', mockCapability1, 3, true);
    });
    ...
    it('should filter by market when specified', () => {
      ...
      expect(service.getBestProvider('get-stock-quote', 'CN')).toBe('cn-only-provider');

      // Should return US provider for US market (CN provider doesn't support US)
      expect(service.getBestProvider('get-stock-quote', 'US')).toBe('high-priority-provider');
    });
    ...
    it('should return null when no providers support the specified market', () => {
      const bestProvider = service.getBestProvider('get-stock-quote', 'UNSUPPORTED_MARKET');
      expect(bestProvider).toBeNull();
    });

    it('should ignore disabled providers', () => {
      // Disable the high-priority provider
      service.registerCapability('high-priority-provider', mockCapability1, 1, false);
  ...
  describe('Capability Retrieval', () => {
    beforeEach(() => {
      service.registerCapability('test-provider', mockCapability1, 1, true);
      service.registerCapability('disabled-provider', mockCapability2, 1, false);
    });

    it('should return null for non-existent provider', () => {
      const capability = service.getCapability('non-existent', 'get-stock-quote');
    ...
    it('should return null for disabled capability', () => {
      const capability = service.getCapability('disabled-provider', 'get-market-data');
      expect(capability).toBeNull();
    });

    it('should return capability for valid provider and capability name', () => {
      const capability = service.getCapability('test-provider', 'get-stock-quote');
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle capabilities with duplicate names in same provider', () => {
      ...
      const updatedCapability = { ...mockCapability1, description: 'Updated description' };
      service.registerCapability('test-provider', updatedCapability, 2, true);

      // Should overwrite the previous capability
      const retrieved = service.getCapability('test-provider', 'get-stock-quote');
    ...
    it('should handle very large number of capabilities', () => {
      ...
      const specificCapability = service.getCapability('bulk-provider', 'capability-500');
    ...
    it('should handle special characters in provider and capability names', () => {
      ...
      service.registerCapability('provider@123', specialCharCapability, 1, true);

      const retrieved = service.getCapability('provider@123', 'get-data.with-special@chars#123');
    ...
    it('should handle market filtering edge cases', () => {
      ...
      service.registerCapability('no-markets-provider', noMarketsCapability, 1, true);
      ...
      expect(service.getBestProvider('no-markets-capability')).toBe('no-markets-provider');
    });
  });
