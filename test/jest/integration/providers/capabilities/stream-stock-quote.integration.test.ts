...
describe("StreamStockQuote Capability Integration", () => {
  ...
  beforeAll(async () => {
    ...
    enhancedRegistryService = module.get<EnhancedCapabilityRegistryService>(
      EnhancedCapabilityRegistryService,
    );
    longportStreamService = module.get<LongportStreamContextService>(
  ...
  describe("订阅和数据流测试", () => {
    ...
    it("应该能够正确处理符号数量超过限制的情况", async () => {
      // 创建超过500个符号的数组
      const symbols = Array(510)
        .fill("")
        .map((_, i) => `${i}.HK`);

      // 应该抛出错误
      await expect(
        streamCapability.subscribe(symbols, longportStreamService),
      ).rejects.toThrow("符号数量超过LongPort限制");
    });
  ...
  describe("错误处理测试", () => {
    it("应该处理初始化错误", async () => {
      ...
      longportStreamService.initializeWebSocket = jest
        .fn()
        .mockRejectedValue(new Error("初始化失败"));

      await expect(
        streamCapability.initialize(longportStreamService),
      ).rejects.toThrow("LongPort WebSocket 流初始化失败");
    ...
    it("应该处理订阅错误", async () => {
      ...
      longportStreamService.subscribe = jest
        .fn()
    ...
    it("应该处理取消订阅错误", async () => {
      ...
      longportStreamService.unsubscribe = jest
        .fn()
