...
describe('Security Cache Integration', () => {
  ...
  describe('IP分析缓存测试', () => {
    it('应该缓存IP地址分析数据', async () => {
      ...
      // Assert - 验证IP分析数据被缓存
      const ipAnalysis = await securityAuditService.getIPAnalysis(securityEvent.clientIP);
    ...
    it('应该检测并缓存可疑IP', async () => {
      ...
      const failureEvents: Array<Omit<SecurityEvent, 'id' | 'timestamp'>> = Array.from(
        { length: 5 },
        (_, i) => ({
          type: 'authentication',
          severity: 'medium',
      ...
      // Assert - 验证IP被标记为可疑
      const isSuspicious = await securityAuditService.isIPSuspicious(suspiciousIP);
