...
import { BatchRequestHelper, BatchRequestConfig } from '../../../utils/batch-request-helper';
...
describe('Monitoring Performance Integration (Optimized)', () => {
  ...
  describe('优化的性能指标收集', () => {
    it('应该通过少量请求生成有效的性能指标', async () => {
      ...
      expect(performanceResponse[0].body.data.summary.totalRequests).toBeGreaterThanOrEqual(0);
    });
    ...
    it('应该有效验证不同端点的性能差异', async () => {
      ...
      const responses = await batchHelper.executeBatchRequests(endpointConfigs, {
      ...
      responses.forEach((response, index) => {
        ...
        console.log(`端点 ${endpointConfigs[index].endpoint} 响应状态: ${response.status}`);
      });
  ...
  describe('优化的系统资源监控', () => {
    it('应该通过最少请求收集系统资源指标', async () => {
      ...
      const responses = await batchHelper.executeBatchRequests(resourceLoadConfigs, {
        batchSize: 2,
        delayBetweenBatches: 100,
      });
  ...
  describe('优化的监控面板数据', () => {
    it('应该高效聚合监控数据', async () => {
      ...
      // 批量执行请求
      const responses = await batchHelper.executeBatchRequests(dashboardConfigs, {
        batchSize: 3,
