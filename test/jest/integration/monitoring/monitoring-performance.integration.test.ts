...
describe('Monitoring Performance Integration', () => {
  ...
  // 批量执行请求的辅助函数
  async function executeBatchRequests(requests: any[], batchSize: number = 3, delay: number = 200) {
    const results = [];
    for (let i = 0; i < requests.length; i += batchSize) {
      ...
      try {
        ...
        console.log(`批次 ${Math.floor(i/batchSize) + 1} 执行失败:`, error.message);
        // 继续执行下一批次
      }
    }
    return results;
  ...
  describe('性能指标数据生成和收集', () => {
    ...
    it('应该通过模拟请求生成性能指标数据', async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          const validatedResponse = validatePerformanceMetricsResponse(response.body);
          expect(validatedResponse.data.summary.totalRequests).toBeGreaterThanOrEqual(0);
          expect(validatedResponse.data.summary.averageResponseTime).toBeGreaterThanOrEqual(0);
        }
      } catch (error) {
    ...
    it('应该正确统计错误率', async () => {
      ...
      try {
        ...
        await executeBatchRequests(errorRequests.map(req => req.catch(() => {})), 1, 300);
      } catch (error) {
      ...
      try {
        ...
        if (response.status === 200) {
          const validatedResponse = validatePerformanceMetricsResponse(response.body);
          expect(validatedResponse.data.summary.errorRate).toBeGreaterThanOrEqual(0);
          expect(validatedResponse.data.summary.totalRequests).toBeGreaterThanOrEqual(0);
        }
      } catch (error) {
    ...
    it('应该正确监控内存使用情况', async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          expect(validatedResponse.data.memoryUsage).toBeGreaterThan(0);
          expect(validatedResponse.data.heapUsed).toBeGreaterThan(0);
          expect(validatedResponse.data.heapTotal).toBeGreaterThan(validatedResponse.data.heapUsed);
        }
      } catch (error) {
    ...
    it('应该生成性能优化建议', async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          expect(Array.isArray(response.body.data.recommendations)).toBeTruthy();
