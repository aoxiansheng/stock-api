...
describe('Monitoring Auth Integration', () => {
  ...
  describe('监控接口访问权限测试', () => {
    ...
    it('管理员也应该能访问公共监控接口', async () => {
      ...
      if (!apiKey || !apiSecret) {
        console.warn('测试API Key不可用，跳过测试', { apiKey: !!apiKey, apiSecret: !!apiSecret });
        return;
      }
    ...
    it('开发者用户也应该能访问公共监控接口', async () => {
      ...
      if (response.status === 201) {
        ...
        for (const endpoint of publicEndpoints) {
          console.log(`Testing public endpoint with developer auth: ${endpoint}`);
          const response = await request(httpServer)
            .get(endpoint)
  ...
  describe('认证中间件集成测试', () => {
    it('应该正确处理认证中间件链', async () => {
      // Arrange - 设置认证中间件链测试
      const testCases = [
        { headers: { 'Authorization': `Bearer ${adminToken}` }, expectedStatus: 200 },
  ...
  afterAll(async () => {
    try {
      // 清理测试数据
      if (testDataPrefix) {
        await userModel.deleteMany({ username: { $regex: `^${testDataPrefix}_` } });
        await apiKeyModel.deleteMany({ name: { $regex: `^${testDataPrefix}_` } });
      }
