...
describe("Monitoring Database Integration", () => {
  ...
  async function setupTestAuth() {
    ...
    await authService.createApiKey(testUser.id, {
  ...
  describe("数据库指标数据生成", () => {
    ...
    it("应该正确统计数据库连接池指标", async () => {
      ...
      for (let i = 0; i < 8; i++) {
        concurrentQueries.push(userModel.find({ role: UserRole.ADMIN }));
      }
      ...
      const validatedResponse = validateDatabaseMetricsResponse(response.body);
      expect(validatedResponse.data.connectionPoolSize).toBeGreaterThanOrEqual(
        0,
      );
      expect(validatedResponse.data.activeConnections).toBeGreaterThanOrEqual(
        0,
      );
      expect(validatedResponse.data.waitingConnections).toBeGreaterThanOrEqual(
        0,
      );
    });
  ...
  describe("数据库性能分析", () => {
    ...
    it("应该正确处理数据库连接异常", async () => {
      ...
      const queries = [];
      for (let i = 0; i < 10; i++) {
        queries.push(userModel.find({ role: UserRole.ADMIN }));
      }
  ...
  describe("数据库监控集成", () => {
    it("应该正确集成到整体性能监控", async () => {
      ...
      expect(
        performanceResponse.body.data.database.totalQueries,
    ...
    it("应该在监控面板中显示数据库指标", async () => {
      ...
      const operations = [];
      for (let i = 0; i < 7; i++) {
        operations.push(userModel.find({ role: UserRole.ADMIN }));
      }
      ...
      expect(
        dashboardResponse.body.data.performance.database.totalQueries,
      ).toBeGreaterThanOrEqual(0);
    });
  });
  ...
  describe("数据库告警和阈值", () => {
    it("应该生成数据库优化建议", async () => {
      ...
      const queries = [];
      for (let i = 0; i < 12; i++) {
        queries.push(userModel.find({ role: UserRole.DEVELOPER }));
      }
  ...
  describe("数据库监控边界情况", () => {
    ...
    it("应该处理高并发数据库访问", async () => {
      ...
      for (let i = 0; i < 20; i++) {
        concurrentOperations.push(userModel.find({ role: UserRole.ADMIN }));
      }
  ...
  afterAll(async () => {
    try {
      ...
      await userModel.deleteMany({
        username: {
          $regex:
            /^(db_admin|db_test_user_|write_test_user_|integration_user_)/,
        },
      });
