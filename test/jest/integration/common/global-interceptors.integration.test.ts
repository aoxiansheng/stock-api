...
describe('Global Interceptors Integration', () => {
  ...
  describe('协作测试 - 拦截器和异常过滤器配合', () => {
    it('应该确保异常响应也符合标准格式', async () => {
      ...
      expect(response.body.error.details).toHaveProperty('path', '/test-global-interceptors/bad-request');
  ...
  describe('边界条件测试', () => {
    ...
    it('应该处理不支持的 HTTP 方法', async () => {
      const response = await request(httpServer).patch('/test-global-interceptors/bad-request').expect(404);
  ...
  describe('性能测试', () => {
    it.skip('应该在合理时间内处理大量并发请求', async () => {
      ...
      console.log(`并发测试完成: ${concurrentRequests} 个请求耗时 ${duration}ms`);
    });
  });
});