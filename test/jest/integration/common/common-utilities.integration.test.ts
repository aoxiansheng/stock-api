...
describe('common Utilities Integration', () => {
  ...
  describe('Market 常量集成测试', () => {
    ...
    it('应该在实际 API 响应中正确使用市场常量', async () => {
      ...
      expect(response.body).toMatchObject({
        ...
        data: {
          markets: expect.arrayContaining(['HK', 'US', 'SZ', 'SH', 'CN', 'CRYPTO']),
  ...
  describe('性能测试', () => {
    it.skip ('Header 处理应该在大量并发下保持性能', async () => {
      ...
      console.log(`并发 Header 处理测试完成: ${concurrentRequests} 个请求耗时 ${duration}ms`);
    });
    ...
    it('市场常量访问应该高效', () => {
      ...
      console.log(`市场常量性能测试完成: ${iterations} 次访问耗时 ${duration}ms`);
    });
  });
