...
describe('SymbolValidationUtils Integration', () => {
  describe('Market Recognition Integration', () => {
    it('should correctly identify Hong Kong market symbols', () => {
      ...
      hkSymbols.forEach(symbol => {
        ...
        expect(SymbolValidationUtils.getMarketFromSymbol(symbol)).toBe(Market.HK);
      });
    });
    ...
    it('should correctly identify US market symbols', () => {
      ...
      usSymbols.forEach(symbol => {
        ...
        expect(SymbolValidationUtils.getMarketFromSymbol(symbol)).toBe(Market.US);
      });
    });
    ...
    it('should correctly identify Shenzhen market symbols', () => {
      ...
      szSymbols.forEach(symbol => {
        ...
        expect(SymbolValidationUtils.getMarketFromSymbol(symbol)).toBe(Market.SZ);
      });
    });
    ...
    it('should correctly identify Shanghai market symbols', () => {
      ...
      shSymbols.forEach(symbol => {
        ...
        expect(SymbolValidationUtils.getMarketFromSymbol(symbol)).toBe(Market.SH);
      });
    });
  });
  ...
  describe('Symbol Format Validation Integration', () => {
    ...
    it('should handle bulk symbol validation', () => {
      ...
      expect(result.valid).toEqual(['700.HK', 'AAPL.US', '000001.SZ', '600000.SH']);
      expect(result.invalid).toEqual(['INVALID', '', '123.UNKNOWN']);
    });

    it('should validate symbol count limits', () => {
      const symbols = Array.from({ length: 10 }, (_, i) => `${String(i).padStart(5, '0')}.HK`);
      
      expect(SymbolValidationUtils.isSymbolCountExceeded(symbols, 15)).toBe(false);
      expect(SymbolValidationUtils.isSymbolCountExceeded(symbols, 5)).toBe(true);
  ...
  describe('Edge Cases and Error Handling', () => {
    it('should handle null and undefined symbols', () => {
      ...
      expect(SymbolValidationUtils.getMarketFromSymbol(null as any)).toBeUndefined(); // 应该返回undefined
      expect(SymbolValidationUtils.getMarketFromSymbol(undefined as any)).toBeUndefined(); // 应该返回undefined
    });
  ...
  describe('Performance and Scale Testing', () => {
    it('should handle large batches of symbols efficiently', () => {
      ...
      const largeSymbolSet = [
        ...Array.from({ length: 100 }, (_, i) => `${String(i).padStart(5, '0')}.HK`),
        ...Array.from({ length: 100 }, (_, i) => `${String.fromCharCode(65 + i % 26).repeat(4)}.US`),
        ...Array.from({ length: 100 }, (_, i) => `000${String(i).padStart(3, '0')}.SZ`),
        ...Array.from({ length: 100 }, (_, i) => `600${String(i).padStart(3, '0')}.SH`),
      ];
