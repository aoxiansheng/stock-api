  ...
  describe('Gateway准备度评估', () => {
    test('应该评估Legacy移除的整体准备度', () => {
      ...
      const assessment = {
        ...
        overallReadiness: 'unknown' as 'ready' | 'not_ready' | 'partial' | 'unknown'
      };
      
      // 计算整体准备度
      if (readinessCheck.ready && healthStatus.status === 'healthy' && stats.isAvailable) {
...
export class GatewayHealthChecker {
  static async performComprehensiveCheck(provider: WebSocketServerProvider): Promise<{
