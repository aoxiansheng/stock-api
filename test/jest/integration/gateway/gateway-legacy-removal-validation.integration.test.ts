      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            validateApiKey: jest
              .fn()
              .mockResolvedValue({ isValid: true, apiKey: { name: "test" } }),
          },
        },
      ],
    }).compile();
  ...
  describe("Gateway健康状态验证", () => {
    ...
    test("应该通过Legacy移除准备检查", () => {
      ...
      if (!readinessCheck.ready) {
        ...
        expect(
          readinessCheck.details.gatewayValidation.validationTime,
        ).toBeDefined();
      }
    });
  });
    ...
    test("应该能够通过Gateway广播消息到房间", async () => {
      ...
      await new Promise<void>((resolve) => {
        clientSocket.on("data", (receivedData) => {
          ...
          expect(receivedData.price).toBe(100.5);
          resolve();
        });
      });
    });
  ...
  describe("Legacy移除准备状态", () => {
    test("应该确认Gateway-Provider集成完成", () => {
      ...
      if (readinessCheck.ready) {
        ...
        console.log(
          "Gateway验证详情:",
          readinessCheck.details.gatewayValidation,
        );
      } else {
        // Gateway尚未准备好，需要先修复
...
class GatewayValidationHelper {
  static async validateGatewayReadiness(
