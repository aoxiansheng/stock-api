...
describe('Auth Database Integration', () => {
  ...
  describe('User Registration and Login Flow', () => {
    ...
    it('应该成功验证JWT令牌', async () => {
      ...
      // Act
      const validatedUser: any = await tokenService.validateUserFromPayload(jwtPayload);
    ...
    it('密码应该被正确加密存储', async () => {
      ...
      // 验证密码哈希是否正确
      const isPasswordValid = await bcrypt.compare('securepassword123', dbUser.passwordHash);
      expect(isPasswordValid).toBe(true);
      
      // 验证错误密码
      const isWrongPasswordValid = await bcrypt.compare('wrongpassword', dbUser.passwordHash);
  ...
  describe('API Key Management Flow', () => {
    ...
    it('应该异步更新API Key使用统计', async () => {
      ...
      do {
        ...
        attempts++;
      } while (updatedApiKey.usageCount < initialUsageCount + 2 && attempts < maxAttempts);
  ...
  describe('Rate Limiting Integration', () => {
    ...
    beforeAll(async () => {
      ...
      const createdApiKey: any = await authService.createApiKey(user.id, apiKeyData);
    ...
    it('应该正确获取当前使用统计', async () => {
      // Act
      const usage = await rateLimitService.getCurrentUsage(testApiKey, RateLimitStrategy.FIXED_WINDOW);
    ...
    it('应该成功重置限流计数器', async () => {
      // Act
      await rateLimitService.resetRateLimit(testApiKey, RateLimitStrategy.FIXED_WINDOW);
    ...
    it('应该正确计算使用统计', async () => {
      ...
      // Act
      const stats = await rateLimitService.getCurrentUsage(testApiKey, RateLimitStrategy.FIXED_WINDOW);
  ...
  describe('Database Performance and Indexes', () => {
    it('应该快速查找用户（用户名索引）', async () => {
      ...
      await tokenService.validateUserFromPayload({
    ...
    it('应该快速查找API Key（复合索引）', async () => {
      ...
      await authService.validateApiKey('nonexistent-key', 'nonexistent-token')
        .catch(() => {}); // 忽略错误，只测试性能
  ...
  describe('Error Scenarios', () => {
    it('应该在重复用户名时抛出冲突异常', async () => {
      ...
      // Assert
      await expect(authService.register(userData2)).rejects.toThrow('用户名或邮箱已存在');
    });
