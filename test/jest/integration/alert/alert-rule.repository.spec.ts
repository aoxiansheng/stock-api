...
describe('AlertRuleRepository (Integration)', () => {
  ...
  beforeAll(async () => {
    ...
    alertRuleModel = testModule.get<Model<AlertRuleDocument>>(getModelToken(AlertRule.name));
  });
  ...
  const createTestRule = (override: Partial<CreateAlertRuleDto> = {}): CreateAlertRuleDto & { id: string } => ({
    id: uuidv4(),
    name: 'Test Rule',
    metric: 'cpu.usage',
    operator: 'gt',
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException when updating a non-existent rule', async () => {
      ...
      await expect(repository.update('non-existent-id', updateDto)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('findAll and findAllEnabled', () => {
    it('should find all rules correctly', async () => {
      await repository.create(createTestRule({ name: 'Rule 1', enabled: true }));
      await repository.create(createTestRule({ name: 'Rule 2', enabled: false }));
