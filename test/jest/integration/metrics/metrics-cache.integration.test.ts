...
describe('Metrics Cache Integration', () => {
  ...
  describe('性能指标缓存策略测试', () => {
    it('应该在Redis可用时正确缓存性能指标', async () => {
      ...
      if (Object.keys(statsData).length > 0) {
        ...
        expect(parseInt(statsData.successfulRequests || '0')).toBeGreaterThan(0);
        
        // 检查响应时间数据
        const responseTimeData = await cacheService.listRange(responseTimeKey, 0, -1);
    ...
    it('应该在Redis不可用时正确容错处理', async () => {
      ...
      // Act - 验证系统不会崩溃
      await expect(
        performanceMonitorService.recordRequest('/test/fault-tolerance', 'GET', 100, true)
  ...
  describe('数据库查询性能缓存测试', () => {
    it('应该缓存数据库查询性能指标', async () => {
      ...
      // Assert - 使用后端实际的缓存键格式
      const cachedMetrics = await performanceMetricsRepository.getDatabaseQueryTimes();
