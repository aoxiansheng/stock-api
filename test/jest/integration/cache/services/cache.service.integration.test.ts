...
describe("CacheService 容错机制集成测试", () => {
  ...
  describe("Redis连接断开时的容错行为", () => {
    ...
    describe("写操作异常行为", () => {
      it("通用写操作应该抛出异常", async () => {
        ...
        await expect(
          cacheService.hashSet("hash", "field", "value"),
        ).rejects.toThrow();
  ...
  describe("性能监控场景", () => {
    it("应该在指标收集场景中正确容错", async () => {
      ...
      // 写操作会抛出异常（这是正确的设计）
      await expect(
        cacheService.hashSet(endpointStatsKey, "totalRequests", "1"),
      ).rejects.toThrow();
      await expect(
        cacheService.hashSet(endpointStatsKey, "successfulRequests", "1"),
      ).rejects.toThrow();
      await expect(
        cacheService.listPush(responseTimeKey, "120"),
      ).rejects.toThrow();

      // Assert - 读取操作应该返回默认值，不抛出异常（这是容错的部分）
      const stats = await cacheService.hashGetAll(endpointStatsKey);
      const responseTimes = await cacheService.listRange(
        responseTimeKey,
        0,
        -1,
      );
