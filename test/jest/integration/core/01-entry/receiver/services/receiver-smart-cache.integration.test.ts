...
describe("Receiver Smart Cache Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    if (!app) {
      throw new Error(
        "集成测试环境未初始化。请确保test/config/integration.setup.ts正确配置。",
      );
    }
  ...
  describe("强时效性缓存策略验证", () => {
    it("应该为美股实时数据使用Receiver API端点", async () => {
      ...
      console.log(
        `✅ 美股强时效策略: AAPL (${responseTime}ms, HTTP状态: ${response.status})`,
      );
    });
    ...
    it("应该在重复请求时验证缓存行为", async () => {
      ...
      console.log(
        `🔄 缓存行为验证: MSFT (第一次: ${responseTime1}ms, 第二次: ${responseTime2}ms, 改善比: ${improvementRatio.toFixed(1)}x)`,
      );
    });
    ...
    it("应该验证批量请求的缓存策略", async () => {
      ...
      console.log(
        `🎯 批量缓存策略: ${requestPayload.symbols.join(", ")} (${responseTime}ms)`,
      );
    });
  });
  ...
  describe("SDK调用频率控制验证", () => {
    it("应该通过连续请求测试缓存效果", async () => {
      ...
      const avgSubsequentTime =
        results.slice(1).reduce((sum, r) => sum + r.responseTime, 0) / 4;
      const improvementRatio = firstRequestTime / avgSubsequentTime;

      console.log(
        `📊 SDK调用频率控制: ${symbol} (首次: ${firstRequestTime}ms, 后续平均: ${avgSubsequentTime.toFixed(1)}ms, 改善: ${improvementRatio.toFixed(1)}x)`,
      );
    });
    ...
    it("应该验证不同符号的独立缓存管理", async () => {
      ...
      console.log(
        `🎯 独立缓存管理: ${symbols.length}个符号，平均响应时间: ${(results.reduce((sum, r) => sum + r.responseTime, 0) / results.length).toFixed(1)}ms`,
      );
    });
  });
  ...
  describe("实时数据新鲜度保证", () => {
    ...
    it("应该验证市场状态对缓存策略的影响", async () => {
      ...
      for (const { symbol, description } of testCases) {
        ...
        console.log(
          `📈 ${description}市场状态测试: ${symbol} (${responseTime}ms, HTTP状态: ${response.status})`,
        );
      }
    });
  });
  ...
  describe("性能和监控指标验证", () => {
    it("应该验证监控指标服务可用性", async () => {
      ...
      try {
        ...
        console.log(
          `📊 监控指标验证: receiverRequestsTotal = N/A (metricsRegistry未初始化)`,
        );
      } catch (error) {
        console.log(
          `📊 监控指标验证: 指标查询失败 (${error.message})，但服务正常注入`,
        );
      }
    });
    ...
    it("应该记录详细的性能指标", async () => {
      ...
      console.log(
        `📊 性能指标记录: INTC (${responseTime}ms, HTTP状态: ${response.status})`,
      );
    });
  });
