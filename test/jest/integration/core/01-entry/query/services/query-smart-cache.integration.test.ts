...
describe("Query Smart Cache Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    smartCacheOrchestrator = moduleFixture.get<SmartCacheOrchestrator>(
    ...
    marketStatusService =
      moduleFixture.get<MarketStatusService>(MarketStatusService);
    metricsRegistry = moduleFixture.get<MetricsRegistryService>(
  ...
  describe("智能缓存命中率验证", () => {
    it("应该在缓存命中时提供快速响应", async () => {
      ...
      (marketStatusService.getBatchMarketStatus as jest.Mock).mockResolvedValue(
        mockMarketStatus,
      );
    ...
    it("应该在多个符号请求中混合命中和未命中", async () => {
      ...
      console.log(
        `📊 Mixed Cache Test: ${hitResults.length} hits (avg: ${avgHitTime}ms), ${missResults.length} misses (avg: ${avgMissTime}ms)`,
      );
    });
  });
  ...
  describe("后台更新机制验证", () => {
    it("应该在缓存即将过期时调度后台更新", async () => {
      ...
      (
        smartCacheOrchestrator.scheduleBackgroundUpdate as jest.Mock
      ...
      expect(result).toBeDefined();
      expect(
        smartCacheOrchestrator.scheduleBackgroundUpdate,
    ...
    it("应该实现TTL节流机制避免频繁更新", async () => {
      ...
      console.log(
        `⏱️ TTL Throttling Test: Background update called once despite 5 rapid requests`,
      );
    });
  });
      ...
      for (const testCase of testCases) {
        const mockMarketStatus = {
          [testCase.market]: {
            ...
            timezone: "UTC",
            realtimeCacheTTL:
              testCase.status === MarketStatus.TRADING ? 30 : 300,
        ...
        (
          marketStatusService.getBatchMarketStatus as jest.Mock
        ...
        expect(result).toBeDefined();
        expect(
          smartCacheOrchestrator.getDataWithSmartCache,
        ).toHaveBeenCalledWith(
          expect.objectContaining({
            strategy: testCase.expectedStrategy,
          }),
        ...
        console.log(
          `📈 Strategy Test: ${testCase.symbol} (${testCase.status}) → ${testCase.expectedStrategy}`,
        );
      }
    });
  });
  ...
  describe("性能和错误处理验证", () => {
    it("应该在缓存服务故障时优雅降级", async () => {
      ...
      console.log(
        `🛡️ Graceful Degradation Test: Fallback to direct service successful`,
      );
    });
    ...
    it("应该记录详细的性能指标", async () => {
      ...
      console.log(
        `📊 Metrics Test: ${symbol} recorded ${responseTime}ms response time`,
      );
    });
  });
  ...
  describe("Deprecated updateCache字段向后兼容性验证", () => {
    ...
    it("应该正确处理现代化的options（不含updateCache字段）", async () => {
      ...
      console.log(
        `✅ Modern Options Test: Request processed successfully without deprecated fields`,
      );
    });
  });
  ...
  describe("综合性能报告", () => {
    afterAll(() => {
      ...
      const hitRate =
        totalRequests > 0
          ? (performanceMetrics.filter((m) => m.hit).length / totalRequests) *
            100
          : 0;
      ...
      const avgHitTime =
        hitMetrics.length > 0
          ? hitMetrics.reduce((sum, m) => sum + m.responseTime, 0) /
            hitMetrics.length
          : 0;

      const avgMissTime =
        missMetrics.length > 0
          ? missMetrics.reduce((sum, m) => sum + m.responseTime, 0) /
            missMetrics.length
          : 0;
      ...
      console.log(
        `🎯 Cache Hit Rate: ${hitRate.toFixed(1)}% (${performanceMetrics.filter((m) => m.hit).length}/${totalRequests})`,
      );
      ...
      // Strategy breakdown
      const strategyCounts = performanceMetrics.reduce(
        (acc, m) => {
