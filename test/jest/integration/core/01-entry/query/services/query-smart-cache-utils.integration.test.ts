...
import { 
  ...
  validateCacheKey
} from '../../../../../../../src/core/05-caching/smart-cache/utils/symbol-smart-cache-request.utils';
...
describe('Query Smart Cache Utils Integration Tests', () => {
  ...
  describe('缓存键构建工具验证', () => {
    ...
    it('应该支持额外参数的缓存键', () => {
      const cacheKey = buildUnifiedCacheKey(
        'receiver:stock-data', 
        ['700.HK'],
        { provider: 'longport', market: 'HK' }
      );
      
      expect(cacheKey).toBe('receiver:stock-data:700.HK:market:HK|provider:longport');
      expect(validateCacheKey(cacheKey)).toBe(true);
  ...
  describe('错误处理验证', () => {
    it('应该拒绝空的缓存键前缀', () => {
      expect(() => buildUnifiedCacheKey('', ['AAPL'])).toThrow('缓存键前缀不能为空');
      expect(() => buildUnifiedCacheKey('   ', ['AAPL'])).toThrow('缓存键前缀不能为空');
    });

    it('应该拒绝空的符号列表', () => {
      expect(() => buildUnifiedCacheKey('test', [])).toThrow('符号列表不能为空');
    ...
    it('应该处理异常符号格式', () => {
      ...
      console.log(`❓ 异常处理: '' → ${market1}, 'UNKNOWN123' → ${market2}, [] → ${market3}`);
    });
  });

  describe('性能和缓存键效率验证', () => {
    it('应该测试大量符号哈希性能', () => {
      const largeSymbolSet = Array.from({ length: 1000 }, (_, i) => `STOCK${i.toString().padStart(4, '0')}`);
      ...
      console.log(`⚡ 大量符号哈希性能: ${largeSymbolSet.length}个符号 → ${hash} (${endTime - startTime}ms)`);
    });

    it('应该测试缓存键长度优化', () => {
      const shortSymbols = ['AAPL', 'MSFT'];
      const longSymbols = Array.from({ length: 20 }, (_, i) => `LONG_SYMBOL_NAME_${i}`);
      ...
      console.log(`📏 缓存键长度: 短键=${shortKey.length}, 长键=${longKey.length}`);
