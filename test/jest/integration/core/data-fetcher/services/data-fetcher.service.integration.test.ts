...
import { DataFetcherModule } from '../../../../../../src/core/data-fetcher/module/data-fetcher.module';
...
describe('DataFetcherService Integration', () => {
  ...
  beforeAll(async () => {
    ...
    jest.spyOn(capabilityRegistryService, 'getCapability').mockReturnValue(mockCapability);
    jest.spyOn(capabilityRegistryService, 'getProvider').mockReturnValue(mockProvider);
  });
  ...
  describe('end-to-end data fetching', () => {
    ...
    it('should handle capability not found errors', async () => {
      jest.spyOn(capabilityRegistryService, 'getCapability').mockReturnValue(null);
  ...
  describe('capability support checking', () => {
    it('should correctly identify supported capabilities', async () => {
      ...
      const isSupported = await service.supportsCapability('test-provider', 'get-stock-quote');
    ...
    it('should correctly identify unsupported capabilities', async () => {
      ...
      const isSupported = await service.supportsCapability('test-provider', 'unsupported-capability');

      expect(isSupported).toBe(false);
    });

    it('should handle capability registry errors gracefully', async () => {
      jest.spyOn(capabilityRegistryService, 'getCapability').mockImplementation(() => {
  ...
  describe('provider context retrieval', () => {
    it('should successfully retrieve provider context service', async () => {
      ...
      expect(capabilityRegistryService.getProvider).toHaveBeenCalledWith('test-provider');
    ...
    it('should return undefined for providers without context service', async () => {
      ...
      jest.spyOn(capabilityRegistryService, 'getProvider').mockReturnValue(providerWithoutContext);
    ...
    it('should return undefined for nonexistent providers', async () => {
      jest.spyOn(capabilityRegistryService, 'getProvider').mockReturnValue(null);
  ...
  describe('batch processing integration', () => {
    it('should process multiple requests concurrently', async () => {
      ...
      jest.spyOn(capabilityRegistryService, 'getCapability')
  ...
  describe('performance and timing', () => {
    ...
    it('should track processing time accurately', async () => {
      ...
      expect(result.metadata.processingTime).toBeGreaterThanOrEqual(simulatedDelay - 50);
  ...
  describe('real-world scenarios', () => {
    it('should handle mixed symbol formats from different markets', async () => {
      const mockMixedData = {
        secu_quote: [
          ...
          { symbol: '00001.SZ', last_done: 12.50, market: 'SZ' },
          { symbol: '600519.SH', last_done: 1890.00, market: 'SH' },
        ],
      };
    ...
    it('should handle large symbol lists efficiently', async () => {
      const largeSymbolList = Array.from({ length: 50 }, (_, i) => `STOCK${i}.HK`);
      const mockLargeData = {
