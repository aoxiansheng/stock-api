...
describe('TTL计算功能集成测试', () => {
  ...
  describe('CommonCacheService.calculateOptimalTTL静态方法', () => {
    ...
    it('应该支持开市/闭市状态的TTL调整', () => {
      ...
      const openMarketResult = CommonCacheService.calculateOptimalTTL(512, 'hot');

      // Act - 闭市状态 - 使用大数据的冷缓存TTL
      const closedMarketResult = CommonCacheService.calculateOptimalTTL(8192, 'cold');
    ...
    it('应该支持新鲜度要求的TTL优化', () => {
      ...
      // Act - 分析需求 - 中等数据温缓存模式
      const analyticalResult = CommonCacheService.calculateOptimalTTL(2048, 'warm');
