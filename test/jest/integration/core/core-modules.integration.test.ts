...
describe('Core Modules Integration Tests', () => {
  ...
  beforeAll(() => {
    ...
    capabilityRegistryService = app.get<CapabilityRegistryService>(CapabilityRegistryService);
  });
  ...
  beforeEach(async () => {
    ...
    testApiKey = await TestDataHelper.createFullAccessApiKey(apiKeyModel, user.id, {
      rateLimit: {
        requests: 1000,
        window: '1h',
      },
  ...
  describe('🎯 Receiver Module - 系统入口点', () => {
    ...
    beforeEach(async () => {
      ...
      jest.spyOn(capabilityRegistryService, 'getCapability').mockImplementation((provider, capability) => {
        ...
        return jest.requireActual('../../../../src/providers/capability-registry.service').CapabilityRegistryService.prototype.getCapability.call(
  ...
  describe('🔀 Symbol Mapping Module', () => {
    it('should create symbol mapping configuration', async () => {
      ...
      expect(response.body.data.dataSourceName).toBe(mappingData.dataSourceName);
      expect(response.body.data.mappingRules).toHaveLength(2);

      // Verify in database
      const savedMapping = await symbolMappingModel.findById(response.body.data.id);
  ...
  describe('Data Mapping Module', () => {
    it('should create data mapping rules', async () => {
      ...
      // Verify in database
      const savedMapping = await dataMappingModel.findById(response.body.data.id);
      expect(savedMapping).toBeTruthy();
    ...
    it('should suggest field mappings', async () => {
      const suggestionRequest = {
        sourceFields: ['symbol', 'last_done', 'volume', 'turnover', 'pre_close'],
        targetFields: ['symbol', 'lastPrice', 'volume', 'turnover', 'previousClosePrice'],
      };
  ...
  describe('🔄 Cross-Module Integration - 跨模块协作', () => {
    ...
    beforeEach(async () => {
      ...
      jest.spyOn(capabilityRegistryService, 'getCapability').mockImplementation((provider, capabilityName) => {
        if (provider === uniqueDataSourceName) {
          return {
            name: capabilityName,
            ...
            execute: jest.fn().mockResolvedValue([{ symbol: 'AAPL.US', price: 151 }]), // Return some data
          } as any;
        }
        return jest.requireActual('../../../../src/providers/capability-registry.service').CapabilityRegistryService.prototype.getCapability.call(
          capabilityRegistryService, provider, capabilityName
        );
      });
      ...
      const originalFindBestMatchingRule = dataMapperService.findBestMatchingRule;
      jest.spyOn(dataMapperService, 'findBestMatchingRule').mockImplementation(async (provider: string, ruleListType: string) => {
        // 如果请求的是dataType格式，转换为正确的ruleListType
        const mappedRuleListType = TestDataHelper.mapDataTypeToRuleListType(ruleListType);
        return await originalFindBestMatchingRule.call(dataMapperService, provider, mappedRuleListType);
      });
    });
    ...
    it('应该完成完整的数据请求生命周期', async () => {
      ...
      expect(response.body.statusCode).toBe(200);
      expect(response.body.data.metadata.provider).toBe(uniqueDataSourceName);
      expect(response.body.data.data).toEqual([{ symbol: 'AAPL.US', price: 151 }]);
    });
    ...
    it('应该在Symbol Mapper和Data Mapper之间保持数据一致性', async () => {
      ...
      expect(dataTransformResponse.body.data.transformedData[0]).toHaveProperty('lastPrice', 503.0);
      expect(dataTransformResponse.body.data.transformedData[0]).toHaveProperty('volume', 1000000);
    });
  ...
  describe('🔧 Transformer Module', () => {
    ...
    it('should preview transformation without saving', async () => {
      ...
      // 验证字段映射包含我们创建的字段
      const lastPriceMapping = fieldMappings.find(m => m.sourceField === 'last_price');
  ...
  describe('Query Module', () => {
    ...
    beforeEach(async () => {
      ...
      const { DataFetchingService } = require('../../../../src/core/shared/services/data-fetching.service');
      ...
      mockFetchSingleData = jest.spyOn(dataFetchingService, 'fetchSingleData').mockResolvedValue({
      ...
      jest.spyOn(capabilityRegistryService, 'getCapability').mockImplementation((provider, capabilityName) => {
        if (provider === uniqueDataSourceName) {
          return {
            ...
            execute: jest.fn().mockResolvedValue([{ symbol: '700.HK', name: 'Tencent' }]), // Return some data
          } as any;
        }
        return jest.requireActual('../../../../src/providers/capability-registry.service').CapabilityRegistryService.prototype.getCapability.call(
          capabilityRegistryService, provider, capabilityName
    ...
    it('should execute symbol-based query', async () => {
      ...
      expect(response.body.statusCode).toBe(201);
      expect(response.body.data.data).toEqual([{ symbol: '700.HK', name: 'Tencent', lastPrice: 503.0 }]);
    });

    it('should handle bulk query execution', async () => {
      const bulkRequest = {
        queries: [{ queryType: 'by_symbols', symbols: ['700.HK'], provider: uniqueDataSourceName }],
        parallel: true,
      };
      ...
      expect(response.body.data.results[0].data).toEqual([{ symbol: '700.HK', name: 'Tencent', lastPrice: 503.0 }]);
    });
  ...
  describe('💾 Storage Module - 双存储策略', () => {
    ...
    it('应该在缓存失效时从数据库获取数据', async () => {
      ...
      try {
        ...
        if (StoredDataModel) {
          const dbRecord = await StoredDataModel.findOne({ key: 'cache-miss-test-key' });
          console.log(`📊 数据库记录检查:`, {
            found: !!dbRecord,
            key: dbRecord?.key,
      ...
      if (redis) {
        try {
          ...
          console.log(`🗑️ Redis删除结果 - 主键: ${deletedCount}, 元数据键: ${deletedMetaCount}`);
      ...
      expect(retrieveResponse.body.data.metadata.key).toBe('cache-miss-test-key');
    });
  ...
  describe('🔍 Error Handling and Edge Cases - 错误处理和边界情况', () => {
    ...
    beforeEach(async () => {
      ...
      jest.spyOn(capabilityRegistryService, 'getCapability').mockImplementation((provider, capabilityName) => {
        if (provider === uniqueDataSourceName) { // 这里只处理已知的测试provider
          return {
    ...
    it('应该在并发请求下正确处理错误隔离', async () => {
      const mixedRequests = [
        request(httpServer).post('/api/v1/receiver/data').set('X-App-Key', testApiKey.appKey).set('X-Access-Token', testApiKey.accessToken).send({ symbols: ['700.HK'], dataType: 'stock-quote', options: { preferredProvider: uniqueDataSourceName } }),
        request(httpServer).post('/api/v1/receiver/data').set('X-App-Key', testApiKey.appKey).set('X-Access-Token', testApiKey.accessToken).send({ symbols: ['INVALID-CODE'], dataType: 'stock-quote', options: { preferredProvider: uniqueDataSourceName } }),
      ];
      ...
      expect(successResponse.body.data.metadata.provider).toBe(uniqueDataSourceName);
