...
describe('SymbolTransformerService Integration Test', () => {
  ...
  beforeEach(async () => {
    ...
    symbolMapperCacheService = module.get<SymbolMapperCacheService>(SymbolMapperCacheService);
  ...
  describe('Architecture Integration', () => {
    it('should properly depend on SymbolMapperCacheService for actual transformations', async () => {
      ...
      (symbolMapperCacheService.mapSymbols as jest.Mock).mockResolvedValue(mockResponse);
  ...
  describe('Error Handling Integration', () => {
    it('should handle SymbolMapperCacheService errors gracefully', async () => {
      ...
      const result = await service.transformSymbols('longport', ['700.HK'], TRANSFORM_DIRECTIONS.TO_STANDARD);
  ...
  describe('Performance and Validation Integration', () => {
    it('should enforce batch size limits', async () => {
      ...
      await expect(
        service.transformSymbols('longport', oversizedBatch, TRANSFORM_DIRECTIONS.TO_STANDARD)
