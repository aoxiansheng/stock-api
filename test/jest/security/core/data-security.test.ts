...
describe("Core Data Security Tests", () => {
  ...
  describe("Data Receiver Security", () => {
    ...
    it("应该限制批量请求数量防止DoS攻击", async () => {
      const massiveSymbolList = Array.from(
        { length: 10000 },
        (_, i) => `SYMBOL${i}.HK`,
      );
  ...
  describe("Symbol Mapper Security", () => {
    ...
    it("应该限制批量映射请求防止DoS", async () => {
      const massiveSymbols = Array.from(
        { length: 50000 },
        (_, i) => `SYMBOL${i}`,
      );
  ...
  describe("Business Logic Security", () => {
    it("应该防护竞态条件攻击", async () => {
      ...
      const successfulResponses = actualResponses.filter(
        (r) => r.status === 200,
      );
    ...
    it("应该防护资源枯竭攻击", async () => {
      ...
      const longRunningRequests = Array(5)
        .fill(0)
        .map(async () => {
          const response = await request
            .post("/api/v1/receiver/data")
