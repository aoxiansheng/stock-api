...
describe('Monitoring Security Tests', () => {
  ...
  beforeEach(async () => {
    ...
    // 使用动态生成的用户名避免重复键冲突
    const { token: adminUserToken } = await global.createAuthenticatedUser({ role: UserRole.ADMIN });
    adminToken = adminUserToken;

    const { token: developerUserToken } = await global.createAuthenticatedUser({ role: UserRole.DEVELOPER });
  ...
  describe('Monitoring Endpoint Access Control', () => {
    ...
    it('应该防护JWT令牌权限提升', async () => {
      // 创建一个拥有普通用户角色的令牌
      const baseUserToken = await global.createTestJWTToken({ role: UserRole.DEVELOPER, sub: 'test-user-id' });

      // 尝试伪造令牌
      const manipulatedTokens = [
        // 1. 无效签名 - 将签名部分替换为任意字符串
        baseUserToken.substring(0, baseUserToken.lastIndexOf('.') + 1) + 'invalid-signature',
        // 2. 伪造 payload - 尝试将角色更改为 'admin' 但不更新签名
        baseUserToken.split('.')[0] + '.' + Buffer.from(JSON.stringify({ role: 'admin' })).toString('base64') + '.' + baseUserToken.split('.')[2],
  ...
  describe('Monitoring Data Security', () => {
    ...
    it('应该防护监控数据查询注入攻击', async () => {
      ...
      for (const param of maliciousParams) {
        ...
        const isSecurityViolation = message.includes('不安全的内容') || message.includes('验证失败'); // 修正：适配新的验证错误消息
        const isInvalidSeverity = message.includes('无效的告警级别') || message.includes('必须是有效的枚举值');
    ...
    it('应该限制监控查询范围防止数据泄露', async () => {
      ...
      expect(response.status).toBe(400);
      expect(response.body.message).toContain('limit must not be greater than 100'); // 修正：根据新的验证规则调整
    });
    ...
    it('应该防护安全审计事件的手动注入', async () => {
      ...
      if (auditResponse.status === 200 && auditResponse.body?.events?.length > 0) {
  ...
  describe('Security Monitoring Attacks', () => {
    ...
    it.skip('应该限制安全扫描频率防止DoS', async () => {
      const scanRequests = Array(5).fill(0).map(() =>
        request
  ...
  describe('Monitoring Performance Security', () => {
    it('应该防护端点指标查询注入', async () => {
      const maliciousSortBy = 'averageResponseTime; UPDATE users SET role = "admin"; --';
    ...
    it('应该防护监控查询时间炸弹', async () => {
      ...
      expect(veryLargeTimeRangeResponse.body.message).toContain('验证失败: 无效的时间范围');
    });
  });
