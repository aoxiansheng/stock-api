...
describe("DataFetcher Pipeline E2E", () => {
  ...
  beforeAll(async () => {
    ...
    dataFetcherService =
      moduleFixture.get<DataFetcherService>(DataFetcherService);
    ...
    capabilityRegistryService = moduleFixture.get<CapabilityRegistryService>(
  ...
  describe("Complete Data Pipeline Integration", () => {
    it("should process data request through complete refactored pipeline", async () => {
      ...
      jest
        .spyOn(receiverService, "determineOptimalProvider" as any)
  ...
  describe("SymbolMapper Enhanced Integration", () => {
    ...
    it("should handle mixed symbol formats correctly", async () => {
      ...
      expect(Object.keys(result.mappingResults.transformedSymbols)).toEqual([
        "00700",
        "600000",
      ]);
      expect(result.mappingResults.transformedSymbols["00700"]).toBe("700.HK");
    ...
    it("should handle high-volume symbol requests efficiently", async () => {
      ...
      jest
        ...
        .mockResolvedValue({
          ...
          mappingResults: {
            transformedSymbols: largeSymbolSet.reduce(
              (acc, symbol) => {
                acc[symbol] = symbol;
                return acc;
              },
