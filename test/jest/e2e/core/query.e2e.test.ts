...
describe('Query E2E Tests', () => {
  ...
  async function setupAuthentication() {
    ...
    const jwtToken = loginResponse.body.data?.accessToken || loginResponse.body.accessToken;

    const apiKeyData = {
      name: 'Query Test API Key',
      permissions: ['data:read', 'query:execute', 'providers:read', 'transformer:preview', 'system:monitor'],
  ...
  describe('Query Execution by Different Criteria', () => {
    it.skip('should execute queries by market', async () => { // 🚧 功能未实现，暂时跳过
      const response = await httpServer
        .post('/api/v1/query/execute')
        .set('X-App-Key', authTokens.apiKey)
        .set('X-Access-Token', authTokens.accessToken)
    ...
    it.skip('should execute queries by provider', async () => { // 🚧 功能未实现，暂时跳过
      const response = await httpServer
        .post('/api/v1/query/execute')
        .set('X-App-Key', authTokens.apiKey)
        .set('X-Access-Token', authTokens.accessToken)
    ...
    it.skip('should execute queries by data type', async () => { // 🚧 功能未实现，暂时跳过
      const response = await httpServer
        .post('/api/v1/query/execute')
        .set('X-App-Key', authTokens.apiKey)
        .set('X-Access-Token', authTokens.accessToken)
  ...
  describe('Advanced Query Features', () => {
    it.skip('should handle advanced filtering', async () => { // 🚧 功能未实现，暂时跳过
      const response = await httpServer
        .post('/api/v1/query/execute')
        .set('X-App-Key', authTokens.apiKey)
        .set('X-Access-Token', authTokens.accessToken)
  ...
  describe('Query Performance and Validation', () => {
    it('should provide query statistics if endpoint exists', async () => {
      ...
      expect(response.body.data.performance).toHaveProperty('averageExecutionTime');
    });
