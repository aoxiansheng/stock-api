...
describe('Error Handling and Recovery E2E Tests', () => {
  ...
  async function setupAuthentication() {
    ...
    const apiKeyData = {
      ...
      permissions: ['data:read', 'query:execute', 'providers:read', 'transformer:preview'],
      rateLimit: {
        requests: 100,
        window: '1h',
      },
  ...
  describe('Error Handling and Recovery', () => {
    ...
    it('should handle malformed request data', async () => {
      const invalidRequests = [
        ...
        },
        {
          ...
          body: { symbols: ['700.HK'], dataType: 'invalid-type' }
        },
        {
    ...
    it('should handle database connection errors', async () => {
      ...
      const response = await httpServer
        ...
        .expect((res: any) => {
          ...
          if (!validStatuses.includes(res.status)) {
            throw new Error(`Expected one of ${validStatuses.join(', ')}, got ${res.status}`);
          }
        });
      ...
      if (response.status === 503) {
        ...
        expect(response.body.error).toMatch(/DatabaseConnectionError|Service Unavailable/);
    ...
    it('should handle Redis connection errors', async () => {
      ...
      if (response.status === 503) {
        // 如果返回503，验证是服务不可用错误
        expect(response.body.statusCode).toBe(503);
        expect(response.body.message).toMatch(/服务.*不可用|缓存.*失败|连接.*失败/);
        expect(response.body.timestamp).toBeDefined();
      } else if (response.status === 200) {
