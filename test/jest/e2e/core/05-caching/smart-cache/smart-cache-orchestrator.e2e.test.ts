  ...
  describe("Receiver Smart Cache E2E Flow", () => {
    it("应该通过Receiver端点完整验证强时效缓存策略", async () => {
      ...
      if (firstResponse.body.data.metadata.cacheStrategy) {
        expect(firstResponse.body.data.metadata.cacheStrategy).toBe(
      ...
      const firstResponseTime =
        firstResponse.body.data.metadata.processingTime || 0;
      const secondResponseTime =
        secondResponse.body.data.metadata.processingTime || 0;
  ...
  describe("Query Smart Cache E2E Flow", () => {
    it("应该通过Query端点验证弱时效缓存策略", async () => {
      ...
      if (firstQueryResponse.body.data.metadata.cacheStrategy) {
        expect(firstQueryResponse.body.data.metadata.cacheStrategy).toBe(
      ...
      // 验证数据一致性
      expect(secondQueryResponse.body.data.data.items.length).toBe(
        firstQueryResponse.body.data.data.items.length,
      );
    });
    ...
    it("应该验证市场感知策略的动态行为", async () => {
      ...
      if (response.body.data.metadata.cacheStrategy) {
        expect(response.body.data.metadata.cacheStrategy).toBe(
      ...
      if (response.body.data.metadata.marketStatus) {
        expect(response.body.data.metadata.marketStatus).toHaveProperty(
          "isOpen",
        );
  ...
  describe("Background Update E2E Verification", () => {
    it("应该正确触发和执行后台更新任务", async () => {
      ...
      if (response.body.data.metadata.backgroundUpdateScheduled !== undefined) {
        expect(
          response.body.data.metadata.backgroundUpdateScheduled,
        ).toBeDefined();
      }
  ...
  describe("Error Handling and Fault Tolerance E2E", () => {
    it("应该在Provider失败时优雅降级", async () => {
      ...
      if (response.body.data.errors) {
        ...
        response.body.data.metadata.hasPartialFailures
      ) {
