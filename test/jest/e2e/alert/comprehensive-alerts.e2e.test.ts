...
describe('Comprehensive Alerting E2E Tests', () => {
  ...
  describe('Alert System Monitoring', () => {
    ...
    it('should filter alerts by severity', async () => {
      ...
      criticalAlerts.forEach(alert => expect(alert.severity).toBe('critical'));
  ...
  describe('Alert Rule Management (CRUD)', () => {
    ...
    it('should create a new alert rule successfully', async () => {
      const ruleData = {
        ...
        severity: 'critical',
        enabled: true,
        channels: [{ name: 'Log Channel', type: 'log', enabled: true, config: { level: 'error' } }],
        cooldown: 600,
      };
  ...
  describe('Alert Lifecycle and Permissions', () => {
    ...
    beforeAll(async () => {
      ...
      const ruleData = {
        ...
        severity: 'warning',
        enabled: true,
        channels: [{ name: 'Lifecycle Log Channel', type: 'log', enabled: true, config: { level: 'warn' } }],
        cooldown: 0,
      };
      const response = await httpServer
        .post('/api/v1/alerts/rules')
    ...
    it('should trigger a new alert', async () => {
      ...
      console.log('Rule found:', JSON.stringify(ruleResponse.body.data, null, 2));
      console.log('==================');
      ...
      console.log('Sending trigger data:', JSON.stringify(triggerData, null, 2));
      console.log('=====================');
      ...
      console.log('Trigger response:', JSON.stringify(triggerResponse.body, null, 2));
      console.log('========================');
      ...
      console.log('Active alerts response:', JSON.stringify(activeAlertsResponse.body, null, 2));
      console.log('Number of alerts found:', activeAlertsResponse.body.data ? activeAlertsResponse.body.data.length : 'data is undefined');
      if (activeAlertsResponse.body.data && activeAlertsResponse.body.data.length > 0) {
        console.log('First alert ruleId:', activeAlertsResponse.body.data[0].ruleId);
        console.log('All alert ruleIds:', activeAlertsResponse.body.data.map((a: any) => a.ruleId));
      }
      console.log('===================');
      
      const newAlert = activeAlertsResponse.body.data.find((a: any) => a.ruleId === ruleId);
    ...
    it('should resolve the acknowledged alert', async () => {
      ...
      const activeAlertsResponse = await httpServer
        .get('/api/v1/alerts/active')
        .set('Authorization', `Bearer ${jwtToken}`);
      const resolvedAlert = activeAlertsResponse.body.data.find(a => a.id === alertId);
