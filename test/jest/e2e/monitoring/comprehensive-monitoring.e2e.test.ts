...
describe('Comprehensive Monitoring E2E Tests', () => {
      ...
      if (response.status === 200) {
        ...
        if (dashboardData.performance) {
          ...
          if (dashboardData.performance.summary) {
            expect(dashboardData.performance.summary).toHaveProperty('averageResponseTime');
            expect(dashboardData.performance.summary).toHaveProperty('errorRate');
            expect(dashboardData.performance.summary).toHaveProperty('totalRequests');
          }
        }
    ...
    it('should categorize recommendations properly', async () => {
      ...
      if (response.status === 200) {
        ...
        if (recommendations.length > 0) {
          ...
          const cacheRecs = recommendations.filter(rec => rec.category === 'cache');
          const databaseRecs = recommendations.filter(rec => rec.category === 'database');
          const securityRecs = recommendations.filter(rec => rec.category === 'security');
          
          // 验证各类建议的有效性
          [performanceRecs, cacheRecs, databaseRecs, securityRecs].forEach(categoryRecs => {
  ...
  describe('Real-time Monitoring Data Flow', () => {
    it('should reflect system activity in monitoring data', async () => {
      if (!authTokens.apiKey || !authTokens.accessToken) {
        console.log('Skipping real-time monitoring test - no API tokens available');
        return;
      }
