  ...
  describe('Standard Response Format Validation', () => {
    it('should enforce consistent response format across all successful API calls', async () => {
      ...
      const endpoints = [
        ...
        { path: '/api/v1/monitoring/performance', method: 'GET', auth: adminToken }
      ];
    ...
    it('should format error responses consistently across all endpoints', async () => {
      ...
      for (const scenario of errorScenarios) {
        ...
        if ((scenario as any).auth) {
          response = response.set('Authorization', `Bearer ${(scenario as any).auth}`);
    ...
    it('should properly format numeric values and currencies', async () => {
      ...
      if (response.status === 200 && response.body.data) {
        ...
        if (data.lastPrice !== undefined) {
          expect(typeof data.lastPrice === 'number' || typeof data.lastPrice === 'string').toBe(true);
    ...
    it('should maintain data type consistency across similar endpoints', async () => {
      ...
      const endpoints = [
        { 
          path: '/api/v1/receiver/data',
          method: 'POST',
          body: { symbols: ['AAPL.US'], dataType: 'stock-quote' },
          headers: { 'X-App-Key': userApiKey, 'X-Access-Token': userAccessToken }
        },
        {
  ...
  describe('Pagination and Large Dataset Formatting', () => {
    beforeAll(async () => {
      ...
      const alertRule = {
        ...
        duration: 60,
        enabled: true,
        channels: [{ name: 'Log Channel', type: 'log', enabled: true, config: { level: 'info' } }],
        cooldown: 0,
      };
  ...
  describe('Cache Headers and Response Optimization', () => {
    it('should set appropriate cache headers for cacheable responses', async () => {
      ...
      if (response.headers['etag']) {
        expect(response.headers['etag']).toMatch(/^(W\/)?"[a-zA-Z0-9+\/=\-_]+"/);
      }

      if (response.headers['last-modified']) {
        expect(new Date(response.headers['last-modified']).getTime()).toBeGreaterThan(0);
      }
    });
    ...
    it('should compress large responses when appropriate', async () => {
      ...
      if (response.status === 200) {
        ...
        if (responseSize > 1024) { // 如果响应大于1KB
          // 检查是否使用了压缩
