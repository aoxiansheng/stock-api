...
import { INestApplication, HttpException, HttpStatus, ValidationPipe } from '@nestjs/common';
...
} from '../../../../src/metrics/interceptors/performance.interceptor';
...
describe('Error Handling Flow E2E Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const createdApiKey = await apikeyService.createApiKey(testUser.id, apiKeyDto);
    validApiKey = {
  ...
  describe('Authentication and Authorization Errors', () => {
    ...
    it('should handle expired JWT token gracefully', async () => {
      ...
      const expiredToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyLCJleHAiOjE1MTYyMzkwMjJ9.invalid'; // 已过期
...
;
    ...
    it('should handle request size limits', async () => {
      // Act - 发送过大的请求体
      const largeSymbolArray = Array(1000).fill(0).map((_, i) => `STOCK${i}.US`);
...
;
  ...
  describe('Error Recovery and Resilience', () => {
    ...
    it('should track error patterns for monitoring', async () => {
      // Act - 生成一系列相似错误
      const errorRequests = Array(5).fill(0).map(() =>
        request(app.getHttpServer())
          .post('/api/v1/receiver/data')
          .set('X-App-Key', validApiKey.appKey)
          .set('X-Access-Token', validApiKey.accessToken)
    ...
    it('should provide consistent error format across all endpoints', async () => {
      ...
      for (const test of errorTests) {
        ...
        if (response && response.status === test.expectedStatus) {
          // Assert - 验证错误格式一致性
          expect(response.body).toHaveProperty('statusCode', test.expectedStatus);
          expect(response.body).toHaveProperty('message');
          expect(response.body).toHaveProperty('timestamp');
          expect(response.body).toHaveProperty('data', null);
          expect(response.body).toHaveProperty('error');
  ...
  describe('Error Logging and Metrics', () => {
    it('should generate error logs with appropriate severity levels', async () => {
      ...
      const errorScenarios = [
        { endpoint: '/api/v1/receiver/data', body: { symbols: [] }, expectedLevel: 'warning' },
