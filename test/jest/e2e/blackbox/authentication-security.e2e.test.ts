...
describe("Real Environment Black-box: Authentication & Security E2E", () => {
  ...
  async function setupAuthentication() {
    try {
      ...
      const developerRegisterResponse = await httpClient.post("/api/v1/auth/register", developerData);
      if (developerRegisterResponse.status !== 201) {
        console.warn('开发者注册失败，可能已存在，尝试直接登录');
      }

      const developerLoginResponse = await httpClient.post("/api/v1/auth/login", {
  ...
  describe("🔐 API Key 认证 - 17个细粒度权限测试", () => {
    ...
    permissionMatrix.forEach(({ permission, endpoint, method, testData }) => {
      it(`应该严格执行 ${permission} 权限控制`, async () => {
        ...
        if (authorizedResponse.status >= 400 && ![404, 429].includes(authorizedResponse.status)) {
          console.log(`\n[ERROR] ${permission} 权限测试失败:`);
          console.log(`  端点: ${method.toUpperCase()} ${endpoint}`);
          console.log(`  状态码: ${authorizedResponse.status}`);
          console.log(`  响应消息: ${authorizedResponse.data?.message || 'N/A'}`);
          console.log(`  错误详情: ${JSON.stringify(authorizedResponse.data, null, 2)}`);
        }
  ...
  describe("🎫 JWT 认证 - 角色权限继承测试", () => {
    it("管理员应该继承所有开发者权限", async () => {
      // 测试管理员访问开发者端点
      const adminProfileResponse = await httpClient.get("/api/v1/auth/profile", {
        headers: { Authorization: `Bearer ${adminJWT}` }
      });
    ...
    it("开发者应该可以访问开发者端点但不能访问管理员端点", async () => {
      ...
      const createKeyResponse = await httpClient.post("/api/v1/auth/api-keys", {
  ...
  describe("🚦 分布式限流系统测试", () => {
    it("应该实现Redis分布式限流机制", async () => {
      ...
      const apiKeyResponse = await httpClient.post("/api/v1/auth/api-keys", limitedApiKeyData, {
        headers: { Authorization: `Bearer ${adminJWT}` }
      });
  ...
  describe("🛡️ 安全边界测试", () => {
    ...
    it("应该验证请求数据完整性", async () => {
      ...
      for (const payload of maliciousPayloads) {
        const response = await httpClient.post("/api/v1/receiver/data", payload, {
          headers: {
            "X-App-Key": apiKey.appKey,
            "X-Access-Token": apiKey.accessToken,
          }
  ...
  describe("🔒 会话管理测试", () => {
    it("应该支持安全的令牌刷新机制", async () => {
      ...
      if (loginResponse.status === 401) {
        const registerResponse = await httpClient.post('/api/v1/auth/register', {
          username: sessionTestUsername,
