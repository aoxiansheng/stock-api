...
export class CoverageTrendChecker {
  ...
  constructor(config?: TrendCheckerConfig) {
    this.historyFile = config?.historyFile || 'coverage/history/coverage-history.json';
   ...
   * 更新历史数据
   */
  private async updateHistoryData(history: CoverageHistoryEntry[], current: CoverageSummary): Promise<void> {
    ...
    const existingIndex = history.findIndex(h => 
      Math.abs(new Date(h.timestamp).getTime() - new Date(newEntry.timestamp).getTime()) < 60000 // 1分钟内
    );
  ...
  private analyzeTrends(history: CoverageHistoryEntry[]): TrendAnalysis {
    ...
    const longTermTrend = this.calculateTrend(history, Math.min(20, history.length)); // 最近20次或全部
   ...
   * 计算趋势
   */
  private calculateTrend(history: CoverageHistoryEntry[], period: number): TrendDetail {
    const recentHistory = history.slice(-period);
    
    if (recentHistory.length < 2) {
      return { direction: 'stable', change: 0, period: `${recentHistory.length} 条记录` };
    }
   ...
   * 计算波动性
   */
  private calculateVolatility(history: CoverageHistoryEntry[]): 'low' | 'medium' | 'high' {
    if (history.length < 3) {
      return 'low';
    }
   ...
   * 生成预测
   */
  private generatePrediction(shortTerm: TrendDetail, longTerm: TrendDetail): string {
    if (shortTerm.direction === 'improving' && longTerm.direction === 'improving') {
      return '覆盖率持续改善，预计将继续提升';
    } else if (shortTerm.direction === 'declining' && longTerm.direction === 'declining') {
      return '覆盖率持续下降，需要立即关注';
    } else if (shortTerm.direction === 'improving' && longTerm.direction === 'stable') {
      return '短期改善明显，长期保持稳定';
    } else if (shortTerm.direction === 'declining' && longTerm.direction === 'stable') {
  ...
  private generateRecommendations(history: CoverageHistoryEntry[]): string[] {
    ...
    if (trends.longTerm.direction === 'stable' && trends.longTerm.change === 0) {
  ...
  private generateSummary(result: TrendCheckResult): string {
    ...
    const errorAlerts = result.alerts.filter(a => a.severity === 'error').length;
  ...
  private printTrendResult(result: TrendCheckResult): void {
    ...
    console.log(`\n📈 趋势分析:`);
    console.log(`短期趋势: ${result.trends.shortTerm.direction} (${result.trends.shortTerm.change > 0 ? '+' : ''}${result.trends.shortTerm.change}%)`);
    console.log(`长期趋势: ${result.trends.longTerm.direction} (${result.trends.longTerm.change > 0 ? '+' : ''}${result.trends.longTerm.change}%)`);
    ...
    if (result.alerts.length > 0) {
      ...
      result.alerts.forEach((alert, index) => {
        const icon = alert.severity === 'critical' ? '🚫' : alert.severity === 'error' ? '❌' : alert.severity === 'warning' ? '⚠️' : 'ℹ️';
