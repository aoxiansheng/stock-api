...
export class ConcurrentRequestHelper {
  ...
  async executeConcurrentRequests(config: ConcurrentRequestConfig) {
    ...
    for (let i = 0; i < finalConfig.totalRequests; i += finalConfig.batchSize) {
      ...
      const totalBatches = Math.ceil(finalConfig.totalRequests / finalConfig.batchSize);
      results.batches = totalBatches;

      console.log(`🔍 执行批次 ${currentBatch}/${totalBatches}...`);

      const batch = Array(Math.min(finalConfig.batchSize, finalConfig.totalRequests - i))
        .fill(0)
        .map((_, j) => this.createSingleRequest(finalConfig, {
      ...
      try {
        ...
        responses.push(...batchResponses);
         ...
         batchResponses.forEach(response => {
           if (response.status === 'fulfilled' && !(response.value as any).error) {
         ...
         const batchSuccessful = batchResponses.filter(r => 
           r.status === 'fulfilled' && !(r.value as any).error).length;
        const batchFailed = batchResponses.length - batchSuccessful;

        console.log(`✅ 批次 ${currentBatch} 完成: ${batchSuccessful}成功, ${batchFailed}失败`);
        ...
        const failedBatch = Array(Math.min(finalConfig.batchSize, finalConfig.totalRequests - i))
          .fill(0)
    ...
    responses.forEach(response => {
      if (response.status === 'fulfilled' && !response.value.error) {
        ...
        const error = response.status === 'rejected' 
          ? response.reason?.message || response.reason?.code || 'Unknown error'
    ...
    return {
      responses,
      results,
      successfulRequests: responses.filter(r => r.status === 'fulfilled' && !r.value.error),
      failedRequests: responses.filter(r => r.status === 'rejected' || 
   ...
   * 创建单个请求（带重试机制）
   */
  private async createSingleRequest(config: ConcurrentRequestConfig, queryParams: any) {
    ...
    let lastError: any;
       ...
       try {
        ...
        // 设置超时
        const response = await requestBuilder.timeout(config.requestTimeout || this.defaultOptions.requestTimeout);
        return response;
        ...
        if (attempt < maxRetries) {
          ...
          console.log(`🔄 重试请求 ${queryParams.batch}-${queryParams.request} (尝试 ${attempt + 2}/${maxRetries + 1})`);
        }
      }
    }

    // 所有重试都失败，返回错误
    return { error: lastError?.message || lastError?.code || 'Request failed after retries' };
  }

  /**
   * 验证内存使用情况
   */
  async checkMemoryUsage(initialMemory: number, maxIncreasePercent: number = 50): Promise<MemoryAnalysis> {
    // 强制垃圾回收
    if (global.gc) {
      console.log('🧹 执行垃圾回收...');
      global.gc();
    ...
    console.log(`  - 是否在限制内: ${increasePercent <= maxIncreasePercent ? '✅' : '❌'}`);
   ...
   * 生成测试报告
   */
     generateReport(results: ConcurrentTestResult, memoryAnalysis: MemoryAnalysis, errorPatterns?: Record<string, number>) {
     console.log('\n📊 并发测试报告:');
     console.log('='.repeat(50));
     console.log(`请求统计:`);
     console.log(`  - 总请求数: ${results.total}`);
     console.log(`  - 成功请求: ${results.successful} (${(results.successRate * 100).toFixed(1)}%)`);
     console.log(`  - 失败请求: ${results.failed} (${((1 - results.successRate) * 100).toFixed(1)}%)`);
     console.log(`  - 总耗时: ${results.endTime - results.startTime}ms`);
     ...
     console.log(`\n内存使用:`);
     console.log(`  - 内存增长: ${(memoryAnalysis.memoryIncrease / 1024 / 1024).toFixed(2)}MB`);
     console.log(`  - 增长百分比: ${memoryAnalysis.increasePercent.toFixed(1)}%`);
     console.log(`  - 是否在限制内: ${memoryAnalysis.isWithinLimit ? '✅' : '❌'}`);
...
export class PerformanceBenchmark {
   ...
   * 检查性能是否在基准范围内
   */
  checkPerformance(name: string, actualValue: number, tolerancePercent: number = 20): boolean {
