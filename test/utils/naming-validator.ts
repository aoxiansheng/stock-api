...
export class NamingValidator {
  ...
  constructor() {
    this.namingRules = {
      ...
      'test/jest/security': {
        ...
        examples: ['auth-security.security.test.ts', 'input-validation.security.test.ts']
      },
      'test/k6': {
    ...
    for (const [dirPattern, rule] of Object.entries(this.namingRules)) {
      ...
      for (const file of testFiles) {
        ...
        if (!rule.pattern.test(fileName)) {
          const suggestedName = this.generateSuggestedName(fileName, rule.pattern);
  ...
  private async validateFileNameConsistency(): Promise<void> {
    ...
    for (const [testTarget, files] of filesByTestTarget.entries()) {
      if (files.length > 1) {
        const baseNames = files.map(f => this.getBaseNameWithoutExtension(path.basename(f)));
  ...
  private async validateSpecialNamingConventions(): Promise<void> {
    ...
    for (const file of setupFiles) {
      const fileName = path.basename(file);
      if (!fileName.includes('setup') || (!fileName.endsWith('.ts') && !fileName.endsWith('.js'))) {
        this.violations.push({
  ...
  private generateFixCommands(): string[] {
    ...
    const fixableViolations = this.violations.filter(v => v.fixCommand && !v.fixCommand.includes('#'));
