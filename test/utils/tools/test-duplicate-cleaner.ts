...
class TestDuplicateCleaner {
  ...
  constructor(testDirectory: string, targetFilePath?: string) {
    ...
    if (targetFilePath) {
      this.targetContent = this.normalizeContent(fs.readFileSync(targetFilePath, 'utf-8'));
    } else {
      this.targetContent = '';
    }
  }
  ...
  private isPlaceholderTestFile(content: string): boolean {
    ...
    // 基本模式检查
    const hasImport = lines.some(line => line.includes("import { Test, TestingModule }"));
    const hasDescribe = lines.some(line => line.includes("describe("));
    const hasBeforeEach = lines.some(line => line.includes("beforeEach"));
    const hasBasicTest = lines.some(line => line.includes("should be defined"));
  ...
  public generateReport(): void {
    ...
    if (this.targetFilePath) {
      console.log(`🎯 参考文件: ${path.relative(process.cwd(), this.targetFilePath)}`);
    } else {
      console.log(`🎯 扫描模式: 通用占位文件检测`);
    }
    ...
    if (this.duplicateFiles.length > 0) {
      ...
      console.log('\n📝 样例清理后内容:');
      if (this.duplicateFiles.length > 0) {
        const sampleContent = this.generateCleanedContent(this.duplicateFiles[0].path);
        console.log('   ' + sampleContent.split('\n').join('\n   '));
      }
    }

    console.log('\n💡 使用说明:');
    console.log('   - 预览清理: ts-node tools/test-duplicate-cleaner.ts --dry-run');
    console.log('   - 执行清理: ts-node tools/test-duplicate-cleaner.ts --execute');
    console.log('   - 查看报告: ts-node tools/test-duplicate-cleaner.ts --report');
    console.log('   - 通用模式: ts-node tools/test-duplicate-cleaner.ts --universal --execute');
  }
}

// 主函数
function main() {
  const testDirectory = '/Users/honor/Documents/code/newstockapi/backend/test/jest';
  const defaultTargetFilePath = '/Users/honor/Documents/code/newstockapi/backend/test/jest/unit/providers/longport-sg/module/longport-sg.module.spec.ts';
