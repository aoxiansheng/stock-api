...
class NamingValidator {
  ...
  async validateNaming(): Promise<NamingValidationResult> {
    ...
    const directoryAnalysis = this.analyzeDirectoryStructure(
      testFiles,
      srcStructure,
    );
    ...
    const result: NamingValidationResult = {
      summary: {
        ...
        complianceRate:
          testFiles.length > 0
            ? (validFiles.length / testFiles.length) * 100
            : 0,
      },
      directoryAnalysis,
      invalidFiles,
  ...
  private scanTestFiles(): TestFileInfo[] {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory()) {
          ...
          const relativePath = path.relative(
            path.join(this.testDir, testType),
            fullPath,
          );
          ...
          testFiles.push({
            ...
            suggestedName: this.isValidNaming(item, testType)
              ? undefined
              : this.suggestCorrectName(item, testType),
    ...
    // 扫描所有测试类型目录
    const testTypeDirectories = fs.readdirSync(this.testDir).filter((item) => {
  ...
  private suggestCorrectName(fileName: string, testType: string): string {
    ...
    let baseName = fileName.replace(
      /\.(spec|test|e2e|integration|security|perf)\.ts$/,
      "",
    );
  ...
  private findCorrespondingSrcFile(
    ...
    // 移除测试文件的扩展名和测试后缀
    let srcPath = testRelativePath.replace(
      /\.(spec|test|e2e|integration|security|perf)\.ts$/,
      "",
    );
  ...
  private analyzeDirectoryStructure(
    ...
    // 按测试类型分组
    const testsByType = testFiles.reduce(
      (acc, file) => {
    ...
    for (const [testType, files] of Object.entries(testsByType)) {
      ...
      const missingCorrespondingSrc = files.filter(
        (f) => !f.correspondingSrcFile,
      );

      // 检查目录结构
      const testTypePath = path.join(this.testDir, testType);
      const testDirs = this.getDirectoryStructure(testTypePath);
      const srcDirs = Array.from(srcFiles)
        .map((f) => path.dirname(f))
        .filter((v, i, a) => a.indexOf(v) === i);

      const extraDirectories = testDirs.filter((dir) => !srcDirs.includes(dir));
      const missingDirectories = srcDirs.filter(
  ...
  private generateSuggestions(
    ...
    if (invalidFiles.length > 0) {
      ...
      const byType = invalidFiles.reduce(
        (acc, file) => {
    ...
    directoryAnalysis.forEach((analysis) => {
      if (
        analysis.missingDirectories.length > 0 ||
        analysis.extraDirectories.length > 0
      ) {
        suggestions.push(`\n🏗️  ${analysis.testType} 目录结构调整:`);

        if (analysis.missingDirectories.length > 0) {
          suggestions.push(
            `   缺失目录 (${analysis.missingDirectories.length}): ${analysis.missingDirectories.slice(0, 3).join(", ")}${analysis.missingDirectories.length > 3 ? "..." : ""}`,
          );
        }

        if (analysis.extraDirectories.length > 0) {
          suggestions.push(
            `   多余目录 (${analysis.extraDirectories.length}): ${analysis.extraDirectories.slice(0, 3).join(", ")}${analysis.extraDirectories.length > 3 ? "..." : ""}`,
          );
        }
      }
    });
    ...
    // 合规性建议
    const totalFiles = directoryAnalysis.reduce(
      (sum, analysis) => sum + analysis.totalFiles,
      0,
    );
    const totalValid = directoryAnalysis.reduce(
      (sum, analysis) => sum + analysis.validNaming,
      0,
    );
    ...
    result.directoryAnalysis.forEach((analysis) => {
      const complianceRate =
        analysis.totalFiles > 0
          ? (analysis.validNaming / analysis.totalFiles) * 100
          : 0;
  ...
  async generateRenameScript(result: NamingValidationResult): Promise<string> {
    ...
    result.invalidFiles.forEach((file) => {
      if (file.suggestedName) {
        ...
        scriptLines.push(
          `echo "重命名: ${file.fileName} -> ${file.suggestedName}"`,
        );
