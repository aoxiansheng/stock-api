  ...
  "test/": {
    ...
    children: {
      ...
      "fixtures/": {
        ...
        files: ["test-data.ts", "mock-responses.ts", "sample-data.json"],
      },
    },
  },
} as const;
...
export class NamingValidator {
  static validateFileName(filePath: string): {
    isValid: boolean;
    violations: string[];
  } {
    ...
    for (const [testType, config] of Object.entries(NAMING_CONVENTIONS)) {
      if (filePath.includes(testType)) {
        ...
        if (!config.pattern.test(fileName)) {
          violations.push(
            `${testType}测试文件应该匹配模式: ${config.pattern.toString()}`,
          );
        }
      }
    }
  ...
  static getRecommendedFileName(
    testType: keyof typeof NAMING_CONVENTIONS,
    baseName: string,
  ): string {
...
export class DirectoryValidator {
  static validateDirectory(filePath: string): {
    isValid: boolean;
    violations: string[];
  } {
    const violations: string[] = [];
    ...
    if (relativePath.length < 3) {
      violations.push(
        "测试文件应该至少在三级目录中: test/技术栈/测试类型/功能模块/",
      );
    }
