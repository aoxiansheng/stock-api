...
describe("StreamDataValidator provider matrix", () => {
  ...
  describe("symbol format 统一严格标准", () => {
    it("接受 US/CN/SG 标准后缀格式以及 CRYPTO 裸 pair 标准格式", () => {
      ...
      expect(validator.isValidSymbolFormat("BTCUSDT.CRYPTO")).toBe(false);
    });
    ...
    it("extractMarket 将 SH/SZ 归一到 CN", () => {
      ...
      expect(validator.extractMarket("BTCUSDT.CRYPTO")).toBeNull();
