...
/**
 * 本地项目 get-stock-basic-info 能力专项测试脚本
 *
 * 目标：
 * 1. 调用本地 POST /api/v1/receiver/data
 * 2. 使用 receiverType=get-stock-basic-info + preferredProvider=infoway
 * 3. 校验返回结构、关键字段、标的覆盖情况与 provider 路由结果
 *
 * 常用环境变量：
 ...
 * - TEST_MARKET: 可选，强制传入 options.market（例如 US）
 * - TEST_PROVIDER: 期望 provider（默认 infoway）
 * - REQUIRE_PROVIDER_MATCH: 是否要求 metadata.provider 匹配 TEST_PROVIDER（默认 true）
...
function extractReceiverBusinessPayload(respData) {
  if (!respData || typeof respData !== "object") {
    return {};
  }
  if (respData.data && typeof respData.data === "object" && "metadata" in respData.data) {
    return respData.data;
  }
  if ("metadata" in respData) {
    return respData;
  }
...
function toRows(businessPayload) {
  if (Array.isArray(businessPayload)) {
    return businessPayload;
  }
  if (Array.isArray(businessPayload?.data)) {
    ...
    for (const field of numericFields) {
      if (row[field] !== undefined && row[field] !== null) {
        ...
        if (!Number.isFinite(value)) {
          issues.push(`data[${index}].${field} 不是数字`);
        }
      }
    }
...
const TEST_PROVIDER = String(process.env.TEST_PROVIDER || "infoway")
  .trim()
  .toLowerCase();
const REQUIRE_PROVIDER_MATCH = isTrue(process.env.REQUIRE_PROVIDER_MATCH, true);
...
async function main() {
  ...
  console.log("[config]", {
    ...
    requireProviderMatch: REQUIRE_PROVIDER_MATCH,
  ...
  const business = extractReceiverBusinessPayload(resp.data);
  const rows = toRows(business);
  const metadata = business?.metadata && typeof business.metadata === "object"
    ? business.metadata
    : {};

  assert(rows.length > 0, "返回 data 为空", { business });

  const issues = validateBasicInfoRows(rows);
  assert(issues.length === 0, "字段结构校验失败", issues.slice(0, 20));

  const requestedSet = new Set(SYMBOLS.map((item) => item.toUpperCase()));
  ...
  if (missingSymbols.length > 0) {
    if (STRICT_SYMBOL_MATCH) {
      throw new Error(`返回缺少请求标的: ${missingSymbols.join(",")}`);
    }
    console.warn("[WARN] 返回缺少部分请求标的", missingSymbols);
  }

  const provider = String(metadata.provider || "").trim().toLowerCase();
  if (REQUIRE_PROVIDER_MATCH) {
    assert(provider === TEST_PROVIDER, "metadata.provider 与预期不一致", {
      expected: TEST_PROVIDER,
  ...
  console.log("[PASS] get-stock-basic-info 本地能力验证通过");
  console.log(
    JSON.stringify(
      {
        timelinessMode: "strong",
